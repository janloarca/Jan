...
- [ ] ClubCashIn: la gráfica de "Valor" no funciona bien, misma naturaleza que el caso IDC/VITALI ya documentado (reportado por el usuario, SOLO documentado, no investigado a fondo ni tocado — sin acceso a los datos reales de esa institución no se puede confirmar la causa exacta). Síntoma visto en captura: filtrado a ClubCashIn en el tab "Valor" (YTD), la línea está PLANA en ~$711 desde enero hasta ~junio, salta de golpe a ~$1.6K, y se queda plana ahí; el tooltip de un punto de abril muestra "+$0.00 (+0.00%) from start" a pesar de estar varios meses adentro del período. Esa forma (plana-baja → salto → plana-alta) es la misma que ya se documentó para activos manuales tipo Bono agregados a mitad de período dentro de una institución que ya tenía otro activo desde antes (mismo patrón que VITALI agregado en enero mientras XOCHI ya existía desde 2024, FASE DV/EJ/EO): candidatos directos son `indexBalanceEvents`/`applyStaticHistory` (`lib/historicalValues.js`, reconstrucción por rebobinado) y el encadenado de tramos de `computeAnchoredReturnSeries`/`computeMWRSeries` (`components/dashboard/analytics.js`) cuando hay más de un fondeo nuevo en la ventana. También candidato el filtro por institución de un ancla de calibración (FASE DO: un `_calibrated:true` de portafolio completo se descarta si `selectedInst !== 'ALL'`, pero ClubCashIn no aparecía en la lista de calibraciones activas de la captura de Calibrate Return, así que probablemente no es esto). Antes de tocar nada: confirmar con el usuario qué activos tiene ClubCashIn, sus fechas de adquisición, y si hay más de uno agregado en fechas distintas — exactamente los datos que expone `lib/assetLogic/corporateBondWithEntryFee.js` como necesarios para reproducir este tipo de caso.
