...
- [ ] Sospecha de "bumps" en la gráfica durante sincronización de precios: reportado por el usuario (captura del chart "Todas las instituciones", YTD: sube gradual hasta junio, salta a ~$26K entre el 14 jul y principios de agosto, y cae de vuelta al total real ~$23K) — SOLO investigado, no tocado, sin confirmación del usuario para proceder. Candidato con código verificado: `hooks/useMarketPrices.js` solo prende `loading:true` en el PRIMERÍSIMO fetch de toda la sesión (`if (Object.keys(_cachedPrices).length === 0) setLoading(true)`); cada actualización de fondo posterior (cada 5 min) corre con `loading:false` de punta a punta. El efecto que graba el snapshot diario (`hooks/useDashboardData.js`) usa esa misma bandera (`pricesLoading`) como semáforo para saber si es seguro escribir — con la bandera rota, ese semáforo no protege nada durante un refresh de fondo: si Yahoo/CoinGecko entrega un precio momentáneamente malo justo en ese instante y el efecto de snapshot corre ahí, graba ese número malo como snapshot PERMANENTE del día. Con broker conectado es peor: una vez escrito el snapshot del día, el código se niega a corregirlo el resto de ese día (a propósito, para no perder trabajo mid-día), así que un precio malo transitorio queda pegado hasta que cambia la fecha. El arreglo obvio (prender `loading` en TODOS los fetches) tiene un efecto secundario a revisar antes de tocar nada: `loading` también se usa en otros lados para mostrar spinners, y prenderlo en cada refresh de fondo podría meter un parpadeo cada 5 minutos que hoy no existe a propósito (según el comentario del código). No se encontró evidencia de doble-conteo por una escritura no atómica de IBKR (`bulkImport` escribe todo en un solo batch), así que esa hipótesis se descartó.
