...
- [ ] Modo oscuro del Spreadsheet: hoy `PortfolioSpreadsheet.jsx` tiene el cuerpo de la tabla (filas, celdas, encabezados de mes) con clases Tailwind fijas de tema claro, no con las variables de tema (`var(--text-primary)` etc.) que usa el resto de la app. FASE EQ3 ya modernizó SOLO la toolbar (iconos, pills). Falta el cuerpo de la tabla. Alcance explícito del usuario: diseño y colores únicamente, NUNCA tocar los cálculos, el matching de meses, ni ningún dato mostrado.
- [ ] Enrich Data ("Completar información" / `AccountReviewModal`, `EnrichModal`, `lib/dataCompleteness.js`): mejorar y hacer más inteligente. Sin alcance definido todavía, pendiente de discutir con el usuario qué significa "más inteligente" en este flujo antes de implementar.
