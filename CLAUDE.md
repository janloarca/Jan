...
- [ ] (usuario) Google sign-in: el lado de código está agotado (FASE FV/FZ0: helper same-origin + gesto + getRedirectResult + error detallado en el banner); lo que falta es CONSOLA: (1) Firebase Console > Authentication > Settings > Authorized domains con chispu.xyz y www.chispu.xyz; (2) Google Cloud Console > Credentials > OAuth client web: https://chispu.xyz en JavaScript origins y https://chispu.xyz/__/auth/handler en redirect URIs. Después probar en iPhone Y en una compu (discriminador Safari vs config) y mandar el banner nuevo si falla.
