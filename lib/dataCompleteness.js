...
export function analyzeDataCompleteness({ items = [], transactions = [], lots = [], convert, baseCurrency = 'USD', now } = {}) {
  ...
  for (const it of items) {
    ...
    // currentPrice never synced or was left blank shows as zero on every
    // card, as if the position vanished. Placed BEFORE the dust-floor
    // `continue` below on purpose: the bug IS that its value reads as dust,
    // so the floor would hide exactly the case this exists to catch.
    if (!broker && isTracked(it) && isMarket(it) && (Number(it.quantity) || 0) > 0 && !((Number(it.currentPrice) || 0) > 0)) {
    ...
    // assumes it cost $0, inflating the return on every P&L card.
    if (isMarket(it) && (Number(it.quantity) || 0) > 0 && (Number(it.currentPrice) || 0) > 0 && !((Number(it.purchasePrice) || 0) > 0)) {
