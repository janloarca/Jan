...
function effectivePrice(it, marketPrices) {
...
function itemBalance(it, marketPrices) {
  ...
  const price = effectivePrice(it, marketPrices) || Number(it.purchasePrice) || 0
...
export function analyzeDataCompleteness({ items = [], transactions = [], lots = [], convert, baseCurrency = 'USD', now, marketPrices = null } = {}) {
  ...
  for (const it of items) {
    ...
    const balance = itemBalance(it, marketPrices)
    ...
    if (!broker && isTracked(it) && isMarket(it) && (Number(it.quantity) || 0) > 0 && !(effectivePrice(it, marketPrices) > 0)) {
    ...
    if (isMarket(it) && (Number(it.quantity) || 0) > 0 && effectivePrice(it, marketPrices) > 0 && !((Number(it.purchasePrice) || 0) > 0)) {
  ...
  for (const [sym, g] of marketBySym) {
    ...
    if (gap > g.qty * 0.01) {
      const biggest = g.items.reduce((a, b) => (itemBalance(a, marketPrices) >= itemBalance(b, marketPrices) ? a : b))
  ...
  for (const it of items) {
    ...
    const w = Math.max(toBase(itemBalance(it, marketPrices), itemCurrency(it)), 1)
    weightSum += w
