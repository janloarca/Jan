...
import { isExcludedFromNetWorth, getGeographyFromItem } from '@/components/dashboard/utils'
  ...
  for (const [sym, g] of marketBySym) {
    ...
    if (gap > g.qty * 0.01) {
      ...
      push('uncovered-shares', 'medium', biggest.id,
        `${fmtQty(gap)} de ${fmtQty(g.qty)} unidades de ${sym} no tienen registro de compra: confirma la fecha de adquisición del activo para anclar su historial.`,
        `${fmtQty(gap)} of ${fmtQty(g.qty)} ${sym} units have no purchase record: confirm the asset's acquisition date to anchor its history.`,
        { kind: 'edit-item' })
    }
  }
