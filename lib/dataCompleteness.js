...
export function analyzeDataCompleteness({ items = [], transactions = [], lots = [], convert, baseCurrency = 'USD', now, marketPrices = null, resolvedPrices = null, pricesReady = true, incomeVerification = null } = {}) {
  ...
  // `idSuffix` (opcional): se agrega al id computado SIN tocar `itemId`, que
  // sigue siendo el id real del ítem (runAction/applySuggestion lo necesitan
  // tal cual para encontrar el ítem). Sirve para bucketizar un hallazgo cuya
  // "situación" cambia en pasos discretos (stale-value, por tiempo
  // transcurrido) sin volver a ofrecerlo por cada centavo/día que se mueva —
  // mismo patrón que NotificationCenter ya usa (FASE ND) para sus ids de
  // vencimiento, en vez de un TTL/expiry genérico.
  const push = (code, severity, itemId, textEs, textEn, action, suggestion, idSuffix) => {
    findings.push({ id: `${code}:${itemId || 'global'}${idSuffix ? ':' + idSuffix : ''}`, code, severity, itemId: itemId || null, textEs, textEn, action, suggestion: suggestion || null })
  }
  ...
  for (const tx of transactions) {
    ...
    if (ty === 'DEPOSIT' && tx._linkedItemId) {
      ...
      // FASE OE. El lado del DESTINO se lee con transferCredit, la única
      // definición (FASE JD): en una transferencia cruzada el banco acreditó
      // `_toAmount` en SU moneda (con su spread), no el monto enviado
      // convertido a la tasa de la app. Sin esto, "faltan aportes por $X" se
      // calculaba sobre dinero que nunca llegó a la cuenta (prellenaba 675.32
      // donde faltaban 700). Una fila vieja sin `_toAmount` cae al monto
      // enviado, que es lo que esta línea hacía antes.
      if (tx._linkedItemId) {
        const credit = transferCredit(tx)
        const dest = byId.get(tx._linkedItemId) || {}
        touch(tx._linkedItemId, cv(credit.amount, credit.currency || txCur, itemCurrency(dest)), ts)
      }
    } else if (ty === 'DIVIDEND') {
  ...
  for (const it of items) {
    ...
    // isTracked(it): a diferencia de sus hermanos pre-gate (no-market-price,
    // income-mismatch), este check no tenía su propio chequeo de isTracked —
    ...
    if (!broker && isTracked(it) && !it.acquisitionDate) {
      ...
      push('no-acq-date', 'high', it.id,
        ...
        { kind: 'edit-item', field: 'acquisitionDate' }, suggestion)
    }
    ...
    if (pricesReady && !broker && isTracked(it) && isMarket(it) && (Number(it.quantity) || 0) > 0 && !(effectivePrice(it, marketPrices, resolvedPrices) > 0)) {
      ...
      if (fallback > 0) {
        push('no-market-price-stale', 'medium', it.id,
          ...
          { kind: 'edit-item', field: 'currentValue' })
      } else {
    ...
    if (verdict && verdict.status === 'mismatch') {
      ...
      push('income-mismatch', 'medium', it.id,
        ...
        { kind: 'edit-item', field: 'incomeConfig' })
    }
    ...
    if (isMarket(it) && (Number(it.quantity) || 0) > 0 && effectivePrice(it, marketPrices, resolvedPrices) > 0 && !((Number(it.purchasePrice) || 0) > 0)) {
      push('no-cost-basis', 'medium', it.id,
        ...
        { kind: 'edit-item', field: 'purchasePrice' })
    }
    ...
    // out of all three.
    // ⛔ action: null, verificado y no supuesto: NO existe ningún <input> para
    // editar el símbolo en EditAccountModal.jsx (grep exhaustivo, 21 matches,
    // todos display-only). Ofrecer "edit-item" llevaba a una edición genérica
    // que no podía resolver nada; el usuario confirmó no agregar ese campo.
    ...
    if (!it.institution) {
      push('no-institution', 'low', it.id,
        ...
        { kind: 'edit-item', field: 'institution' })
    }

    const hasIncome = (Number(it.incomeRate) || 0) > 0 || (Number(it.incomeAmount) || 0) > 0 || (Number(it.dividendYield) || 0) > 0

    // 6b. FASE OA. Un BONO sin ningun calendario de pago. El recorrido guiado
    // no pregunta el ingreso (pide lo minimo y el resto lo reclama este
    // boletin), asi que un bono creado ahi nace mudo: la Hoja, Ingresos
    // Pasivos y el motor de cupones no tienen nada que leer, y desde afuera
    // "no paga" y "no se configuro" se ven igual. Solo bonos: una accion
    // trae su dividendo de la cotizacion, y un alternativo puede no pagar.
    if (/bond|bono/i.test(it.type || '') && !hasIncome && it.rateType !== 'variable' && it.rateType !== 'continuous') {
      push('bond-no-income', 'medium', it.id,
        `${name} no tiene cupon configurado: sus pagos no entran a la Hoja ni a Ingresos Pasivos. Si paga interes, pon la tasa o el monto y los meses.`,
        `${name} has no coupon configured: its payments never reach the Sheet or Passive Income. If it pays interest, set the rate or amount and the months.`,
        { kind: 'edit-item', field: 'incomeConfig' })
    }
    ...
    if (hasIncome && !(Array.isArray(it.incomeMonths) && it.incomeMonths.length > 0)) {
      ...
      push('income-no-months', 'medium', it.id,
        ...
        { kind: 'edit-item', field: 'incomeMonths' }, suggestion)
    }
    ...
    if (hasIncome && it.dividendAction !== 'reinvest' && !it.incomeDestination) {
      ...
      push('income-no-dest', 'low', it.id,
        ...
        { kind: 'edit-item', field: 'incomeDestination' }, suggestion)
    }
    ...
    // Código propio ('broken-link-income'), separado del de #9b: los dos
    // compartían 'broken-link' y con AMBAS referencias rotas en el mismo ítem
    // colisionaban en el mismo id (`code:itemId`) — descarte/aplicado cruzado
    // entre dos hallazgos distintos.
    if (it.incomeDestination && !resolveRef(it.incomeDestination)) {
      push('broken-link-income', 'medium', it.id,
        ...
        { kind: 'edit-item', field: 'incomeDestination' }, {
    ...
    // Código propio ('broken-link-debt'): ver el comentario de #9 arriba.
    if (it.linkedDebtId && !resolveRef(it.linkedDebtId)) {
      push('broken-link-debt', 'medium', it.id,
        ...
        { kind: 'edit-item', field: 'linkedDebtId' }, {
    ...
    if (!it.assetCountry && !isMarket(it) && getGeographyFromItem(it) === 'US') {
      push('no-country', 'low', it.id,
        ...
        { kind: 'edit-item', field: 'assetCountry' })
    }
    ...
    if (maturity && maturity < nowDate) {
      push('past-maturity', 'medium', it.id,
        ...
        { kind: 'edit-item', field: 'maturityDate' })
    }
    ...
    if (!isMarket(it)) {
        ...
        if (!originConfirmed && balance > 0 && explained < 0) {
          if (toBase(-explained, cur) > UNEXPLAINED_FLOOR_BASE) {
            push('flows-exceed-balance', 'medium', it.id,
              ...
              { kind: 'edit-item', field: 'movements' })
          }
      ...
      if (!it.isIlliquid && lastTouch > 0 && (nowDate.getTime() - lastTouch) > STALE_DAYS * 86400000) {
        // Bucketizado por tiempo transcurrido (mismo patrón que
        // NotificationCenter/FASE ND): cada STALE_DAYS de más es una
        // "situación" nueva y se vuelve a preguntar; mientras el mismo bucket
        // siga vigente, el id no cambia y un descarte se mantiene descartado.
        const staleBucket = Math.floor((nowDate.getTime() - lastTouch) / (STALE_DAYS * 86400000))
  ...
  for (const [sym, g] of marketBySym) {
    ...
    if (gap > g.qty * 0.01) {
      ...
      // ⛔ action: null, y no `{kind:'edit-item'}` como decía el código antes de
      // esta pasada: el comentario de arriba YA afirmaba que la acción es
      // irresoluble ("ningún componente del repo edita lotes"), pero el push
      // seguía ofreciendo un botón que llevaba a una edición genérica. Ahora el
      // código deja de contradecir a su propio comentario.
