...
export function analyzeDataCompleteness({ items = [], transactions = [], lots = [], convert, baseCurrency = 'USD', now, marketPrices = null, resolvedPrices = null } = {}) {
  ...
  for (const it of items) {
    ...
    if (!broker && isTracked(it) && isMarket(it) && (Number(it.quantity) || 0) > 0 && !(effectivePrice(it, marketPrices, resolvedPrices) > 0)) {
