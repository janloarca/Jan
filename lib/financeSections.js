...
import { buildSpendingRhythm, windowRange, WIDEST_RHYTHM_MONTHS } from '@/lib/spendingByWeekday'
...
// ¿Hay algo que el ritmo de gasto pueda mostrar? La card se auto-oculta con
// esta MISMA función, así que la pregunta y la respuesta no pueden divergir.
//
// A PROPÓSITO NO entra a `commitmentsHaveContent`: esa contesta "¿a qué estoy
// amarrado hacia adelante?" y un gasto suelto no es un compromiso. El ritmo es
// una lectura de patrón, no una obligación, y meterlo ahí haría que un solo
// café encendiera la sección de compromisos.
//
// Se juzga con la ventana MÁS ANCHA y no con la seleccionada: si no, elegir
// "este mes" en un mes vacío haría desaparecer la card entera junto con su
// selector, y no habría forma de volver.
export function weekdayRhythmHasContent(transactions, { convert = null, today = null, now = new Date() } = {}) {
  const range = windowRange(WIDEST_RHYTHM_MONTHS, today || nowDateKey(now))
  if (!range) return false
  return buildSpendingRhythm(transactions || [], {
    fromDate: range.fromDate, toDate: range.toDate, convert,
