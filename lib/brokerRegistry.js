...
export const IBKR_DISCONNECTED_FIELDS = {
  ...
  // ⛔ FASE KN. El presupuesto de intentos del día y el último mensaje crudo del
  // servidor: los dos describen la conexión anterior. Conservar el presupuesto
  // dejaría a una cuenta recién conectada sin intentos hasta mañana.
  _ibkrAttemptsToday: null,
  _ibkrLastUpstreamError: null,
  // ⛔ FASE KK. El resumen forense del último sync (días de NAV, trades,
  // flujos, costos, secciones del XML). Describe una CONEXIÓN, no hechos del
  // broker, así que sobrevivir a un desconectar lo convierte en un fósil: la
  // cuenta reconectada mostraba "0 flujos · 0 costos" y "+317 operaciones" de
  // una era anterior como si fueran de ahora, mientras la línea "Última sync"
  // de al lado no aparecía (esos timestamps SÍ se limpiaban). Distinto de
  // `settings.ibkrPaSummary`, que sí se conserva a propósito (FASE GO): eso
  // describe hechos reales de la cuenta en el broker, no de nuestra conexión.
  _ibkrLastSyncSummary: null,
}
