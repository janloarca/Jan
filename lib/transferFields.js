...
function balanceFields(item, target) {
  ...
  if (value === 0) {
    // ⛔ FASE OM: ver el comentario de la rama de abajo. Vaciar la cuenta
    // también es una foto nueva ("hoy vale $0"), y sellarla es lo que impide
    // que un retiro que la deja en cero se lea como un interés negativo.
    return isBankLike(item)
      ? { quantity: 0, currentPrice: 0, purchasePrice: 0, balanceAsOf: todayLocalISO() }
      : { quantity: 0 }
  }
  ...
  if (isBankLike(item)) {
    ...
    // ⛔ FASE OM (extensión de la spec congelada liquidFundYield.js, sección
    // 5, OK explícito del usuario). Una transferencia (TransferModal, en
    // cualquiera de los dos lados) o el crédito de una venta (SellModal) es
    // exactamente la misma FOTO que las otras tres puertas ya sellan: hasta
    // acá esta función movía el saldo sin fecha, así que el motor de
    // rendimiento deducido (lib/liquidYield.js) medía contra un
    // `balanceAsOf` viejo y leía el monto transferido como interés que la
    // cuenta nunca generó. Sella HOY, nunca la fecha elegida en el modal
    // (que se puede backdatear).
    fields.balanceAsOf = todayLocalISO()
    return fields
  }
