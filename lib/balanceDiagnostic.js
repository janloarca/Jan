...
import { looksEmptied } from '@/lib/zeroQuantityHeal'
...
function verdictOf(item, fields) {
  ...
  // Una cuenta VACIADA se juzga con su propia vara, y con la MISMA definición
  // que usan los sanadores (`looksEmptied`): si el diagnóstico marcara como
  // defecto un estado que los sanadores dejan quieto a propósito, las dos
  // superficies se contradirían y volvería la ambigüedad que este archivo
  // existe para cerrar.
  if (looksEmptied(item)) {
    if (value !== 0) {
    ...
    // Vaciada y en cero: eso es lo correcto, no un defecto. Se dice igual
    // (con el residuo a la vista si lo hay) porque "la vacié" es justo lo que
    // el usuario viene a confirmar.
    const residuo = FALLBACK_FIELDS.filter((k) => Number(item?.[k]) > 0)
    return {
      code: 'vaciada',
      ok: true,
      es: `vaciada y en cero, correcto${residuo.length ? ` (queda un residuo en ${residuo.join('/')}, inofensivo con la cantidad en 0)` : ''}`,
      en: `emptied and at zero, correct${residuo.length ? ` (leftover in ${residuo.join('/')}, harmless while quantity is 0)` : ''}`,
    }
  }
...
export function balanceDiagnosticText(rows, { build = null, lang = 'es' } = {}) {
  ...
  for (const r of rows || []) {
    ...
    if (r.verdict) lines.push(`  ${r.verdict.ok ? '✓' : '⚠'} ${es ? r.verdict.es : r.verdict.en}`)
    lines.push('')
  }
  return lines.join('\n')
}
