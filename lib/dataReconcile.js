...
import { isDemoItem } from './demoData'
...
export function assessDataSources({ items = [], transactions = [], snapshots = [] } = {}) {
  // El filtro de demo preguntaba por `it._demo`, un campo que NINGÚN escritor
  // del repo pone: la convención es `_source: 'demo'` (lib/demoData.js), la
  // misma que usan el borrado selectivo, el veto a los snapshots y el banner de
  // salida. O sea el filtro era INERTE y el puntaje de calidad de datos se
  // calculaba sobre las posiciones de ejemplo, diciéndole a quien está mirando
  // una demo qué tan completos están "sus" datos. Se pregunta con el predicado
  // compartido y no con una segunda copia, que es cómo nació esto.
  const positions = (items || []).filter((it) => it && !isDemoItem(it))
