...
export function enrichmentFor(row, existing) {
  ...
  // Solo si no está ya puesto, y eso NO es cosmético: escribirlo siempre hacía
  // que `updates` nunca estuviera vacío, o sea el guard del caller
  // (`Object.keys(updates).length === 0`) no podía dispararse NUNCA. En un
  // re-import del mismo estado eso es un write a Firestore por CADA fila
  // confirmada (79 en un estado real) sobre datos que no cambiaron, y la
  // pantalla final reportando "N actualizadas" cuando no se actualizó nada.
  // Es la misma lección que el neteo: emparejar sí, escribir no.
  //
  // El campo se conserva porque es un hecho real (el banco confirmó esta fila,
  // que no es lo mismo que "solo la capturó el atajo"), pero hoy NINGUNA
  // superficie lo lee; si sigue así, borrarlo es mejor que dejarlo sugiriendo
  // una capacidad que no existe.
  if (existing._confirmedBy !== 'statement') updates._confirmedBy = 'statement'
