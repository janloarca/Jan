...
// Which card a row belongs to. Rows parsed from a statement carry `cardKey`;
// rows imported before that field existed only carry the account label the
// import stamped ("Bi (Contecnica) •9856"), so the last four digits are read
// back out of it rather than leaving that older data unprotected.
function cardOf(tx) {
...
function cardsConflict(row, tx) {
  const a = cardOf(row), b = cardOf(tx)
...
function classify({ relation, days, sim, bankText }) {
  if (relation === 'exact') {
...
export function reconcileStatement(statementRows, existing = []) {
  ...
  rows.forEach((row, ri) => {
    for (const cand of pool) {
      ...
      const verdict = classify({ relation, days, sim, bankText: isBankText(cand.tx) })
      if (!verdict) continue
  ...
  if (dates.length) {
    ...
    orphans = pool
      .filter((c) => !c.claimed)
      .filter((c) => {
        const own = cardOf(c.tx)
        return own == null || statementCard == null || own === statementCard
      })
      .filter((c) => {
