...
export const BROKER_HOWTO = {
  ibkr: {
    ...
    // FASE GM parte 2: título = SOLO la acción, en una línea; toda especificación
    // (qué secciones, qué marcar adentro, por qué) vive en `detail`, detrás del
    // botón "i" — el usuario lo pidió explícito ("es muy agobiante" la pared de
    // 7 instrucciones). Antes el título YA era una oración con la enumeración
    // completa adentro, así que ni colapsando se sentía corto.
    csv: {
      steps: [
        { es: 'Andá a "Performance & Reports" → "Flex Queries"', en: 'Go to "Performance & Reports" → "Flex Queries"',
        ...
        { es: 'Creá un Activity Flex Query con 5 secciones', en: 'Create an Activity Flex Query with 5 sections',
          detail: { es: 'Open Positions, Trades, Cash Transactions, Cash Report y Net Asset Value (NAV) in Base. La sección NAV trae el valor diario de tu cuenta: sin ella el historial y el % de retorno arrancan en blanco (dentro del archivo esa sección se llama "EquitySummaryByReportDateInBase": si la ves ahí, es la correcta). Cash Report trae tu efectivo: sin ella no aparece en el portafolio. En CADA sección, marca "Select All" en la lista de campos: agregar la sección no incluye sus columnas, y una sección con filas pero sin los campos que leemos llega vacía. Y dentro de "Cash Transactions" marca también el tipo que habla de depósitos y retiros (aparece como "Deposits/Withdrawals"): sin esos movimientos tus depósitos se cuentan como ganancia en los retornos.', en: 'Open Positions, Trades, Cash Transactions, Cash Report and Net Asset Value (NAV) in Base. The NAV section brings your daily account value: without it the history and the % return chart start blank (inside the file that section is called "EquitySummaryByReportDateInBase": if you see it there, it is the right one). Cash Report brings your cash: without it it never shows in the portfolio. In EVERY section, tick "Select All" in the field list: adding the section does not include its columns, and a section with rows but without the fields we read arrives empty. And inside "Cash Transactions" also tick the type about deposits and withdrawals (it shows as "Deposits/Withdrawals"): without those movements your deposits count as gains in your returns.' } },
        { es: 'Configurá "Open Positions" para traer lotes con fecha', en: 'Set "Open Positions" to bring dated lots',
          detail: { es: 'Pon "Level of Detail" en "Lots" y marca "Open Date Time", así como la columna "Asset Class": sin ella los bonos y el cash se importan como acciones, y cada posición llega con su fecha real de compra.', en: 'Set "Level of Detail" to "Lots" and tick "Open Date Time", plus the "Asset Class" column: without it bonds and cash import as stocks, and each position arrives with its real purchase date.' } },
        { es: 'Período: desde hace un año hasta hoy, en XML', en: 'Period: from a year ago to today, in XML',
          detail: { es: '"Custom Date Range". El tope de 365 días es de IBKR, no nuestro, y es por CUENTA, no por archivo: no existe un rango que llegue más atrás, ni pidiendo un año calendario distinto ni con varios archivos.', en: '"Custom Date Range". The 365-day cap is IBKR\'s, not ours, and it is per ACCOUNT, not per file: no range reaches further back, not by asking for a different calendar year and not with several files.' } },
        { es: 'Descargá el archivo y subilo aquí', en: 'Download the file and upload it here' },
        // journeyStep: esta instrucción ES un paso propio del viaje guiado de
        // IBKR (app/dashboard/page.jsx). Dentro del viaje se omite de esta
        // lista: repetir como sub-instrucción algo que el usuario va a hacer
        // en la pantalla siguiente alarga la pared de texto sin agregar nada.
        // Fuera del viaje (importar un archivo suelto) se queda, porque ahí
        // nadie le va a ofrecer esos pasos después.
        { es: 'Para lo anterior a esos ~365 días, transcribilo una vez', en: 'For anything before those ~365 days, transcribe it once', journeyStep: 3,
          detail: { es: '"Portfolio Analyst" → "Holdings" → "Since Inception" + "Quarterly", quita los benchmarks y copia el valor de cada trimestre. Portfolio Analyst SÍ muestra toda tu historia, pero solo como gráfica: no hay exportación detrás. Son unos 4 números por año y solo el total de la cuenta. Chispu tiene una pantalla para escribirlos.', en: '"Portfolio Analyst" → "Holdings" → "Since Inception" + "Quarterly", remove the benchmarks and copy each quarter\'s value. Portfolio Analyst DOES show your whole history, but only as a picture: there is no export behind it. It is about 4 numbers per year, only the account total. Chispu has a screen to type them in.' } },
        { es: 'Copiá los retornos que IBKR te muestra hoy', en: 'Copy the returns IBKR shows you today', journeyStep: 4,
          detail: { es: 'Hoy, 1W, MTD, 1M, 3M, YTD, 1Y y desde el inicio. Cada porcentaje ancla la curva en su fecha, así que lo que quede estimado entre medio se apoya en números reales de tu broker en vez de en una reconstrucción nuestra.', en: 'Today, 1W, MTD, 1M, 3M, YTD, 1Y and since inception. Each percentage anchors the curve at its own date, so whatever stays estimated in between leans on real numbers from your broker instead of on a reconstruction of ours.' } },
      ],
      note: { es: 'Un Flex Query solo alcanza los últimos ~365 días, sin importar el rango que pidas: es un límite de IBKR por cuenta, no por archivo. Al terminar, el resumen final te dice cuántos días de historial se importaron. Si dice 0, al archivo le falta "Net Asset Value (NAV) in Base": generalo de nuevo incluyéndola.', en: 'A Flex Query only reaches the last ~365 days, no matter what range you request: it is IBKR\'s limit per account, not per file. When done, the final summary tells you how many days of history were imported. If it says 0, the file is missing "Net Asset Value (NAV) in Base": generate it again including it.' },
    },
    api: {
      steps: [
        { es: 'Andá a "Performance & Reports" → "Flex Queries"', en: 'Go to "Performance & Reports" → "Flex Queries"' },
        { es: 'Creá un Activity Flex Query con 5 secciones', en: 'Create an Activity Flex Query with 5 sections',
          detail: { es: 'Open Positions, Trades, Cash Transactions, Cash Report y Net Asset Value (NAV) in Base. La sección NAV trae el valor diario de tu cuenta (dentro del archivo se llama "EquitySummaryByReportDateInBase": si la ves ahí, es la correcta). Cash Report trae tu efectivo: sin ella no aparece en el portafolio. En CADA sección, marca "Select All" en la lista de campos: agregar la sección no incluye sus columnas, y una sección con filas pero sin los campos que leemos llega vacía. Y dentro de "Cash Transactions" marca también el tipo que habla de depósitos y retiros (aparece como "Deposits/Withdrawals"): sin esos movimientos tus depósitos se cuentan como ganancia en los retornos.', en: 'Open Positions, Trades, Cash Transactions, Cash Report and Net Asset Value (NAV) in Base. The NAV section brings your daily account value (inside the file it is called "EquitySummaryByReportDateInBase": if you see it there, it is the right one). Cash Report brings your cash: without it it never shows in the portfolio. In EVERY section, tick "Select All" in the field list: adding the section does not include its columns, and a section with rows but without the fields we read arrives empty. And inside "Cash Transactions" also tick the type about deposits and withdrawals (it shows as "Deposits/Withdrawals"): without those movements your deposits count as gains in your returns.' } },
        { es: 'Configurá "Open Positions" para traer lotes con fecha', en: 'Set "Open Positions" to bring dated lots',
          detail: { es: 'Pon "Level of Detail" en "Lots" y marca "Open Date Time", así como la columna "Asset Class": sin ella los bonos y el cash se importan como acciones, y cada posición llega con su fecha real de compra aunque el período sea corto.', en: 'Set "Level of Detail" to "Lots" and tick "Open Date Time", plus the "Asset Class" column: without it bonds and cash import as stocks, and each position arrives with its real purchase date even if the period is short.' } },
        { es: 'Período: "Last 365 Calendar Days", y formato XML', en: 'Period: "Last 365 Calendar Days", and XML format',
          detail: { es: 'Es el máximo real que IBKR entrega, así que no hace falta tocar el rango a mano: cada sync trae los ~365 días vigentes en ese momento, ventana que se renueva sola. El formato TIENE que ser XML: el Flex Web Service devuelve el formato que tenga guardado la query, y con CSV la sincronización automática se queda esperando un reporte que nunca va a poder leer.', en: 'It is the real maximum IBKR hands over, so there is no need to touch the range by hand: each sync brings whatever the trailing ~365 days are at that moment, a window that renews itself. The format MUST be XML: the Flex Web Service returns whatever format the query has saved, and with CSV the automatic sync waits forever for a report it can never read.' } },
        { es: 'Generá el Token', en: 'Generate the Token',
          detail: { es: 'En la MISMA página de Flex Queries, junto a "Flex Web Service" tocá el engranaje ⚙, activalo y tocá "Generate a New Token". Copialo de inmediato (no se vuelve a mostrar): el token NO está en Settings.', en: 'On the SAME Flex Queries page, next to "Flex Web Service" click the gear ⚙, enable it, then click "Generate a New Token". Copy it immediately (it will not show again): the token is NOT under Settings.' } },
        { es: 'Copiá el Query ID', en: 'Copy the Query ID',
          detail: { es: 'Es el número junto a tu query guardado en la lista de Flex Queries. El token y el Query ID son dos cosas distintas.', en: 'It is the number next to your saved query in the Flex Queries list. The token and the Query ID are two different things.' } },
      ],
      note: { es: 'Solo lectura: el token no puede operar ni mover dinero. Un Flex Query alcanza como máximo ~365 días atrás desde hoy: es un límite de IBKR por cuenta, no por archivo. Para historial más viejo, transcribe los trimestres desde Portfolio Analyst.', en: 'Read-only: the token cannot trade or move money. A Flex Query reaches at most ~365 days back from today: it is IBKR\'s limit per account, not per file. For older history, transcribe the quarters from Portfolio Analyst.' },
    },
  },
