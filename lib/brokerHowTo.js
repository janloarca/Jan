  ...
  ibkr: {
    ...
    csv: {
      steps: [
        ...
        { es: 'Cada archivo cubre máximo 365 días: descarga UNO por año calendario (1 ene a 31 dic) desde que abriste la cuenta, en formato XML', en: 'Each file covers at most 365 days: download ONE per calendar year (Jan 1 to Dec 31) since you opened the account, in XML format',
          detail: { es: 'El tope de 365 días es de IBKR, no nuestro: no hay forma de pedir más en un solo archivo. Usa "Custom Date Range" y repite por año. El año en curso no lo bajes aquí: lo cubre la sincronización automática (o súbelo también si no usas la API).', en: 'The 365-day cap is IBKR\'s, not ours: there is no way to ask for more in one file. Use "Custom Date Range" and repeat per year. Skip the current year here: the auto-sync covers that one (or upload it too if you do not use the API).' } },
        ...
        { es: 'Si no puedes bajar un año, transcríbelo: "Portfolio Analyst" → "Holdings" → "Since Inception" + "Quarterly", quita los benchmarks y copia el valor de cada trimestre', en: 'If you cannot download a year, transcribe it: "Portfolio Analyst" → "Holdings" → "Since Inception" + "Quarterly", remove the benchmarks and copy each quarter\'s value',
    ...
    api: {
      ...
      note: { es: 'Solo lectura: el token no puede operar ni mover dinero. Un Flex Query alcanza máximo 365 días (límite de IBKR), así que esta query cubre SOLO el año en curso: para años anteriores usa la pestaña de subir archivo, una query por año, o transcribe los trimestres desde Portfolio Analyst. Cada enero, actualiza el rango de esta query a "1 de enero a hoy" del año nuevo, y sube el año que acaba de cerrar como historial.', en: 'Read-only: the token cannot trade or move money. A Flex Query reaches at most 365 days (IBKR\'s limit), so this query covers ONLY the current year: for prior years use the file-upload tab, one query per year, or transcribe the quarters from Portfolio Analyst. Each January, update this query\'s range to "January 1st to today" of the new year, and upload the year that just closed as history.' },
    },
  },
