  ...
  ibkr: {
    ...
    csv: {
      steps: [
        ...
        { es: 'Abre "Flex Queries" (no "Statements" → "Activity": ese formato no trae el valor diario de la cuenta, solo posiciones y trades)', en: 'Open "Flex Queries" (not "Statements" → "Activity": that format has no daily account value, only positions and trades)' },
        { es: 'Crea un "Activity Flex Query" con TODAS estas secciones: Open Positions, Trades, Cash Transactions, Cash Report y Equity Summary by Report Date in Base. Esta última es la que trae el valor diario: sin ella el historial y el % de retorno arrancan en blanco', en: 'Create an "Activity Flex Query" with ALL of these sections: Open Positions, Trades, Cash Transactions, Cash Report and Equity Summary by Report Date in Base. That last one is what brings the daily value: without it the history and the % return chart start blank' },
        { es: 'En "Open Positions", cambia "Level of Detail" a "Lots" y marca "Open Date Time", para que cada posición llegue con su fecha real de compra', en: 'Under "Open Positions", set "Level of Detail" to "Lots" and tick "Open Date Time", so every position arrives with its real purchase date' },
        { es: 'LO MÁS IMPORTANTE: un Flex Query cubre como máximo 365 días. Si tu cuenta tiene más de un año, usa "Custom Date Range" y descarga UNA query por cada año calendario completo (1 ene - 31 dic) desde que abriste la cuenta. El año en curso no lo bajes aquí: ese lo trae la sincronización automática de abajo', en: 'THE KEY STEP: a Flex Query covers at most 365 days. If your account is older than a year, use "Custom Date Range" and download ONE query per full calendar year (Jan 1 - Dec 31) since you opened the account. Skip the current year here: the auto-sync below covers that one' },
        { es: 'Formato: XML. Descarga cada año y súbelo aquí, uno por uno: se van sumando', en: 'Format: XML. Download each year and upload it here, one at a time: they add up' },
      ],
      note: { es: 'Si ya conectaste por API, al subir cada año elige "Enriquecer con historial": enlaza cada posición con la que ya tienes en vez de duplicarla. Sin la sección Equity Summary, el import "tiene éxito" pero deja el historial vacío en silencio: revisa que el resumen final diga cuántos días de valor trajo.', en: 'If you already connected via API, pick "Enrich with history" when uploading each year: it links each position to the one you already have instead of duplicating it. Without the Equity Summary section, the import "succeeds" but silently leaves history empty: check that the final summary shows how many value-days it brought in.' },
    },
    api: {
      steps: [
        { es: 'En IBKR: "Performance & Reports" → "Flex Queries"', en: 'In IBKR: "Performance & Reports" → "Flex Queries"' },
        { es: 'Crea un "Activity Flex Query" con: Open Positions, Trades, Cash Transactions, Cash Report y Equity Summary by Report Date in Base', en: 'Create an "Activity Flex Query" with: Open Positions, Trades, Cash Transactions, Cash Report and Equity Summary by Report Date in Base' },
        { es: 'En "Open Positions", cambia "Level of Detail" a "Lots" y marca el campo "Open Date Time". Así cada posición llega con su fecha real de compra, aunque el período sea corto', en: 'Under "Open Positions", set "Level of Detail" to "Lots" and tick the "Open Date Time" field. That way every position arrives with its real purchase date, even if the period is short' },
        { es: 'Período: "Custom Date Range" del 1 de enero del año en curso a hoy (NO "Last 365 Calendar Days": esa ventana se va moviendo y con el tiempo empieza a pisar los años que ya subiste como historial)', en: 'Period: "Custom Date Range" from January 1st of the current year to today (NOT "Last 365 Calendar Days": that window keeps sliding and eventually overlaps the years you already uploaded as history)' },
