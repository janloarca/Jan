...
export const BROKER_HOWTO = {
  ibkr: {
    ...
    csv: {
      steps: [
        ...
        { es: 'Crea un "Activity Flex Query" con TODAS estas secciones: Open Positions, Trades, Cash Transactions, Cash Report y Net Asset Value (NAV) in Base (en portales antiguos aparece como "Equity Summary by Report Date in Base": es la misma sección). Esta última es la que trae el valor diario: sin ella el historial y el % de retorno arrancan en blanco', en: 'Create an "Activity Flex Query" with ALL of these sections: Open Positions, Trades, Cash Transactions, Cash Report and Net Asset Value (NAV) in Base (on older portals it shows as "Equity Summary by Report Date in Base": it is the same section). That last one is what brings the daily value: without it the history and the % return chart start blank' },
        ...
        { es: 'LO MÁS IMPORTANTE: un Flex Query cubre como máximo 365 días. Si tu cuenta tiene más de un año, usa "Custom Date Range" y descarga UNA query por cada año calendario completo (1 ene - 31 dic) desde que abriste la cuenta. El año en curso no lo bajes aquí: ese lo trae la sincronización automática de abajo (o súbelo también si no usas la API)', en: 'THE KEY STEP: a Flex Query covers at most 365 days. If your account is older than a year, use "Custom Date Range" and download ONE query per full calendar year (Jan 1 - Dec 31) since you opened the account. Skip the current year here: the auto-sync below covers that one (or upload it too if you do not use the API)' },
        { es: 'Formato: XML. Descarga un XML por año y súbelos aquí uno por uno: se van sumando. A partir del segundo archivo elige "Enriquecer" para no duplicar posiciones.', en: 'Format: XML. Download one XML per year and upload them here one at a time: they add up. From the second file on, pick "Enrich" so positions are not duplicated.' },
      ],
    ...
    api: {
      steps: [
        ...
        { es: 'Crea un "Activity Flex Query" con: Open Positions, Trades, Cash Transactions, Cash Report y Net Asset Value (NAV) in Base (en portales antiguos aparece como "Equity Summary by Report Date in Base": es la misma sección)', en: 'Create an "Activity Flex Query" with: Open Positions, Trades, Cash Transactions, Cash Report and Net Asset Value (NAV) in Base (on older portals it shows as "Equity Summary by Report Date in Base": it is the same section)' },
  ...
  hapi: {
    ...
    csv: {
      steps: [
        ...
        { es: 'Sube ese PDF aquí mismo: Chispu lo lee con IA y lo convierte a nuestra plantilla. Tú revisas y confirmas cada dato antes de que se importe nada', en: 'Upload that PDF right here: Chispu reads it with AI and converts it to our template. You review and confirm every value before anything is imported' },
      ],
  ...
  alpaca: {
    ...
    api: {
      steps: [
        { es: 'Entra a tu Dashboard de Alpaca', en: 'Go to your Alpaca Dashboard' },
        { es: 'Confirma que estás en el entorno correcto: "Paper" (prueba) o "Live" (real), cada uno tiene sus propias keys', en: 'Confirm you\'re in the right environment: "Paper" (test) or "Live" (real), each one has its own keys' },
  ...
  vanguard: {
    ...
    csv: {
      steps: [
        ...
        { es: 'Para movimientos: "My Accounts" → "Transaction History" → "Download" → elige formato CSV, rango de fechas y cuentas', en: 'For movements: "My Accounts" → "Transaction History" → "Download" → choose CSV format, date range and accounts' },
      ],
  ...
  degiro: {
    ...
    csv: {
      steps: [
        { es: 'Entra a DEGIRO → "Cartera" (Portfolio)', en: 'Go to DEGIRO → "Cartera" (Portfolio)' },
  ...
  traderepublic: {
    ...
    csv: {
      steps: [
        ...
        { es: 'Entra a "Actividad" (Activity) → "Extractos" (Statements)', en: 'Go to "Activity" → "Extracts" (Statements)' },
        { es: 'Busca "Extracto de cuenta" (Account Statement)', en: 'Look for "Extracto de cuenta" (Account Statement)' },
      ...
      note: { es: 'Trade Republic entrega PDF; súbelo aquí directamente y Chispu lo lee con IA. También puedes usar un conversor de PDF a Excel y subir el CSV.', en: 'Trade Republic delivers PDF; upload it right here and Chispu reads it with AI. You can also use a PDF-to-Excel converter and upload the CSV.' },
    },
    api: null,
  },
  ...
  kraken: {
    ...
    csv: {
      steps: [
        { es: 'Entra a Kraken → "History" (historial)', en: 'Go to Kraken → "History" (history)' },
  ...
  binance: {
    ...
    api: {
      steps: [
        { es: 'Entra a Binance → ícono de perfil → "API Management"', en: 'Go to Binance → profile icon → "API Management"' },
  ...
  bitso: {
    ...
    csv: {
      steps: [
        ...
        { es: 'Filtra por fecha y tipo de movimiento', en: 'Filter by date and movement type' },
