...
export const IBKR_STEPS = [
  ...
  },
  {
    ...
    desc: {
      es: 'Hoy, 1W, MTD, 1M, 3M, YTD, 1Y y desde el inicio: ancla tu curva a los números reales de tu broker. En PortfolioAnalyst cambia "Performance Measure" a MWR antes de copiar (nuestro cálculo es money-weighted; si no hiciste depósitos ni retiros en el período, TWR y MWR coinciden).',
      en: 'Today, 1W, MTD, 1M, 3M, YTD, 1Y and since inception: anchors your curve to your broker\'s real numbers. In PortfolioAnalyst switch "Performance Measure" to MWR before copying (our math is money-weighted; with no deposits or withdrawals in the period, TWR and MWR match).',
    },
    done: ({ hasIbkrCalibration }) => !!hasIbkrCalibration,
    // FASE NT. Una calibración de IBKR que la app está IGNORANDO (FASE NN: el
    // % copiado no cuadra con el NAV que el broker reporta para esa fecha).
    // NO cambia `done`: re-copiar el % es lo correcto y el paso sigue
    // pendiente. Lo que cambia es lo que se DICE: "Pendiente" a secas sobre un
    // paso que el usuario ya hizo se lee como que la app deshizo su trabajo.
    // Devuelve el texto bilingüe o null; el panel lo pinta en ámbar.
    attention: ({ ibkrCalibrationIgnored }) => ((ibkrCalibrationIgnored || 0) > 0
      ? {
        es: `Copiaste ${ibkrCalibrationIgnored === 1 ? 'un %' : `${ibkrCalibrationIgnored} %`} pero la app lo está ignorando: el valor que sale de ese % no cuadra con el NAV que IBKR reporta para esa fecha. No se deshizo nada: cópialo otra vez desde PortfolioAnalyst (con Performance Measure en MWR).`,
        en: `You copied ${ibkrCalibrationIgnored === 1 ? 'a %' : `${ibkrCalibrationIgnored} %`} but the app is ignoring it: the value that % solves to does not match the NAV IBKR reports for that date. Nothing was undone: copy it again from PortfolioAnalyst (with Performance Measure set to MWR).`,
      }
      : null),
  },
]
