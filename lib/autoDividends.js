...
// ⛔ EXPORTADA porque hay DOS reglas distintas en el repo para "¿este pago es de
// este activo?" y difieren justo donde importa: `getDividendIncomeByItem`
// (lógica congelada) exige `_linkedItemId` a secas, y un dividendo importado de
// IBKR NO lo trae (lib/ibkrSync.js lo escribe con símbolo, `_ibkrTxnId` y
// `_source`, nunca con vínculo). Esta sí cae al símbolo, y por eso el motor
// automático nunca duplicó un pago que el broker ya había traído.
//
// Todo consumidor nuevo usa ESTA, no una tercera copia: una segunda definición
// del emparejamiento es exactamente cómo el motor automático y el de retorno
// terminarían discrepando sobre el mismo dinero.
export const dividendBelongsToItem = (tx, item) => {
...
const belongsTo = dividendBelongsToItem
...
export function dividendCreditTarget(tx, items) {
  ...
  // FASE OB. La fila DICE a qué cuenta llegó (`_destinationItemId`, lo
  // estampa un pago registrado a mano desde "Registrar movimiento"), y eso
  // manda sobre el destino CONFIGURADO en el activo: eran dos respuestas a la
  // misma pregunta y ganaba la equivocada, así que borrar un pago tecleado
  // debitaba la cuenta que el activo tiene puesta hoy y no la que de verdad
  // lo recibió. Sin `_destinationItemId` se conserva la regla de siempre.
  //
  // ⛔ EL HUECO QUE QUEDA, Y POR QUÉ NO SE PUEDE CERRAR LEYENDO (FASE ON).
  // Esta función NO consulta `dividendAction`, así que un cambio de modo no la
  // afecta directamente: solo la bandera `_reinvested`, que es un hecho del
  // pasado. Pero el editor LIMPIA `incomeDestination` al elegir "reinvertir"
  // (FASE OB), y un pago que escribió el motor no lleva `_destinationItemId`,
  // así que después de ese cambio `destKey` queda vacío y esto devuelve null:
  // "nunca movió un saldo", que es FALSO cuando la fila trae
  // `_destinationCredited: true`, o sea la prueba de que sí lo movió.
  // Consecuencias medidas: borrar ese cupón ya no revierte el crédito (la
  // cuenta destino se queda alta para siempre) y el neteo interno del desglose
  // del YTD deja de verlo como traspaso entre cuentas propias.
  //
  // Adivinar el destino está prohibido acá (no hay ningún dato que diga a cuál
  // de las cuentas fue), así que el arreglo real es de ESCRITURA: que el motor
  // estampe `_destinationItemId` en todo pago ruteado, igual que ya lo hace un
  // pago registrado a mano. Eso toca lo que la fila LLEVA, y esa fila la leen
  // superficies congeladas, así que espera OK explícito del usuario.
  const destKey = tx._destinationItemId || source?.incomeDestination
  if (!destKey) return null
  const dest = (items || []).find((it) => (it.id || it.symbol) === destKey)
  if (!dest) return null
...
/**
 * FASE OB. Una acción cuyo dividendo se corrigió a mano en el alta quedaba
 * guardada con `dividendYield` y `incomeMonths` pero SIN `incomeRate` ni
 * `incomeAmount` (el override escribía el rendimiento en el campo que la
 * pantalla lee, no en el que el motor lee), así que el motor no la veía y
 * "Editar manualmente" apagaba los pagos en silencio. El alta ya escribe los
 * dos campos; esto rescata lo que quedó guardado con la forma vieja: si no
 * hay monto por acción ni tasa, el rendimiento anual detectado es la tasa.
 * Solo con calendario, porque sin meses de pago un porcentaje anual no
 * produce ningún pago fechado.
 */
export function marketYieldFallback(item) {
  if (!item) return null
  if (item.incomeAmount > 0 || item.incomeRate > 0) return null
  if (item.rateType === 'variable' || item.rateType === 'continuous') return null
  const y = Number(item.dividendYield)
  if (!(y > 0)) return null
  if (!Array.isArray(item.incomeMonths) || item.incomeMonths.length === 0) return null
  return { incomeMode: 'percent', incomeRate: y }
}
