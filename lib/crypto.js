...
if (typeof process !== 'undefined' && process.env?.NODE_ENV === 'production' && !process.env?.NEXT_PUBLIC_CRYPTO_SALT) {
  console.warn('[crypto] NEXT_PUBLIC_CRYPTO_SALT not set — using default. Set this env var in production.')
}
...
export async function decryptToken(stored, uid) {
  ...
  const keyFns = [deriveKey, deriveLegacyKey, deriveV1Key]
