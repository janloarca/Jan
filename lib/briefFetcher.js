...
export async function fetchCryptoWindow(symbols, { days = 7, cgWindow = null } = {}) {
  // CoinGecko acepta una lista CERRADA de ventanas (1h, 24h, 7d, 14d, 30d,
  // 200d, 1y): la anual se pide como '1y', nunca como '365d'.
  const win = cgWindow || `${days}d`
  const field = `price_change_percentage_${win}_in_currency`
  const ids = symbols.map((s) => CRYPTO_MAP[s.toUpperCase()]).filter(Boolean)
  if (ids.length === 0) return {}
  try {
    const url = `https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=${ids.join(',')}&price_change_percentage=${win}`
...
// Cierres sintéticos suficientes para CUALQUIER ventana que este repo usa
// (252 días hábiles es la anual, la más larga). windowChangePct solo mira el
// primero y el último de su ventana, así que rellenar con el precio previo
// reproduce el % exacto; quedarse corto haría que la ventana no alcance y la
// fila saliera sin porcentaje teniendo el dato.
const CRYPTO_PAD = 300

export function makeBriefFetcher({ days = 7, cgWindow = null, yahooRange = '6mo' } = {}) {
  let cryptoCache = null
  return async (symbol, kind) => {
    if (kind === 'crypto') {
      if (!cryptoCache) cryptoCache = await fetchCryptoWindow(['BTC', 'ETH'], { days, cgWindow })
      ...
      return { closes: Array(CRYPTO_PAD).fill(prior).concat([q.price]) }
    }
