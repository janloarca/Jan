...
export const REASONS = {
  ...
  SELF_REPORTED: 'self-reported',       // los únicos pagos los escribió NUESTRO propio motor: no son una segunda fuente
...
// ⛔ Un pago que escribió NUESTRO propio motor NO es una segunda fuente.
//
// `processDividends` genera una fila DIVIDEND por cada pago del calendario, con
// el monto que sale del rendimiento DECLARADO. Sumar esas filas y compararlas
// contra la proyección es comparar el número contra sí mismo: da ratio 1.0000 y
// estampa "verificado" sobre algo que nadie verificó, que es peor que no tener
// la verificación (una falsa sensación de seguridad sobre dinero).
//
// Medido antes de excluirlas: un bono con `incomeAmount: 240` y sus dos cupones
// automáticos del año devolvía `{status:'verified', ratio:1, payments:2}`.
//
// `inferred_yield` (lib/liquidYield.js) también lo escribimos nosotros: lo
// DEDUCE del saldo, así que técnicamente es independiente del rendimiento
// declarado, pero ese motor ya trae sus propias honestidades (rehúsa ante una
// tasa implausible y ante un residuo negativo) y sus cuentas REINVIERTEN, que
// acá ya se rehúsa antes. Queda fuera por conservador, no por descuido.
//
// Lo que SÍ cuenta: el ledger del broker (`ibkr`), y lo que el usuario tecleó a
// mano, que es una observación suya de lo que de verdad recibió.
const SELF_WRITTEN_SOURCES = /^(auto|inferred_yield)$/i
...
export function verifyItemIncome({
  ...
  //
  // La salida es que los pagos de ESTE activo estén vinculados por id, y ahí
  // solo se cuentan ESOS. Exigir "alguna transacción vinculada" no alcanzaba y
  // se midió: una COMPRA vinculada satisfacía el guard y después los dividendos
  // seguían emparejando por símbolo con el gemelo, o sea justo la adivinanza
  // que este guard existe para impedir.
  const onlyLinked = sharedSymbol
  if (onlyLinked && !transactions.some((tx) => isIncomeTx(tx) && tx._linkedItemId === item.id)) {
  ...
  for (const tx of transactions) {
    ...
    if (onlyLinked && tx._linkedItemId !== item.id) continue
    const amt = receivedAmount(tx)
    if (!(amt > 0)) continue
    if (SELF_WRITTEN_SOURCES.test(tx._source || '')) { selfWritten += 1; continue }
    actual += cv(amt, tx.currency || itemCurrency, itemCurrency)
    payments += 1
    if (ts < earliestTs) earliestTs = ts
  }

  // El orden importa: "solo hay pagos nuestros" es más informativo que "no hay
  // pagos", porque dice que este activo NO TIENE segunda fuente en absoluto.
  if (payments === 0) return refuse(selfWritten > 0 ? REASONS.SELF_REPORTED : REASONS.NO_PAYMENTS)
