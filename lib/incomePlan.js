...
export function normalizePlan(raw, currentYear) {
  ...
  const incomeGrowth = numOrNull(raw?.incomeGrowthPct)
  return {
    ...
    returnRate,
    // Cuánto crece el plan cada año después del año en curso. Null es "sin
    // configurar", que el consumidor lee como 0: repetir el plan tal cual.
...
// Los doce meses de un año FUTURO, que no es lo mismo que repetir el año en
// curso tal cual.
//
// La diferencia y su razón, porque confundirlas cambia el número:
//   - Un cuadrito MENSUAL (el salario) arranca en un mes de ESTE año y de ahí
//     en adelante corre siempre. Un sueldo que empieza en septiembre de 2026
//     paga los doce meses de 2027, así que `startMonth` es una fecha de
//     arranque y no una propiedad que se repita cada año.
//   - `skip` es lo mismo: "quitá el salario de este noviembre" es una decisión
//     sobre ESTE noviembre, no sobre todos los noviembres que vienen.
//   - Un cuadrito de UNA VEZ (la cátedra, el bono) sí conserva su mes: es lo
//     que lo hace repetible año con año.
export function planTotalsForFutureYear(plan, { convert, to = PLAN_CURRENCY } = {}) {
  const out = Array(12).fill(0)
  for (const chip of plan?.chips || []) {
    const amt = chipAmountIn(chip, convert, to)
    if (chip.repeat === REPEAT_MONTHLY) {
      for (let m = 0; m < 12; m++) out[m] += amt
    } else {
      out[clampMonth(chip.month)] += amt
    }
  }
  return out
}
...
export function serializePlan(plan) {
  return {
    ...
    incomeGrowthPct: plan.incomeGrowthPct == null ? null : plan.incomeGrowthPct,
  }
}
