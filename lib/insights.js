...
export function generateInsights(items, profile, netWorth, estimatedAnnualIncome) {
  ...
  // Items without an explicit currency are summed as USD — for a GTQ balance
  // that's a ~7.8× silent error. Surface it instead of guessing quietly.
  const noCurrency = items.filter(it => !it.currency && !it._originalCurrency)
  if (noCurrency.length > 0) {
    insights.push({
      type: 'warning',
      titleEs: 'Moneda asumida', titleEn: 'Assumed currency',
      descEs: `${noCurrency.length} activo${noCurrency.length > 1 ? 's' : ''} sin moneda definida — se asume USD. Edítalo${noCurrency.length > 1 ? 's' : ''} si está${noCurrency.length > 1 ? 'n' : ''} en otra moneda.`,
      descEn: `${noCurrency.length} asset${noCurrency.length > 1 ? 's' : ''} without a currency — USD is assumed. Edit if held in another currency.`,
      priority: 1,
    })
  }
  ...
  if (profile) {
    ...
    if (profile.monthlyExpenses > 0 && netWorth > 0) {
      ...
      insights.push({
        ...
        // Disclose the assumption: FIRE number = 25× annual expenses (4% rule).
        descEs: `Estás al ${firePct.toFixed(0)}% de tu número FIRE (25× tus gastos anuales, regla del 4%).`,
        descEn: `You're ${firePct.toFixed(0)}% to your FIRE number (25× annual expenses, the 4% rule).`,
        priority: 5,
      })
    }
