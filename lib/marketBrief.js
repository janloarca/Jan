...
export async function buildMarketBrief({ fetchSeries, windowSize = 5 }) {
  const rows = []
  ...
  for (const { inst, closes } of results) {
    ...
    if (inst.key === 'spx') {
      ...
      if (move) {
        context.push(`S&P 500 ${move.pct >= 0 ? 'up' : 'down'} ${Math.abs(move.pct).toFixed(1)}%, its biggest weekly move in ${move.weeks} weeks.`)
      }
    }
    if (inst.key === 'vix') {
      const extreme = describeLevelExtreme(c, { size: windowSize })
      if (extreme) {
        context.push(`VIX at ${extreme.value.toFixed(1)}, its ${extreme.kind === 'high' ? 'highest' : 'lowest'} close in ${extreme.weeks} weeks.`)
      }
    }
  }
