...
export function planEquitySnapshotWrites(equityHistory, existingSnapshots, toUSD, { todayStr = null } = {}) {
  const t = typeof toUSD === 'function' ? toUSD : (v) => v || 0
  // FASE NU. Un NAV fechado DESPUES de hoy no es una observacion: describe un
  // dia que no ocurrio. El Flex nunca lo emite (reportDate es un cierre real),
  // pero el CSV de PortfolioAnalyst fecha el mes en curso a su fin de mes, y
  // este planificador es el UNICO escritor de NAV para los dos caminos, asi
  // que el guard va aca aunque el parser ya lo recorte: dos capas, porque un
  // doc futuro en el archivo se convierte en "el ultimo snapshot".
