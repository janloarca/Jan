...
export function buildContributionFields({ item, amount, date, isAdd, currency }) {
  ...
  if (isBankLikeItem(item)) {
    ...
    const itemFields = {
      ...
      // 5, OK explícito del usuario). Esta rama escribe una FOTO nueva del
      // saldo, exactamente igual que las otras tres puertas que sellan
      // `balanceAsOf`, y hasta ahora no lo hacía: un aporte o retiro registrado
      // desde CashFlowModal (o desde "Agregar/Retirar" del editor) dejaba la
      // foto vieja. El motor de rendimiento deducido (lib/liquidYield.js) lee
      // `knownContributions` con un `asOfTs` fijo en la fecha de esa foto, así
      // que una transferencia posterior al sello viejo quedaba FUERA de "lo
      // conocido" y se leía como interés inventado por el mismo monto exacto.
      // Sella HOY (el usuario está mirando el campo y registrando el
      // movimiento AHORA), nunca la fecha `date` del movimiento, que puede
      // estar backdateada.
