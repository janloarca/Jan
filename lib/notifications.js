...
export function checkPriceAlerts(alerts, prices, onTrigger) {
  ...
  for (const alert of alerts) {
    ...
    if (hit) {
      ...
      // FASE OE: la moneda es la de la COTIZACIÓN (ver lib/priceAlertCurrency.js);
      // un número pelado en la notificación se lee en la base del usuario.
      const cur = priceData.currency ? ` ${priceData.currency}` : ''
      sendNotification(
        `${sym} ${dir} ${alert.targetPrice}${cur}`,
        { body: `${sym} is now at ${current.toFixed(2)}${cur}`, tag: `alert-${alert.id}` }
      )
    }
  }
  return triggered
