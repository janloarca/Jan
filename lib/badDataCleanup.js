...
// La única dependencia, y es otro módulo puro: la llave de un documento tiene
// que salir de la MISMA definición que la escribe (FASE MO), o el sanador de la
// transición terminaría buscando una forma que ya nadie escribe.
import { transactionDocId } from './transactionDocId'
...
export function detectAccountQualifiedIdDuplicates(transactions = []) {
  ...
  // ⛔ Las DOS formas se le preguntan al ESCRITOR, no se re-arman acá.
  //
  // `transactionDocId` es la MISMA función que decide bajo qué id se guarda una
  // fila, así que "el id que tenía ANTES" y "el que tiene AHORA" salen de una
  // sola definición y no pueden divergir de lo que de verdad se escribe.
  //
  // Mi primera versión re-armaba la llave a mano y ya divergía en dos detalles
  // (`Math.abs` sobre los centavos, `??` en vez de `||`): dos filas del mismo
  // monto con signo opuesto caían en el mismo grupo y una podía quedar sin
  // revisar. Es el defecto que este archivo documenta una y otra vez, cometido
  // dentro del sanador que existe para arreglarlo.
  //
  // La condición pide las DOS mitades, y esa es la parte que impide borrar de
  // más: el doc tiene que estar EXACTAMENTE donde el código VIEJO lo ponía (su
  // id sin el sufijo de cuenta) Y el doc que el código nuevo escribe tiene que
  // existir ya. Una regla más ancha ("su id no es el canónico") también
  // funcionaría para la transición y borraría además cualquier doc de forma
  // desconocida: preferimos dejar un duplicado, que el usuario ve y borra, a
  // tragarnos un movimiento, que no se recupera.
  const byId = new Set(eligible.map((tx) => String(tx.id)))
  const flagged = eligible.filter((tx) => {
    const viejo = transactionDocId({ ...tx, _ibkrAccountId: undefined })
    if (String(tx.id) !== viejo) return false
    const nuevo = transactionDocId(tx)
    return nuevo !== viejo && byId.has(nuevo)
  })
  if (flagged.length > 1000) return []
  return flagged.map((tx) => ({
    id: tx.id, date: tx.date, symbol: tx.symbol, amount: tx.totalAmount,
