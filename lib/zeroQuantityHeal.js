...
// La firma literal de un vaciado: los DOS campos de precio en cero EXPLÍCITO.
// Un campo ausente no cuenta (eso es una cuenta a la que nunca se le escribió
// ese campo, no una que se vació), que es la misma vara que usa
// `resurrectedBalanceFixes` más abajo. Los dos sanadores TIENEN que compartir
// esta definición: ver el comentario de abajo.
export function looksEmptied(item) {
  const cur = Number(item?.currentPrice)
  const pur = Number(item?.purchasePrice)
  return Number.isFinite(cur) && cur === 0 && Number.isFinite(pur) && pur === 0
}
...
export function zeroQuantityBalanceFixes(items) {
  ...
  for (const it of items || []) {
    ...
    // ⛔ Una cuenta VACIADA no se resucita, aunque le quede un residuo.
    //
    // `getItemPrice` salta todo candidato que no sea > 0, así que con los dos
    // precios en cero cae en cascada a `price`/`cost`/`averagePrice` y un
    // residuo ahí se lee como "precio vivo". Sin este guard los dos sanadores
    // se DESHACEN entre sí a través de sesiones: `resurrectedBalanceFixes`
    // apaga la cantidad de una cuenta vaciada (valor 0, correcto) y en la
    // sesión siguiente esta función ve "cantidad cero con precio 240" y la
    // vuelve a encender, o sea el saldo resucita justo como lo reportó el
    // usuario. Reproducido con las funciones reales antes de arreglarlo.
    //
    // El test que daba confianza medía a los dos sanadores en un ÚNICO
    // instante ("nunca reclaman el mismo ítem a la vez"), que es cierto y es
    // el invariante equivocado: el que importa es que aplicar uno no cree la
    // precondición del otro. Ese caso vive en `resurrectedBalanceFixes`, que
    // es quien sabe qué hacer con un residuo, y por eso acá se cede en vez de
    // competir.
    if (looksEmptied(it)) continue
...
export function resurrectedBalanceFixes(items) {
  ...
  for (const it of items || []) {
    ...
    if (!isBankLike(it)) continue
    if (it.soldFully || it.saleDate) continue
    // Los dos precios EXPLÍCITAMENTE en cero, con la MISMA definición que usa
    // `zeroQuantityBalanceFixes` para cederle este caso. Una sola definición a
    // propósito: con dos copias, la de arriba podría ensancharse y volver a
    // resucitar lo que esta apaga.
    if (!looksEmptied(it)) continue
    const qty = Number(it.quantity)
    if (!(Number.isFinite(qty) && qty > 0)) continue
