...
export async function renderSpreadsheetXlsx({ model, year }) {
  ...
  const notes = model.missingMonths.length > 0
    ? [`Months without data (${model.missingMonths.map(monthLabel).join(', ')}): open the Spreadsheet in the app once and they fill in on their own.`]
    : []
