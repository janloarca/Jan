...
import { renderStyledSheet } from './xlsxSheet'
...
export function buildSpreadsheetModel({ items, monthDocs = {}, monthKeys = [], liveMonthKey = null, baseCurrency = 'USD', convert = null }) {
    ...
    for (const [instName, instItems] of insts) {
      ...
      for (const it of instItems) {
        ...
        const values = monthKeys.map((mk) => {
          ...
          // ⛔ FASE MN. La MISMA exclusión que la rama del mes en vivo. Una
          // cuenta por cobrar que el usuario sacó del patrimonio a propósito
          // (`countInNetWorth: false`) quedaba FUERA del TOTAL en la columna de
          // hoy y DENTRO en todos los meses históricos: la misma fila contada
          // distinto según la columna, o sea la fila TOTAL se contradecía a sí
          // misma a lo ancho.
          //
          // El subtotal de CATEGORÍA sigue incluyéndola, igual que la Hoja en
          // pantalla (`catMap[cat].total += val` sin condición): esa asimetría
          // es deliberada ahí, y por eso se DICE al pie en vez de dejar dos
          // números que no cuadran sin explicación.
          if (!isExcludedFromNetWorth(it)) add(mk, signed)
      ...
      // FASE NS: las posiciones ya VENDIDAS de esta institución y categoría, una
      // fila por símbolo, contadas solo con el broker conectado (misma regla
      // que el bucket y que la Hoja en pantalla). La celda trae institución y
      // categoría; la llave no se parsea.
      if (hasIbkrItems) {
        const closed = new Map()
        for (const mk of monthKeys) {
          for (const [k, v] of Object.entries(hist[mk] || {})) {
            if (!k.startsWith(IBKR_CLOSED_KEY_PREFIX) || !v || v.institution !== instName || v.category !== catKey) continue
            if (v.value == null) continue
            if (!closed.has(k)) closed.set(k, { symbol: v.symbol || k, months: {} })
            closed.get(k).months[mk] = v.value
          }
        }
        for (const { symbol, months } of closed.values()) {
          const values = monthKeys.map((mk) => {
            const v = months[mk]
            if (v == null) return null
            add(mk, v); catAcc[mk] = (catAcc[mk] || 0) + v
            return round2(v)
          })
          rows.push({ name: `${symbol} (sold)`, values, synthetic: true })
        }
      }
  ...
  return {
    ...
    baseCurrency,
    liveMonthKey,
    // Hay al menos una fila que se muestra y que NO cuenta en el TOTAL: sin
    // decirlo, las partes no suman el todo y no hay forma de saber por que.
    hasExcludedFromTotal: (items || []).some((it) => isExcludedFromNetWorth(it)),
  }
}

// ── El workbook con formato (FASE IE4) ────────────────────────────────────
//
// El ESTILO vive en lib/xlsxSheet.js, compartido con el botón "Excel" de la
// pantalla del Spreadsheet (FASE IE5): los dos archivos que Chispudo produce
// tienen que verse igual, y dos copias del formato es cómo una se queda atrás.
// Aquí solo se traduce el modelo a las filas que ese renderer espera.
...
export async function renderSpreadsheetXlsx({ model, year }) {
  const rows = []
  for (const cat of model.categories) {
    rows.push({ kind: 'category', label: cat.label, values: cat.subtotals })
    for (const inst of cat.institutions) {
      for (const it of inst.items) {
        ...
        const label = inst.name && inst.name !== 'No institution'
          ? `${it.name}  ·  ${inst.name}`
          : it.name
        rows.push({ kind: 'item', label, values: it.values, muted: !!it.synthetic })
      }
    }
  }
  rows.push({ kind: 'total', label: 'TOTAL', values: model.totals })

  const notes = []
  if (model.missingMonths.length > 0) {
    notes.push(`Months without data (${model.missingMonths.map(monthLabel).join(', ')}): open the Spreadsheet in the app once and they fill in on their own.`)
  }
  // La Hoja en pantalla marca esa categoria por la misma razon: el subtotal la
  // incluye y el TOTAL no, asi que sin la nota son dos numeros que no cuadran.
  if (model.hasExcludedFromTotal) {
    notes.push('Some receivables are shown but left out of the TOTAL, because you chose not to count them toward net worth. Category subtotals still include them.')
  }
  ...
  const columns = model.monthLabels.map((l, i) =>
    model.monthKeys[i] === model.liveMonthKey ? `${l} (current)` : l)

  const ab = await renderStyledSheet({
    title: 'Portfolio Spreadsheet',
    subtitle: `${monthRangeLabel(model)} ${year}  ·  Values in ${model.baseCurrency || 'USD'}`,
    sheetName: `Portfolio ${year}`,
    columns,
    rows,
    notes,
  })
  return { buffer: Buffer.from(ab), filename: `chispudo-spreadsheet-${year}.xlsx` }
}
