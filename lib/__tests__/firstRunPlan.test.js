...
describe('firstRunPlan: el recorrido no puede exigir lo que no pregunta', () => {
  ...
  test('el deposito de apertura cae a hoy cuando no hay fecha', () => {
    ...
    // FASE OL: el respaldo vive UNA vez en `purchaseDate` (la fecha de ESTA
    // compra: lote y DEPOSIT la usan), y en un merge el item conserva la suya.
    expect(src).toMatch(/const purchaseDate = effectiveAcqDate \|\| new Date\(\)\.toISOString\(\)\.split\('T'\)\[0\]/)
    const uses = src.match(/(acquisitionDate|date): purchaseDate,/g) || []
