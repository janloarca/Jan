...
describe('parseTrades', () => {
  it('parses self-closing and paired trades, preferring ibCommission', () => {
    ...
    // tradeDate se espera NORMALIZADA. Este assert decía '20260115' (el crudo de
    // IBKR) y con eso fijaba el defecto: esa cadena es Invalid Date, así que
    // buildTxEvents descartaba cada compra y venta y el ledger del broker no
    // entraba en la reconstrucción histórica. Actualizar el número esperado es
    // correcto acá porque el valor viejo describía el bug, no un invariante.
    expect(r[0]).toMatchObject({ symbol: 'AAPL', buySell: 'BUY', commission: 1.25, tradeDate: '2026-01-15' })
...
describe('parseCashTransactions', () => {
  ...
  // Real-world attribute order: a query with ALL fields selected emits
  // securityIDType="" (and securityID="", securityIDType, cusip...) BEFORE
  // type=. Without a boundary in the attr() regex, attr('type') matched the
  // tail of securityIDType="" (case-insensitive) and read every row as
  // empty-type: ZERO cash transactions parsed from a file whose section
  // counter saw 175 rows (user's real Chispu_5.xml, FASE GH). This row is a
  // faithful trim of one of that file's deposit rows.
  it('parses rows where securityIDType precedes type (all-fields query)', () => {
    const real = '<CashTransaction accountId="U12093588" acctAlias="Jan" model="" currency="USD" fxRateToBase="1" '
      + 'assetCategory="" subCategory="" symbol="" description="CASH RECEIPTS / ELECTRONIC FUND TRANSFERS" conid="" '
      + 'securityID="" securityIDType="" cusip="" isin="" figi="" listingExchange="" underlyingConid="" '
      + 'underlyingSymbol="" underlyingSecurityID="" underlyingListingExchange="" issuer="" issuerCountryCode="" '
      + 'multiplier="0" strike="" expiry="" putCall="" principalAdjustFactor="" dateTime="20260127" settleDate="20260127" '
      + 'amount="1450" type="Deposits/Withdrawals" tradeID="" code="" transactionID="37590199980" reportDate="20260127" '
      + 'clientReference="" actionID="" levelOfDetail="DETAIL" serialNumber="" deliveryType="" commodityType="" fineness="0.0" weight="0.0 ()" />'
    const r = parseCashTransactions(real)
    expect(r).toHaveLength(1)
    expect(r[0].kind).toBe('flow')
    expect(r[0].amount).toBe(1450)
    expect(r[0].date).toBe('2026-01-27')
    expect(r[0].txnId).toBe('37590199980')
  })
})
...
describe('classifyError: el texto crudo de IBKR (FASE KN)', () => {
  it('conserva lo que IBKR dijo literalmente, aunque el mensaje mostrado sea el nuestro', () => {
    const r = classifyError('Token has expired', '1012')
    expect(r.errorCode).toBe('TOKEN_EXPIRED')
    // El texto que se muestra es NUESTRO, mas accionable...
    expect(r.error).toMatch(/Genera uno nuevo/)
    // ...y el del servidor sigue disponible como evidencia.
    expect(r.raw).toBe('Token has expired')
  })

  it('REDACTA el Flex Token: viaja en la URL como ?t= y el crudo se persiste', () => {
    const msg = 'fetch failed for https://gdcdyn.interactivebrokers.com/x?t=SUPERSECRETO123&q=456&v=3'
    const r = classifyError(msg)
    expect(r.raw).not.toContain('SUPERSECRETO123')
    expect(r.raw).toContain('t=***')
    // El query id no es secreto y se conserva: sirve para diagnosticar.
    expect(r.raw).toContain('q=456')
  })

  it('recorta un mensaje enorme en vez de guardar una pagina entera', () => {
    const r = classifyError('x'.repeat(5000))
    expect(r.raw.length).toBeLessThanOrEqual(300)
  })

  it('sin mensaje no inventa un campo vacio', () => {
    expect(classifyError('', '1012').raw).toBeUndefined()
  })
})
