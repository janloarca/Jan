...
describe('buildFirebaseInitJson', () => {
  ...
  test('reports the host actually serving the helper as its authDomain', () => {
    // Under the same-origin setup the helper runs on our domain, so saying
    // otherwise would point it back at the cross-site host this whole
    // arrangement exists to avoid.
    expect(buildFirebaseInitJson(ENV, { authDomain: 'chispu.xyz' }).authDomain).toBe('chispu.xyz')
