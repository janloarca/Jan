...
describe('parseIBKRFile — activity statement (sectioned format)', () => {
  ...
  it('stamps YYYYMM monthly NAV rows at month-END, not month-start', () => {
    // PortfolioAnalyst monthly NAV rows carry YYYYMM dates and the value is
    // the month-END NAV. Month-start stamping put Jan-31's NAV on Jan 1,
    // where the YTD anchor picked it up and Modified Dietz double-counted
    // January's deposit.
    const csv = [
      'Allocation by Financial Instrument,Header,Date,NAV',
      'Allocation by Financial Instrument,Data,202512,98000',
      'Allocation by Financial Instrument,Data,202601,100000',
      'Allocation by Financial Instrument,Data,202602,102500',
    ].join('\n')
    const result = parseIBKRFile(csv)
    const dates = result.equityHistory.map((e) => e.date)
  ...
  // FASE NU. La fila del mes EN CURSO tambien viene como YYYYMM, pero su valor
  // es el NAV al dia del export, no un cierre: el CSV real del usuario traia
  // '202609' un 4 de septiembre, y salia estampado '2026-09-30', una fecha que
  // no existia todavia.
  it('recorta el YYYYMM del mes en curso a HOY en vez de a un fin de mes futuro', () => {
    const now = new Date()
    const today = now.toISOString().slice(0, 10)
    const ym = today.slice(0, 7).replace('-', '')
    const prev = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth() - 1, 1))
    const prevYm = prev.toISOString().slice(0, 7).replace('-', '')
    const prevEnd = new Date(Date.UTC(prev.getUTCFullYear(), prev.getUTCMonth() + 1, 0)).toISOString().slice(0, 10)
    const csv = [
      'Allocation by Financial Instrument,Header,Date,NAV',
      `Allocation by Financial Instrument,Data,${prevYm},9800`,
      `Allocation by Financial Instrument,Data,${ym},10008.97`,
    ].join('\n')
    const dates = parseIBKRFile(csv).equityHistory.map((e) => e.date).sort()
    // El mes cerrado conserva su fin de mes; el mes en curso aterriza en hoy.
    expect(dates).toEqual([prevEnd, today].sort())
...
describe('full-range activity statement: flows, income and cash', () => {
  const csv = [
    'Statement,Header,Field Name,Field Value',
    'Statement,Data,Period,"January 1, 2026 - July 27, 2026"',
    ...
    'Deposits & Withdrawals,Data,USD,U1,2026-01-27,Electronic Fund Transfer,1450',
    'Deposits & Withdrawals,Data,USD,U1,2026-02-26,Electronic Fund Transfer,1350',
    'Deposits & Withdrawals,Data,USD,U1,2026-04-20,Disbursement,-5',
    'Deposits & Withdrawals,Data,Total,,,,2795',
    'Dividends,Header,Currency,Account,Date,Description,Amount',
    'Dividends,Data,USD,U1,2026-01-06,PEP(US7134481081) Cash Dividend USD 1.4225 per Share,6.19',
    'Dividends,Data,USD,U1,2026-01-08,VICI(US9256521090) Cash Dividend USD 0.45 per Share,14.64',
    'Dividends,Data,Total,,,,20.83',
    'Withholding Tax,Header,Currency,Account,Date,Description,Amount,Code',
    'Withholding Tax,Data,USD,U1,2026-01-08,VICI(US9256521090) Cash Dividend - US Tax,-2.20,',
    'Withholding Tax,Data,USD,U1,2026-02-11,VICI(US9256521090) Cash Dividend - US Tax REVERSAL,0.70,',
    'Withholding Tax,Data,Total,,,,-1.50,',
    'Cash Report,Header,Currency Summary,Currency,Total,Securities,Futures,Month to Date,Year to Date',
    'Cash Report,Data,Starting Cash,Base Currency Summary,0.2450,0.2450,0,,',
    'Cash Report,Data,Deposits,Base Currency Summary,0,0,0,,3950',
    'Cash Report,Data,Dividends,Base Currency Summary,0,0,0,,198.36',
    'Cash Report,Data,Trades (Purchase),Base Currency Summary,0,0,0,,-9898.39',
    'Cash Report,Data,Ending Cash,Base Currency Summary,1.3944,1.3944,0,,',
    'Cash Report,Data,Ending Settled Cash,Base Currency Summary,1.3944,1.3944,0,,',
  ].join('\n')

  const parsed = () => parseIBKRFile(csv)

  it('ignores the "Total" row so deposits are not double counted', () => {
    // The Total row has no DataDiscriminator and no date; it used to import as a
    // real deposit dated today, doubling the user's contributions and wrecking
    // the Modified Dietz return.
    const flows = parsed().trades.filter(t => /DEPOSIT|WITHDRAWAL/.test(t.type))
    expect(flows).toHaveLength(3)
    const net = flows.reduce((s, f) => s + (f.type === 'DEPOSIT' ? f.totalAmount : -Math.abs(f.totalAmount)), 0)
    expect(net).toBe(2795)
  })

  it('imports dividends with their symbol and date', () => {
    const divs = parsed().trades.filter(t => t.type === 'DIVIDEND')
    expect(divs).toHaveLength(2)
    expect(divs.map(d => d.symbol).sort()).toEqual(['PEP', 'VICI'])
    expect(divs.find(d => d.symbol === 'PEP').date).toBe('2026-01-06')
    expect(divs.reduce((s, d) => s + d.totalAmount, 0)).toBeCloseTo(20.83, 2)
  })

  it('keeps IBKR signs on withholding tax so a reversal is a refund', () => {
    const tax = parsed().trades.filter(t => t.type === 'TAX')
    expect(tax).toHaveLength(2)
    expect(tax.reduce((s, t) => s + t.totalAmount, 0)).toBeCloseTo(-1.5, 2)
  })

  it('creates ONE cash holding from the ending balance, not one per line item', () => {
    // Every Cash Report line ("Deposits", "Dividends", "Trades (Purchase)"...)
    // used to become its own fake holding, inventing tens of thousands of
    // dollars of assets that do not exist.
    const cash = parsed().cashPositions
    expect(cash).toHaveLength(1)
    expect(cash[0].symbol).toBe('CASH-USD')
    expect(cash[0].currentPrice).toBeCloseTo(1.3944, 4)
  })

  it('does not leak Cash Report line items into imported items', () => {
    const items = formatIBKRFileResult(parsed()).items
    expect(items.filter(i => /CASH-(Deposits|Dividends|Commissions|Trades)/i.test(i.symbol))).toHaveLength(0)
  })
})

describe('purchase dates only when the statement can prove them', () => {
  // A statement covers a window. For a holding bought BEFORE that window, the
  // earliest buy inside it is just a top-up, not the purchase date: stamping it
  // would claim a position owned for years started a few months ago and erase
  // its earlier value from the chart.
  const build = (posQty, trades) => [
    'Open Positions,Header,DataDiscriminator,Asset Category,Currency,Symbol,Quantity,Cost Price,Close Price,Value',
    `Open Positions,Data,Summary,Stocks,USD,UNH,${posQty},289.82,417.64,${(posQty * 417.64).toFixed(2)}`,
    'Trades,Header,DataDiscriminator,Asset Category,Currency,Symbol,Date/Time,Quantity,T. Price,Proceeds,Comm/Fee,Buy/Sell,Realized P/L',
    ...trades,
  ].join('\n')

  it('dates the position when the window explains every share', () => {
    const csv = build(3, ['Trades,Data,Order,STK,USD,UNH,2026-01-27,3,289.82,-869,-1,BUY,0'])
    expect(parseIBKRFile(csv).positions[0].acquisitionDate).toBe('2026-01-27')
  })

  it('leaves the date OFF when the window only explains part of the position', () => {
    const csv = build(3, ['Trades,Data,Order,STK,USD,UNH,2026-01-27,0.5347,289.82,-155,-1,BUY,0'])
    const pos = parseIBKRFile(csv).positions[0]
    expect(pos.acquisitionDate).toBeUndefined()
    expect(pos._historyIncomplete).toBe(true)
  })

  it('leaves the date OFF when the window is net negative (sold more than bought)', () => {
    const csv = build(2.8256, [
      'Trades,Data,Order,STK,USD,UNH,2026-04-17,1,289.82,-290,-1,BUY,0',
      'Trades,Data,Order,STK,USD,UNH,2026-05-02,-2.17,300,651,-1,SELL,20',
    ])
    expect(parseIBKRFile(csv).positions[0].acquisitionDate).toBeUndefined()
  })

  it('flags the incomplete holding through to the imported item', () => {
    const csv = build(3, ['Trades,Data,Order,STK,USD,UNH,2026-01-27,0.5,289.82,-145,-1,BUY,0'])
    expect(formatIBKRFileResult(parseIBKRFile(csv)).items[0]._historyIncomplete).toBe(true)
  })
})

describe('PortfolioAnalyst export: aggregates are not transactions', () => {
  const csv = [
    'Trade Summary,MetaInfo,Analysis Period,"June 23, 2023 - July 27, 2026"',
    ...
    'Trade Summary,Data,ETFs,United States Dollar,EWH,ISHARES MSCI HONG KONG ETF,Broad,3.0464,15.61,-47.57,-47.57,-3.0464,16.04,48.87,48.87',
    'Dividends,MetaInfo,Analysis Period,"June 23, 2023 - July 27, 2026"',
    'Dividends,Header,PayDate,Ex-Date,Symbol,Quantity,DividendPerShare,Amount',
    'Dividends,Data,20231215,20231130,KO,1,0.46,0.46',
    'Dividends,Data,20240701,20240614,KO,8,0.48,3.88',
  ].join('\n')

  it('never turns a lifetime "Trade Summary" into dated trades', () => {
    // The aggregate has no dates, so every synthesized trade landed on today:
    // 77 fake purchases in the movement ledger on a real export.
    const trades = parseIBKRFile(csv).trades.filter(t => t.type === 'BUY' || t.type === 'SELL')
    expect(trades).toHaveLength(0)
  })

  it('imports each dividend exactly once with its real symbol', () => {
    // The Dividends section is understood by BOTH the activity and the
    // PortfolioAnalyst parsers, and their output is concatenated: income used
    // to double, with the second copy filed under a generic CASH symbol.
    const divs = parseIBKRFile(csv).trades.filter(t => t.type === 'DIVIDEND')
    expect(divs).toHaveLength(2)
    expect(divs.filter(d => d.symbol === 'CASH')).toHaveLength(0)
    expect(divs.map(d => d.symbol)).toEqual(['KO', 'KO'])
    expect(divs.reduce((s, d) => s + d.totalAmount, 0)).toBeCloseTo(4.34, 2)
  })

  it('still reads dividends from an Activity Statement layout (no Symbol column)', () => {
    const activityLayout = [
      'Dividends,Header,Currency,Account,Date,Description,Amount',
      'Dividends,Data,USD,U1,2026-01-06,PEP(US7134481081) Cash Dividend USD 1.4225 per Share,6.19',
    ].join('\n')
    const divs = parseIBKRFile(activityLayout).trades.filter(t => t.type === 'DIVIDEND')
    expect(divs).toHaveLength(1)
    expect(divs[0].symbol).toBe('PEP')
  })
})

// ⛔ FASE NR. La fila de EFECTIVO que se cotizaba como si fuera una accion.
//
// El caso real, con las cifras del CSV del usuario (Since Inception Detailed de
// PortfolioAnalyst): la cuenta cierra en 10,008.97 y la app mostraba 17,212.36.
// Los 7,203.39 de diferencia son UNA fila: "USD - United States Dollar", 82.53
// dolares de efectivo, leidos como 82.53 ACCIONES del ETF cuyo ticker tambien
// es "USD" (ProShares Ultra Semiconductors, ~88.50).
//
// La cadena: (1) el filtro de secciones del parser de Activity Statement
// matcheaba tambien "Open Position Summary", que es de PortfolioAnalyst; (2) esa
// seccion no trae columna "Asset Category", asi que `mapAssetCategory('')` caia
// a su default 'Stock'; (3) el merge pone las posiciones del activity primero y
// solo deja ganar a PA cuando el costo es 0, asi que la lectura mala ganaba;
// (4) con tipo 'Stock', `isMarketPriced` deja pasar el simbolo y useMarketPrices
// lo cotiza. Con tipo 'Bank' habria dado false y nunca se habria pedido precio.
describe('FASE NR: una fila de efectivo no es una posicion de mercado', () => {
  // La forma REAL del header de PortfolioAnalyst, verbatim del CSV del usuario.
  const PA_HEADER = 'Date,FinancialInstrument,Currency,Symbol,Description,Sector,Quantity,ClosePrice,Value,Cost Basis,UnrealizedP&L,FXRateToBase'
  const PA_CASH = '09/03/2026,Cash,USD,USD,United States Dollar,Cash,82.534123,1,82.534123, , ,1'
  const PA_STOCK = '09/03/2026,Stocks,USD,MA,MASTERCARD INC - A,Technology,1.69,585.71,989.85,1000.32541,-10.47541,1'

  // El caso del usuario, de punta a punta. No tiene dientes contra UN solo
  // cambio (las dos mitades del arreglo lo cubren por separado, a proposito):
  // los dientes van en los dos tests de abajo, uno por mitad.
  it('el efectivo de PortfolioAnalyst entra como efectivo, no como el ETF "USD"', () => {
    const csv = [
      `Open Position Summary,Header,${PA_HEADER}`,
      `Open Position Summary,Data,${PA_CASH}`,
      `Open Position Summary,Data,${PA_STOCK}`,
    ].join('\n')
    const { positions } = parseIBKRFile(csv)
    const usd = positions.filter(p => p.symbol === 'USD')
    expect(usd).toHaveLength(1)
    expect(usd[0].type).toBe('Bank')
    expect(usd[0].quantity).toBeCloseTo(82.534123, 6)
    expect(usd[0].currentPrice).toBe(1)
    // 82.53 + 989.85 = el valor de la cuenta, no 82.53 x 88.50 encima.
    const total = positions.reduce((s, p) => s + p.quantity * p.currentPrice, 0)
    expect(total).toBeCloseTo(1072.38, 2)
  })

  it('la puerta de abajo: un item de efectivo nunca se cotiza', () => {
    // El eslabon que convierte el tipo equivocado en dinero inventado. Se fija
    // aca porque es lo que hace que el tipo IMPORTE.
    const { isMarketPriced } = require('../../components/dashboard/utils')
    const cash = { symbol: 'USD', name: 'United States Dollar', type: 'Bank', quantity: 82.53 }
    expect(isMarketPriced(cash)).toBe(false)
    expect(isMarketPriced({ ...cash, type: 'Stock' })).toBe(true)
  })

  it('DIENTES del guard: sin columna de categoria no se afirma un tipo', () => {
    // Misma forma de PA pero bajo el nombre exacto que el activity SI reclama.
    // Solo el guard de `catIdx < 0` impide que esta fila salga como 'Stock'.
    const csv = [
      `Open Positions,Header,${PA_HEADER}`,
      `Open Positions,Data,${PA_CASH}`,
    ].join('\n')
    const { positions } = parseIBKRFile(csv)
    expect(positions.filter(p => p.symbol === 'USD').map(p => p.type)).toEqual(['Bank'])
  })

  it('DIENTES del alcance: "Open Position Summary" es de PA, no del activity', () => {
    // Con columna de categoria el guard no puede disparar, asi que lo unico que
    // impide que el activity reclame esta seccion es que su filtro ya no
    // matchee el prefijo. Las dos columnas se contradicen a proposito: es la
    // unica forma de ver CUAL de los dos parsers gano.
    const csv = [
      'Open Position Summary,Header,Asset Category,Currency,Symbol,Description,Sector,Quantity,Cost Price,Close Price',
      'Open Position Summary,Data,Stocks,USD,USD,United States Dollar,Cash,82.534123,1,1',
    ].join('\n')
    const { positions } = parseIBKRFile(csv)
    expect(positions.filter(p => p.symbol === 'USD').map(p => p.type)).toEqual(['Bank'])
  })

  it('CONTROL: un Activity Statement de verdad se sigue leyendo igual', () => {
    // Sin esto, "no reclamo la seccion" podria pasar por haber dejado de leer
    // Open Positions del todo.
    const csv = [
      'Open Positions,Header,DataDiscriminator,Asset Category,Currency,Symbol,Description,Quantity,Cost Price,Close Price,Value',
      'Open Positions,Data,Summary,STK,USD,AAPL,Apple Inc,100,145.50,182.30,18230',
    ].join('\n')
    ...
    expect(positions[0]).toMatchObject({ symbol: 'AAPL', type: 'Stock', quantity: 100 })
  })
})
