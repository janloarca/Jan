import { parseBankAlert, htmlToText, splitMerchantLocation, extractTime } from '../parsers/bankAlertParser'
...
describe('extractTime', () => {
  it('reads a labelled hour', () => {
    expect(extractTime('Monto: Q17.00\nHora: 14:32')).toBe('14:32:00')
    expect(extractTime('Hora de transacción: 08:05:41')).toBe('08:05:41')
  })

  it('honours the meridiem when the alert uses one', () => {
    expect(extractTime('Hora: 2:32 p.m.')).toBe('14:32:00')
    expect(extractTime('Hora: 12:10 a.m.')).toBe('00:10:00')
  })

  it('reads the hour glued to a date', () => {
    expect(extractTime('03/08/2026 20:32')).toBe('20:32:00')
    expect(extractTime('2026-08-03T20:32:11')).toBe('20:32:11')
  })

  it('returns null when there is no hour, instead of inventing one', () => {
    // Most alerts print none, and then the message's Date header is the honest
    // approximation. A made-up midnight would make every alert of a day look
    // like the same instant.
    expect(extractTime('Compra en RALLY PADEL por Q17.00')).toBeNull()
    expect(extractTime('')).toBeNull()
  })

  it('rejects an impossible hour rather than clamping it', () => {
    expect(extractTime('Hora: 99:99')).toBeNull()
  })

  it('parseBankAlert carries the hour through', () => {
    const alert = parseBankAlert({ text: 'Compra por Q17.00 en RALLY PADEL\nFecha: 03/08/2026\nHora: 14:32' })
    expect(alert.date).toBe('2026-08-03')
    expect(alert.time).toBe('14:32:00')
  })
})

describe('CREDIT_HINTS solo mira el encabezado (FASE NH)', () => {
  // Clasificar como 'credit' termina en `skip`: el gasto NO se escribe y el
  // correo se marca leido. Descartar un cobro real lo hace desaparecer sin
  // dejar rastro, asi que la palabra tiene que estar donde vive el verbo de la
  // alerta, no en el pie.
  const FOOTER = '\n\nConsulte nuestra politica de devoluciones en el sitio web.\nEl abono a su cuenta puede tardar 48 horas.'

  it('un cobro REAL con "devolucion" y "abono" en el pie sigue siendo un cobro', () => {
    const alert = parseBankAlert({
      subject: 'Compra aprobada',
      text: `Compra por Q250.00 en LIBRERIA SOPHOS\nFecha: 03/08/2026${FOOTER}`,
    })
    expect(alert.kind).toBe('debit')
    expect(alert.amount).toBe(250)
  })

  it('un reverso de verdad se sigue descartando: la palabra esta en el encabezado', () => {
    const alert = parseBankAlert({
      subject: 'Reverso de transaccion',
      text: 'Reverso por Q250.00 en LIBRERIA SOPHOS\nFecha: 03/08/2026',
    })
    expect(alert.kind).toBe('credit')
  })

  it('tambien lo reconoce en las primeras lineas del cuerpo, no solo en el asunto', () => {
    const alert = parseBankAlert({
      subject: 'Notificacion',
      text: 'Devolucion aplicada por Q80.00 de FARMACIA GALENO\nFecha: 03/08/2026',
    })
    expect(alert.kind).toBe('credit')
  })

  it('el guard de rechazadas no se movio', () => {
    expect(parseBankAlert({
      subject: 'Transaccion rechazada',
      text: 'Compra rechazada por Q99.00 en TIENDA X',
    }).kind).toBe('declined')
  })
})
