...
describe('fillFallback', () => {
  ...
  it('flags fallback cells as estimated (held-flat, no real market price)', () => {
    const result = {}
    months.forEach(mk => { result[mk] = {} })
    fillFallback(result, { id: 'item1', symbol: 'VTI', quantity: 10, purchasePrice: 200 }, months)
    expect(result['2025-01']['item1'].estimated).toBe(true)
  })
})
...
describe('applyStaticHistory', () => {
  ...
  it('shows flat value when there are no events', () => {
    ...
    months.forEach(mk => {
      ...
      expect(result[mk]['flat'].estimated).toBe(true)
    })
  })

  // FASE DS: a bond's held-flat value is exact (it only moves through tracked
  // coupon events), not a guess — the spreadsheet's "~" marker must not show
  // on it. The default (no 8th arg) stays `estimated:true` for the market-
  // fallback caller, which genuinely IS guessing.
  it('marks the value exact (not estimated) when trueStatic is passed', () => {
    const result = makeResult()
    const item = { id: 'vitali', name: 'Vitali', quantity: 1, currentPrice: 6000 }
    applyStaticHistory(result, item, months, null, 'USD', {}, {}, true)

    months.forEach(mk => expect(result[mk]['vitali'].estimated).toBe(false))
  })

  it('defaults to estimated:true (market-fallback caller) when trueStatic is omitted', () => {
    const result = makeResult()
    const item = { id: 'stock', name: 'SomeStock', quantity: 1, currentPrice: 100 }
    applyStaticHistory(result, item, months, null, 'USD', {}, {})

    months.forEach(mk => expect(result[mk]['stock'].estimated).toBe(true))
  })
