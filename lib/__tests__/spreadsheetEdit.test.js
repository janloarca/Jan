...
describe('flujo: entro o salio dinero', () => {
  ...
  test('un banco con cantidad distinta de 1 TAMPOCO ofrece flujo', () => {
    // ⛔ Encontrado por verificacion adversarial: la promesa "flujo y correccion
    // escriben el mismo parche" solo vale a cantidad 1, porque la rama de banco
    // de buildContributionFields desplaza los PRECIOS por el delta completo.
    expect(canRecordFlow({ type: 'Bank', quantity: 2 })).toBe(false)
  ...
  test('banco qty 2 + "meti dinero": ni ingreso inventado ni ganancia fantasma', () => {
    // Defensa en profundidad (la UI ni ofrece el boton): la respuesta FLOW sin
    // forma de registrarse cae al parche de CORRECCION, nunca a la rama de
    // rendimiento, que sobre una cuenta liquida inventaria un manual_yield.
    const it = { id: 'k2', type: 'Bank', quantity: 2, purchasePrice: 500, currentPrice: 500, currency: 'GTQ' }
