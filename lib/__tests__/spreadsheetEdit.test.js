...
describe('a quien se le pregunta', () => {
  ...
  test('un item sincronizado o de demo tampoco se pregunta', () => {
    // La verdad de un item sincronizado vive en el broker (el proximo sync lo
    // pisa), y el modo demo promete cero side-effects persistentes.
    expect(editNeedsAnswer({ item: { ...BANCO, _source: 'ibkr' }, oldValue: 1000, newValue: 1200, isMarket: false })).toBe(false)
    expect(editNeedsAnswer({ item: { ...BANCO, _source: 'demo' }, oldValue: 1000, newValue: 1200, isMarket: false })).toBe(false)
    // Un origen manual SI se pregunta: es dato del usuario.
    expect(editNeedsAnswer({ item: { ...BANCO, _source: 'manual_new_account' }, oldValue: 1000, newValue: 1200, isMarket: false })).toBe(true)
  })
...
describe('flujo: entro o salio dinero', () => {
  ...
  // ⛔ FASE NF. El valor esperado viejo era `false` para un banco con cantidad
  // 2, y describia la RESTRICCION de FASE JJ3 (esconder el boton), no el
  // invariante. El invariante es que el valor no puede quedar en 1600 y que no
  // se invente ganancia, y ese se sigue cumpliendo: ahora se logra escribiendo
  // el parche correcto en vez de negandole al usuario la respuesta verdadera.
  test('una CUENTA DE SALDO ofrece flujo con la cantidad que tenga', () => {
    expect(canRecordFlow({ type: 'Bank', quantity: 2 })).toBe(true)
    // La forma "cantidad = saldo con precio 1" (FASE JD3), que es la que dejaba
    // al usuario sin poder decir que saco dinero de su fondo liquido.
    expect(canRecordFlow({ type: 'Cuenta monetaria', quantity: 602.25, currentPrice: 1 })).toBe(true)
    // Sin cantidad, o en cero, la convencion efectiva de planCellEdit es 1.
    expect(canRecordFlow({ type: 'Bank' })).toBe(true)
    expect(canRecordFlow({ type: 'Bank', quantity: 0 })).toBe(true)
    // Lo que NO se ensancha: un estatico por CANTIDAD real sigue fuera, porque
    // ahi sacar dinero es cerrar lotes.
    expect(canRecordFlow({ type: 'Bono', quantity: 3 })).toBe(false)
    expect(canRecordFlow({ type: 'Alternative', quantity: 4 })).toBe(false)
  })

  test('regresion negativa: por que un banco qty 2 no puede usar la rama de flujo', () => {
    // Con cantidad 2, desplazar los precios por el delta completo DUPLICA el
    // efecto sobre el valor: un deposito de 300 subiria el valor en 600 y los
    // 300 sobrantes se leerian como ganancia inventada.
    const it = { id: 'k2', type: 'Bank', quantity: 2, purchasePrice: 500, currentPrice: 500, currency: 'GTQ' }
    const { itemFields } = buildContributionFields({ item: it, amount: 300, date: '2026-08-25', isAdd: true })
    expect(getItemValue({ ...it, ...itemFields })).toBe(1600) // no 1300
  })

  test('banco qty 2 + "meti dinero": aterriza en 1300, sin ganancia fantasma', () => {
    // ⛔ EL INVARIANTE DE FASE JJ3, intacto: el valor tecleado se respeta y la
    // ganancia no se mueve. Lo que cambia (FASE NF) es que ahora ADEMAS queda
    // constancia del movimiento, en vez de que la respuesta del usuario se
    // degrade en silencio a una correccion. El 1600 del test de arriba es lo
    // que pasaria si esta rama usara buildContributionFields con cantidad 2.
    const it = { id: 'k2', type: 'Bank', quantity: 2, purchasePrice: 500, currentPrice: 500, currency: 'GTQ' }
    const { patch, income, flow } = planCellEdit({ item: it, oldValue: 1000, newValue: 1300, answer: ANSWER_FLOW })
    expect(flow).toEqual(expect.objectContaining({ type: 'DEPOSIT', amount: 300 }))
    expect(income).toBe(null)
    expect(getItemValue({ ...it, ...patch })).toBeCloseTo(1300, 6)
    expect(gainOf({ ...it, ...patch })).toBeCloseTo(0, 6)
  })

  test('FASE NF: sacar dinero de un fondo con cantidad = saldo aterriza exacto', () => {
    // El caso reportado: IDC-CASH guardado como cantidad 602.25 con precio 1.
    // Antes ni siquiera ofrecia la respuesta, asi que "saque dinero" terminaba
    // registrado como "perdio valor" y le desinflaba el retorno.
    const it = { id: 'idc', type: 'Cuenta monetaria', quantity: 602.25, purchasePrice: 1, currentPrice: 1, currency: 'GTQ' }
    const { patch, income, flow } = planCellEdit({ item: it, oldValue: 602.25, newValue: 210, answer: ANSWER_FLOW })
    expect(flow).toEqual(expect.objectContaining({ type: 'WITHDRAWAL', amount: 392.25, currency: 'GTQ' }))
    expect(income).toBe(null)
    expect(getItemValue({ ...it, ...patch })).toBeCloseTo(210, 6)
    expect(gainOf({ ...it, ...patch })).toBeCloseTo(0, 6)
  })

  test('si no se puede registrar el flujo, no se inventa ninguno', () => {
    const it = { id: 'x', type: 'Alternative', quantity: 4, purchasePrice: 100, currentPrice: 120 }
    const { flow, income, patch } = planCellEdit({ item: it, oldValue: 480, newValue: 400, answer: ANSWER_FLOW })
    expect(flow).toBe(null)
    expect(income).toBe(null)
    // Y cae al parche de correccion: la ganancia de 80 sobrevive.
    expect(gainOf({ ...it, ...patch })).toBeCloseTo(80, 6)
  })
})
