...
import { investmentIncomeOfMonth } from '../financeMonth'
...
describe('Flujo: un pago reinvertido nunca toco una cuenta bancaria', () => {
  const items = [FUND_REINVEST, FUND_CASH]

  it('lo deja fuera del ingreso del mes', () => {
    const r = investmentIncomeOfMonth([legacyPayment, realCash], { month: 2, year: 2026 }, null, items)
    expect(r.total).toBeCloseTo(40, 2)
    expect(r.count).toBe(1)
  })

  it('sin items entraba como ingreso (comportamiento anterior)', () => {
    const r = investmentIncomeOfMonth([legacyPayment, realCash], { month: 2, year: 2026 }, null)
    expect(r.total).toBeCloseTo(140, 2)
