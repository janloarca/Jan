...
describe('transferReversalPlan', () => {
  ...
  // el saldo de HOY, y una cuenta no puede quedar en negativo.
  test('quitar mas de lo que hay hoy deja la cuenta en cero, no en negativo', () => {
    ...
    const plan = transferReversalPlan(tx, [origen, destino])
    expect(accountValue({ ...destino, ...plan.to.fields })).toBe(0)
  })
})
