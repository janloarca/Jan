import { projectWealth, suggestSavingsRate, annualizedReturnPct, savingsRateFromProfile, projectToGoal, savingsRateForGoal, goalChartGeometry } from '../wealthProjection'
...
describe('projectToGoal', () => {
  ...
  it('los puntos mensuales cubren el recorrido completo, con el arranque adelante', () => {
    const r = projectToGoal(base)
    // 4 meses de 2026 (sep..dic) mas 12 de 2027 y 12 de 2028, mas el punto de
    // partida.
    expect(r.points.length).toBe(1 + 4 + 12 + 12)
    expect(r.points[0].value).toBe(100000)
    expect(r.points[r.points.length - 1].value).toBeCloseTo(r.endValue, 6)
  })

  it('los puntos van hacia adelante en el tiempo, sin saltos ni repetidos', () => {
    const r = projectToGoal(base)
    const key = (p) => p.year * 12 + p.month
    for (let i = 1; i < r.points.length; i++) {
      expect(key(r.points[i])).toBe(key(r.points[i - 1]) + 1)
    }
  })

  it('el cierre de cada anio de la grafica es EXACTAMENTE el de la lista', () => {
    // Sin esto la grafica y la lista de anios podrian contarle al usuario dos
    // historias distintas del mismo recorrido.
    const r = projectToGoal(base)
    for (const y of r.years) {
      const dic = r.points.find((p) => p.year === y.year && p.month === 11)
      expect(dic.value).toBeCloseTo(y.value, 6)
    }
  })
...
describe('goalChartGeometry', () => {
  const sube = [100, 200, 300, 400].map((value, i) => ({ year: 2026, month: i, value }))

  it('dibuja la meta cuando cabe en el eje, y marca donde se cruza', () => {
    const g = goalChartGeometry(sube, 300)
    expect(g.goalOffChart).toBe(false)
    expect(g.goalY).not.toBeNull()
    expect(g.cross.index).toBe(2)
  })

  it('una meta muy por encima NO se dibuja: aplastaria la trayectoria contra el piso', () => {
    const g = goalChartGeometry(sube, 40000)
    expect(g.goalOffChart).toBe(true)
    expect(g.goalY).toBeNull()
    // Y la trayectoria conserva su forma: el maximo sigue en lo mas alto que
    // el margen permite.
    expect(g.line.endsWith('6.00')).toBe(true)
  })

  it('una meta que cabe estira el eje, o sea la trayectoria ya no toca el techo', () => {
    const g = goalChartGeometry(sube, 500)
    expect(g.goalOffChart).toBe(false)
    expect(g.line.endsWith('6.00')).toBe(false)
    expect(Number(g.goalY)).toBeCloseTo(6, 6)
  })

  it('ningun punto toca el borde: el trazo no se come su propio grosor', () => {
    // Vale para el caso con la meta dentro del eje y para el que la deja fuera.
    for (const meta of [300, 500, 40000]) {
      const g = goalChartGeometry(sube, meta)
      const ys = g.line.split(' ').map((pt) => Number(pt.split(',')[1]))
      if (g.goalY != null) ys.push(g.goalY)
      expect(Math.min(...ys)).toBeGreaterThanOrEqual(6)
      expect(Math.max(...ys)).toBeLessThanOrEqual(94)
    }
  })

  it('sin cruce no inventa un marcador', () => {
    expect(goalChartGeometry(sube, 500).cross).toBeNull()
  })

  it('sin movimiento no dibuja nada, en vez de una raya pegada al borde', () => {
    const plano = [1, 2, 3].map((_, i) => ({ year: 2026, month: i, value: 500 }))
    expect(goalChartGeometry(plano, 500)).toBeNull()
  })

  it('con menos de dos puntos no hay linea que trazar', () => {
    expect(goalChartGeometry([{ year: 2026, month: 0, value: 100 }], 200)).toBeNull()
    expect(goalChartGeometry([], 200)).toBeNull()
  })
})
