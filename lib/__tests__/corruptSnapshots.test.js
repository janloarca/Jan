...
import { corruptSnapshotRunIds, feEraSuspectDailyIds, DELETABLE_SNAPSHOT_SOURCES, FE_CORRUPT_ERA } from '@/lib/corruptSnapshots'
...
describe('FASE GA: purga guiada por era (el residuo +$2K protegido por un deposito real)', () => {
  // Docs de la era FE (15 jul - 6 ago 2026) en ~25.2K, nivel real post-era
  // ~23.18K. El salto de entrada estaba medio-explicado por el deposito real
  // de XOCHI, asi que las bandas estadisticas lo perdonaban.
  const eraSnap = (dateStr, value, source = 'daily') => ({ id: dateStr, date: dateStr, netWorthUSD: value, _source: source })

  function eraScenario() {
    return [
      eraSnap('2026-07-10', 22300),
      eraSnap('2026-07-14', 22350),
      eraSnap('2026-07-15', 25200),
      eraSnap('2026-07-16', 25210),
      eraSnap('2026-07-19', 25208),
      eraSnap('2026-08-07', 23180),
      eraSnap('2026-08-08', 23200),
    ]
  }

  it('borra los dailies de la era desviados >5% del nivel post-era', () => {
    const ids = feEraSuspectDailyIds(eraScenario())
    expect(ids.sort()).toEqual(['2026-07-15', '2026-07-16', '2026-07-19'])
  })

  it('un daily de la era AL nivel real se respeta', () => {
    const rows = [...eraScenario(), eraSnap('2026-07-20', 23300)]
    const ids = feEraSuspectDailyIds(rows)
    expect(ids).not.toContain('2026-07-20')
  })

  it('sin evidencia post-era no hay veredicto', () => {
    const rows = eraScenario().filter((s) => s.date <= FE_CORRUPT_ERA.to)
    expect(feEraSuspectDailyIds(rows)).toHaveLength(0)
  })

  it('backfill/manual/broker dentro de la era jamas se tocan', () => {
    const rows = [
      ...eraScenario(),
      eraSnap('2026-07-21', 25300, 'backfill'),
      eraSnap('2026-07-22', 25300, 'manual'),
      { id: '2026-07-23~nav~ibkr', date: '2026-07-23', netWorthUSD: 10012, _source: 'ibkr' },
    ]
    const ids = feEraSuspectDailyIds(rows)
    expect(ids).not.toContain('2026-07-21')
    expect(ids).not.toContain('2026-07-22')
    expect(ids).not.toContain('2026-07-23~nav~ibkr')
  })

  it('docs fuera de la era jamas se tocan aunque esten desviados', () => {
    const rows = [...eraScenario(), eraSnap('2026-06-01', 30000)]
    expect(feEraSuspectDailyIds(rows)).not.toContain('2026-06-01')
  })
})
