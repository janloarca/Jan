import {
  ...
  publishBlockedBy, PUBLISH_GATES,
} from '../friendsPublish'
import fs from 'fs'
...
describe('qué cuenta como "hay algo que publicar"', () => {
  ...
  // ⛔ FASE NA. Con datos de DEMO no se publica NADA — y filtrar los items no
  // alcanzaría: los retornos llegan calculados sobre el conjunto completo, así
  // que UN item de ejemplo contamina la cifra entera. El contrato del modo demo
  // es "cero side-effects persistentes" (lib/demoData.js) y este era el que
  // faltaba: publicarle a un grupo un ranking con dinero inventado.
  test('con UN item de demo entre items reales no se publica nada', () => {
    const items = [item(), item({ id: 'demo-1', symbol: 'AAPL', _source: 'demo' })]
    const stats = buildPublishStats({ enrichedItems: items, returnYTD: 5, dailyChange: 1, totalAssets: 1000 })
    expect(hasSomethingToPublish({ stats, enrichedItems: items })).toBe(false)
    expect(buildPublishPayload({ enrichedItems: items, returnYTD: 5, dailyChange: 1, totalAssets: 1000 })).toBe(null)
  })

  test('una cartera 100% demo tampoco publica, aunque tenga cifras medibles', () => {
    const items = [item({ id: 'demo-1', _source: 'demo' })]
    const stats = buildPublishStats({ enrichedItems: items, returnYTD: 7, dailyChange: 2, totalAssets: 500 })
    expect(hasSomethingToPublish({ stats, enrichedItems: items })).toBe(false)
  })
})
...
// ⛔ Publicar un número EQUIVOCADO es peor que publicar uno viejo: acá lo leen
// otras personas y queda rankeado al lado de los suyos. Estas compuertas son las
// mismas de los cuatro escritores de snapshots, y la lista YA divergió una vez:
// el tablero la escribió a mano y se olvidó de las dos últimas mientras su
// propio comentario afirmaba que eran idénticas, y /friends no miraba ninguna.
describe('cuándo es seguro publicar', () => {
  const open = {
    dataLoading: false, pricesLoading: false, pricesFetching: false,
    ratesLoading: false, bulkWriting: false, ibkrAutoSyncing: false, ytdResolved: true,
  }

  test('con todo asentado no bloquea nada', () => {
    expect(publishBlockedBy(open)).toBe(null)
  })

  // Recorre la lista en vez de nombrar cada compuerta: así, agregar una a
  // PUBLISH_GATES y no respetarla falla acá en vez de pasar en silencio.
  test.each(PUBLISH_GATES)('%s cerrada bloquea la publicación', (gate) => {
    expect(publishBlockedBy({ ...open, [gate]: true })).toBe(gate)
  })

  test('el ancla del año sin resolver bloquea (es la única que bloquea en falso)', () => {
    expect(publishBlockedBy({ ...open, ytdResolved: false })).toBe('ytdResolved')
  })

  // El caso literal del defecto: estas dos faltaban. `bulkWriting` cubre un
  // import masivo, cuyos estados intermedios muestran el portafolio a medias o
  // duplicado (FASE GB); `ibkrAutoSyncing` cubre la descarga+escritura de un
  // sync de broker.
  test('bulkWriting e ibkrAutoSyncing están en la lista', () => {
    expect(PUBLISH_GATES).toContain('bulkWriting')
    expect(PUBLISH_GATES).toContain('ibkrAutoSyncing')
  })

  // Un objeto vacío es "no sé nada": tiene que bloquear, nunca dar permiso.
  test('sin información no se publica', () => {
    expect(publishBlockedBy()).toBeTruthy()
    expect(publishBlockedBy({})).toBeTruthy()
  })

  // GUARDIÁN DE FUENTE. Las dos superficies que publican tienen que usar la
  // función compartida y no una lista propia: el bug original fue exactamente
  // eso, dos listas escritas a mano que decían cosas distintas. Se lee el
  // ARCHIVO porque el efecto del hook y el de la pantalla no se pueden montar
  // acá sin arrastrar Firestore entero.
  describe('las dos superficies usan la definición compartida', () => {
    ...
    ])('%s llama a publishBlockedBy', (file) => {
      const src = read(file)
      expect(src).toMatch(/publishBlockedBy\(/)
      expect(src).toMatch(/from '@\/lib\/friendsPublish'/)
    })

    // Y le pasan el juego COMPLETO: pasarle un subconjunto deja abierta en
    // silencio la compuerta que se omitió, que es el mismo defecto con otra
    // forma.
    test.each([
      ['hooks/useDashboardData.js'],
      ['app/friends/page.jsx'],
    ])('%s le pasa todas las compuertas', (file) => {
      const src = read(file)
      const call = src.slice(src.indexOf('publishBlockedBy('))
      const body = call.slice(0, call.indexOf('})') + 2)
      for (const gate of [...PUBLISH_GATES, 'ytdResolved']) {
