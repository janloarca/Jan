...
describe('desglose del YTD: colocación en el tablero', () => {
  ...
  it('la sección se monta DENTRO del grid superior, después de las dos cards', () => {
    const gridStart = page.indexOf('grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5')
    ...
    // El cierre del grid: el primer </ErrorBoundary> después de su apertura.
    const gridEnd = page.indexOf('</ErrorBoundary>', gridStart)
    expect(networth).toBeGreaterThan(gridStart)
    expect(chart).toBeGreaterThan(networth)
    expect(ytd).toBeGreaterThan(chart)
    expect(ytd).toBeLessThan(gridEnd)
  })
