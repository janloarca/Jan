...
// ── El rastro por transporte ────────────────────────────────────────────────
//
// Este camino resolvía el token (lo que estampa `lastUsedAt`) y nunca estampaba
// CÓMO terminó, así que desde Ajustes "el reenvío no está llegando" y "llega y
// lo descartamos" se veían idénticos.
describe('el barrido deja constancia de que el correo llegó', () => {
  const stamped = []
  beforeEach(() => { stamped.length = 0 })

  it('estampa el resultado marcado como correo, no como atajo', async () => {
    jest.resetModules()
    jest.doMock('../ingestTokens', () => ({
      TOKEN_RE: /^[a-f0-9]{32}$/,
      resolveIngestToken: async (_db, token) => (token === TOKEN ? { uid: 'u1' } : null),
      readUserRules: async () => [],
      readUserBaseCurrency: async () => 'GTQ',
      stampIngestResult: async (_db, token, result, source) => { stamped.push({ token, result, source }) },
    }))
    const { ingestParsedEmail } = require('../emailIngest')
    const headers = new Map([['delivered-to', `gastos+${TOKEN}@chispu.xyz`]])
    const out = await ingestParsedEmail({ db: {}, parsed: { subject: 'Alerta', text: 'x' }, headers })
    expect(out.status).toBe('created')
    expect(stamped).toEqual([{ token: TOKEN, result: 'created', source: 'email' }])
  })
})
