...
describe('staleBackfillDates', () => {
  ...
  describe('buildNavByDate (FASE HN)', () => {
    ...
    it('ignora anclas de calibracion', () => {
      const out = buildNavByDate([
        { id: '2026-01-01~cal~ibkr', date: '2026-01-01', _source: 'ibkr', _account: 'ibkr', netWorthUSD: 5000 },
        { id: '2026-01-05', date: '2026-01-05', _source: 'ibkr', _calibrated: true, netWorthUSD: 5100 },
      ])
      expect(out.size).toBe(0)
    })

    // FASE NV. Este test fijaba antes que la transcripcion trimestral se
    // IGNORABA ("ignora anclas de calibracion y transcripciones trimestrales").
    // Ese valor esperado describia el comportamiento (el default en ['ibkr']),
    // no un invariante: la Hoja y staleBackfillDates ya la trataban como
    // observacion real del broker, y la cadena de composicion no la veia.
    // Actualizarlo es correcto aca, a diferencia del candado de 3.94%.
    it('FASE NV: una transcripcion trimestral SI es NAV real del broker', () => {
      const out = buildNavByDate([
        { id: '2025-03-31', date: '2025-03-31', _source: 'ibkr_quarterly', netWorthUSD: 7000 },
      ])
      expect(out.get('2025-03-31')).toBe(7000)
      expect(out.size).toBe(1)
    })

    it('FASE NV: en la misma fecha el NAV diario gana al trimestral, en los DOS ordenes', () => {
      const daily = { id: '2025-12-31~nav~ibkr', date: '2025-12-31', _source: 'ibkr', netWorthUSD: 5432.98 }
      const quarterly = { id: '2025-12-31', date: '2025-12-31', _source: 'ibkr_quarterly', netWorthUSD: 5400 }
      expect(buildNavByDate([daily, quarterly]).get('2025-12-31')).toBe(5432.98)
      expect(buildNavByDate([quarterly, daily]).get('2025-12-31')).toBe(5432.98)
    })

    it('FASE NV: dos docs diarios de la misma fecha conservan la regla de siempre (gana el ultimo)', () => {
      const out = buildNavByDate([
        { id: '2026-08-06', date: '2026-08-06', _source: 'ibkr', netWorthUSD: 10000 },
        { id: '2026-08-06~nav~ibkr', date: '2026-08-06', _source: 'ibkr', netWorthUSD: 10050 },
      ])
      expect(out.get('2026-08-06')).toBe(10050)
    })

    it('FASE NV: un caller que pase su propia lista de fuentes la manda', () => {
      const out = buildNavByDate([
        { date: '2025-03-31', _source: 'ibkr_quarterly', netWorthUSD: 7000 },
        { date: '2025-04-01', _source: 'ibkr', netWorthUSD: 7100 },
      ], ['ibkr'])
      expect(out.size).toBe(1)
      expect(out.get('2025-04-01')).toBe(7100)
    })
  })

  // FASE NV. Lo que un NAV trimestral puede y no puede hacer dentro de la
  // composicion: llena SU fecha y los dias sin mercado pegados a ella, y nada
  // mas. Un cierre de trimestre arrastrado a lo ancho del trimestre archivaria
  // meses enteros desde una sola medicion gruesa.
  describe('composeDailyTotals con NAV trimestral (FASE NV)', () => {
    const t = (d) => Date.parse(`${d}T00:00:00Z`)
    const nav = buildNavByDate([
      { date: '2025-03-31', _source: 'ibkr_quarterly', netWorthUSD: 7000 },
    ])
    const manualPoints = [
      { ts: t('2025-03-31'), total: 13000 },
      { ts: t('2025-04-02'), total: 13100 },
      { ts: t('2025-04-30'), total: 13500 },
    ]

    it('el cierre del trimestre y su fin de semana se componen con el NAV transcrito', () => {
      const out = composeDailyTotals({
        gaps: ['2025-03-31', '2025-04-01', '2025-04-02'],
        manualPoints, navByDate: nav, hasBrokerItems: true,
      })
      expect(out).toEqual([
        { date: '2025-03-31', total: 20000, composed: true },
        { date: '2025-04-01', total: 20000, composed: true },
        { date: '2025-04-02', total: 20100, composed: true },
      ])
    })

    it('lejos del cierre el dia sigue siendo un hueco: el trimestral no se arrastra un mes', () => {
      const out = composeDailyTotals({
        gaps: ['2025-04-30'],
        manualPoints, navByDate: nav, hasBrokerItems: true,
      })
