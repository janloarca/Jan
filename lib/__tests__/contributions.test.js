...
// timers falsos para que las aserciones de abajo sean deterministas. La suite
// corre en America/Guatemala (jest.config.js), así que un instante a media
// mañana local no puede cruzar ningún borde de día por accidente.
...
describe('buildContributionFields — bank-like (balance) assets', () => {
  ...
  it('add shifts both balance fields up', () => {
    ...
    expect(itemFields).toEqual({ purchasePrice: 6500, currentPrice: 6500, balanceAsOf: TODAY })
  ...
  it('withdraw shifts both balance fields down, floored at 0 (and empties the quantity)', () => {
    ...
    expect(itemFields).toEqual({ purchasePrice: 0, currentPrice: 0, quantity: 0, balanceAsOf: TODAY })
  })
  ...
  it('preserves a market-value gap on static assets (bond above cost)', () => {
    ...
    expect(itemFields).toEqual({ purchasePrice: 11000, currentPrice: 11500, balanceAsOf: TODAY })
  })
  ...
  it('falls back to purchasePrice when currentPrice is missing', () => {
    ...
    expect(itemFields).toEqual({ purchasePrice: 2500, currentPrice: 2500, balanceAsOf: TODAY })
  })

  // ⛔ FASE OM: la aserción con dientes contra el fix en sí. Sin sellar
  // `balanceAsOf`, el motor de rendimiento deducido (lib/liquidYield.js) mide
  // un aporte de HOY contra una foto vieja y lo lee como interés que la cuenta
  // nunca generó — ver la integración completa en liquidYield.test.js.
  it('sella balanceAsOf a HOY, sin importar la fecha `date` del movimiento (que puede estar backdateada)', () => {
    const backdated = buildContributionFields({
      item: bank, amount: 100, date: '2020-01-01', isAdd: true,
    })
    expect(backdated.itemFields.balanceAsOf).toBe(TODAY)
  })
})
