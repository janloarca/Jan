...
describe('uncovered-shares', () => {
  ...
  // action: null (esta pasada): el propio comentario del código ya decía que
  // esto es irresoluble ("ningún componente del repo edita lotes") pero el
  // push seguía ofreciendo {kind:'edit-item'}, contradicción directa entre el
  // comentario y el código.
  it('has no action to offer (editing a lot is not possible anywhere in the app)', () => {
    const lots = [{ symbol: 'VOO', quantity: 4, status: 'open', acquisitionDate: '2025-01-01' }]
    const f = byCode(run([stock()], [], lots), 'uncovered-shares')
...
describe('metadata checks', () => {
  ...
  // isTracked gate (esta pasada): a diferencia de sus hermanos pre-gate
  // (no-market-price, income-mismatch), no-acq-date no chequeaba isTracked —
  // podía disparar sobre deuda o un ítem excluido del patrimonio, que no
  // tienen "fecha de adquisición" como algo que preguntarles.
  it('does not flag debt or net-worth-excluded items even with no acquisitionDate (isTracked gate)', () => {
    const debt = bank({ id: 'd1', acquisitionDate: '', isDebt: true })
    // isExcludedFromNetWorth: isReceivable && !countInNetWorth (mismo fixture
    // que el test de exención de arriba, "exempts broker-sourced, debt,
    // excluded and dust items").
    const excluded = bank({ id: 'e1', acquisitionDate: '', isReceivable: true })
    const r = run([debt, excluded])
    expect(codes(r)).not.toContain('no-acq-date')
  })
  ...
  // FASE (esta pasada): 'broken-link' colisionaba con el hallazgo hermano de
  // linkedDebtId (mismo código → mismo id `code:itemId` si un ítem tuviera las
  // dos referencias rotas a la vez). Separados en 'broken-link-income' y
  // 'broken-link-debt'.
  it('flags broken income destination links, resolves by id/symbol/name', () => {
    ...
    const brokenFindings = r.findings.filter((f) => f.code === 'broken-link-income')
    expect(brokenFindings).toHaveLength(1)
    expect(brokenFindings[0].itemId).toBe('c')
  })

  it('broken-link-income always suggests clearing the dead reference (nothing real to point at instead)', () => {
    const broken = bank({ id: 'c', incomeRate: 5, incomeMonths: [1], incomeDestination: 'deleted-id' })
    const r = run([broken])
    const f = byCode(r, 'broken-link-income')
  ...
  describe('stale-value: id bucketizado por tiempo (mismo patrón que NotificationCenter/FASE ND)', () => {
    // STALE_DAYS = 180 en el módulo (no exportado). daysAgo() deriva la
    // fecha de NOW en vez de escribirla a mano, para que el bucket
    // (Math.floor(diffDays/180)) sea exacto y no dependa de contar días de
    // calendario: 200 y 210 caen los dos en (180,360] → bucket 1; 381 cae
    // en (360,540] → bucket 2. Los tres offsets se eligen con margen de
    // sobra de cualquier frontera de 180 días.
    const daysAgo = (days) => {
      const d = new Date(NOW)
      d.setUTCDate(d.getUTCDate() - days)
      return d.toISOString().slice(0, 10)
    }
    const withLastTouch = (days) => bank({ acquisitionDate: daysAgo(days), createdAt: `${daysAgo(days)}T00:00:00Z` })

    it('el mismo bucket (lastTouch distinto, misma ventana de 180 días) produce el MISMO id', () => {
      const idA = byCode(run([withLastTouch(200)]), 'stale-value').id
      const idB = byCode(run([withLastTouch(210)]), 'stale-value').id
      expect(idA).toBe(idB)
      expect(idA).toBe('stale-value:b1:1')
    })

    it('lastTouch desplazado más de STALE_DAYS produce un id DISTINTO', () => {
      const idNear = byCode(run([withLastTouch(200)]), 'stale-value').id
      const idFar = byCode(run([withLastTouch(381)]), 'stale-value').id
      expect(idNear).not.toBe(idFar)
      expect(idFar).toBe('stale-value:b1:2')
    })
  })
...
describe('no-market-price: a real holding silently worth $0', () => {
  ...
  // Field-routing (esta pasada): las dos ramas apuntan al mismo campo
  // editable (resolveFocusTarget's 'currentValue' → isMarket → precio de
  // compra, la cascada real detrás de ambos hallazgos), para que el botón
  // de "Ver campo"/acción lleve directo ahí en vez de a una edición genérica.
  it('both no-market-price and no-market-price-stale route to field "currentValue"', () => {
    const noPrice = byCode(run([stock({ currentPrice: 0, purchasePrice: 0 })]), 'no-market-price')
    expect(noPrice.action).toEqual({ kind: 'edit-item', field: 'currentValue' })
    const stale = byCode(run([stock({ currentPrice: 0 })]), 'no-market-price-stale')
    expect(stale.action).toEqual({ kind: 'edit-item', field: 'currentValue' })
  })
...
describe('no-symbol: a market asset that cannot be price-synced or matched', () => {
  ...
  // action: null (esta pasada): NO existe ningún campo editable de símbolo en
  // EditAccountModal.jsx (grep exhaustivo, verificado); ofrecer un botón de
  // acción llevaba a una edición genérica sin sentido. El caller (Chispu-
  // Suggestions) tiene que guardar contra esto, no este módulo — se fija acá
  // el contrato que ese guard asume.
  it('has no action to offer (nothing editable exists for this field)', () => {
    const f = byCode(run([stock({ symbol: '' })]), 'no-symbol')
    expect(f.action).toBeNull()
  })
})
...
describe('scores and ordering', () => {
  it('stable ids, severity ordering, and weighted global score', () => {
    ...
    // El id de `stale-value` lleva un sufijo de bucket temporal A PROPÓSITO
    // (dedup consciente del tiempo, mismo patrón que NotificationCenter/
    // FASE ND) — el invariante real es que el id EMPIEZA con `code:itemId`,
    // no que sea exactamente eso. `dirty` (creado en 2024-12-01, sin
    // acquisitionDate) es justo un ítem que ahora cae en esa rama.
    expect(r.findings.every((f) => f.id.startsWith(`${f.code}:${f.itemId || 'global'}`))).toBe(true)
...
// El caller (útil desde dividendVerify.js) le pasa el veredicto ya calculado;
// este módulo solo lo traduce a hallazgo. Sin veredicto no hay nada que decir
// — verificado antes como no cubierto por ningún test previo.
describe('income-mismatch: declared yield vs what the broker actually paid', () => {
  it('flags a mismatch verdict and routes to the income config field', () => {
    const item = stock({ id: 's1' })
    const incomeVerification = new Map([['s1', { status: 'mismatch', projected: 500, actual: 50, payments: 2 }]])
    const r = run([item], [], [], { incomeVerification })
    const f = byCode(r, 'income-mismatch')
    expect(f).toBeDefined()
    expect(f.severity).toBe('medium')
    expect(f.action).toEqual({ kind: 'edit-item', field: 'incomeConfig' })
  })

  it('without an incomeVerification map, no finding (older callers unaffected)', () => {
    const item = stock({ id: 's1' })
    const r = run([item])
    expect(codes(r)).not.toContain('income-mismatch')
  })

  it('a non-"mismatch" verdict does not flag', () => {
    const item = stock({ id: 's1' })
    const incomeVerification = new Map([['s1', { status: 'verified', projected: 500, actual: 500, payments: 2 }]])
    const r = run([item], [], [], { incomeVerification })
    expect(codes(r)).not.toContain('income-mismatch')
  })
})
...
describe('broken-link cubre el préstamo vinculado de un inmueble', () => {
  ...
  it('avisa cuando el préstamo vinculado ya no existe', () => {
    ...
    const f = r.findings.find((x) => x.code === 'broken-link-debt' && x.itemId === 'casa')
    expect(f).toBeTruthy()
  ...
  it('con el préstamo presente no dice nada', () => {
    ...
    expect(r.findings.some((x) => x.code === 'broken-link-debt')).toBe(false)
  })

  it('broken-link-income y broken-link-debt no colisionan cuando el mismo ítem tiene las dos referencias rotas', () => {
    const r = run([casa({ id: 'z', linkedDebtId: 'borrada', incomeDestination: 'tampoco-existe', incomeRate: 5, incomeMonths: [1] })])
    const income = r.findings.find((x) => x.code === 'broken-link-income' && x.itemId === 'z')
    const debt = r.findings.find((x) => x.code === 'broken-link-debt' && x.itemId === 'z')
    expect(income).toBeTruthy()
    expect(debt).toBeTruthy()
    expect(income.id).not.toBe(debt.id)
  })
})
...
// El hermano de partial-history del otro lado: los movimientos netean
// NEGATIVO sobre un saldo positivo (salió más de lo que entró y la cuenta
// igual tiene dinero). Sin cobertura previa en este archivo.
describe('flows-exceed-balance: movements net negative on a positive balance', () => {
  const cuenta = (over = {}) => ({
    id: 'fx', type: 'Bank', name: 'Cuenta con fuga', symbol: 'FX', institution: 'IDC',
    quantity: 1, purchasePrice: 500, currentPrice: 500, currency: 'GTQ',
    acquisitionDate: '2024-01-01', createdAt: '2024-01-01T00:00:00Z', ...over,
  })
  // 1,000 salieron (sin ningún ingreso) contra un saldo de 500: el exceso
  // (1,000) supera el piso de 200, así que dispara.
  const txs = [{ type: 'WITHDRAWAL', date: '2024-02-01', totalAmount: 1000, currency: 'GTQ', _linkedItemId: 'fx' }]

  it('flags it and routes to the movements field', () => {
    const f = byCode(run([cuenta()], txs), 'flows-exceed-balance')
    expect(f).toBeTruthy()
    expect(f.severity).toBe('medium')
    expect(f.action).toEqual({ kind: 'edit-item', field: 'movements' })
    expect(f.textEs).toContain('1,000')
  })

  it('a confirmed origin (_newMoneyConfirmed) does not re-ask', () => {
    expect(codes(run([cuenta({ _newMoneyConfirmed: true })], txs))).not.toContain('flows-exceed-balance')
  })

  // Mismo caso que documenta el comentario del código: 500 de entrada, 520 de
  // salida, saldo 500. El exceso (20) no llega al piso de flows-exceed-
  // balance, pero la rama negativa CORTA igual (un `else if` encadenado
  // dejaba caer esto a partial-history diciendo "faltan aportes por 520"
  // sobre una cuenta de 500 — dos afirmaciones falsas a la vez).
  it('the negative branch cuts even below its own floor — never falls through to partial-history', () => {
    const mixed = [
      { type: 'DEPOSIT', date: '2024-01-15', totalAmount: 500, currency: 'GTQ', _linkedItemId: 'fx' },
      { type: 'WITHDRAWAL', date: '2024-02-01', totalAmount: 520, currency: 'GTQ', _linkedItemId: 'fx' },
    ]
    const r = run([cuenta()], mixed)
    expect(codes(r)).not.toContain('flows-exceed-balance')
    expect(codes(r)).not.toContain('partial-history')
  })
})
