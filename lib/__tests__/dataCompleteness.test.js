...
describe('metadata checks', () => {
  it('flags missing acquisitionDate even for broker items', () => {
    const r = run([bank({ acquisitionDate: '', _source: 'ibkr' })])
    expect(byCode(r, 'no-acq-date')).toBeDefined()
  })
  ...
  it('flags duplicate suspects sharing symbol+institution', () => {
    const r = run([bank({ id: 'p1' }), bank({ id: 'p2', name: 'Cuenta Líquida 2' })])
    expect(byCode(r, 'dup-suspect')).toBeDefined()
  })
})
