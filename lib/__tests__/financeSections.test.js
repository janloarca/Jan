import { commitmentsHaveContent, yearInViewHasContent, weekdayRhythmHasContent, nowMonthKey, nowDateKey } from '@/lib/financeSections'
import fs from 'fs'
...
describe('¿la sección tiene algo debajo?', () => {
  ...
  // El ritmo NO es un compromiso: un gasto suelto no puede encender la sección
  // de "a qué estoy amarrado". Las dos afirmaciones juntas son las que fijan
  // que son dos preguntas distintas.
  test('un gasto suelto enciende el ritmo pero no los compromisos', () => {
    const txs = [
      { id: '1', type: 'EXPENSE', date: daysAgo(10), amount: 120, currency: 'GTQ', merchant: 'Cafe', category: 'Alimentación' },
    ]
    expect(weekdayRhythmHasContent(txs)).toBe(true)
    expect(commitmentsHaveContent(txs)).toBe(false)
  })

  test('sin gastos, o con gastos fuera de la ventana, el ritmo no existe', () => {
    expect(weekdayRhythmHasContent([])).toBe(false)
    expect(weekdayRhythmHasContent(null)).toBe(false)
    expect(weekdayRhythmHasContent([
      { id: '1', type: 'EXPENSE', date: daysAgo(400), amount: 120, currency: 'GTQ', category: 'Alimentación' },
    ])).toBe(false)
  })

  // Una transferencia degradada (lib/cardPaymentNetting.js) no es gasto: si lo
  // fuera, pagar la tarjeta dibujaría una card de ritmo sobre dinero que solo
  // se movió entre cuentas propias.
  test('una transferencia no enciende el ritmo', () => {
    expect(weekdayRhythmHasContent([
      { id: '1', type: 'EXPENSE', date: daysAgo(5), amount: 8175, currency: 'GTQ', category: 'Transferencia Enviada' },
    ])).toBe(false)
  })
...
describe('las cards siguen preguntándole al mismo selector', () => {
  ...
  test.each([
    ...
    ['WeekdayRhythmCard.jsx', 'weekdayRhythmHasContent'],
