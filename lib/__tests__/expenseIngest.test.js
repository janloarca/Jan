...
import { resolveOccurredAt } from '../sameCharge'
...
describe('una hora sin zona no es un instante', () => {
  

  it('la descarta y usa la hora de llegada, que sí es cierta', () => {
    // El caso real: el atajo arma occurredAt con "Format Current Date → ISO
    // 8601". Si iOS no le pone el offset, el servidor (UTC) leería 07:17 de
    // Guatemala como 07:17 UTC: SEIS horas corrido. Y seis horas rompen justo
    // lo que occurredAt existe para decidir (mismo cobro vs dos cobros), así
    // que el mismo consumo capturado por atajo y por correo se guardaría dos
    // veces.
    const out = resolveOccurredAt({
      reported: '2026-08-19T07:17:00',
      received: '2026-08-19T13:17:30Z',
    })
    expect(out).toEqual({ occurredAt: '2026-08-19T13:17:30.000Z', timeSource: 'received' })
  })

  it('con offset la usa tal cual, que es el caso bueno', () => {
    expect(resolveOccurredAt({ reported: '2026-08-19T07:17:00-06:00' }))
      .toEqual({ occurredAt: '2026-08-19T13:17:00.000Z', timeSource: 'reported' })
    expect(resolveOccurredAt({ reported: '2026-08-19T13:17:00Z' }))
      .toEqual({ occurredAt: '2026-08-19T13:17:00.000Z', timeSource: 'reported' })
    // Sin los dos puntos en el offset, que también es ISO 8601 válido.
    expect(resolveOccurredAt({ reported: '2026-08-19T07:17:00-0600' })?.occurredAt)
      .toBe('2026-08-19T13:17:00.000Z')
  })

  it('sin nada con zona prefiere no dar hora antes que dar una corrida', () => {
    // Sin hora se cae al emparejado por comercio del mismo día, que es peor pero
    // honesto. Una hora inventada haría veredictos de "cobro distinto" firmes y
    // equivocados.
    expect(resolveOccurredAt({ reported: '2026-08-19T07:17:00' })).toBeNull()
  })

  it('un Date de verdad sigue funcionando', () => {
    // toISOString() siempre termina en Z, así que nunca cae en el guard.
    expect(resolveOccurredAt({ reported: new Date('2026-08-19T13:17:00Z') })?.timeSource).toBe('reported')
  })
})
