...
describe('assessDataSources', () => {
  ...
  // El filtro preguntaba por `it._demo`, un campo que ningún escritor pone: la
  // convención real es `_source: 'demo'`. Con el filtro inerte, quien está
  // explorando con datos de ejemplo recibía un puntaje de completitud sobre
  // posiciones que no son suyas.
  describe('las posiciones de la demo no cuentan como datos del usuario', () => {
    const demo = items.map((it) => ({ ...it, _source: 'demo' }))

    it('una cartera que es SOLO demo no tiene posiciones', () => {
      const a = assessDataSources({ items: demo, snapshots: months(6) })
      expect(a.stages.find((s) => s.key === 'positions').done).toBe(false)
      expect(a.stages.find((s) => s.key === 'positions').count).toBe(0)
      expect(a.next).toBe('positions')
    })

    it('con la demo al lado, solo cuentan las posiciones reales', () => {
      const a = assessDataSources({
        items: [...demo, { id: 'real', symbol: 'NVDA', _source: 'manual' }],
        snapshots: months(6),
        transactions: [tx('BUY', '2026-02-01', 100, 'NVDA')],
      })
      expect(a.stages.find((s) => s.key === 'positions').count).toBe(1)
      // Los símbolos de la demo tampoco entran al denominador de cobertura: con
      // ellos adentro, importar el ledger completo nunca llegaría al 100%.
      expect(a.symbolCoverage).toBe(1)
    })
  })
})
