...
describe('planEquitySnapshotWrites: donde aterriza cada NAV', () => {
  // FASE NU: un NAV fechado despues de hoy no es una observacion y no se
  // escribe; el de HOY si (el cierre de hoy es la ultima fecha posible).
  it('no escribe un NAV fechado en el FUTURO, y si el de hoy', () => {
    const out = planEquitySnapshotWrites(
      [eq('2026-09-30', 10008.97), eq('2026-09-04', 10008.97), eq('2026-09-03', 9990)],
      [], toUSD, { todayStr: '2026-09-04' }
    )
    expect(out.map((s) => s.date).sort()).toEqual(['2026-09-03', '2026-09-04'])
  })
