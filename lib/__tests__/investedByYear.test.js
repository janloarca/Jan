...
describe('computeInvestedByYear', () => {
  ...
  // ⛔ Este test afirmaba lo CONTRARIO ("un anio sin datos no aparece"), o sea
  // documentaba el defecto: la lista salia 2026, 2025, 2024, 2022, 2021 y el
  // hueco de 2023 se leia como que la app perdio un anio. Actualizar el valor
  // esperado es correcto ACA (a diferencia del candado de 3.94%) porque
  // describia el comportamiento, no un invariante, y el comportamiento era el
  // bug. Ver el comentario del `continue` en investedByYear.js.
  test('un anio DENTRO del rango del archivo tiene fila aunque este vacio', () => {
    const r = computeInvestedByYear({ transactions: [], items: [], series, convert, baseCurrency: 'USD', nowTs: NOW })
    const y2026 = r.rows.find((x) => x.year === 2026)
    expect(y2026).toBeDefined()
    expect(y2026.empty).toBe(true)
    expect(y2026.gainAbs).toBeNull()
  })

  // El caso REAL que lo destapo: la tabla salteaba 2023 en medio.
  test('un anio intermedio sin nada NO deja un hueco en la cronologia', () => {
    const txs = [
      { type: 'DEPOSIT', date: '2022-03-01', totalAmount: 800, currency: 'USD' },
      { type: 'DEPOSIT', date: '2024-03-01', totalAmount: 500, currency: 'USD' },
    ]
    const r = computeInvestedByYear({ transactions: txs, items: [], series, convert, baseCurrency: 'USD', nowTs: NOW })
    const anios = r.rows.map((x) => x.year)
    // Consecutivos, sin saltos: un anio que falta se lee como app rota.
    for (let i = 1; i < anios.length; i++) expect(anios[i - 1] - anios[i]).toBe(1)
    expect(anios).toContain(2023)
    expect(r.rows.find((x) => x.year === 2023).empty).toBe(true)
  })

  // El invariante que SI hay que conservar: un anio anterior al primer dato
  // del archivo no es un hueco, es que no existia.
  test('un anio ANTERIOR al archivo sigue fuera', () => {
    const r = computeInvestedByYear({ transactions: [], items: [], series, convert, baseCurrency: 'USD', nowTs: NOW })
    expect(r.rows.find((x) => x.year === 2019)).toBeUndefined()
  })
