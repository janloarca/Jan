import { recordedOriginTotal, isOriginFullyRecorded, unlinkedOpeningDeposits } from '../originDeposits'
...
describe('unlinkedOpeningDeposits', () => {
  const items = [{ id: 'vitali', symbol: 'VITALI' }, { id: 'fondo', symbol: 'IDC-CASH' }]
  const orphan = { id: 'd1', type: 'DEPOSIT', symbol: 'VITALI', totalAmount: 6098, _source: 'manual_new_account' }

  it('heals the deposit our own onAdd wrapper left unlinked', () => {
    expect(unlinkedOpeningDeposits([orphan], items)).toEqual([{ id: 'd1', itemId: 'vitali' }])
  })

  it('leaves an already-linked deposit alone', () => {
    expect(unlinkedOpeningDeposits([{ ...orphan, _linkedItemId: 'vitali' }], items)).toEqual([])
  })

  it('never touches a deposit from another flow', () => {
    expect(unlinkedOpeningDeposits([{ ...orphan, _source: 'manual_cashflow' }], items)).toEqual([])
    expect(unlinkedOpeningDeposits([{ ...orphan, _source: 'ibkr' }], items)).toEqual([])
  })

  it('refuses to guess when two accounts share the symbol', () => {
    const twoBrokers = [...items, { id: 'vitali2', symbol: 'VITALI' }]
    expect(unlinkedOpeningDeposits([orphan], twoBrokers)).toEqual([])
  })

  it('ignores a symbol that matches nothing', () => {
    expect(unlinkedOpeningDeposits([{ ...orphan, symbol: 'OTRO' }], items)).toEqual([])
  })
})
