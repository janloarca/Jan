...
describe('FASE NN: la calibración por cuenta que el broker contradice', () => {
  ...
  // FASE NV. La transcripción trimestral de PortfolioAnalyst es una observación
  // del broker (más gruesa que el NAV diario, pero suya), así que también sirve
  // de evidencia: sin ella, quien solo transcribió trimestres conservaba una
  // calibración envenenada que su propia transcripción contradice.
  it('FASE NV: el cierre trimestral del 31 de diciembre también juzga', () => {
    const quarterly = [{ date: '2025-12-31', _source: 'ibkr_quarterly', netWorthUSD: 5432.98 }]
    expect(contradictedAccountCalibrations([cal(9305.22)], quarterly)).toHaveLength(1)
    expect(contradictedAccountCalibrations([cal(5450)], quarterly)).toEqual([])
  })

  it('FASE NV: con NAV diario y trimestral el mismo día, el diario es la vara', () => {
    // Una transcripción del 31 de diciembre que discrepe del Flex no puede
    // cambiar el veredicto: el diario gana la fecha.
    const both = [
      { date: '2025-12-31', _source: 'ibkr_quarterly', netWorthUSD: 9300 },
      { date: '2025-12-31', _source: 'ibkr', netWorthUSD: 5432.98 },
    ]
    expect(contradictedAccountCalibrations([cal(9305.22)], both)).toHaveLength(1)
  })
