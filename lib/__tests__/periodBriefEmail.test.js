...
describe('buildMonthlyBriefEmail', () => {
  test('asunto y cifras del mes y del año', () => {
    // FASE OO: el patrimonio pasó a ser opt-in (la vista previa del teléfono lo
    // enseñaba sin desbloquear), así que este test lo PIDE explícito. Sigue
    // fijando lo mismo de siempre: la forma del correo completo. Que por
    // default el total NO viaje lo fija lib/__tests__/emailPreview.test.js.
    const { subject, text } = buildMonthlyBriefEmail({ monthLabel: 'July 2026', portfolio: PORTFOLIO, showNetWorth: true })
    expect(subject).toBe('Chispudo Monthly · July 2026')
    // La variación del período vive en la cifra principal (FASE IE7), no en
    // una fila de la tabla: el patrimonio es lo que se abre a ver.
    expect(text).toContain('Net worth: $23,156.99')
    expect(text).toContain('+$210.40 (+0.92%) this month')
...
describe('buildAnnualBriefEmail', () => {
  ...
  test('asunto, encabezado y fila propios del anual', () => {
    // Con el total pedido explícito (FASE OO): el sufijo "this year" vive en la
    // pastilla de la variación, que solo existe cuando la cifra grande es el
    // patrimonio. Sin él, la etiqueta de la cifra grande ya dice el período.
    const { subject, text } = buildAnnualBriefEmail({ yearLabel: '2026', portfolio: ANNUAL, showNetWorth: true })
    expect(subject).toBe('Chispudo Annual · 2026')
    expect(text).toContain('Chispudo Annual')
    expect(text).toContain('YOUR YEAR')
    expect(text).toContain('+$3,120.55 (+13.40%) this year')
  })
