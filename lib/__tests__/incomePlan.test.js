import { normalizePlan, firstPlannedMonth, chipsForMonth, expandPlan, planTotalsByMonth, realIncomeByMonth, upsertChip, removeChip, moveChip, serializePlan, REPEAT_MONTHLY, REPEAT_ONCE, planTotalsForFutureYear } from '../incomePlan'
...
describe('planTotalsForFutureYear', () => {
  // El plan real: BAM Q15,000 mensual desde septiembre, mas UFM Q4,000 y bono
  // Q7,000 en diciembre.
  const plan = normalizePlan({
    chips: [
      { id: 'bam', label: 'BAM', amount: 15000, repeat: REPEAT_MONTHLY, startMonth: 8 },
      { id: 'ufm', label: 'UFM', amount: 4000, repeat: REPEAT_ONCE, month: 11 },
      { id: 'bono', label: 'Bono', amount: 7000, repeat: REPEAT_ONCE, month: 11 },
    ],
  }, 2026)

  it('un salario que arranca en septiembre paga los DOCE meses del anio siguiente', () => {
    const out = planTotalsForFutureYear(plan)
    expect(out[0]).toBe(15000)
    expect(out[7]).toBe(15000)
    expect(out[11]).toBe(15000 + 4000 + 7000)
    expect(out.reduce((a, b) => a + b, 0)).toBe(15000 * 12 + 11000)
  })

  it('el anio EN CURSO sigue respetando el mes de arranque: son preguntas distintas', () => {
    const esteAnio = planTotalsByMonth(plan)
    expect(esteAnio[0]).toBe(0)
    expect(esteAnio[8]).toBe(15000)
  })

  it('quitar el salario de un mes es una decision sobre ESTE anio, no sobre todos', () => {
    const conSkip = normalizePlan({
      chips: [{ id: 'bam', label: 'BAM', amount: 15000, repeat: REPEAT_MONTHLY, startMonth: 0, skip: [10] }],
    }, 2026)
    expect(planTotalsByMonth(conSkip)[10]).toBe(0)
    expect(planTotalsForFutureYear(conSkip)[10]).toBe(15000)
  })
})
