...
const healthy = {
  ...
  [HELPER_PATHS.initJs]: { status: 200, body: 'firebase.initializeApp({})' },
...
describe('runAuthDiagnostics', () => {
  test('everything healthy sends the case to support, not back around the console', async () => {
    ...
    // Every console setting has already been exhausted (domains, redirect URIs,
    // rotated secret, recreated OAuth client, published Hosting) and sign-in
    // still fails. Naming any of them here sends the user to redo finished
    // work, which is how a previous round burned an hour. What is left is only
    // visible server-side.
    const console_ = byId(res, 'console')
    expect(console_).toBeDefined()
    expect(console_.detail).toContain('soporte de Firebase')
    expect(console_.detail).not.toContain('Authorized domains')
    expect(console_.detail).not.toContain('Client secret')
  })
...
describe('Hosting never published (the mechanism behind the whole failure)', () => {
  // The helper page is not self-contained: it loads the SDK files Hosting
  // serves under /__/firebase/. A site that exists but never had a release
  // 404s that whole prefix, so the page dies client-side before reaching any
  // server -- which is exactly an internal-error with no server payload.
  const noHosting = { ...healthy }
  delete noHosting[HELPER_PATHS.initJs]

  test('init.json alone cannot answer it: our own fallback fills that in', async () => {
    // The 200 here comes from us, not Firebase, so a check built on init.json
    // would report a healthy proxy over a Hosting that serves nothing.
    const res = await run({ ...noHosting, [HELPER_PATHS.init]: { ...healthy[HELPER_PATHS.init], source: 'local' } })
    expect(byId(res, 'proxy').status).toBe('ok')
    expect(byId(res, 'hosting').status).toBe('fail')
  })

  test('names the fix instead of just failing', async () => {
    const check = byId(await run(noHosting), 'hosting')
    expect(check.status).toBe('fail')
    expect(check.detail).toContain('firebase deploy --only hosting')
  })

  test('suppresses the generic console advice, which would point at the wrong place', async () => {
    // Re-linking the OAuth provider fixes nothing when the handler cannot even
    // load its own JavaScript.
    expect(byId(await run(noHosting), 'console')).toBeUndefined()
  })

  test('a published site passes and leaves the rest of the diagnosis intact', async () => {
    const res = await run(healthy)
    expect(byId(res, 'hosting').status).toBe('ok')
    expect(byId(res, 'console')).toBeDefined()
  })
})
