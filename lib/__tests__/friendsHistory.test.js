...
import {
  snapshotAllGroups, readRecentHistory, HISTORY_LOOKBACK,
  purgeUserFromGroupHistory, deleteGroupHistory,
} from '../friendsHistoryStore'
...
// ⛔ Un EMPATE no puede fabricar un ascenso. `posIn` repartía puestos
// estrictamente distintos (i+1) mientras `groupStandings` usa ranking de
// competencia, así que en un empate el desempate lo decidía el ORDEN DEL
// ARREGLO: el correo decía "pasaste a una persona" en el mismo cuerpo donde la
// tabla mostraba a las dos en el primer puesto. Medido con 152k escenarios
// aleatorios: TODO movimiento fabricado sin movimiento real involucraba un
// empate, y ninguno ocurría sin él.
describe('empates: nadie pasa a nadie', () => {
  const prev = { weekKey: '2026-08-23', rows: [
    { uid: 'a', displayName: 'A', ytd: 20, rank: 1 },
    { uid: 'b', displayName: 'B', ytd: 10, rank: 2 },
  ] }
  // A bajó de 20 a 10 y quedó empatado con B: nadie pasó a nadie.
  const tied = [
    { uid: 'a', displayName: 'A', ytd: 10 },
    { uid: 'b', displayName: 'B', ytd: 10 },
  ]

  test('un empate no mueve a nadie, sin importar el orden del arreglo', () => {
    for (const rows of [tied, [...tied].reverse()]) {
      const out = withMovement({ rows, previous: prev })
      expect(out.map((r) => r.rankDelta)).toEqual([0, 0])
    }
  })

  test('control positivo: un adelantamiento REAL sí se reporta', () => {
    const out = withMovement({
      rows: [{ uid: 'b', displayName: 'B', ytd: 30 }, { uid: 'a', displayName: 'A', ytd: 10 }],
      previous: prev,
    })
    expect(out.find((r) => r.uid === 'b').rankDelta).toBe(1)
    expect(out.find((r) => r.uid === 'a').rankDelta).toBe(-1)
  })
})

// ⛔ "Seguidas" quiere decir seguidas. `snapshotAllGroups` salta los grupos que
// esa semana tuvieron menos de dos personas con número, así que la historia
// puede tener huecos.
describe('la racha exige semanas contiguas', () => {
  const wk = (key) => ({ weekKey: key, rows: [{ uid: 'a', displayName: 'A', ytd: 9, rank: 1 }] })

  test('semanas seguidas cuentan', () => {
    expect(leaderStreak([wk('2026-08-30'), wk('2026-08-23'), wk('2026-08-16')]).weeks).toBe(3)
  })

  test('un hueco corta la racha ahí', () => {
    expect(leaderStreak([wk('2026-08-30'), wk('2026-08-23'), wk('2026-05-31')]).weeks).toBe(2)
  })

  test('sin llave de semana no se afirma contigüidad', () => {
    expect(leaderStreak([wk('2026-08-30'), { rows: wk('x').rows }])).toBe(null)
  })
})

// ⛔ PRIVACIDAD. Apagar Amigos promete "se borra tu perfil público al instante",
// y la memoria semanal guardaba uid, nombre y porcentajes de cada persona sin
// que ese borrado la tocara.
describe('la purga de historia', () => {
  const mkDb = (docs) => {
    ...
    const mk = (d) => ({
      ...
      data: () => d,
      ref: {
        delete: async () => { d._deleted = true },
        update: async (patch) => { Object.assign(d, patch) },
      },
    })
    return {
      store,
      collection: () => ({
        doc: () => ({
          collection: () => ({
            limit: () => ({ get: async () => ({ size: store.length, docs: store.map(mk) }) }),
          }),
        }),
      }),
    }
  ...
  test('saca tus filas y CONSERVA las de los demás', async () => {
    const db = mkDb([
      { weekKey: '2026-08-23', rows: [{ uid: 'me', ytd: 5 }, { uid: 'otro', ytd: 3 }] },
      { weekKey: '2026-08-16', rows: [{ uid: 'otro', ytd: 1 }] },
    ])
    const out = await purgeUserFromGroupHistory({ db, groupId: 'g1', uid: 'me' })
    expect(out.cleaned).toBe(1)
    expect(db.store[0].rows).toEqual([{ uid: 'otro', ytd: 3 }])
    // La historia de las otras personas es suya: no se destruye al irte.
    expect(db.store[1].rows).toEqual([{ uid: 'otro', ytd: 1 }])
    expect(db.store[1]._deleted).toBeUndefined()
  })

  test('una foto que se queda sin nadie se borra', async () => {
    const db = mkDb([{ weekKey: '2026-08-23', rows: [{ uid: 'me', ytd: 5 }] }])
    await purgeUserFromGroupHistory({ db, groupId: 'g1', uid: 'me' })
    expect(db.store[0]._deleted).toBe(true)
  })

  test('borrar el grupo se lleva su historia (Firestore no borra subcolecciones)', async () => {
    const db = mkDb([{ weekKey: '2026-08-23', rows: [] }, { weekKey: '2026-08-16', rows: [] }])
    const out = await deleteGroupHistory({ db, groupId: 'g1' })
    expect(out.deleted).toBe(2)
    expect(db.store.every((d) => d._deleted)).toBe(true)
  })
})
