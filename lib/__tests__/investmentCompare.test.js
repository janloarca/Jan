...
describe('leadershipSegments: el cruce, que es lo que de verdad decide', () => {
  const corto = newScenario('a', 'MAU', { initial: 5000, ratePct: 9, compound: 'annually', years: 3 })
  const largo = newScenario('b', 'CAYALA', { initial: 5000, ratePct: 7.25, compound: 'quarterly', years: 7 })

  it('nombra los dos tramos del caso real del usuario', () => {
    // MAU rinde más pero dura 3 años; CAYALA la pasa cuando MAU se congela.
    const segs = leadershipSegments(compareScenarios([corto, largo]))
    expect(segs).toHaveLength(2)
    expect(segs[0]).toMatchObject({ index: 0, startYear: 1, endYear: 3 })
    expect(segs[1]).toMatchObject({ index: 1, startYear: 4, endYear: 7 })
  })

  it('el año 0 no abre tramo: todos valen su monto inicial y no hay líder', () => {
    const segs = leadershipSegments(compareScenarios([corto, largo]))
    expect(segs[0].startYear).toBe(1)
  })

  it('sin cruce hay un solo tramo, y ahí la fila "mejor" ya lo dice todo', () => {
    const peor = newScenario('c', 'C', { initial: 5000, ratePct: 3, compound: 'annually', years: 7 })
    const segs = leadershipSegments(compareScenarios([largo, peor]))
    expect(segs).toHaveLength(1)
    expect(segs[0].index).toBe(0)
  })

  it('con un solo escenario no hay nada que comparar', () => {
    expect(leadershipSegments(compareScenarios([largo]))).toEqual([])
  })

  it('dos escenarios idénticos empatan siempre y no producen tramos', () => {
    const gemelo = { ...largo, id: 'z', name: 'Z' }
    expect(leadershipSegments(compareScenarios([largo, gemelo]))).toEqual([])
  })
})
