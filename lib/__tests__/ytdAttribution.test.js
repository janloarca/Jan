...
describe('attributeYtd', () => {
  ...
  test('rejects a negative start: an account was never worth negative money', () => {
    // The shape of the original blow-up: a broker whose reconstructed January
    // value came out around -$3,195, which turned its entire balance into
    // "gain" (+$13,207 on a $10K account) while the total still added up.
    expect(attributeYtd({
  ...
  test('the total adding up does NOT excuse an impossible row', () => {
    // Rows that cancel each other sum perfectly to the headline. A total-only
    // check waves this through; the per-account guard is what stops it.
    const accounts = [acc('ok', 12000, 0, 9000, true), acc('impossible', 1000, 0, -1000, true)]
    const totalGain = accounts.reduce((s, a) => s + (a.endVal - a.start - a.flow), 0)
    expect(totalGain).toBeCloseTo(5000, 6) // reconciles with the headline below
    expect(attributeYtd({ accounts, portfolioStart: 8000, headlineGain: 5000 })).toBeNull()
  })

  test('a start of exactly zero is fine: the account simply did not exist yet', () => {
    // OSMO, opened in February: its January value is 0, not an estimate, and the
    // real -$26.65 falls straight out of end - 0 - deposits.
    const res = attributeYtd({
      accounts: [acc('osmo', 116.88, 143.53, 0, true), acc('other', 10000, 0, 9500, true)],
      portfolioStart: 9500,
      headlineGain: 116.88 - 143.53 + 500,
    })
    expect(res).not.toBeNull()
    expect(res.groups.find((g) => g.key === 'osmo').gain).toBeCloseTo(-26.65, 2)
  })
