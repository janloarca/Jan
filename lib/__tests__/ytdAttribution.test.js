import { attributeYtd, attributionRefusalText, MAX_UNEXPLAINED_FLOOR, MAX_UNEXPLAINED_START_RATIO } from '../ytdAttribution'
...
describe('attributeYtd', () => {
  ...
  test('rows are ordered by how much they moved the needle, in dollars', () => {
    const accounts = [acc('small', 1010, 0, 1000), acc('big', 9400, 0, 9000)]
    const res = attributeYtd({ accounts, portfolioStart: 10000, headlineGain: headlineFor(accounts, 10000) })
    expect(rowsOf(res).map((g) => g.key)).toEqual(['big', 'small'])
    // Ordered by dollars, not by return: the small account returned more (1.0%
    // vs 4.4%) but contributed a tenth as much to the year.
    expect(rowsOf(res)[0].gain).toBeCloseTo(400, 6)
  })

  test('a small net with large opposing accounts no longer prints absurd percentages', () => {
    // The share-of-total column divided each row by the portfolio's NET gain,
    // so this year printed 1750% and -1650%. Both rows now report the return on
    // their own capital, whose denominator cannot collapse toward zero.
    const accounts = [acc('win', 10700, 0, 10000), acc('lose', 4340, 0, 5000)]
    const res = attributeYtd({ accounts, portfolioStart: 15000, headlineGain: headlineFor(accounts, 15000) })
    expect(res.totalGain).toBeCloseTo(40, 6)
    const byKey = Object.fromEntries(rowsOf(res).map((g) => [g.key, g.ret]))
    expect(byKey.win).toBeCloseTo(7, 6)
    expect(byKey.lose).toBeCloseTo(-13.2, 6)
    expect(rowsOf(res).every((g) => g.share === undefined)).toBe(true)
  })
  ...
  // FASE HY: el residuo es (suma de arranques - ancla), dos reconstrucciones de
  // una cantidad del tamaño del PORTAFOLIO, así que su ruido escala con el
  // portafolio y no con la ganancia del año. El caso del usuario: YTD +$241
  // sobre un arranque de ~$12K dejaba el techo en $84 y el panel rehusaba casi
  // permanentemente por un desacuerdo de reconstrucción menor al 1.5%.
  test('a low-gain year tolerates reconstruction noise proportional to the START, not the gain', () => {
    // Residual of 180 = 1.5% of a 12,000 start, against a headline of only 100.
    // The gain-only cap (max(25, 35)) refused this; the start-scaled term
    // (2% of 12,000 = 240) lets it render, with the leftover named as its own row.
    const accounts = [acc('A', 12100, 0, 12180)]
    const res = attributeYtd({ accounts, portfolioStart: 12000, headlineGain: 100 })
    expect(res).not.toBeNull()
    const residual = residualOf(res)
    expect(residual).not.toBeNull()
    expect(residual.gain).toBeCloseTo(180, 6)
    // Everything shown still adds up to the headline, and the account keeps its start.
    expect(res.groups.reduce((s, g) => s + g.gain, 0)).toBeCloseTo(100, 6)
    expect(rowsOf(res)[0].start).toBeCloseTo(12180, 6)
  })

  test('beyond the start-scaled bound the split still refuses: that is a stale anchor, not noise', () => {
    // Starts sum to 13,000 against an anchor of 12,000: an 8.3% disagreement is
    // the sawtooth-era signature (a manual-only January doc), not rounding.
    // Two accounts so no single start trips the per-row ceiling first: this
    // test is about the SUM disagreeing with the anchor.
    expect(MAX_UNEXPLAINED_START_RATIO).toBeLessThanOrEqual(0.05)
    const accounts = [acc('A', 6050, 0, 6500), acc('B', 6050, 0, 6500)]
    expect(attributeYtd({ accounts, portfolioStart: 12000, headlineGain: 100 })).toBeNull()
  })

  test('the refusal text carries the numbers when given the diag detail', () => {
    const diag = {}
    const accounts = [acc('A', 6050, 0, 6500), acc('B', 6050, 0, 6500)]
    expect(attributeYtd({ accounts, portfolioStart: 12000, headlineGain: 100 }, diag)).toBeNull()
    expect(diag.reason).toBe('unexplained-too-large')
    const es = attributionRefusalText(diag.reason, 'es', diag.detail)
    expect(es).toMatch(/no cuadran/)
    expect(es).toMatch(/diferencia de \$1,000\.00/)
    expect(es).toMatch(/tolerancia de \$240\.00/)
    // Without detail the text is byte-identical to what it always was.
    expect(attributionRefusalText('unexplained-too-large', 'es')).not.toMatch(/diferencia/)
  })
})
