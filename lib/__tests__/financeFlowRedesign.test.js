...
describe('categoryLabel', () => {
  it('las 22 categorías tienen inglés', () => {
...
describe('el desglose cuadra consigo mismo', () => {
  ...
  it('el ingreso de inversión entra a su grupo, así el total sigue cuadrando', () => {
    const a = computeMonthlyAnalysis(data, { month: 7, year: 2026 }, null, { now: CLOSED, extraIncome: 733 })
    expect(a.income).toBeCloseTo(9000 + 1500 + 400 + 733, 6)
    expect(a.incomeGroups.reduce((s, g) => s + g.amount, 0)).toBeCloseTo(a.income, 6)
    const inv = a.incomeGroups.find((g) => g.key === 'inversion')
    expect(inv.amount).toBe(733)
    // Va como fila propia y marcada, porque no es una transacción de Flujo.
    expect(inv.categories).toEqual([{ category: 'Inversiones', amount: 733, auto: true }])
  })

  it('un depósito importado como "Inversiones" ya no es invisible', () => {
    // Antes la página saltaba el grupo `autoOnly` al armar los tiles, así que
    // esta fila contaba en el total y no aparecía en ninguna parte.
    const a = computeMonthlyAnalysis(
      [...data, tx('2026-08-08', 'INCOME', 250, 'Inversiones')],
      { month: 7, year: 2026 }, null, { now: CLOSED, extraIncome: 733 },
    )
    const inv = a.incomeGroups.find((g) => g.key === 'inversion')
    expect(inv.amount).toBe(983)
    expect(inv.categories.reduce((s, c) => s + c.amount, 0)).toBe(983)
    expect(a.incomeGroups.reduce((s, g) => s + g.amount, 0)).toBeCloseTo(a.income, 6)
  })
})
