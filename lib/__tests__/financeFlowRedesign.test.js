...
describe('comparable: cuándo una variación significa algo', () => {
  ...
  it('mes EN CURSO: las variaciones comparan a MISMO día del mes (windowDays)', () => {
    // Comportamiento nuevo, elegido por el usuario: antes esto fijaba que un
    // mes en curso callara TODO. La comparación honesta existe (los primeros N
    // días de cada mes son ventanas del mismo tamaño), así que las flechas
    // vuelven, rotuladas con la ventana.
    const a = computeMonthlyAnalysis(data, { month: 7, year: 2026 }, null, { now: new Date(2026, 7, 18) })
    expect(a.partialMonth).toBe(true)
    expect(a.windowDays).toBe(18)
    expect(a.momComparable).toBe(true)
    // Todos los movimientos de julio del fixture caen en los primeros 18 días,
    // así que la cifra coincide con la de mes completo; lo que cambia es que
    // ahora SÍ se dibuja.
    const ali = a.groups.find((g) => g.key === 'alimentacion')
    expect(ali.momPct).toBeCloseTo(-66.67, 1)
    expect(ali.comparable).toBe(true)
  })

  it('la ventana recorta el mes anterior a los mismos días: un gasto del día 25 no entra', () => {
    const windowed = [
      tx('2026-08-03', 'EXPENSE', 400, 'Alimentación'),
      tx('2026-07-05', 'EXPENSE', 100, 'Alimentación'),
      tx('2026-07-25', 'EXPENSE', 900, 'Alimentación'), // fuera de la ventana de 18 días
    ]
    const a = computeMonthlyAnalysis(windowed, { month: 7, year: 2026 }, null, { now: new Date(2026, 7, 18) })
    const ali = a.groups.find((g) => g.key === 'alimentacion')
    // 400 contra 100 (solo los primeros 18 días de julio), nunca contra 1000.
    expect(ali.prevAmount).toBe(100)
    expect(ali.momPct).toBeCloseTo(300, 6)
    expect(ali.comparable).toBe(true)
    // Y con el mes CERRADO se compara julio completo, como siempre.
    const closed = computeMonthlyAnalysis(windowed, { month: 7, year: 2026 }, null, { now: CLOSED })
    expect(closed.windowDays).toBe(null)
    expect(closed.groups.find((g) => g.key === 'alimentacion').prevAmount).toBe(1000)
  })

  it('si la ventana NO cabe en el mes anterior, se vuelve a callar', () => {
    // 30 de marzo contra febrero (28 días): ventana grande contra chica, que es
    // exactamente lo que isComparable existe para prohibir.
    const marzo = [
      tx('2026-03-10', 'EXPENSE', 400, 'Alimentación'),
      tx('2026-02-10', 'EXPENSE', 200, 'Alimentación'),
    ]
    const a = computeMonthlyAnalysis(marzo, { month: 2, year: 2026 }, null, { now: new Date(2026, 2, 30) })
    expect(a.partialMonth).toBe(true)
    expect(a.windowDays).toBe(null)
    expect(a.momComparable).toBe(false)
    expect(a.groups.find((g) => g.key === 'alimentacion').comparable).toBe(false)
  })

  it('una fila sin datos en la ventana del mes anterior sigue sin comparar', () => {
    // El sueldo del mes pasado cayó el día 25: en los primeros 18 días no hay
    // base contra la cual medir, y eso es "no hay datos", no "+infinito".
    const lateSalary = [
      tx('2026-08-04', 'INCOME', 9000, 'Salario'),
      tx('2026-07-25', 'INCOME', 9000, 'Salario'),
    ]
    const a = computeMonthlyAnalysis(lateSalary, { month: 7, year: 2026 }, null, { now: new Date(2026, 7, 18) })
    expect(a.momIncomePct).toBe(null)
    const fijos = a.incomeGroups.find((g) => g.key === 'fijos')
    expect(fijos.comparable).toBe(false)
  })
...
describe('los insights hablan del gasto, no del calendario', () => {
  ...
  it('con el mes EN CURSO el mover habla de la VENTANA, y el YoY sigue callado', () => {
    ...
    // Ya no se calla: compara los primeros 18 días de cada mes y LO DICE.
    expect(text).toContain('vs los primeros 18 días del mes pasado')
    expect(text).not.toContain('vs el mes pasado (')
    // El YoY sí sigue siendo de mes completo y por eso no se emite a medias.
    expect(text).not.toContain('que en 2025-08')
  })
...
// ⛔ FASE JA5. Un mes EN CURSO no es un veredicto sobre cómo ahorraste.
//
// El sueldo cae UNA vez y los gastos llegan repartidos, así que a los cuatro
// días la tasa de ahorro está inflada por el calendario, no por lo que hiciste.
// Las dos ramas NO tienen la misma verdad, y por eso no se tratan igual.
describe('los insights de un mes en curso no felicitan por adelantado', () => {
  const DAY4 = new Date(2026, 7, 4)   // 4 de agosto
  const CLOSED_SEP = new Date(2026, 8, 15)

  // Sueldo adentro, dos gastos registrados: la tasa da ~97%.
  const earlyMonth = [
    tx('2026-08-01', 'INCOME', 15000, 'Salario'),
    tx('2026-08-02', 'EXPENSE', 300, 'Alimentación'),
    tx('2026-08-03', 'EXPENSE', 200, 'Transporte'),
  ]

  it('el dia 4 no dice "tasa de ahorro solida"', () => {
    const a = computeMonthlyAnalysis(earlyMonth, { month: 7, year: 2026 }, null, { now: DAY4 })
    expect(a.partialMonth).toBe(true)
    expect(a.savingsRate).toBeGreaterThan(90)   // la cifra existe y es enorme
    const text = buildFinanceInsights(a, 'es').map((i) => i.textEs).join(' | ')
    expect(text).not.toContain('sólida')
  })

  it('el MISMO mes, ya cerrado, si lo dice', () => {
    const a = computeMonthlyAnalysis(earlyMonth, { month: 7, year: 2026 }, null, { now: CLOSED_SEP })
    expect(a.partialMonth).toBe(false)
    const text = buildFinanceInsights(a, 'es').map((i) => i.textEs).join(' | ')
    expect(text).toContain('sólida')
  })

  it('gastar mas de lo que entro SI se avisa a media ventana: es cierto hoy y solo puede empeorar', () => {
    const overspent = [
      tx('2026-08-01', 'INCOME', 3000, 'Salario'),
      tx('2026-08-02', 'INCOME', 500, 'Freelance'),
      tx('2026-08-03', 'EXPENSE', 5000, 'Vivienda'),
    ]
    const a = computeMonthlyAnalysis(overspent, { month: 7, year: 2026 }, null, { now: DAY4 })
    expect(a.incomeLooksUnlogged).toBe(false)
    const text = buildFinanceInsights(a, 'es').map((i) => i.textEs).join(' | ')
    // Pero hablando de lo que VA del mes, no del mes entero.
    expect(text).toContain('Llevas')
    expect(text).not.toContain('Este mes gastaste')
  })

  it('con dos movimientos, "X se lleva el 100%" es aritmetica y no se emite', () => {
    const a = computeMonthlyAnalysis(earlyMonth, { month: 7, year: 2026 }, null, { now: DAY4 })
    const text = buildFinanceInsights(a, 'es').map((i) => i.textEs).join(' | ')
    expect(text).not.toContain('se lleva el')
  })

  it('con movimientos suficientes si describe la concentracion, y dice que es parcial', () => {
    const many = [
      tx('2026-08-01', 'INCOME', 15000, 'Salario'),
      ...Array.from({ length: 8 }, (_, i) => tx(`2026-08-0${i + 1}`, 'EXPENSE', 1000, 'Vivienda')),
      tx('2026-08-02', 'EXPENSE', 200, 'Transporte'),
    ]
    const a = computeMonthlyAnalysis(many, { month: 7, year: 2026 }, null, { now: DAY4 })
    const text = buildFinanceInsights(a, 'es').map((i) => i.textEs).join(' | ')
    expect(text).toContain('se lleva el')
    expect(text).toContain('de lo que llevas gastado este mes')
  })
})
