...
describe('el correo', () => {
  ...
  test('no afirma movimientos de posicion, que hoy no se pueden saber', () => {
    // Sin memoria por semana no hay contra que comparar: "subiste dos puestos"
    // seria un dato con cara de dato.
    const body = buildFriendsWeeklyEmail({ groups: [group], now: NOW }).text.toLowerCase()
    for (const w of ['moved up', 'moved down', 'last week', 'streak']) expect(body).not.toContain(w)
  })
})

// ---- el builder completo, contra un Firestore falso -------------------------
function fakeDb({ groups = [], profiles = {} } = {}) {
