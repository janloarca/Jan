import { hasDividendInMonth, redundantAutoDividendIds, creditableBackfills, creditDestinationBalance, dividendCreditTarget, marketYieldFallback } from '../autoDividends'
...
describe('creditDestinationBalance', () => {
  ...
  // El bug que el usuario reportó el 28 ago 2026: "Hoy pagan XOCHI, el bono sí
  // lo detectó pero el spreadsheet no". El cupón SÍ se archivó y SÍ se veía en
  // el historial de la cuenta, y la Hoja marcaba 0.00 igual.
  //
  // La cuenta se había VACIADO antes (una transferencia más un retiro), y vaciar
  // escribe `quantity: 0` (lib/transferFields.js, a propósito). Este camino
  // escribía el saldo nuevo en el PRECIO y no tocaba la cantidad, así que
  // `getItemValue` (= cantidad × precio) daba 0 por más que el precio dijera
  // 1,000. Las dos superficies se contradecían de forma diagnóstica: el pie del
  // editor de cuenta cae a cantidad 1 y mostraba el saldo bien.
  describe('una cuenta vaciada (cantidad 0) que recibe un cupón', () => {
    const vaciada = { id: 'fondoVacio', currency: 'GTQ', quantity: 0, purchasePrice: 0, currentPrice: 0 }

    it('devuelve la cantidad a 1, o el saldo escrito en el precio vale CERO', () => {
      const { newBalance, newPrice, newQuantity } = creditDestinationBalance({}, vaciada, 600, 'GTQ', null)
      expect(newBalance).toBeCloseTo(600, 2)
      expect(newPrice).toBeCloseTo(600, 2)
      expect(newQuantity).toBe(1)
      // La consecuencia, que es el bug: sin la cantidad, esto es 0 × 600.
      expect(0 * newPrice).toBe(0)
      expect(newQuantity * newPrice).toBeCloseTo(600, 2)
    })

    it('una cuenta con cantidad SANA no se toca (null = no escribir el campo)', () => {
      const { newQuantity } = creditDestinationBalance({}, fondoQ, 78.70, 'USD', null)
      expect(newQuantity).toBeNull()
    })

    it('una cantidad fraccionaria legítima tampoco se pisa', () => {
      const cripto = { id: 'x', currency: 'USD', quantity: 0.25, purchasePrice: 400 }
      const { newPrice, newQuantity } = creditDestinationBalance({}, cripto, 100, 'USD', null)
      expect(newQuantity).toBeNull()
      // El divisor sigue siendo la cantidad real: 0.25 × 800 = 200 = 100 + 100.
      expect(newPrice).toBeCloseTo(800, 2)
    })
  })
})
...
// ⛔ EL CASO REAL DEL USUARIO, reportado CINCO veces (29 ago 2026): vaciaba su
// FONDO LIQUIDO $ y el saldo volvia a 240, que es exactamente el cupon
// semestral de VITALI, el bono cuyo `incomeDestination` es esa cuenta. Por eso
// "solo pasa en fondo liquido dolares".
//
// `creditableBackfills` se dispara con saldo <= 0, y un saldo en cero tiene DOS
// causas: una cuenta que nunca se fondeo (FASE DV, hay que acreditar) y una que
// el usuario VACIO (acreditar la vuelve a llenar). Tres vaciados seguidos
// acreditaron tres cupones atrasados, uno por vez.
describe('creditableBackfills: vaciar una cuenta no es lo mismo que no haberla fondeado', () => {
  const vitali = { id: 'vitali', symbol: 'VITALI', incomeDestination: 'fondo' }
  const cupon = (id, date) => ({
    id, type: 'DIVIDEND', date, totalAmount: 240, currency: 'USD',
    _linkedItemId: 'vitali', _destinationCredited: false,
  })

  it('cuenta que NUNCA se fondeo: se acredita (comportamiento de FASE DV, intacto)', () => {
    const txs = [cupon('c1', '2026-05-15')]
    expect(creditableBackfills(txs, vitali, 0).map((t) => t.id)).toEqual(['c1'])
  })

  it('cuenta VACIADA por transferencia: el cupon anterior YA NO se acredita', () => {
    const txs = [
      cupon('c1', '2026-05-15'),
      { id: 't1', type: 'TRANSFER', date: '2026-08-24', _originItemId: 'fondo', _linkedItemId: 'monetaria' },
    ]
    expect(creditableBackfills(txs, vitali, 0)).toEqual([])
  })

  it('cuenta VACIADA por retiro: mismo resultado', () => {
    const txs = [
      cupon('c1', '2026-05-15'),
      { id: 'w1', type: 'WITHDRAWAL', date: '2026-08-24', _linkedItemId: 'fondo' },
    ]
    expect(creditableBackfills(txs, vitali, 0)).toEqual([])
  })

  // El defecto exacto: tres vaciados, tres cupones atrasados, uno por vez.
  it('varios cupones atrasados no pueden re-llenar la cuenta uno por uno', () => {
    const txs = [
      cupon('c1', '2025-05-15'), cupon('c2', '2025-12-15'), cupon('c3', '2026-05-15'),
      { id: 't1', type: 'TRANSFER', date: '2026-08-25', _originItemId: 'fondo' },
    ]
    expect(creditableBackfills(txs, vitali, 0)).toEqual([])
  })

  // Un cupon POSTERIOR a la ultima salida si es dinero nuevo que de verdad
  // llego a una cuenta vacia: ese se acredita.
  it('un cupon posterior al vaciado SI se acredita', () => {
    const txs = [
      cupon('c1', '2026-05-15'),
      { id: 't1', type: 'TRANSFER', date: '2026-06-01', _originItemId: 'fondo' },
      cupon('c2', '2026-12-15'),
    ]
    expect(creditableBackfills(txs, vitali, 0).map((t) => t.id)).toEqual(['c2'])
  })

  // Una salida de OTRA cuenta no dice nada de esta.
  it('la salida de otra cuenta no bloquea nada', () => {
    const txs = [
      cupon('c1', '2026-05-15'),
      { id: 't1', type: 'TRANSFER', date: '2026-08-24', _originItemId: 'otra-cuenta' },
    ]
    expect(creditableBackfills(txs, vitali, 0).map((t) => t.id)).toEqual(['c1'])
  })

  it('un cupon sin fecha se REHUSA cuando hubo una salida, en vez de adivinar', () => {
    const txs = [
      { id: 'c1', type: 'DIVIDEND', totalAmount: 240, _linkedItemId: 'vitali', _destinationCredited: false },
      { id: 't1', type: 'TRANSFER', date: '2026-08-24', _originItemId: 'fondo' },
    ]
    expect(creditableBackfills(txs, vitali, 0)).toEqual([])
  })

  it('con saldo vivo no se acredita nada, pase lo que pase', () => {
    expect(creditableBackfills([cupon('c1', '2026-05-15')], vitali, 240)).toEqual([])
  })
})

describe('FASE OB: dividendCreditTarget resuelve el destino que la FILA declara', () => {
  const msft = { id: 'msft', symbol: 'MSFT', type: 'Stock', incomeDestination: 'bankA', _originalCurrency: 'USD' }
  const bankA = { id: 'bankA', symbol: 'BANK-A', type: 'Bank' }
  const bankB = { id: 'bankB', symbol: 'BANK-B', type: 'Bank' }
  const manualYield = { type: 'DIVIDEND', totalAmount: 83, currency: 'USD', _linkedItemId: 'msft', _source: 'manual_cashflow', _destinationItemId: 'bankB' }

  it('un pago tecleado a mano que llego a la cuenta B se revierte de B, no del destino configurado A', () => {
    // Antes: debitaba bankA (la configuracion de hoy) por un pago que bankB recibio.
    expect(dividendCreditTarget(manualYield, [msft, bankA, bankB]).dest.id).toBe('bankB')
  })
  it('sin _destinationItemId manda el destino configurado del activo, como siempre', () => {
    const { _destinationItemId, ...auto } = manualYield
    expect(dividendCreditTarget({ ...auto, _source: 'auto' }, [msft, bankA, bankB]).dest.id).toBe('bankA')
  })
  it('sin ninguno de los dos no hay nada que revertir', () => {
    const { _destinationItemId, ...auto } = manualYield
    expect(dividendCreditTarget(auto, [{ ...msft, incomeDestination: '' }, bankA])).toBeNull()
  })
})

describe('FASE OB: marketYieldFallback rescata el override guardado con la forma vieja', () => {
  it('dividendYield + meses, sin incomeRate ni incomeAmount => tasa anual', () => {
    expect(marketYieldFallback({ dividendYield: 0.7, incomeMonths: [2, 5, 8, 11], incomeAmount: 0 }))
      .toEqual({ incomeMode: 'percent', incomeRate: 0.7 })
  })
  it('con monto por accion o tasa propia no interfiere', () => {
    expect(marketYieldFallback({ dividendYield: 0.7, incomeMonths: [2], incomeAmount: 0.83 })).toBeNull()
    expect(marketYieldFallback({ dividendYield: 0.7, incomeMonths: [2], incomeRate: 5 })).toBeNull()
  })
  it('sin calendario no produce ningun pago fechado, asi que no aplica', () => {
    expect(marketYieldFallback({ dividendYield: 0.7, incomeMonths: [] })).toBeNull()
    expect(marketYieldFallback({ dividendYield: 0.7 })).toBeNull()
  })
})
