...
// ── 6. `no-market-price` afirmaba que un activo valía $0 sin serlo ────────
//
// Verificado ejecutando `getItemValue`, la funcion que de verdad suma el
// patrimonio: un ítem de 10 unidades sin cotización viva pero con precio de
// compra 56 vale 560, no 0. Tercera recurrencia de este mismo check (FASE EZ4
// y FASE HV11 arreglaron sus otras dos formas).
describe('no se puede afirmar que un activo vale $0 cuando la app lo valúa', () => {
  const ACCION = {
    id: 's1', name: 'ACME', symbol: 'ACME', type: 'Stock', currency: 'USD',
    quantity: 10, currentPrice: 0, purchasePrice: 56,
    acquisitionDate: '2025-01-01', createdAt: '2025-01-01T00:00:00.000Z',
    institution: 'X', assetCountry: 'US', balanceAsOf: '2026-08-01',
  }
  const go = (over = {}) => analyzeDataCompleteness({ items: [{ ...ACCION, ...over }], transactions: [], baseCurrency: 'USD', now: NOW })

  it('con precio de compra no dice "$0": dice que el precio quedó quieto', () => {
    const c = codes(go())
    expect(c).not.toContain('no-market-price')
    expect(c).toContain('no-market-price-stale')
    expect(find(go(), 'no-market-price-stale').textEs).not.toContain('$0')
  })

  it('sin ninguna fuente de precio sí lo dice, y sigue siendo el aviso más fuerte', () => {
    const f = find(go({ purchasePrice: 0 }), 'no-market-price')
    expect(f.severity).toBe('high')
    expect(f.textEs).toContain('$0')
  })
})

// ── 7. `uncovered-shares` no puede prescribir lo que no lo resuelve ───────
describe('un hallazgo sin acción posible no se presenta como pendiente', () => {
  const CRIPTO = {
    id: 'c1', name: 'BTC', symbol: 'BTC', type: 'Crypto', currency: 'USD',
    quantity: 0.5, currentPrice: 60000, purchasePrice: 50000,
    acquisitionDate: '2025-01-01', createdAt: '2025-01-01T00:00:00.000Z',
    institution: 'X', assetCountry: 'US', balanceAsOf: '2026-08-01',
  }
  const go = (over = {}) => analyzeDataCompleteness({ items: [{ ...CRIPTO, ...over }], transactions: [], lots: [], baseCurrency: 'USD', now: NOW })

  it('deja de mandar a confirmar una fecha que NO lo limpia', () => {
    // Comprobado ejecutando el motor: con la fecha puesta el hallazgo sigue
    // ahí, porque lo que falta es un LOTE y una fecha no crea ninguno.
    expect(codes(go())).toContain('uncovered-shares')
    const f = find(go(), 'uncovered-shares')
    expect(f.textEs).not.toContain('confirma la fecha')
  })

  it('dice la consecuencia real y que el valor de hoy no se toca', () => {
    const f = find(go(), 'uncovered-shares')
    expect(f.textEs).toContain('estimado')
    expect(f.textEs).toContain('Su valor de hoy no se ve afectado')
  })

  it('no pesa más que los avisos que sí se pueden resolver', () => {
    expect(find(go(), 'uncovered-shares').severity).toBe('low')
  })
})
