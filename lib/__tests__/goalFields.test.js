import { legacyProfileGoals, goalAdoptionPatch, goalsDiffer, goalsMigratedStamp, PROFILE_GOAL_CURRENCY, portfolioValue } from '../goalFields'
...
describe('portfolioValue', () => {
  it('una meta de portafolio se mide contra los ACTIVOS, no contra el patrimonio neto', () => {
    // Con una deuda viva las dos cifras difieren, y contra el neto pagar la
    // deuda contaria como crecimiento del portafolio sin que un activo se
    // hubiera movido.
    expect(portfolioValue(30000, 26000)).toBe(30000)
  })

  it('sin activos cae al neto, que es la misma cifra cuando no hay deuda', () => {
    expect(portfolioValue(null, 26000)).toBe(26000)
    expect(portfolioValue(undefined, 26000)).toBe(26000)
    expect(portfolioValue(0, 26000)).toBe(26000)
  })

  it('sin nada usable devuelve cero, jamas NaN', () => {
    expect(portfolioValue(null, null)).toBe(0)
    expect(portfolioValue('x', 'y')).toBe(0)
  })
})

// Guardian de FUENTE: las DOS superficies que preguntan "¿llego a mi meta?"
// tienen que medir contra la misma base. Se lee el ARCHIVO y no una copia de
// la expresion, porque el calculo vive dentro de JSX que jest no puede montar
// sin el dashboard entero (precedente: moneyInputs.test.js).
describe('la meta se mide igual en las dos pantallas', () => {
  const fs = require('fs')
  ...
  const SURFACES = [
    ...
    'components/dashboard/WealthProjectionCard.jsx',
  ]

  it.each(SURFACES)('%s importa portfolioValue del modulo compartido', (rel) => {
    const src = read(rel)
    expect(src).toMatch(/import \{[^}]*\bportfolioValue\b[^}]*\} from '@\/lib\/goalFields'/)
  })

  it.each(SURFACES)('%s no trae su propia copia de portfolioValue', (rel) => {
    const src = read(rel)
    expect(src).not.toMatch(/function portfolioValue\s*\(/)
  })

  it('el arranque de la carrera hacia la meta sale de portfolioValue, no de netWorth crudo', () => {
    const src = read('components/dashboard/WealthProjectionCard.jsx')
    expect(src).toMatch(/startValue: portfolioValue\(totalAssets, netWorth\)/)
  })
})
