...
import {
  ...
  getIncomeReceivedByItem, getOwnReinvestedYieldByItem, getInvestedCapital,
  computeModifiedDietz, entryFeeAddbacks,
...
describe('el cupón que aterriza en otra cuenta no es capital invertido', () => {
  ...
  // ⛔ FASE KZ3 (extensión de la superficie E, OK del usuario el 26 ago 2026):
  // el rendimiento PROPIO de una cuenta bank-like que reinvierte también sale
  // del capital invertido. El guard que protege al caso congelado: sobre el
  // BONO no cambia NADA, aunque reinvierta, porque su costo no contiene el
  // rendimiento. El 3.94% no se mueve.
  it('KZ3: un rendimiento reinvertido sobre el BONO no toca su capital invertido', () => {
    const reinvested = {
      id: 'own', type: 'DIVIDEND', date: '2026-06-30', totalAmount: 100,
      currency: 'USD', _linkedItemId: 'vitali', _reinvested: true,
    }
    const own = getOwnReinvestedYieldByItem([...transactions, reinvested], items, null, 'USD')
    expect(own.get('vitali')).toBeUndefined() // Bond: no es bank-like
    const received = getIncomeReceivedByItem(transactions, items, null, 'USD')
    expect(getInvestedCapital(bond, received.get('vitali'), own.get('vitali'))).toBe(VITALI.cashOutOfPocket)
  })
})
...
describe('FASE IA: compra DENTRO de la ventana, con el ancla quieta en el 1 de enero', () => {
  ...
  it('un aporte posterior (manual_balance_flow) NO desbloquea la comisión', () => {
    // Solo el depósito de APERTURA lleva la comisión adentro (lógica congelada
    // G). Un aporte desde la Hoja es puro principal: si sirviera de llave, una
    // ventana que nunca neteó el depósito de apertura devolvería una comisión
    // que no restó. Encontrado por verificación adversarial de FASE JJ2.
    const laterFlow = {
      id: 'flow', type: 'DEPOSIT', date: '2026-07-01',
      totalAmount: 500, currency: 'USD', _linkedItemId: 'vitali', _source: 'manual_balance_flow',
    }
    const addback = entryFeeAddbacks(items, [laterFlow, coupon], { fromTs: yearStart, toTs: today })
    expect(addback.size).toBe(0)
  })
})
