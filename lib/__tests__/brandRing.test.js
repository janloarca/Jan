...
describe('nadie vuelve a definir la geometría por su cuenta', () => {
  ...
  // FASE ME7: la lista de tres archivos de arriba dejó pasar una CUARTA copia
  // (el pill de IBKR en Header.jsx, con su propio R=13.5, su 0.22/0.78 y su
  // propio keyframe de barrido). Este barrido cubre TODO components/: la firma
  // de la copia es un keyframe *Sweep propio, o la fracción 0.22 escrita a
  // mano junto a una circunferencia propia (los donuts legítimos usan 2πr sin
  // ninguna de las dos cosas).
  it('ningún componente del repo define su propio barrido ni su propia fracción 0.22', () => {
    const walk = (dir, out = []) => {
      for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
        const full = path.join(dir, e.name)
        if (e.isDirectory()) { if (e.name !== 'node_modules' && e.name !== '__tests__') walk(full, out) }
        else if (e.name.endsWith('.jsx')) out.push(full)
      }
      return out
    }
    const offenders = []
    for (const f of walk(path.join(process.cwd(), 'components'))) {
      const src = fs.readFileSync(f, 'utf8')
      if (/@keyframes \w*Sweep/.test(src)) offenders.push(`${path.relative(process.cwd(), f)}: keyframe Sweep propio`)
      if (/Math\.PI/.test(src) && /\* 0\.22\b/.test(src)) offenders.push(`${path.relative(process.cwd(), f)}: fracción 0.22 a mano`)
    }
