...
describe('shouldClaim: arrastre propio del contenido', () => {
  const pull = { dx: 0, dy: 30, atTop: true, insideMain: true }

  it('no reclama cuando el toque empezó sobre algo que arrastra lo suyo', () => {
    // El calendario del plan de ingresos: arrastrar un cuadrito hacia abajo
    // cumple TODAS las demás condiciones de un tirón. Sin esta compuerta, el
    // gesto le haría preventDefault al arrastre y dispararía un refresco en
    // vez de mover el ingreso de mes.
    expect(shouldClaim({ ...pull, blockedByDragHandle: true })).toBe(false)
  })

  it('el mismo tirón fuera de esa zona sí se reclama', () => {
    expect(shouldClaim({ ...pull, blockedByDragHandle: false })).toBe(true)
  })

  it('la compuerta manda incluso sobre un tirón perfectamente vertical', () => {
    expect(shouldClaim({ dx: 0, dy: 200, atTop: true, insideMain: true, blockedByDragHandle: true })).toBe(false)
  })
})
