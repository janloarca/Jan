...
describe('balanceDiagnostic', () => {
  ...
  // ⛔ El diagnostico y los sanadores tienen que decir lo MISMO de un estado.
  // Una cuenta vaciada (los dos precios en cero) con la cantidad ya apagada
  // esta CORRECTA en cero, y los sanadores la dejan quieta a proposito; si el
  // diagnostico la marcara como defecto, las dos superficies se contradirian y
  // volveria la ambiguedad que este modulo existe para cerrar.
  it('una cuenta vaciada y en cero se declara correcta, no defectuosa', () => {
    const rows = balanceDiagnostic([banco({ quantity: 0, currentPrice: 0, purchasePrice: 0, cost: 240 })])
    expect(rows[0].value).toBe(0)
    expect(rows[0].verdict.code).toBe('vaciada')
    expect(rows[0].verdict.ok).toBe(true)
    // El residuo se NOMBRA igual: es lo que explica por que sigue ahi.
    expect(rows[0].verdict.es).toContain('cost')
  })

  it('vaciada sin ningun residuo tambien es correcta', () => {
    const rows = balanceDiagnostic([banco({ quantity: 0, currentPrice: 0, purchasePrice: 0 })])
    expect(rows[0].verdict.ok).toBe(true)
    expect(rows[0].verdict.es).not.toContain('residuo')
  })
...
describe('balanceDiagnosticText', () => {
  ...
  // Un estado correcto no puede imprimirse con la misma marca que un defecto:
  // el reporte se lee en un telefono y la marca es lo unico que se escanea.
  it('lo correcto se marca distinto de lo roto', () => {
    const ok = balanceDiagnostic([banco({ quantity: 0, currentPrice: 0, purchasePrice: 0, cost: 240 })])
    const txt = balanceDiagnosticText(ok)
    expect(txt).toContain('✓')
