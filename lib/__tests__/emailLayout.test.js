...
// ⛔ Un dato que FALTA no puede verse como una pérdida.
//
// Los cuatro correos usan "-" para "no hay dato" (`if (v == null) return '-'`),
// y ese guión caía en la rama de negativo del color por signo: en el correo
// salía en el MISMO rojo que -11.20%. Lo destapó el render en navegador del
// correo de grupos, donde una persona sin datos para el alcance del grupo
// aparecía pintada como si hubiera perdido dinero; pero el defecto vive en el
// renderer compartido, así que venía afectando también al semanal, al mensual
// y al anual desde que existen.
describe('el color por signo exige un numero', () => {
  const render = (value) => renderEmail({
    title: 'T', sections: [{ heading: 'S', rows: [['label', value]] }],
  }).html

  const NEG = '#b91c1c'
  const POS = '#047857'

  test('un guion suelto NO se pinta de perdida', () => {
    expect(render('-')).not.toContain(NEG)
  })

  test('ningun texto sin cifras se colorea', () => {
    for (const v of ['-', '--', 'n/a', '(sin datos)', '+']) {
      const html = render(v)
  ...
  test('y los numeros de verdad SIGUEN coloreandose', () => {
    expect(render('-11.20%')).toContain(NEG)
    expect(render('+24.51%')).toContain(POS)
    expect(render('(3.47%)')).toContain(NEG)
  })
})
