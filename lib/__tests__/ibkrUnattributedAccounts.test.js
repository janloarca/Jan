...
describe('el aviso del multi-cuenta llega a la pantalla (FASE MP)', () => {
  ...
  it('el aviso nombra el arreglo real, que vive del lado del usuario', () => {
    // FASE NB: la condición se izó a la const `multiAccount` para que el
    // auto-cierre del modal y el render compartan UNA definición, así que el
    // mensaje ya no vive a 1400 chars del PRIMER uso del conteo. Se busca
    // desde donde esa const se renderiza; si el render desaparece, indexOf
    // devuelve -1 y el test falla igual que antes.
    const at = modalSrc.indexOf('{multiAccount && (')
