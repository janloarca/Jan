...
describe('serie de cripto para el brief', () => {
  ...
  test('la ventana anual se pide como "1y", nunca como "365d" (FASE IE6)', async () => {
    // La lista de ventanas que CoinGecko acepta es CERRADA (1h, 24h, 7d, 14d,
    // 30d, 200d, 1y): pedir "365d" devuelve la fila sin ese campo y la cripto
    // saldría sin porcentaje teniendo el dato.
    mockFetch([{ id: 'bitcoin', current_price: 120, price_change_percentage_1y_in_currency: 20 }])
    const series = await makeBriefFetcher({ cgWindow: '1y' })('BTC', 'crypto')
    expect(global.fetch.mock.calls[0][0]).toContain('price_change_percentage=1y')
    expect(global.fetch.mock.calls[0][0]).not.toContain('365d')
    // La serie sintética alcanza para la ventana anual (252 días hábiles).
    expect(windowChangePct(series.closes, 252)).toBeCloseTo(20, 4)
  })
