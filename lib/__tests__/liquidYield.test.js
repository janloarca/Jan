import {
  ...
  yieldSignature, supersededYieldTxIds, CANDIDATE_STATUSES, dismissalFor,
} from '../liquidYield'
import { buildContributionFields } from '../contributions'
import { planCellEdit, ANSWER_CORRECTION } from '../spreadsheetEdit'
import { isMarketPriced, getEffectiveYield } from '@/components/dashboard/utils'
...
// ⛔ FASE OM (extensión de la spec congelada liquidFundYield.js, sección 5, OK
// explícito del usuario, 6 sep 2026). La integración de punta a punta: lo que
// `buildContributionFields` sella en `balanceAsOf` es EXACTAMENTE el dato que
// `knownContributions`/`computeLiquidYield` leen como corte. No son dos
// motores que coinciden por casualidad: son el mismo campo, escrito por uno y
// consumido por el otro, y esta suite fija que sigan de acuerdo.
//
// El defecto real: un fondo abierto con un depósito y sellado ese mismo día
// (la convención de las tres puertas que ya sellaban) recibía DESPUÉS un
// aporte nuevo (CashFlowModal) sin que nada volviera a sellar la foto. Esa
// aporte quedaba fechado DESPUÉS del sello viejo, así que `knownContributions`
// lo descartaba como "no conocido" y `computeLiquidYield` lo leía como
// interés que la cuenta nunca generó, por el monto exacto del aporte.
describe('FASE OM: buildContributionFields sella la foto que este motor consume', () => {
  const CUENTA = { id: 'fund-x', name: 'Fondo X', currency: 'USD', type: 'Bank' }

  it('sin foto fresca, un aporte posterior al sello se lee como interés inventado (el bug)', () => {
    // Apertura de $5,000 el 1 de enero, con la foto sellada ESE día (como
    // hacían las otras tres puertas antes de este fix). El 15 de agosto entra
    // un aporte de $500 por CashFlowModal, pero la foto se quedó vieja.
    const cuenta = { ...CUENTA, balanceAsOf: '2026-01-01' }
    const txs = [
      { id: 'd1', type: 'DEPOSIT', date: '2026-01-01', totalAmount: 5000, currency: 'USD', _linkedItemId: 'fund-x' },
      { id: 'd2', type: 'DEPOSIT', date: '2026-08-15', totalAmount: 500, currency: 'USD', _linkedItemId: 'fund-x' },
    ]
    const asOfTs = new Date(`${cuenta.balanceAsOf}T00:00:00Z`).getTime()
    const contributions = knownContributions({ item: cuenta, items: [cuenta], transactions: txs, asOfTs })
    // El aporte de agosto queda FUERA de lo conocido: solo entra la apertura.
    expect(contributions.map((c) => c.amount)).toEqual([5000])

    const out = computeLiquidYield({ contributions, finalBalance: 5500, asOfTs })
    // Los $500 reales del aporte se leen como interés que la cuenta jamás ganó.
    expect(out.interest).toBeCloseTo(500, 6)
  })

  it('con la foto fresca que sella buildContributionFields, el mismo aporte se explica y el interés es cero', () => {
    jest.useFakeTimers().setSystemTime(new Date('2026-08-15T15:00:00Z'))
    const cuentaVieja = { id: 'fund-x', currency: 'USD', type: 'Bank', quantity: 1, currentPrice: 5000, purchasePrice: 5000 }
    // El mismo aporte, ahora registrado por el motor real: sella `balanceAsOf`
    // a HOY, sin importar la fecha `date` del movimiento.
    const { itemFields } = buildContributionFields({
      item: cuentaVieja, amount: 500, date: '2026-08-15', isAdd: true, currency: 'USD',
    })
    jest.useRealTimers()
    expect(itemFields.balanceAsOf).toBe('2026-08-15')

    const cuenta = { ...cuentaVieja, ...itemFields }
    const txs = [
      { id: 'd1', type: 'DEPOSIT', date: '2026-01-01', totalAmount: 5000, currency: 'USD', _linkedItemId: 'fund-x' },
      { id: 'd2', type: 'DEPOSIT', date: '2026-08-15', totalAmount: 500, currency: 'USD', _linkedItemId: 'fund-x' },
    ]
    const asOfTs = new Date(`${cuenta.balanceAsOf}T00:00:00Z`).getTime()
    const contributions = knownContributions({ item: cuenta, items: [cuenta], transactions: txs, asOfTs })
    // Con la foto al día, el aporte de agosto SÍ entra a lo conocido.
    expect(contributions.map((c) => c.amount)).toEqual([5000, 500])

    const out = computeLiquidYield({ contributions, finalBalance: cuenta.currentPrice, asOfTs })
    // Sin ningún interés real de por medio, el residuo es cero: nada se
    // inventa sobre el aporte que la app misma acaba de registrar.
    expect(Math.abs(out.interest)).toBeLessThan(0.01)
  })
})

// dismissalFor computa, para un estado HIPOTÉTICO, el mismo veredicto que
// liquidYieldCandidates calcularía para el ya guardado. Estos casos prueban
// la función directo, sin pasar por la Hoja: el escenario de punta a punta
// (con planCellEdit real) está en el describe de FASE OV, más abajo.
describe('dismissalFor', () => {
  const items = [FUND, XOCHI, CREDI]
  // Los mismos cuatro montos y fechas que CONTRIBUTIONS, pero como
  // transacciones reales, para poder pasar por knownContributions.
  const DEPOSIT_TXS = [
    { id: 'd1', type: 'DEPOSIT', date: '2024-07-01', totalAmount: 500, currency: 'GTQ', _linkedItemId: 'fund-q' },
    { id: 'd2', type: 'DEPOSIT', date: '2025-02-15', totalAmount: 600, currency: 'GTQ', _linkedItemId: 'fund-q' },
    { id: 'd3', type: 'DEPOSIT', date: '2025-06-15', totalAmount: 400, currency: 'GTQ', _linkedItemId: 'fund-q' },
    { id: 'd4', type: 'DEPOSIT', date: '2025-08-15', totalAmount: 600, currency: 'GTQ', _linkedItemId: 'fund-q' },
  ]

  it('con un estado "ok", la firma es IDÉNTICA a la que yieldSignature computa por separado', () => {
    const out = dismissalFor({ item: FUND, items, transactions: DEPOSIT_TXS, asOfTs: AS_OF, finalBalance: 2236.73 })
    const contributions = knownContributions({ item: FUND, items, transactions: DEPOSIT_TXS, asOfTs: AS_OF })
    expect(contributions).toHaveLength(4) // las mismas 4 que CONTRIBUTIONS
    expect(out).toEqual({
      signature: yieldSignature({ asOfTs: AS_OF, finalBalance: 2236.73, contributions }),
      dismissed: true,
    })
  })

  it('un residuo NEGATIVO también es candidato: dismissalFor no devuelve null', () => {
    const out = dismissalFor({ item: FUND, items, transactions: DEPOSIT_TXS, asOfTs: AS_OF, finalBalance: 1800 })
    expect(out).toEqual({ signature: expect.any(String), dismissed: true })
  })

  it('sin ningún aporte conocido, null', () => {
    expect(dismissalFor({ item: FUND, items, transactions: [], asOfTs: AS_OF, finalBalance: 2500 })).toBeNull()
  })

  it('con un residuo de puro redondeo (negligible), null', () => {
    // 2,103 sobre estos mismos aportes ya se fijó como 'negligible' arriba.
    expect(dismissalFor({ item: FUND, items, transactions: DEPOSIT_TXS, asOfTs: AS_OF, finalBalance: 2103 })).toBeNull()
  })

  it('cuando el IRR no converge (unsolvable), null', () => {
    // Un único aporte fechado EXACTO en asOfTs: los dos flujos de xirr caen en
    // el mismo instante, sin tiempo transcurrido con el cual resolver una
    // tasa (xirr devuelve null por diseño en ese caso).
    const txs = [{ id: 'd1', type: 'DEPOSIT', date: '2026-08-11', totalAmount: 1000, currency: 'GTQ', _linkedItemId: 'fund-q' }]
    expect(dismissalFor({ item: FUND, items, transactions: txs, asOfTs: AS_OF, finalBalance: 1500 })).toBeNull()
  })
})

// ⛔ FASE OV (extensión de la spec congelada liquidFundYield.js, secciones 5 y
// 7; OK explícito del usuario, 22 sep 2026). Reporte con captura: "Esto ya lo
// habíamos puesto en el excel, me lo preguntó doble" sobre una cuenta
// ("Ahorro GTQ") marcada implausible-rate DESPUÉS de corregirla en la Hoja.
//
// El defecto: ANSWER_CORRECTION ("el número anterior estaba mal") no archiva
// ninguna transacción, pero SÍ re-sella balanceAsOf — así que knownContributions
// nunca se entera de que la corrección ya contestó "esto no fue rendimiento",
// y como la firma depende del saldo Y de balanceAsOf, cambia igual y el mismo
// residuo vuelve a ofrecerse en el modal de cuentas líquidas.
//
// dismissalFor computa, sobre el estado que el patch de la corrección está por
// escribir, el mismo veredicto que liquidYieldCandidates calcularía; si
// produciría un candidato, answerPendingEdit lo estampa en la MISMA escritura.
describe('FASE OV: la corrección de la Hoja pre-sella el candidato que produciría', () => {
  // Ahorro GTQ: saldo mal tecleado (520) que el usuario corrige en la Hoja.
  const RAW = {
    id: 'ahorro-gtq', name: 'Ahorro GTQ', currency: 'GTQ', type: 'Bank',
    quantity: 1, currentPrice: 520, purchasePrice: 520, balanceAsOf: '2026-01-01',
  }
  // El único movimiento que la app conoce: el depósito de apertura. La
  // corrección de la Hoja no escribe ninguno nuevo, así que esto es TODO lo
  // que knownContributions puede encontrar antes y después de corregir.
  const TXS = [{ id: 'd1', type: 'DEPOSIT', date: '2026-01-01', totalAmount: 500, currency: 'GTQ', _linkedItemId: 'ahorro-gtq' }]

  // Reproduce, con las funciones REALES, exactamente lo que liquidYieldCandidates
  // (hooks/useDashboardData.js) le preguntaría a este ítem: mismos gates,
  // misma fórmula de finalBalance, mismo declaredRatePct.
  function candidateFor(item, transactions, asOfTs) {
    if (!item?.id || !item.balanceAsOf) return null
    if (isMarketPriced(item) || item.type === 'Debt' || item.isReceivable) return null
    const finalBalance = (Number(item.quantity) || 1) * (Number(item.currentPrice ?? item.purchasePrice) || 0)
    if (!(finalBalance > 0)) return null
    const contributions = knownContributions({ item, items: [item], transactions, asOfTs })
    if (contributions.length === 0) return null
    const res = computeLiquidYield({ contributions, finalBalance, asOfTs, declaredRatePct: getEffectiveYield(item) || 0 })
    if (!CANDIDATE_STATUSES.includes(res.status)) return null
    const signature = yieldSignature({ asOfTs, finalBalance, contributions })
    if (item._liquidYield?.signature === signature) return null // ya descartado
    return { ...res, signature }
  }

  it('REGRESIÓN NEGATIVA: sin el sello, la corrección deja el candidato para que vuelva a aparecer', () => {
    // El usuario teclea 3,000 donde antes decía 520 (un depósito de apertura
    // de 500 no explica ni de lejos ese saldo, así que sigue implausible
    // DESPUÉS de la corrección) — y ANSWER_CORRECTION no archiva nada que lo
    // explique.
    const plan = planCellEdit({ item: RAW, oldValue: 520, newValue: 3000, answer: ANSWER_CORRECTION })
    const patch = { ...plan.patch, balanceAsOf: '2026-08-20' }
    const postItem = { ...RAW, ...patch } // SIN _liquidYield: el fix no corrió
    const asOfTs = new Date(`${patch.balanceAsOf}T00:00:00Z`).getTime()

    const candidate = candidateFor(postItem, TXS, asOfTs)
    expect(candidate).not.toBeNull()
    expect(candidate.status).toBe('implausible-rate')
  })

  it('con el sello que answerPendingEdit escribe en la MISMA pasada, el candidato ya no aparece', () => {
    const plan = planCellEdit({ item: RAW, oldValue: 520, newValue: 3000, answer: ANSWER_CORRECTION })
    const patch = { ...plan.patch, balanceAsOf: '2026-08-20' }
    let postItem = { ...RAW, ...patch }
    const asOfTs = new Date(`${patch.balanceAsOf}T00:00:00Z`).getTime()
    const finalBalance = (Number(postItem.quantity) || 1) * (Number(postItem.currentPrice ?? postItem.purchasePrice) || 0)

    // El bloque que answerPendingEdit ejecuta ANTES de commitPatch.
    const dismissal = dismissalFor({
      item: postItem, items: [postItem], transactions: TXS, asOfTs, finalBalance,
      declaredRatePct: getEffectiveYield(postItem) || 0,
    })
    expect(dismissal).not.toBeNull()
    patch._liquidYield = { ...dismissal, asOf: patch.balanceAsOf }
    postItem = { ...RAW, ...patch } // el ítem tal como quedó escrito en Firestore

    // La misma evaluación de arriba, ahora contra el ítem ya sellado: el
    // candidato desaparece sin haber escrito ninguna transacción.
    expect(candidateFor(postItem, TXS, asOfTs)).toBeNull()
  })
})
