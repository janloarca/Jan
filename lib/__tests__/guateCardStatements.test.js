...
describe('Contecnica / Banco Industrial', () => {
  ...
  it('cashback stays income: no purchase to net it against', () => {
    ...
    for (const t of cb) {
      ...
      expect(t.category).toBe('Otros Ingresos')
    }
  })
...
describe('el import aplica lo que el usuario ya enseñó', () => {
  ...
  it('no toca el cashback, que no tiene comercio contra el cual netear', () => {
    const rules = [{ match: 'devuelve', category: 'Compras' }]
    const r = parseCardStatement(BI_TEXT, { rules })
    for (const t of r.transactions.filter((x) => x.type === 'INCOME')) {
      expect(t.category).toBe('Otros Ingresos')
    }
  })
