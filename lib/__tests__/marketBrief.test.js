...
describe('marketBrief: orquestador', () => {
  ...
  test('la ventana mensual etiqueta el contexto en MESES, nunca en semanas (FASE IE)', async () => {
    // Cinco meses planos y el último subiendo +10%: el mayor movimiento
    // mensual del rango, con ventana de 21 días hábiles.
    const closes = Array(106).fill(100).concat(Array.from({ length: 21 }, (_, i) => 100 + ((i + 1) * 10) / 21))
    const brief = await buildMarketBrief({
      fetchSeries: async () => ({ closes }),
      windowSize: 21, windowName: 'monthly', windowUnit: 'months',
    })
    const ctx = brief.context.join(' ')
    expect(ctx).toContain('monthly move')
    expect(ctx).toContain('months')
    expect(ctx).not.toContain('week')
  })
})
