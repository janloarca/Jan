...
// ⛔ FASE OM. `balanceAsOf` es un SEGUNDO corte, además de la compra: si el
// saldo tecleado (la "foto") cae DENTRO del mes que se está devengando, ese
// mes se prorratea desde el día SIGUIENTE a la foto, porque los días de antes
// ya están adentro del número que el usuario tecleó. El corte que manda es el
// MÁS TARDE de los dos (compra, foto).
describe('accrualDaysInMonth + balanceAsOf: la foto del saldo tambien recorta su mes', () => {
  it('sin balanceAsOf el comportamiento es identico al de siempre (callers viejos)', () => {
    expect(accrualDaysInMonth({ year: 2026, monthIndex: 7, acquisitionDay: '2026-08-20' }))
      .toBe(accrualDaysInMonth({ year: 2026, monthIndex: 7, acquisitionDay: '2026-08-20', balanceAsOf: undefined }))
  })

  it('la foto a mitad de mes recorta ese mes desde el dia siguiente', () => {
    // Agosto tiene 31 dias; foto al 15 => quedan 16 (del 16 al 31).
    expect(accrualDaysInMonth({ year: 2026, monthIndex: 7, balanceAsOf: '2026-08-15' })).toBe(16)
  })

  it('un mes ANTERIOR al de la foto devenga cero', () => {
    expect(accrualDaysInMonth({ year: 2026, monthIndex: 6, balanceAsOf: '2026-08-15' })).toBe(0)
  })

  it('un mes POSTERIOR al de la foto devenga entero', () => {
    expect(accrualDaysInMonth({ year: 2026, monthIndex: 8, balanceAsOf: '2026-08-15' })).toBe(30)
  })

  it('manda el mas TARDIO entre compra y foto', () => {
    // La foto es posterior a la compra: manda la foto.
    expect(accrualDaysInMonth({ year: 2026, monthIndex: 7, acquisitionDay: '2026-08-05', balanceAsOf: '2026-08-15' })).toBe(16)
    // La compra es posterior a la foto: manda la compra.
    expect(accrualDaysInMonth({ year: 2026, monthIndex: 7, acquisitionDay: '2026-08-20', balanceAsOf: '2026-08-05' })).toBe(11)
  })
})
...
describe('monthlyAccrual', () => {
  ...
  // ⛔ FASE OM. El caso reportado: un fondo de $5,000 al 4% que REINVIERTE, con
  // el saldo tecleado (`balanceAsOf`) al 15 de agosto. El asiento de fin de mes
  // devengaba agosto ENTERO (31 días, $16.68) sobre un saldo que ya contenía
  // media parte de él; con el corte, solo los 16 días de después ($8.60).
  it('la foto a mitad de mes devenga solo los dias de despues, no el mes entero', () => {
    const sinFoto = monthlyAccrual({ balance: 5000, annualRatePct: 4, year: 2026, monthIndex: 7 })
    const conFoto = monthlyAccrual({ balance: 5000, annualRatePct: 4, year: 2026, monthIndex: 7, balanceAsOf: '2026-08-15' })
    // El valor VIEJO (el defecto), para que quede fijado.
    expect(sinFoto).toBeCloseTo(16.68314230566459, 8)
    // El valor corregido: 16 dias (16 al 31), no 31.
    expect(conFoto).toBeCloseTo(8.60371460082665, 8)
    expect(conFoto).toBeCloseTo(5000 * (Math.pow(1.04, 16 / 365) - 1), 8)
    expect(conFoto).toBeLessThan(sinFoto)
  })
})
