...
describe('getBrokerCompletionSteps', () => {
  it('returns the full 3-step IBKR checklist', () => {
    expect(getBrokerCompletionSteps('ibkr')).toBe(IBKR_STEPS)
    expect(IBKR_STEPS.map((s) => s.id)).toEqual(['connect', 'quarterly', 'returns'])
  })
...
describe('IBKR_STEPS.done', () => {
  ...
  it('step 2 (quarterly) is skippable once the sync already reaches the account start', () => {
    expect(IBKR_STEPS[1].skippable({ ibkrSnapshotSpanDays: 500, earliestNeededDays: 400 })).toBe(true)
    expect(IBKR_STEPS[1].skippable({ ibkrSnapshotSpanDays: 200, earliestNeededDays: 400 })).toBe(false)
    expect(IBKR_STEPS[1].skippable({ ibkrSnapshotSpanDays: 500, earliestNeededDays: null })).toBe(false)
  })

  it('step 3 (returns) follows an ibkr-scoped calibration', () => {
    expect(IBKR_STEPS[2].done({ hasIbkrCalibration: true })).toBe(true)
    expect(IBKR_STEPS[2].done({ hasIbkrCalibration: false })).toBe(false)
  })

  // FASE NT: una calibración copiada que la app está IGNORANDO se DICE, sin
  // cambiar el estado del paso.
  it('step 3 (returns) raises attention when a copied calibration is being ignored', () => {
    const a = IBKR_STEPS[2].attention({ hasIbkrCalibration: false, ibkrCalibrationIgnored: 1 })
    expect(a).toEqual({ es: expect.stringMatching(/ignorando/), en: expect.stringMatching(/ignoring/) })
    expect(a.es).toMatch(/No se deshizo nada/)
    // El estado NO cambia: re-copiar sigue siendo lo correcto.
    expect(IBKR_STEPS[2].done({ hasIbkrCalibration: false, ibkrCalibrationIgnored: 1 })).toBe(false)
  })

  it('step 3 (returns) stays silent with nothing ignored, including a missing key', () => {
    expect(IBKR_STEPS[2].attention({ hasIbkrCalibration: false })).toBeNull()
    expect(IBKR_STEPS[2].attention({ ibkrCalibrationIgnored: 0 })).toBeNull()
  })

  it('attention never changes the inference gate', () => {
    const state = {
      ibkrConnected: true, ibkrSnapshotSpanDays: 500,
      hasQuarterlyHistory: true, hasIbkrCalibration: false, ibkrCalibrationIgnored: 2, earliestNeededDays: 400,
    }
    expect(hasCompleteBrokerData('ibkr', null, state)).toBe(false)
  })
})
