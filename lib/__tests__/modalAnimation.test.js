...
function overlayFiles() {
  ...
  for (const dir of DIRS) {
    ...
    for (const f of sourceFiles(abs)) {
      ...
      if (/fixed inset-0 z-50/.test(src)) out.push([path.relative(ROOT, f), src])
    }
  }
  return out
}
...
describe('todo modal entra y sale con animacion', () => {
  ...
  // colores (AccountReviewModal es literalmente ese caso: panel blanco con
  // texto `text-slate-900`, que sobre un fondo de tema oscuro sería invisible).
