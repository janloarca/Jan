...
describe('IBKR safety net — scaled vs held-flat vs gated', () => {
  ...
  it('uncovered early months use held-flat current value, covered months use NAV scaling', async () => {
    ...
    // Jan/Feb: no snapshot at or before → held-flat current value
    expect(r['2026-01']['ibkr1'].value).toBe(1000)
    expect(r['2026-02']['ibkr1'].value).toBe(1000)
    // Mar: scaled to NAV 800 (currentNAV 1000 → scale 0.8)
    expect(r['2026-03']['ibkr1'].value).toBeCloseTo(800)
    // Apr/May carry Mar's NAV forward
    expect(r['2026-04']['ibkr1'].value).toBeCloseTo(800)
    // Jun: scaled to NAV 1000
    expect(r['2026-06']['ibkr1'].value).toBeCloseTo(1000)
  })

  it('is blank in prior years before the asset was added', async () => {
    const r = await getHistoricalItemValues([ibkrItem], months2025, convert, 'USD', [], [], snaps)
    months2025.forEach(mk => {
      expect(r[mk]['ibkr1']).toBeUndefined()
    })
  })
})
