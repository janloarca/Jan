...
// =====================================================================
// Regression: createdAt must NOT blank current-year history
// (createdAt is the app-add date, not the acquisition date)
// =====================================================================
describe('createdAt gate — current year shows, prior years stay blank', () => {
  // A bank account added to the app mid-2026 with NO acquisitionDate.
  const recentBank = {
    id: 'recent', symbol: 'RECENT', name: 'Recent Bank', type: 'Bank',
    quantity: 1, currentPrice: 500, purchasePrice: 500,
    createdAt: '2026-06-15T00:00:00.000Z', _category: 'banks',
  }

  it('shows held-flat across ALL current-year months (not blank Jan–May)', async () => {
    const r = await getHistoricalItemValues([recentBank], months2026, convert, 'USD', [], [], [])
    months2026.forEach(mk => {
      expect(r[mk]['recent']).toBeDefined()
      expect(r[mk]['recent'].value).toBe(500)
    })
  })

  it('stays blank in every prior-year month (no fabricated 2025 value)', async () => {
    const r = await getHistoricalItemValues([recentBank], months2025, convert, 'USD', [], [], [])
    months2025.forEach(mk => {
      expect(r[mk]['recent']).toBeUndefined()
    })
  })
})

// =====================================================================
// IBKR held-flat safety net: scaled where snapshots cover, held-flat in
// uncovered current-year months, blank before the asset existed
// =====================================================================
describe('IBKR safety net — scaled vs held-flat vs gated', () => {
  const ibkrItem = {
    id: 'ibkr1', symbol: 'IBKRX', name: 'IBKR Position', type: 'stock',
    quantity: 1, currentPrice: 1000, purchasePrice: 1000, institution: 'IBKR Co',
    createdAt: '2026-01-10T00:00:00.000Z', _source: 'ibkr', _category: 'stocks',
  }
  const bucketKey = `${IBKR_UNKNOWN_KEY_PREFIX}IBKR Co__stocks`
  // Snapshots only reach back to March; June is current.
  const snaps = [
    { date: '2026-03-31', netWorthUSD: 800, _source: 'ibkr' },
    { date: '2026-06-30', netWorthUSD: 1000, _source: 'ibkr' },
  ]

  it('never writes under the real item id — always the synthetic unknown-positions bucket', async () => {
    const r = await getHistoricalItemValues([ibkrItem], months2026, convert, 'USD', [], [], snaps)
    months2026.forEach(mk => expect(r[mk]['ibkr1']).toBeUndefined())
  })

  it('uncovered early months use held-flat current value, covered months use NAV scaling', async () => {
    const r = await getHistoricalItemValues([ibkrItem], months2026, convert, 'USD', [], [], snaps)
    // Jan/Feb: no snapshot at or before → held-flat current value, flagged estimated
    expect(r['2026-01'][bucketKey].value).toBe(1000)
    expect(r['2026-01'][bucketKey].estimated).toBe(true)
    expect(r['2026-02'][bucketKey].value).toBe(1000)
    // Mar: scaled to real synced NAV 800 (currentNAV 1000 → scale 0.8) — a real
    // Flex NAV is exact, not an estimate.
    expect(r['2026-03'][bucketKey].value).toBeCloseTo(800)
    expect(r['2026-03'][bucketKey].estimated).toBe(false)
    // Apr/May carry Mar's NAV forward
    expect(r['2026-04'][bucketKey].value).toBeCloseTo(800)
    // Jun: scaled to NAV 1000
    expect(r['2026-06'][bucketKey].value).toBeCloseTo(1000)
  })

  it('a transcribed quarterly screenshot NAV is flagged estimated, unlike a real Flex sync', async () => {
    const quarterlySnaps = [{ date: '2026-03-31', netWorthUSD: 800, _source: 'ibkr_quarterly' }]
    const r = await getHistoricalItemValues([ibkrItem], months2026, convert, 'USD', [], [], quarterlySnaps)
    expect(r['2026-03'][bucketKey].value).toBeCloseTo(800)
    expect(r['2026-03'][bucketKey].estimated).toBe(true)
  })

  it('is blank in prior years before the asset was added (createdAt gate)', async () => {
    const r = await getHistoricalItemValues([ibkrItem], months2025, convert, 'USD', [], [], snaps)
    months2025.forEach(mk => {
      expect(r[mk][bucketKey]).toBeUndefined()
    })
  })

  it('does NOT gate on acquisitionDate (sync-stamped, unreliable) — only on createdAt', async () => {
    // No snapshot reaches back this far at all (nothing to carry forward), and
    // acquisitionDate (sync-stamped) is much later than the checked month — but
    // createdAt says the row has existed in the app since 2023, so the month
    // must still get a held-flat estimate, not a blank (FASE FH: this exact gate
    // was blanking a real month sandwiched between two years that DID have data).
    const oldItem = { ...ibkrItem, createdAt: '2023-05-01T00:00:00.000Z', acquisitionDate: '2026-01-10' }
    const laterSnaps = [{ date: '2025-12-31', netWorthUSD: 900, _source: 'ibkr' }]
    const r = await getHistoricalItemValues([oldItem], ['2024-12'], convert, 'USD', [], [], laterSnaps)
    expect(r['2024-12'][bucketKey]).toBeDefined()
    expect(r['2024-12'][bucketKey].value).toBeCloseTo(1000)
    expect(r['2024-12'][bucketKey].estimated).toBe(true)
  })
})
