...
describe('periodRange', () => {
  // Se lee en UTC porque la frontera se CORTA en UTC (FASE KY). Leerla con
  // getMonth() local pasaba con el runner en UTC y fallaba en Guatemala, o sea
  // el test heredaba el mismo defecto que el codigo: comprobado corriendo la
  // suite con TZ=America/Guatemala.
  test('ventanas correctas para ago 2026', () => {
    expect(new Date(periodRange('month', NOW).startTs).getUTCMonth()).toBe(7)
    expect(new Date(periodRange('quarter', NOW).startTs).getUTCMonth()).toBe(6)
    expect(new Date(periodRange('ytd', NOW).startTs).getUTCMonth()).toBe(0)
    expect(periodRange('all', NOW).startTs).toBeNull()
  })

  // La frontera cae en la medianoche UTC del dia nominal, EXACTO. Con la
  // convencion local, al oeste de UTC caia seis horas despues, asi que el
  // snapshot fechado ese mismo dia (ts = medianoche UTC) quedaba FUERA de su
  // propia ventana. Esta asercion es independiente de la zona del runner.
  test('la frontera es medianoche UTC del dia nominal', () => {
    expect(periodRange('ytd', NOW).startTs).toBe(Date.UTC(2026, 0, 1))
    expect(periodRange('month', NOW).startTs).toBe(Date.UTC(2026, 7, 1))
    expect(periodRange('quarter', NOW).startTs).toBe(Date.UTC(2026, 6, 1))
  })
})
