...
describe('expense groups (the 6 main groups)', () => {
  ...
  test('income groups cover all income categories and mark Inversiones auto-only', () => {
    ...
    const inv = INCOME_GROUPS.find((g) => g.autoOnly)
    expect(inv.categories).toContain('Inversiones')
    expect(MANUAL_INCOME_BLOCKED).toContain('Inversiones')
  })
})
