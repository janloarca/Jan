...
// ⛔ FASE OM. El caso reproducido con el hook real: un fondo de $5,000 al 4%
// que REINVIERTE, con un saldo tecleado (`balanceAsOf`) al 15 de agosto. Sin
// el corte, el asiento de fin de mes devengaba AGOSTO ENTERO (31 días) sobre
// un saldo que ya contenía media parte de él; con el corte, solo los 16 días
// posteriores a la foto.
describe('monthlyIncomeAmount + devengo diario: la foto del saldo prorratea el mes en que cae', () => {
  const daily = { balance: 5000, accrual: 'daily', incomeRate: 4, payDate: '2026-08-31' }

  test('sin accrualBalanceAsOf, el mes se devenga entero (el defecto reproducido)', () => {
    expect(monthlyIncomeAmount(daily, 12)).toBeCloseTo(16.68314230566459, 10)
  })

  test('con la foto a mitad de mes, solo se devengan los días de después', () => {
    expect(monthlyIncomeAmount({ ...daily, accrualBalanceAsOf: '2026-08-15' }, 12))
      .toBeCloseTo(8.60371460082665, 10)
  })

  // El corte manda el MÁS TARDE entre la compra y la foto: una compra DESPUÉS
  // de una foto vieja sigue prorrateando desde la compra, sin que la foto
  // adelantada mueva un centavo.
  test('una compra posterior a la foto sigue prorrateando desde la compra, no desde la foto', () => {
    const comprado20 = { ...daily, acquisitionDay: '2026-08-20', accrualBalanceAsOf: '2026-08-01' }
    const soloCompra = { ...daily, acquisitionDay: '2026-08-20' }
    expect(monthlyIncomeAmount(comprado20, 12)).toBeCloseTo(monthlyIncomeAmount(soloCompra, 12), 10)
  })

  // Un mes ANTERIOR o POSTERIOR al de la foto no se recorta: el corte solo
  // aplica al mes en que la foto realmente cae.
  test('un mes que no es el de la foto no se recorta', () => {
    const septiembre = { ...daily, payDate: '2026-09-30', accrualBalanceAsOf: '2026-08-15' }
    expect(monthlyIncomeAmount(septiembre, 12)).toBeCloseTo(monthlyIncomeAmount({ ...daily, payDate: '2026-09-30' }, 12), 10)
  })
})
