...
describe('la hora en el id no puede romper la idempotencia', () => {
  // FASE LO. Este test afirmaba `reason === 'same-event'`, o sea fijaba QUÉ RED
  // atrapaba el reintento, no el invariante. Con la hora de llegada dentro del
  // id (necesario: sin eso Android tragaba cobros reales, porque nunca reporta
  // hora) el reintento resuelve a otro id y lo atrapa la SEGUNDA red,
  // `findNearDuplicate`, con su ventana de cinco minutos.
  //
  // El invariante es y siempre fue "un reintento no escribe un segundo gasto":
  // eso es `_docs.size === 1` y no se movió. Se afirma explícito abajo, junto
  // con que sigue reportándose como duplicado, para que este test no vuelva a
  // depender de por cuál capa pasó.
  it('un reintento con hora de LLEGADA no escribe un segundo gasto', async () => {
    const db = fakeDb()
    ...
    const segundo = await write(db, reintento)
