...
describe('computeCosts — item-level costs (manual assets)', () => {
  ...
  it('accepts the year as a STRING: the page keeps it that way (SegmentedTabs)', () => {
    ...
    const items = [{ id: 'x', managementFee: 120, managementFeeType: 'fixed', acquisitionDate: '2020-01-01', quantity: 1, purchasePrice: 1000, currency: 'USD' }]
    // ⛔ El reloj se FIJA para las dos llamadas. La prorrata corre "hasta hoy",
    // así que dos invocaciones seguidas ven instantes distintos: $120 al año
    // avanzan 3.81e-9 por MILISEGUNDO contra una tolerancia de 5e-10, o sea un
    // solo milisegundo entre ambas tumbaba el test. Pasaba casi siempre porque
    // las dos caían en el mismo ms, y fallaba bajo la carga de la suite
    // completa: una carrera real, no un test "flaky" que se pueda ignorar.
