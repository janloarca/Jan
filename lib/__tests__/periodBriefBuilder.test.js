  ...
  return {
    ...
    doc: (path) => ({
      get: async () => {
        ...
        if (path.endsWith('/settings/profile')) return { exists: !!profile, data: () => profile }
...
// El texto de un PDF de jsPDF sin comprimir: los operadores `(...) Tj` del
// content stream. Se lee del PDF GENERADO, nunca del código que lo intenta.
function pdfTextOf(mail) {
...
describe('buildMonthlyBriefForUser (integración con db falso)', () => {
  test('arma el correo del mes cerrado con sus DOS adjuntos reales', async () => {
    ...
    // FASE OO: con el total oculto (el default), el período es la CIFRA
    // PRINCIPAL y su etiqueta dice cuál es, así que el sufijo "this month" de
    // la pastilla ya no existe.
    expect(mail.text).toContain('This month: +2.30%')
...
describe('el reporte adjunto no imprime ceros falsos', () => {
  const pdfText = async (mail) => pdfTextOf(mail)
...
// ── FASE KB: el nombre del reporte salia SIEMPRE en blanco ────────────────
// Los builders leian `prefs.profileName` (doc `settings/preferences`) y NADIE
// escribe nunca ese campo: ninguno de los 15 call sites de `saveSettings` lo
// pasa. El nombre real vive en OTRO doc, `settings/profile` bajo `.name`, que
// es el que la app usa en pantalla. O sea no faltaba el nombre: el servidor lo
// buscaba donde nunca estuvo, y los reportes salen sin el desde que existen.
//
// Se comprueba sobre el PDF GENERADO (texto extraido del content stream y
// nombre del archivo adjunto), nunca leyendo el codigo que lo intenta.
describe('el nombre del reporte sale de settings/profile', () => {
  const NAME = 'Jan Marco'

  test('con perfil guardado, el PDF lo imprime y lo lleva en su nombre de archivo', async () => {
    const db = makeFakeDb({ profile: { name: NAME } })
    const mail = await buildMonthlyBriefForUser({ db, uid: 'u1', prefs: {}, now: NOW })
    expect(pdfTextOf(mail)).toContain(NAME)
    const pdf = mail.attachments.find((a) => a.filename.endsWith('.pdf'))
    expect(pdf.filename).toMatch(/^chispudo-report-jan-marco-\d{4}-\d\d-\d\d\.pdf$/)
  })

  test('sin perfil, el correo sale igual y sin nombre', async () => {
    // Nadie pierde su resumen porque el doc de perfil no exista.
    const mail = await buildMonthlyBriefForUser({ db: makeFakeDb(), uid: 'u1', prefs: {}, now: NOW })
    expect(mail).toBeTruthy()
    expect(pdfTextOf(mail)).not.toContain(NAME)
    const pdf = mail.attachments.find((a) => a.filename.endsWith('.pdf'))
    expect(pdf.filename).toMatch(/^chispudo-report-\d{4}-\d\d-\d\d\.pdf$/)
  })

  test('un perfil ilegible deja el nombre vacio en vez de tumbar el correo', async () => {
    const base = makeFakeDb()
    const broken = {
      collection: base.collection,
      doc: (p) => (p.endsWith('/settings/profile')
        ? { get: async () => { throw new Error('permission denied') } }
        : base.doc(p)),
    }
    const mail = await buildMonthlyBriefForUser({ db: broken, uid: 'u1', now: NOW })
    expect(mail).toBeTruthy()
    expect(mail.attachments.some((a) => a.filename.endsWith('.pdf'))).toBe(true)
  })

  test('un `name` que no es texto no se cuela al PDF', async () => {
    const db = makeFakeDb({ profile: { name: { first: 'Jan' } } })
    const mail = await buildMonthlyBriefForUser({ db, uid: 'u1', now: NOW })
    const pdf = mail.attachments.find((a) => a.filename.endsWith('.pdf'))
    expect(pdf.filename).toMatch(/^chispudo-report-\d{4}-\d\d-\d\d\.pdf$/)
  })

  test('prefs.profileName ya NO tiene efecto: el campo muerto esta muerto', async () => {
    // Fija que el campo viejo quedo desconectado de verdad. Si alguien lo
    // "restaura" como respaldo, este test lo dice: sugerir que configurarlo
    // por ahi sirve es peor que no tener el campo.
    const mail = await buildMonthlyBriefForUser({
      db: makeFakeDb(), uid: 'u1', prefs: { profileName: 'Nombre Muerto' }, now: NOW,
    })
    expect(pdfTextOf(mail)).not.toContain('Nombre Muerto')
    const pdf = mail.attachments.find((a) => a.filename.endsWith('.pdf'))
    expect(pdf.filename).toMatch(/^chispudo-report-\d{4}-\d\d-\d\d\.pdf$/)
  })

  test('el reporte del SEMANAL usa el mismo nombre', async () => {
    // Los dos builders son modulos aparte: cablear uno solo dejaria los dos
    // PDFs de la misma tanda contradiciendose, como en FASE IE8.
    const { buildWeeklyBriefForUser } = await import('../weeklyBriefBuilder')
    const db = makeFakeDb({ profile: { name: NAME } })
    const mail = await buildWeeklyBriefForUser({ db, uid: 'u1', prefs: {}, now: NOW })
    expect(pdfTextOf(mail)).toContain(NAME)
  })

  // FASE OO. El interruptor del patrimonio (`emailShowNetWorth`) llega DESDE
  // las preferencias hasta el cuerpo del correo. Los dos builders son modulos
  // aparte, asi que se prueban los dos: cablear uno solo dejaria al otro
  // filtrando el total en la vista previa del telefono (leccion FASE IE8).
  test('el total no viaja por default, y el interruptor lo trae de vuelta', async () => {
    const { buildWeeklyBriefForUser } = await import('../weeklyBriefBuilder')
    const builders = [
      ['mensual', buildMonthlyBriefForUser],
      ['semanal', buildWeeklyBriefForUser],
    ]
    for (const [, build] of builders) {
      const off = await build({ db: makeFakeDb(), uid: 'u1', prefs: {}, now: NOW })
      expect(off.text).not.toContain('$6,240.00')
      expect(off.text).toMatch(/kept out of this email/)

      const on = await build({ db: makeFakeDb(), uid: 'u1', prefs: { emailShowNetWorth: true }, now: NOW })
      expect(on.text).toContain('Net worth: $6,240.00')
      expect(on.text).not.toMatch(/kept out of this email/)
    }
  })

  test('el cache de corrida cubre TAMBIEN la lectura del perfil', async () => {
    // El test de cache de FASE IE9 envuelve solo `collection().get()`, asi que
    // no ve las lecturas de `doc()`: sin este, la lectura nueva podria estar
    // repitiendose por cadencia sin que nada lo dijera.
    let profileReads = 0
    const base = makeFakeDb({ profile: { name: NAME } })
    const db = {
      collection: base.collection,
      doc: (p) => {
        const inner = base.doc(p)
        if (!p.endsWith('/settings/profile')) return inner
        return { get: async () => { profileReads++; return inner.get() } }
      },
    }
    const cache = makeContextCache()
    await buildMonthlyBriefForUser({ db, uid: 'u1', now: NOW, cache })
    ...
    await buildAnnualBriefForUser({ db, uid: 'u1', now: NOW, cache })
    expect(profileReads).toBe(1)
    // Otro usuario si vuelve a leer: el cache es por uid, no global.
    await buildMonthlyBriefForUser({ db, uid: 'u2', now: NOW, cache })
