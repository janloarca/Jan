...
describe('detectPhantomFlows', () => {
  ...
  it('never judges user-entered flows, whatever their manual_* source', () => {
    // Todo origen manual lleva prefijo "manual" (manual_new_account,
    // manual_contribution, manual_balance_flow...). El guard viejo solo cubria
    // el literal 'manual' y dejaba juzgar los demas como si fueran de broker.
    const mk = (id, date, amount) => ({
      id, date, type: 'DEPOSIT', totalAmount: amount,
      _source: 'manual_new_account', description: 'Deposit',
    })
    const rows = [mk('a', '2026-01-05', 1450), mk('b', '2026-02-05', 1350), mk('c', '2026-03-05', 1150), mk('t', '2026-07-28', 3950)]
    expect(detectPhantomFlows(rows)).toEqual([])
  })
...
describe('detectPhantomFlows on a realistic multi-year ledger', () => {
  // Regression for the real failure: the test fixtures above only had 8 flows,
  // while a genuine account that imported an inception report plus a year
  // statement has ~25. Fed oldest-first the search exhausted its budget on the
  // 2023-2025 rows and returned nothing, so the detector looked correct in CI
  // and did nothing in production. Values must be consumed newest-first.
  const inception = Array.from({ length: 19 }, (_, i) => ({
    id: `inc${i}`,
    // Deliberately non-round, cent-level amounts: those explode the reachable
    // set fastest and are what real transfers look like.
    date: `202${3 + Math.floor(i / 8)}-${String((i % 12) + 1).padStart(2, '0')}-15`,
    type: 'DEPOSIT',
    totalAmount: 137.43 + i * 41.17,
    _source: 'ibkr',
  ...
  it('finds the totals row among 24 older flows spanning three years', () => {
    const hits = detectPhantomFlows([...inception, ...year, phantom])
    expect(hits.map((h) => h.id)).toEqual(['ghost'])
  })

  it('finds it regardless of the order the ledger arrives in', () => {
    const shuffled = [...inception, ...year, phantom].sort(() => 0.5 - Math.random())
    expect(detectPhantomFlows(shuffled).map((h) => h.id)).toEqual(['ghost'])
  })

  it('still flags nothing on the same ledger without the totals row', () => {
    expect(detectPhantomFlows([...inception, ...year])).toEqual([])
  })
})
