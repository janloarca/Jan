import { monthKeyOf, getMonthStatus, computeMonthlyAnalysis, buildFinanceInsights, investmentIncomeOfMonth, formatFinanceDate } from '../financeMonth'
...
describe('computeMonthlyAnalysis', () => {
  ...
  it('extraIncome folds read-only investment income into income/savings per month', () => {
    const a = computeMonthlyAnalysis(data, { month: 6, year: 2026 }, null, {
      extraIncome: 1000, prevExtraIncome: 500, yoyExtraIncome: 0,
    })
    expect(a.income).toBe(11000)
    expect(a.savings).toBe(11000 - 3050)
    expect(a.savingsRate).toBeCloseTo(((11000 - 3050) / 11000) * 100)
    expect(a.prev.income).toBe(10500)
    expect(a.momIncomePct).toBeCloseTo(((11000 - 10500) / 10500) * 100)
    expect(a.yoy.income).toBe(8000) // untouched when the extra is 0
  })

  it('extraIncome alone does not turn overspending into savings insights', () => {
    const onlyExpenses = [tx('2026-07-03', 'EXPENSE', 500, 'Alimentación')]
    const withDividends = computeMonthlyAnalysis(onlyExpenses, { month: 6, year: 2026 }, null, { extraIncome: 1000 })
    expect(withDividends.savings).toBe(500)
    expect(withDividends.savingsRate).toBeCloseTo(50)
    const insights = buildFinanceInsights(withDividends, 'es')
    expect(insights.some((i) => i.textEs.includes('más de lo que ingresó'))).toBe(false)
  })
