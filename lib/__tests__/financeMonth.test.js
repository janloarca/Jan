...
// FASE LK — el año en una vista. La garantía que importa es de PARIDAD: cada
// columna sale del MISMO summarizeMonth que produce la pantalla del mes, así
// que tocar una columna y aterrizar en su mes no puede mostrar otros números.
describe('yearTotalsByMonth (FASE LK)', () => {
  const { yearTotalsByMonth } = require('../financeMonth')
  const rows = [
    { id: 'a', type: 'INCOME', date: '2026-07-01', amount: 15000, currency: 'GTQ', category: 'Salario' },
    { id: 'b', type: 'EXPENSE', date: '2026-07-15', amount: 39782, currency: 'GTQ', category: 'Seguros' },
    { id: 'c', type: 'EXPENSE', date: '2026-07-20', amount: -500, currency: 'GTQ', category: 'Compras' }, // reembolso
    { id: 'd', type: 'INCOME', date: '2026-07-22', amount: 8175, currency: 'GTQ', category: 'Transferencia Recibida' },
    { id: 'e', type: 'EXPENSE', date: '2026-06-10', amount: 690, currency: 'GTQ', category: 'Alimentación' },
    { id: 'f', type: 'EXPENSE', date: '2025-07-10', amount: 999, currency: 'GTQ', category: 'Compras' }, // otro año
  ]

  it('cada columna coincide con computeMonthlyAnalysis de ese mes, al centavo', () => {
    const cols = yearTotalsByMonth(rows, 2026, null)
    expect(cols).toHaveLength(12)
    const july = cols[6]
    const julyView = computeMonthlyAnalysis(rows, { month: 6, year: 2026 }, null)
    expect(july.key).toBe('2026-07')
    expect(july.income).toBeCloseTo(julyView.income, 2)
    expect(july.expenses).toBeCloseTo(julyView.expenses, 2)
    // Y hereda las reglas: la transferencia no cuenta, el reembolso NETEA.
    expect(july.income).toBeCloseTo(15000, 2)
    expect(july.expenses).toBeCloseTo(39782 - 500, 2)
  })

  it('un año ajeno no se filtra a ninguna columna', () => {
    const cols = yearTotalsByMonth(rows, 2026, null)
    const total = cols.reduce((s, c) => s + c.expenses, 0)
    expect(total).toBeCloseTo(39782 - 500 + 690, 2) // sin los 999 de 2025
  })
})
