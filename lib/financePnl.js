...
export function buildPnlStatement(analysis, model) {
  ...
  // El CERO EXACTO no es superávit. `bottom >= 0` es cierto en cero, así que un
  // mes que gastó exactamente lo que entró se pintaba con el verde de una
  // ganancia. La PALABRA sigue saliendo de `surplus` ("Resultado", no
  // "Déficit", que en cero sería falso); lo que cambia es que el color tiene un
  // tercer estado. Medio centavo de tolerancia, la misma que usa el aviso de
  // pagos sin cuadrar: por debajo de eso es ruido de redondeo entre monedas.
  const bottomZero = Math.abs(bottom) < 0.005
