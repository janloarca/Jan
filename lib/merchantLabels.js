...
// El rótulo que el usuario YA escribió para un comercio, indexado por la MISMA
// llave con la que se aprenden las reglas (`merchantRuleKey`), para que "FINCA
// FELIZ GT" y "FINCA FELIZ" compartan el rótulo igual que comparten la regla.
//
// Dos fuentes, y las dos las tecleó el usuario:
//   · `rule.label` — lo que escribió al enseñar la categoría de ese comercio.
//   · `tx.userLabel` — lo que escribió corrigiendo ESA fila. Gana sobre la
//     regla porque es lo más reciente que dijo sobre el comercio.
export function buildMerchantLabelIndex(transactions = [], rules = []) {
  const index = new Map()
  for (const r of rules || []) {
    if (!r?.match || !r?.label) continue
    index.set(String(r.match), String(r.label))
  }
  for (const tx of transactions || []) {
    if (!tx?.userLabel) continue
    const key = merchantRuleKey(tx.merchant || tx.description || '')
    if (!key) continue
    index.set(key, String(tx.userLabel))
  }
  return index
}

// ⛔ La cadena del banco NUNCA se transforma. Un `capitalize` de CSS convierte
// "iShares" en "IShares" (FASE JL lo quitó justamente por eso) y un title-case
// de JS rompe "BAC CREDOMATIC". Lo único que puede reemplazar a esa cadena es
// lo que el usuario escribió él mismo; sin eso se muestra tal cual, en
// mayúsculas, que es feo pero es la verdad de lo que imprimió el banco.
export function merchantDisplay(description, index) {
  const raw = String(description || '').trim()
  if (!index || typeof index.get !== 'function' || index.size === 0) return raw
  const key = merchantRuleKey(raw)
  return (key && index.get(key)) || raw
}
