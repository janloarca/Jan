...
// ⛔ QUÉ CUENTA COMO GASTO EN FLUJO, EN UN SOLO LUGAR.
//
// Estas dos se exportan para que otra superficie que sume gasto (hoy
// lib/spendingByWeekday.js) le pregunte a la MISMA definición en vez de
// reimplementarla. Es exactamente el par que usa `summarizeMonth` abajo: la
// exclusión de transferencias (sin ella, las dos caras del mes se inflan por el
// mismo monto, ver lib/cardPaymentNetting.js) y la conversión con respaldo al
// monto crudo cuando falta la tasa. Con una copia, la rejilla semanal y el
// total del mes divergirían a la primera transferencia degradada.
export function isSpendRow(tx) {
  return !!tx && tx.type === 'EXPENSE' && !isTransferCategory(tx.category)
}

export function txAmount(tx, convert, to = FINANCE_CURRENCY) {
...
// `dayCap` recorta la ventana a los primeros N días del mes ('YYYY-MM-DD':
// comparar los dos dígitos del día como TEXTO es comparar cronológicamente).
// Es lo que hace posible "los primeros 18 días de agosto contra los primeros
// 18 de julio": dos ventanas del mismo tamaño.
function txsOfMonth(transactions, key, dayCap = null) {
  const cap = dayCap != null ? String(dayCap).padStart(2, '0') : null
  return (transactions || []).filter((tx) => typeof tx.date === 'string' && tx.date.startsWith(key)
    && (cap == null || tx.date.slice(8, 10) <= cap))
}

// Días reales de un mes 'YYYY-MM'. Componentes numéricos, nunca new Date(str).
...
function summarizeMonth(transactions, key, convert, dayCap = null) {
  const txs = txsOfMonth(transactions, key, dayCap)
  let income = 0
  let expenses = 0
  const byGroup = {}
  const incomeByGroup = {}
...
// Las doce columnas del año (feature 5 del plan): cada mes sale del MISMO
// summarizeMonth que produce la pantalla del mes seleccionado, así que la
// columna de julio y la vista de julio no pueden discrepar por construcción
// (misma exclusión de transferencias, misma semántica de reembolsos, misma
// conversión). Muestra lo OCURRIDO: el plan de ingresos (incomePlan) jamás
// entra acá, esa es la regla dura de lib/incomePlan.js.
export function yearTotalsByMonth(transactions, year, convert) {
  return Array.from({ length: 12 }, (_, i) => {
    const key = monthKeyOf(year, i)
    const s = summarizeMonth(transactions, key, convert)
    return { key, month: i + 1, income: s.income, expenses: s.expenses, txCount: s.txCount }
  })
}

// Los totales por GRUPO de un año completo, para la dona en vista anual.
//
// Sale de los MISMOS doce `summarizeMonth` que ya produce `yearTotalsByMonth`,
// así que la dona del año y las doce columnas del año no pueden discrepar: un
// segundo motor sumando el mismo dinero es como nacen esas contradicciones.
// El año va COMPLETO (enero a diciembre) y no hasta hoy: es la lectura natural
// de "por año", y los meses que todavía no ocurrieron suman cero por sí solos.
export function yearGroupTotals(transactions, year, convert) {
  const byGroup = {}
  const incomeByGroup = {}
  // Por CATEGORÍA además de por grupo: la dona agrupa con la taxonomía del
  // USUARIO (que puede haber movido una categoría de grupo), así que necesita
  // el detalle para re-agrupar. Con solo los totales por grupo de fábrica, la
  // dona del año y el estado del mes dirían cosas distintas sobre la misma
  // categoría en cuanto alguien la reubicara.
  const byCategory = {}
  ...
  let txCount = 0
  for (let i = 0; i < 12; i++) {
    const s = summarizeMonth(transactions, monthKeyOf(year, i), convert)
    income += s.income
    expenses += s.expenses
    txCount += s.txCount
    for (const [k, v] of Object.entries(s.byGroup)) byGroup[k] = (byGroup[k] || 0) + v
    for (const [k, v] of Object.entries(s.incomeByGroup)) incomeByGroup[k] = (incomeByGroup[k] || 0) + v
    for (const gk of Object.keys(s.catByGroup)) {
      for (const [c, v] of Object.entries(s.catByGroup[gk])) byCategory[c] = (byCategory[c] || 0) + v
    }
    for (const gk of Object.keys(s.incomeCatByGroup)) {
      for (const [c, v] of Object.entries(s.incomeCatByGroup[gk])) incomeByCategory[c] = (incomeByCategory[c] || 0) + v
    }
  }
  return { year, income, expenses, byGroup, incomeByGroup, byCategory, incomeByCategory, txCount }
}
...
export function computeMonthlyAnalysis(transactions, { month, year }, convert, extras = {}) {
  ...
  // ── Comparación a mismo día del mes ──
  //
  // Con el mes EN CURSO, comparar contra el mes anterior COMPLETO es media
  // ventana contra una entera, y por eso `isComparable` callaba todo (dos de
  // cada tres visitas caen en un mes a medias: la pantalla vivía sin flechas).
  // La comparación honesta existe: los primeros N días de este mes contra los
  // primeros N del anterior. Ahí las dos ventanas miden lo mismo, que es
  // exactamente lo que isComparable exige, así que las variaciones vuelven a
  // encenderse desde el día 2, rotuladas con la ventana.
  //
  // Solo cuando la ventana CABE en el mes comparado: el 30 de marzo contra
  // febrero (28 días) volvería a ser ventana grande contra chica, y en esos
  // 1-3 días del final se calla como antes. `windowDays` no-null es la señal
  // para el caller de que las variaciones son a mismo día.
  const windowFits = progress.partialMonth && progress.daysElapsed <= daysInMonthOf(prevKey)
  const windowDays = windowFits ? progress.daysElapsed : null
  const prevCompare = windowDays != null
    ? summarizeMonth(transactions, prevKey, convert, windowDays)
    : prevSummary
  // La razón por la que las filas siguen calladas: solo aplica cuando NO hay
  // ventana equivalente que ofrecer.
  const stillPartial = progress.partialMonth && windowDays == null
  ...
  // Lo mismo de la ventana ANTERIOR, aplanado por categoría. ADICIÓN PURA: sale
  // del `prevCompare` que este motor ya calculaba y no cambia un solo número de
  // los que ya devolvía. Existe porque el estado de resultados (lib/financePnl)
  // lista CATEGORÍAS y una columna de variación por línea es el estándar de un
  // P&L; el desglose por grupo ya tenía la suya y la de categoría faltaba.
  const flattenCats = (map) => {
    const out = {}
    for (const gk of Object.keys(map || {})) {
      for (const [cat, amt] of Object.entries(map[gk] || {})) {
  ...
  const groups = [...EXPENSE_GROUPS, OTHER_GROUP].map((g) => {
    ...
    // Con el mes en curso, `prev` es la MISMA ventana del mes anterior (los
    // primeros N días); con el mes cerrado, el mes anterior completo.
    const prev = hasPrev ? (prevCompare.byGroup[g.key] || 0) : null
    ...
    return {
      ...
      comparable: isComparable(cur, prev, stillPartial),
  ...
  const incomeGroups = INCOME_GROUPS.map((g) => {
    ...
    const prev = hasPrev ? (prevCompare.incomeByGroup[g.key] || 0) : null
  ...
  // ⛔ FASE MK. El MISMO `!== 0` que el lado del gasto, y por la misma razón:
  // un ingreso puede quedar NEGATIVO en neto. `CORRECCION A PAGO` (FASE KQ)
  // revierte una entrada de dinero, así que un mes cuyas correcciones superan
  // a los pagos de ese grupo cierra abajo de cero — el caso real del estado de
  // BI, con −Q1,731.26 corrigiendo un pago del ciclo anterior.
  //
  // Con `> 0` esa fila DESAPARECÍA mientras su dinero seguía contando en el
  // total de arriba: medido, ingreso de −1,231.26 en el encabezado con CERO
  // grupos listados, o sea las partes dejaban de sumar el todo. Y desde afuera
  // una fila escondida y una fila rota se ven igual (la lección de FASE JI).
  //
  // FASE JW arregló exactamente esto en el lado del gasto y dejó este sin
  // tocar: la asimetría no era una decisión, era la mitad que faltaba.
  ...
  return {
    ...
    incomeLooksUnlogged,
    ...progress,
    // No-null cuando las variaciones (filas Y encabezado) comparan los
    // primeros N días de este mes contra los primeros N del anterior. La UI lo
    // usa para rotular la ventana; los insights también.
    windowDays,
    // Los totales del encabezado se callan por la misma razón que las filas:
    // media ventana contra una completa. Con la ventana a mismo día, vuelven.
    momComparable: hasPrev && !stillPartial,
    ...
    momExpensesPct: hasPrev ? pctDelta(current.expenses, prevCompare.expenses) : null,
    momIncomePct: hasPrev ? pctDelta(current.income, prevCompare.income) : null,
    // Adición pura (ver flattenCats): la MISMA ventana de comparación que ya
    // usan los grupos, ahora también disponible por categoría.
    prevCategories: hasPrev ? flattenCats(prevCompare.catByGroup) : null,
    prevIncomeCategories: hasPrev ? flattenCats(prevCompare.incomeCatByGroup) : null,
    // El YoY se queda de mes completo a propósito: su insight solo se emite con
    // el mes cerrado, y una ventana de 18 días contra el año pasado agrega
    // ruido de calendario (quincenas que caen en otro día de semana) sin que
    // ninguna superficie lo pida todavía.
...
export function buildFinanceInsights(analysis, lang = 'es') {
  ...
  // Con el mes en curso los montos comparados son los primeros N días de cada
  // mes (ver windowDays en computeMonthlyAnalysis): el rótulo TIENE que decir
  // la ventana, o "(Q1,500 → Q600)" se leería como julio completo.
  const vsEs = analysis.windowDays != null
    ? `vs los primeros ${analysis.windowDays} días del mes pasado`
    : 'vs el mes pasado'
  const vsEn = analysis.windowDays != null
    ? `vs the first ${analysis.windowDays} days of last month`
    : 'vs last month'
  for (const g of movers.slice(0, 2)) {
    const dir = g.momPct >= 0 ? t('subió', 'went up') : t('bajó', 'went down')
    out.push({
      severity: g.momPct >= 0 ? 'warn' : 'good',
      textEs: `${g.icon} ${g.label} ${dir} ${Math.abs(g.momPct).toFixed(0)}% ${vsEs} (${fmtQ(g.prevAmount)} → ${fmtQ(g.amount)}).`,
      textEn: `${g.icon} ${g.labelEn} ${dir} ${Math.abs(g.momPct).toFixed(0)}% ${vsEn} (${fmtQ(g.prevAmount)} → ${fmtQ(g.amount)}).`,
    })
  }
