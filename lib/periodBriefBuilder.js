 ...
 */
export async function buildPeriodBriefForUser({ kind = 'monthly', db, uid, prefs = {}, market = null, now = new Date(), cache = null }) {
  ...
  const showNetWorth = prefs?.emailShowNetWorth === true
  const { subject, html, text } = isAnnual
    ? buildAnnualBriefEmail({ yearLabel: periodLabel, portfolio, market, attachmentsInfo, showNetWorth })
    : buildMonthlyBriefEmail({ monthLabel: periodLabel, portfolio, market, attachmentsInfo, showNetWorth })
