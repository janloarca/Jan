...
export function buildHistoryItemsPayload({ items, transactions, lots, convert }) {
  ...
  // ⛔ FASE MZ. Qué ítem ES la caja del broker se resuelve ANTES de construir
  // sus flujos, porque `buildCashFlows` necesita saberlo: un gasto pagado
  // DESDE esta misma caja sí la movió y tiene que deshacerse, mientras que uno
  // pagado desde otra cuenta no (ver el guard de `_paidFromItemId` allá). Se
  // separó de la decisión de USARLA, que sigue exigiendo flujos y un movimiento
  // externo real, para que las dos no se necesiten mutuamente.
  ...
  const cashCandidate = (items || []).find((it) => it._source === 'ibkr' && /^CASH-/i.test(it.symbol || ''))
