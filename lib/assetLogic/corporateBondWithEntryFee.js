...
//       · FASE NN (misma autorización, mismo número reportado): FASE NL arregló
//         la especie EQUIVOCADA de calibración y por eso no movió un centavo.
//         El usuario volvió con la misma captura y el ancla despejada dio
//         9,305.22 OTRA VEZ, al centavo. Esa invariancia es el dato: un doc que
//         un backfill reescribe habría cambiado de valor; uno que se COMPUTA en
//         vivo, no. Una calibración POR CUENTA (`_account`) nunca es el ancla
//         guardada: vive en su propio doc y `combineAccountCalibrations` la
//         aplica EN MEMORIA dentro de este memo, cambiando la rebanada estimada
//         de esa cuenta por el valor despejado del %. O sea el archivo puede
//         quedar perfecto y el arranque sale igual de inflado, porque la
//         calibración se le vuelve a aplicar encima al leerlo: reparar el doc
//         no alcanza, hay que dejar de APLICARLA.
//         Y la razón que FASE NL escribió para saltarlas era FALSA ("no hay
//         evidencia en el archivo con la que juzgarla sola"): el NAV REAL del
//         broker es esa evidencia y también es por cuenta, así que compararlos
//         NO mezcla universos. `contradictedAccountCalibrations`
//         (lib/snapshotBackfill.js) la deja de aplicar con la misma banda (8%,
//         piso de $50) y el mismo arrastre acotado (el 1 de enero es feriado y
//         su NAV es el cierre del 31); una cuenta MANUAL no se juzga, porque
//         ahí no existe una segunda medición. El filtro va en la ÚNICA
//         definición de `accountCalibrations`, no en este memo: la misma
//         calibración se aplica en cuatro sitios y filtrar en uno solo dejaría
//         a los otros tres midiendo contra un arranque ya declarado falso.
//         El doc NO se borra: se deja de aplicar y la tarjeta lo DICE, así que
//         re-calibrar sigue siendo un camino abierto.
//         El candado de 3.94% pasa sin tocarse: VITALI no es una cuenta de
//         broker y no tiene calibraciones por cuenta.
//       · FASE NP (misma autorización, TERCER reporte del mismo número): el
//         ancla despejada volvió a dar 9,305.22 al centavo con NL y NN ya
//         desplegadas, y la gráfica la dibuja como punto del 1 de enero. Eso
//         descarta la especie POR CUENTA (el chart no sintetiza anclas 'ytd',
//         ver CHART_ONLY_KINDS) y deja una sola lectura posible: es un doc
//         GLOBAL calibrado en el archivo, o sea el territorio de FASE NL. Por
//         qué NL no lo tocó: exige `c.composed`, y `composeDailyTotals` SALTA
//         todo día sin NAV arrastrable del broker (invariante 2 de la serie
//         histórica). Sin entrada para ese día no hay con qué juzgarlo NI con
//         qué reescribirlo, así que las tres vías de reparación son inertes.
//         `selfContradictedCalibrationDates` (lib/snapshotBackfill.js) juzga
//         con la evidencia que sí estaba: el día de al lado. Se PROYECTA el
//         vecino hasta la fecha del ancla aplicándole los flujos que hubo en
//         medio; con vecinos de los dos lados que coinciden entre sí y un ancla
//         que dice otra cosa, es un pico aislado y ningún movimiento de mercado
//         lo produce (la firma de round-trip de FASE FR). Si los vecinos NO
//         coinciden, pasó un cambio real de nivel y no hay veredicto.
//         La mitad que importa para este memo: además de repararse, el ancla
//         contradicha se filtra de `snapshots` en la única definición, así que
//         `findYearStartAnchor` cae al siguiente doc real. Reparar no alcanzaba
//         cuando la reparación no puede correr, que es la lección de NN una
//         capa más arriba. Solo se filtra una calibración GLOBAL, jamás una
//         observación real, y la tarjeta lo DICE (ytdAnchorIgnored).
//         CONFIRMADO EN PRODUCCIÓN: el año pasó de -$3,282.11 (-24.77%) a
//         +$491.27 sobre $9,989.06, el ancla cayó al día vecino (5,552.79
//         despejado) y el rótulo de caída máxima pasó del acantilado
//         fabricado "-40.3% (1 ene → 2 ene)" a "-8.6% (2 mar → 27 mar)", que
//         es mercado real. Y el aviso ámbar estaba visible, o sea lo que
//         arregló el número fue el filtro de LECTURA y no la reescritura del
//         doc: sin esa mitad no habría cambiado nada.
//
...
//       · FASE OK (extensión, aprobada explícitamente por el usuario el 7 sep
//         2026; era el pendiente escrito en FASE OB, "vender 'queda en el
//         portafolio' borra toda la historia del activo en la Hoja"). Vender
//         un ítem estático con destino "Queda en el portafolio"
//         (`_destinationItemId`) no crea ninguna WITHDRAWAL para el ítem
//         VENDIDO — solo la crea el destino "Sale del portafolio"
//         (`lib/saleTx.js`). Sin ese evento, `applyStaticHistory` reconstruye
//         desde `curVal` (0, porque vender pone currentPrice/purchasePrice en
//         cero) sin nada que deshacer: TODA la historia del activo, incluidos
//         los meses en que valía dinero real, colapsaba a $0.00. Reproducido
//         con las funciones reales: un bono de $6,000 vendido con destino
//         "queda en el portafolio" mostraba $0.00 en cada mes de su historia.
//         El arreglo espeja lo que YA pasaba (por accidente) con destino
//         "Sale del portafolio": ahí la WITHDRAWAL compañera SÍ empuja un
//         evento -monto para el ítem vendido (ancla el valor pre-venta al
//         PRECIO DE VENTA, no al de compra — la MISMA convención "hold-flat +
//         deshacer" que ya rige para una cuenta VACIADA, FASE LW/MB). Ahora
//         toda venta con destino declarado también empuja ese mismo evento
//         para el ítem VENDIDO, en la misma fecha y moneda que el que ya
//         recibe el destino. Los dos destinos son MUTUAMENTE EXCLUYENTES por
//         construcción (uno trae `_destinationItemId`, el otro la WITHDRAWAL
//         compañera, nunca los dos), así que la rama nueva no puede duplicar
//         el caso "Sale del portafolio" (fijado en test). Verificado además
//         que "dos posiciones del mismo símbolo sin lote con dueño se
//         reparten mal" (la segunda mitad de ese mismo hallazgo de FASE OB) ya
//         estaba resuelto por el guard `sharedSymbol` (symItemCount > 1 →
//         held-flat con la cantidad PROPIA de cada ítem, ignorando lotes
//         compartidos): reproducido con dos posiciones de BTC y lotes legacy
//         sin `itemId`/`institution`, cada una reconstruye su propio valor sin
//         mezclarse. Cero contacto con la fórmula (numerador, denominador,
//         comisión de entrada): cambia solo QUÉ movimiento entra a la
//         reconstrucción del valor. El candado de 3.94% pasa sin tocarse:
//         VITALI no se vende en ese test.
//
