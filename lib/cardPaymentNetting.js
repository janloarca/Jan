...
// estado no vuelva a emparejar una fila que ya dejo de contar.
...
export function planCardPaymentNetting(bankRows, recorded, { windowDays = 5 } = {}) {
  ...
  return {
    ...
    pairs,
    rowIndexes,
    demotions: pairs.filter((p) => p.match?.id).map((p) => ({ id: p.match.id, updates: transferDemotion('INCOME') })),
  }
}
...
export function bankPaymentCandidates(recorded) {
  return (recorded || []).filter(
    (tx) => tx?.type === 'EXPENSE' && tx?.source === 'bi_import' && !tx?._nettedTransfer
  )
}
...
export function planStatementPaymentNetting(cardRows, recorded, { windowDays = 5 } = {}) {
  ...
  return {
    ...
    pairs,
    rowIndexes,
    demotions: pairs.filter((p) => p.match?.id).map((p) => ({ id: p.match.id, updates: transferDemotion('EXPENSE') })),
  }
}
