...
export function buildCashFlows(transactions, amountToBase, { rewindableSymbols, cashAccountId } = {}) {
  ...
  for (const tx of transactions || []) {
    ...
    // sin él, la convención por tipo.
      ...
      //
      // ⛔ FASE MZ (corrección de una regresión de MY): el guard NO puede ser
      // "trae el campo", tiene que ser "lo pagó OTRA cuenta". Si la cuenta que
      // pagó ES esta misma caja, el broker sí movió su efectivo y saltarlo lo
      // PIERDE: `historyPayload` usa esta lista para la caja del broker EN VEZ
      // de `balanceEventsById` (es un o-lo-uno-o-lo-otro), así que el evento
      // que MX sí registra del otro lado nunca lo recoge nadie. Reproducido:
      // el payload de la caja salía con el depósito y sin el gasto.
      //
      // Sin `cashAccountId` (callers que no lo pasan) se conserva el
      // comportamiento de MY, que es el correcto para todo lo que no es la
      // caja de este broker.
      if (tx._paidFromItemId && tx._paidFromItemId !== cashAccountId) continue
