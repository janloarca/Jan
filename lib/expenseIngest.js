...
import { parseAmount, detectCurrencyMarker, resolveCurrency } from './numberParse'
import { chargeDayISO, payerOffsetFromTimestamp } from './localDate'
...
export function normalizeExpenseInput(raw = {}, { offsetMinutes = null } = {}) {
  ...
  // El día del cobro, visto desde donde estaba quien pagó.
  //
  // ⛔ Este campo es el que decide en qué MES cae el gasto, así que un día de
  // error acá mueve dinero entre meses: cambia el total, el desglose por
  // categoría y la tasa de ahorro de DOS meses a la vez. Un instante Zulu no
  // dice de qué día es (`2026-09-01T02:05Z` son las 8pm del 31 de agosto en
  // Guatemala), y tomar sus diez primeros caracteres archivaba esa compra en
  // septiembre. La regla vive en `lib/localDate.js`.
  //
  // El desfase del pagador, en orden: el que el caller conozca (el correo lo
  // saca de la cabecera Date de su propio mensaje), y si no, el que la marca de
  // tiempo declare. Un atajo configurado como pide la documentación
  // (`yyyy-MM-dd'T'HH:mm:ssZ`, que en iOS es el desfase numérico) lo trae
  // escrito, así que para ese camino esto sale gratis. Sin ninguno de los dos
  // el comportamiento queda idéntico al de antes.
  const offset = (offsetMinutes != null && isFinite(offsetMinutes))
    ? Number(offsetMinutes)
    : payerOffsetFromTimestamp(raw.occurredAt || raw.time || raw.receivedAt)

  const date = chargeDayISO(raw.date || raw.occurredAt, offset) || chargeDayISO(new Date(), offset)
  ...
  //
  // El tope también es del PAGADOR y no de UTC, y con un desfase al ESTE eso
  // deja de ser un detalle: en Tokio las nueve de la mañana del 1 de septiembre
  // todavía son 31 de agosto en UTC, así que un tope en UTC arrastraría una
  // fecha local correcta un día hacia atrás.
  const today = chargeDayISO(new Date(), offset)
