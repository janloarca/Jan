...
//
// Solo el instante REPORTADO, nunca el de llegada, y esa distinción es lo que
// mantiene viva la idempotencia: un instante reportado viaja DENTRO del cuerpo,
// así que un reintento del mismo evento manda el mismo valor y da el mismo id;
// la hora de llegada cambia en cada intento, y meterla acá haría que un
// reintento de red escribiera un segundo gasto. Sin hora reportada, la llave
// queda byte-idéntica a la de siempre.
export function expenseDocId(input) {
  const reported = input.timeSource === 'reported' && input.occurredAt ? `|${input.occurredAt}` : ''
