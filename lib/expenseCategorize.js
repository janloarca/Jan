...
export const MERCHANT_RULES = [
  ...
  // ⛔ NO agregar una regla para las dos categorías de transferencia, por más
  // que un comercio diga "TRANSFERENCIA", "zelle", "paypal" o "remesa".
  //
  // `lib/financeCategories.js` deja sus arreglos de palabras clave VACÍOS a
  // propósito y explica por qué: a esas categorías se llega por EVIDENCIA (el
  // neteo empareja el débito del banco con el pago de la tarjeta al centavo) o
  // porque el usuario las elige a mano sabiendo que esa fila movió dinero
  // entre cuentas SUYAS. Desde FASE KV no cuentan en ningún total.
  //
  // Esta tabla SÍ tenía la regla, y es la que corre en el import de banco, el
  // de tarjeta, la captura automática y Reclasificar. "TRANSFERENCIA A JUAN
  // PEREZ" es dinero enviado a OTRA persona, o sea un gasto real: clasificarlo
  // acá lo sacaba del mes, del desglose, de la tasa de ahorro y de la
  // proyección, en silencio. Sin regla cae a 'Otros Gastos', que cuenta.
