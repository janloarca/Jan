...
// La última es un MONTO FIJO CONTRACTUAL ("Monto fijo por pago", "Monto por
...
export function monthlyIncomeAmount({
  ...
  // ⛔ FASE OM. Nombrado `accrualBalanceAsOf` y no `balanceAsOf` a propósito: el
  // guardián de las cuatro puertas (`lib/__tests__/balanceAsOfLocal.test.js`)
  // barre `hooks/` buscando quién SELLA ese campo, y un caller que solo lo
  // REENVÍA como parámetro de una función pura se ve textualmente idéntico a un
  // sello (`balanceAsOf: <valor>`). Este nombre no colisiona con esa firma, así
  // que el barrido no tiene nada que confundir.
  accrualBalanceAsOf,
}, payMonthsCount = 12) {
  ...
  // reparto de siempre.
  //
  // ⛔ FASE OM. `accrualBalanceAsOf` es OPCIONAL y solo tiene sentido cuando el
  // ingreso se REINVIERTE en la propia cuenta: ahí el devengo del mes vuelve a
  // sumarse sobre el MISMO saldo que la foto acaba de sellar, así que si la
  // foto cae a mitad del mes hay que devengar solo los días de después (ver
  // `accrualDaysInMonth`). El caller decide si aplica; no se infiere acá.
  if (accrual === ACCRUAL_DAILY && payDate) {
    ...
    if (rate > 0 && Number.isFinite(y) && Number.isFinite(m)) {
      return monthlyAccrual({ balance, annualRatePct: rate, year: y, monthIndex: m, acquisitionDay, balanceAsOf: accrualBalanceAsOf })
    }
  }
