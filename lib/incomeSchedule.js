...
export function estimateIncomeAmount({ balance, incomeMode, incomeRate, incomeAmount, rateType, rateMin, rateMax, isPerShare, qty, accrual, acquisitionDay, payDate }, payMonthsCount = 12) {
  const divisor = payMonthsCount || 12
  // FASE KT. Devengo diario: el monto de un mes depende de sus DIAS reales, no
  // de un doceavo parejo, asi que la estimacion se hace por fecha de pago. Sin
  // `payDate` (callers viejos) se cae al reparto de siempre.
