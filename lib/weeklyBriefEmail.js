 ...
 */
export function buildWeeklyBriefEmail({ weekLabel, portfolio, market, hasAttachment = false, showNetWorth = false }) {
  const cur = portfolio?.currency || 'USD'
  const sections = []

  // ── Tu semana ──
  // El patrimonio es la cifra que alguien abre el correo a ver, pero también
  // es la que un teléfono muestra en la lista del buzón sin desbloquearse: por
  // default no viaja, y la cifra principal pasa a ser la variación (FASE ON).
  const hero = portfolioHero({
    showTotal: showNetWorth,
    total: { label: 'Net worth', value: fmtMoney(portfolio?.netWorth, cur) },
    change: {
      label: 'This week',
      combo: (portfolio?.weekPct != null || portfolio?.weekAbs != null)
        ? combo(portfolio.weekAbs, portfolio.weekPct, cur) : null,
      abs: portfolio?.weekAbs != null && isFinite(portfolio.weekAbs) ? fmtSigned(portfolio.weekAbs, cur) : null,
      pct: fmtPct(portfolio?.weekPct),
      suffix: 'this week',
    },
  ...
  if (movers.length > 0) {
    // A lo ANCHO: es una lista, no una cifra. En la columna de números, con
    // nowrap, se salía de la tarjeta en el teléfono (captura del usuario).
    rows.push({
      label: 'Movers this week', wide: true,
      value: movers.map((m) => `${m.symbol} ${fmtPct(m.pct)}`).join('   ·   '),
    })
  }
  ...
  // Una ausencia deliberada se DICE: sin esta línea, un correo que dejó de
  // traer el patrimonio se lee como un correo roto.
  if (!showNetWorth) paragraphs.push(HIDDEN_TOTAL_NOTE)

  sections.push({ hero, heading: 'Your week', rows, paragraphs, cta: { label: 'Open Chispudo', url: `${APP_URL}/dashboard` } })
  ...
  const { html, text } = renderEmail({
    ...
    preheader: 'Your week in Chispudo: how the portfolio moved, the income you collected, and where the market closed. Open to see the numbers.',
    sections,
