...
export function buildWeeklyBriefEmail({ weekLabel, portfolio, market, hasAttachment = false }) {
  ...
  // ── Tu semana ──
  // El patrimonio es la cifra que alguien abre el correo a ver: va como número
  // principal con su variación al lado, no como una fila más de la tabla.
  const hero = {
    label: 'Net worth',
    value: fmtMoney(portfolio?.netWorth, cur),
    delta: (portfolio?.weekPct != null || portfolio?.weekAbs != null)
      ? `${combo(portfolio.weekAbs, portfolio.weekPct, cur)} this week`
      : null,
