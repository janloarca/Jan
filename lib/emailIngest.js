...
import { resolveIngestToken, readUserRules, readUserBaseCurrency, stampIngestResult, TOKEN_RE } from './ingestTokens'
...
function imapConfig() {
  ...
  return {
    ...
    // Sin esto, el default de imapflow es NOVENTA segundos, o sea más que la
    // función entera del cron (60s en el plan actual). Un host que traga
    // paquetes en vez de rechazarlos deja colgado el `connect()` hasta ahí, y
    // un `try/catch` atrapa excepciones, no el reloj: el barrido se comía el
    // presupuesto y NINGÚN correo salía, que es exactamente lo contrario de lo
    // que promete el comentario del caller.
...
export async function ingestParsedEmail({ db, parsed, headers }) {
  ...
  // Se deja constancia del resultado, igual que la ruta del atajo.
  //
  // Este camino resolvía el token (lo que estampa `lastUsedAt`) y NUNCA
  // estampaba cómo terminó, así que "el reenvío no está llegando" y "llega y lo
  // descartamos" se veían idénticos desde Ajustes — que es justo la pregunta de
  // quien acaba de configurar la regla de reenvío.
  if (skip) {
    await stampIngestResult(db, token, skip, 'email')
    return { status: 'skipped', reason: skip }
  }
  if (input.error) {
    await stampIngestResult(db, token, input.error, 'email')
    return { status: 'skipped', reason: input.error }
  }
  ...
  await stampIngestResult(db, token, result.status, 'email')
...
// `budgetMs`: cuánto tiempo puede consumir el barrido antes de soltar el resto
// para mañana. NO es una optimización: el barrido comparte función serverless
// con el envío de los correos y corre ANTES (a propósito: un día sin ninguna
// cadencia pendiente retorna temprano y el barrido no depende de eso). Sin
// tope, un backlog de cien mensajes con siete viajes a Firestore cada uno se
// come el presupuesto entero y los correos no salen.
//
// La asimetría que decide quién cede: un correo perdido NO tiene ventana de
// recuperación (el mensual solo se evalúa el día 1), y un mensaje sin barrer
// tiene una mañana. Cortar el barrido es seguro por construcción: lo que no se
// alcanzó queda sin leer y el siguiente lo toma, y un mensaje interrumpido
// DESPUÉS de escribir su gasto y ANTES de marcarse leído se reprocesa contra el
// mismo id determinístico, o sea es un no-op.
export async function sweepInbox({ db, limit = 50, budgetMs = 25000 } = {}) {
  ...
  const summary = { scanned: 0, ours: 0, created: 0, duplicates: 0, skipped: 0, unmatched: 0, failed: 0, deferred: 0 }
  ...
  try {
    ...
    for (const uid of batch) {
      if (Date.now() > deadline) { summary.deferred = batch.length - summary.scanned; break }
      summary.scanned++
      try {
