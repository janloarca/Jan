...
//      alerts to gastos+<ingest token>@ourdomain.
...
// Sweeps the shared mailbox. Only unseen messages are considered, and every
// message we look at is marked \Seen afterwards — including the ones we could not
// use — so a malformed email can never wedge the sweep into reprocessing forever.
export async function sweepInbox({ db, limit = 50 } = {}) {
  ...
  const summary = { scanned: 0, created: 0, duplicates: 0, skipped: 0, unmatched: 0, failed: 0 }
