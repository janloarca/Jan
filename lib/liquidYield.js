...
export function knownContributions({ item, items, transactions, convert, asOfTs } = {}) {
  ...
  for (const tx of transactions || []) {
    ...
    if (ty === 'DEPOSIT' && mine) out.push({ ts, amount, kind: 'deposit', ...meta })
      ...
      //
      // La excepción, y es la que impide contar lo mismo dos veces: un
      // rendimiento que el USUARIO ya declaró al corregir el saldo en la Hoja
      // (`manual_yield`) ya está explicado, así que tiene que salir del
      // residuo. Sin esto el motor volvería a proponer los mismos intereses que
      // acaba de registrar, y aceptarlos los duplicaría. Es distinto de
      // `inferred_yield`, que se excluye arriba a propósito porque una corrida
      // nueva REEMPLAZA a la anterior (supersededYieldTxIds).
      if (mine && tx._source === 'manual_yield') {
        out.push({ ts, amount, kind: 'income', ...meta })
        continue
      }
      if (mine) continue
...
// Los tres estados que liquidYieldCandidates trata como "hay un residuo que
// vale la pena preguntar". Compartida para que ese filtro y el sello
// preventivo de una corrección de la Hoja (dismissalFor, abajo) no puedan
// divergir sobre qué cuenta como "candidato".
export const CANDIDATE_STATUSES = ['ok', 'implausible-rate', 'negative-residual']

// La marca de descarte que le correspondería a un estado HIPOTÉTICO (el que
// un patch está por escribir), con las MISMAS funciones puras que
// liquidYieldCandidates usa para decidir si vale la pena preguntar. Null si
// ese estado no produciría ningún candidato. La usa la Hoja para pre-sellar
// item._liquidYield cuando una respuesta ANSWER_CORRECTION ya contestó
// "esto no fue rendimiento" por otra vía: el residuo exacto que la
// corrección deja nunca vuelve a preguntarse, sin escribir ninguna
// transacción ni tocar knownContributions. Si el saldo, su fecha o los
// aportes vuelven a cambiar después, la firma cambia y la pregunta
// reaparece — igual que tras tocar "No fue rendimiento" a mano.
export function dismissalFor({ item, items, transactions, convert, asOfTs, finalBalance, declaredRatePct } = {}) {
  const contributions = knownContributions({ item, items, transactions, convert, asOfTs })
  if (contributions.length === 0) return null
  const res = computeLiquidYield({ contributions, finalBalance, asOfTs, declaredRatePct })
  if (!CANDIDATE_STATUSES.includes(res.status)) return null
  const signature = yieldSignature({ asOfTs, finalBalance, contributions })
  return { signature, dismissed: true }
}
