...
export function knownContributions({ item, items, transactions, convert, asOfTs } = {}) {
  ...
  for (const tx of transactions || []) {
    ...
    if (ty === 'DEPOSIT' && mine) out.push({ ts, amount, kind: 'deposit', txId: tx.id })
    else if (ty === 'WITHDRAWAL' && mine) out.push({ ts, amount: -amount, kind: 'withdrawal', txId: tx.id })
    else if (ty === 'DIVIDEND') {
      ...
      if (dest && dest.id === item.id) out.push({ ts, amount, kind: 'income', txId: tx.id })
    } else if (ty === 'SELL' && tx._destinationItemId === item.id) {
      out.push({ ts, amount, kind: 'sale', txId: tx.id })
    } else if (ty === 'TRANSFER') {
      if (tx._linkedItemId === item.id) out.push({ ts, amount, kind: 'transfer-in', txId: tx.id })
      else if (tx._originItemId === item.id) out.push({ ts, amount: -amount, kind: 'transfer-out', txId: tx.id })
    }
  }
...
export function supersededYieldTxIds(transactions, itemId, asOfTs) {
  if (!itemId) return []
  return (transactions || []).filter((tx) => {
    ...
    if (tx._reinvested !== true) return false
    const ts = toTs(tx.date)
