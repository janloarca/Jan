...
// La regla, en UN solo lugar: exactamente UN separador, y de un solo tipo, es
// decimal, sin heurística. Con dos o más ("1.234.567") vuelve a ser agrupación,
// que ahí no es ambiguo, así que se delega en `parseAmount`, que además resuelve
// símbolos de moneda, paréntesis contables y sufijos K/M/B.
//
// Vive aparte porque DOS funciones la necesitan y difieren solo en el signo:
// `parseQuantity` pisa los negativos en cero y `parseRate` los conserva. Con la
// regla copiada en las dos, la primera corrección que se le haga a una deja a la
// otra atrás, que es exactamente lo que este archivo ya documenta para la lista
// de códigos ISO.
function loneSeparatorIsDecimal(val) {
  const s = String(val).trim()
  const dots = (s.match(/\./g) || []).length
  const commas = (s.match(/,/g) || []).length
  // FASE OB. Se conserva la `e` de una notación científica: "1.5e-7" es lo
  // que `Number.toString()` devuelve para un polvo de cripto, y sin esto el
  // separador solitario se llevaba el exponente y la lectura daba 1.5.
  return dots + commas === 1
    ? parseFloat(s.replace(',', '.').replace(/[^\d.\-eE]/g, ''))
    : parseAmount(s)
}
...
export function parseQuantity(val) {
  ...
  // sin que nada lo señale, así que el piso es cero.
  const num = loneSeparatorIsDecimal(val)
  return isFinite(num) && num > 0 ? num : 0
}

// Una TASA no es dinero NI una cantidad, y por eso necesita su propia lectura.
//
// Contra `parseAmount`: la convención LatAm de miles es correcta para dinero y
// desastrosa para un porcentaje. `parseAmount('7.500')` son siete mil quinientos,
// y como tasa eso es 7500% en vez de 7.5%.
//
// Contra `parseQuantity`: esa función PISA EN CERO todo negativo, a propósito,
// porque una tenencia negativa no existe. Un rendimiento negativo sí existe: es
// como se teclea un año perdedor. Usarla en `CalibrateReturnModal`, que es la
// pantalla que ANCLA los retornos de toda la app, convertiría un −12% en 0% sin
// que nada lo señale.
//
// Así que: misma regla de separador solitario, signo conservado, y sin piso.
// Un valor ilegible devuelve 0, igual que sus dos hermanas; el caller que
// necesite distinguir "vacío" de "cero" tiene que conservar su propio guard
// (`CalibrateReturnModal` lo hace: '' significa "no calibrar este período", que
// es muy distinto de 0%).
