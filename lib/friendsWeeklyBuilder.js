...
import { withMovement, leaderStreak, weekKeyFor } from '@/lib/friendsHistory'
...
export async function buildFriendsWeeklyForUser({ db, uid, prefs = {}, now = new Date() } = {}) {
  ...
  for (const gd of snap.docs.slice(0, MAX_GROUPS)) {
    ...
    // ⛔ La racha incluye la semana EN CURSO, o sea la tabla que va justo al
    // lado. `readRecentHistory` excluye a propósito la semana de hoy (para que
    // el movimiento no dependa del orden de las dos pasadas del cron), así que
    // pasarle solo la historia hacía que la frase describiera al líder de la
    // semana PASADA: con un cambio de líder, el correo ponía a Ana arriba de la
    // tabla y debajo escribía "Beto has led for 3 weeks running", y sin cambio
    // de líder contaba una semana de menos.
    groups.push({
      name: g.name || 'Group', ...standings, rows,
      streak: leaderStreak([{ weekKey: weekKeyFor(now), rows }, ...history]),
    })
  }
