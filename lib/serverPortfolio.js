...
export function enrichItemsServerSide(items, prices = {}, convert, baseCurrency = 'USD') {
  return (items || []).map((it) => {
    ...
    if (priceData && isMarketPriced(it)) {
      ...
      // FASE KN: paridad con el cliente (hooks/useMarketPrices.js). Qué ventana
      // mide ese change1d y de cuándo es; sin esto el servidor no puede saber
      // que un "movimiento de hoy" es en realidad el de la última sesión.
...
export function trailingDividends(transactions, { convert, baseCurrency = 'USD', now = new Date(), items = null } = {}) {
  ...
  // FASE JW: la MISMA regla que periodIncome, unas líneas más abajo en este
...
// FASE KK: el paso POR ÍTEM, extraído para que el link compartido pueda listar
// las fuentes de ingreso una por una sin re-derivar la regla de moneda (que es
// donde está toda la sutileza: el saldo se toma en su denominación ORIGINAL
// cuando existe, y solo entonces se convierte). El total de abajo es la suma de
// estas partes por construcción, así que la tarjeta y su encabezado no pueden
// discrepar. Devuelve 0 sin el helper o sin ingreso proyectado.
export function itemAnnualIncomeInBase(it, { convert, baseCurrency = 'USD', projectItemAnnualIncome } = {}) {
  if (!projectItemAnnualIncome || !it) return 0
  ...
  return convert ? convert(annual, cur, baseCurrency) : annual
}
...
export function projectedAnnualIncome(items, { convert, baseCurrency = 'USD', projectItemAnnualIncome } = {}) {
  ...
  for (const it of items || []) {
    total += itemAnnualIncomeInBase(it, { convert, baseCurrency, projectItemAnnualIncome })
  }
  return total
}
