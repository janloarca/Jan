...
import { snapshotAssetsUSD, assetOnlyFlows, debtItemIds } from './assetReturns'
...
function snapValueUSD(s) {
  // FASE LV: el valor SOLO-ACTIVOS del doc (lib/assetReturns.js), la misma
  // decisión de FASE LU para el tablero: el rendimiento mide tus inversiones.
  // En docs sin noción de deuda es la lectura de siempre, byte-idéntica.
...
export function buildReportData({
  ...
  // FASE LV: el RETORNO del reporte mide activos, igual que el tablero (FASE
  // LU). El valor final de todo Dietz es el total de ACTIVOS (sin el
  // parámetro, netWorth: en un portafolio sin deuda son el mismo número), y la
  // lista de flujos pasa por assetOnlyFlows: lo vinculado a una deuda no entra
  // (el DEPOSIT envenenado de una deuda vieja contaba como depósito del
  // período) y un pago de deuda desde una cuenta entra como el retiro que es.
  // El patrimonio NETO sigue siendo la cifra de valor del reporte, y la
  // sección de deudas sigue existiendo: cambia qué mide el RENDIMIENTO.
  const endVal = totalAssets != null && isFinite(totalAssets) && totalAssets > 0 ? totalAssets : netWorth
  const assetTxs = assetOnlyFlows(txs, debtItemIds(list))
  ...
  if (range.kind === 'ytd') {
    if (returnYTD != null) periodReturn = { pct: returnYTD, abs: ytdChange, fromTs: startTs, source: 'header' }
      ...
      const d = periodDietz({ series: seriesAll, startTs, endTs: nowTs, endValue: endVal, transactions: assetTxs, convert, baseCurrency })
    ...
    if (returnSinceStart != null) {
      ...
      const d = periodDietz({ series: seriesAll, startTs: seriesAll[0].ts, endTs: nowTs, endValue: endVal, transactions: assetTxs, convert, baseCurrency })
      if (d) periodReturn = { ...d, source: 'dietz' }
    }
  } else {
    const d = periodDietz({ series: seriesAll, startTs, endTs: nowTs, endValue: endVal, transactions: assetTxs, convert, baseCurrency })
  ...
  if (series.length > 0) {
    ...
    // FASE LV: la serie es de activos, así que su punto final también (con la
    // deuda adentro, el "cambio de valor" cerraba contra otro universo).
    valueChange = { first: first.value, firstTs: first.ts, last: endVal, lastTs: nowTs, abs: endVal - first.value }
  }
  ...
  const rollingDietz = (months) => periodDietz({
    ...
    endValue: endVal, transactions: assetTxs, convert, baseCurrency,
  })
  const ytdFallback = returnYTD == null
    ? periodDietz({ series: seriesAll, startTs: periodRange('ytd', now).startTs, endTs: nowTs, endValue: endVal, transactions: assetTxs, convert, baseCurrency })
    : null
  const performance = [
    ...
    { key: 'all', ...(returnSinceStart != null
      ...
      : (seriesAll.length > 0
        ? (periodDietz({ series: seriesAll, startTs: seriesAll[0].ts, endTs: nowTs, endValue: endVal, transactions: assetTxs, convert, baseCurrency }) || { pct: null })
        : { pct: null })) },
  ]
  ...
  if (seriesAll.length > 0) {
    ...
    for (let year = firstYear; year <= nowYear; year++) {
      ...
      const g = calendarYearGain({ series: seriesAll, year, transactions: assetTxs, convert, baseCurrency })
