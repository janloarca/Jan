...
export function periodRange(kind, now = new Date()) {
  ...
  // acaba de cerrar, no un lunes-a-domingo parcial.
...
export function buildReportData({
  ...
  const performance = [
    ...
    { key: 'ytd', pct: returnYTD, abs: ytdChange, fromTs: periodRange('ytd', now).startTs },
    { key: '1y', ...(rollingDietz(12) || { pct: null }) },
    { key: 'all', pct: returnSinceStart, abs: null, fromTs: sinceStartDate ? (Date.parse(sinceStartDate) || null) : null },
  ]
  ...
  if (seriesAll.length > 0) {
    const firstYear = new Date(seriesAll[0].ts).getFullYear()
    ...
    for (let year = firstYear; year <= nowYear; year++) {
      ...
      for (const p of seriesAll) {
        if (p.ts <= yStart && p.ts >= yStart - ANCHOR_BEFORE_MS) startAnchor = p
        if (p.ts > yStart) break
      }
      if (!startAnchor) startAnchor = seriesAll.find((p) => p.ts >= yStart && p.ts <= yStart + ANCHOR_AFTER_MS) || null
      let endAnchor = null
      for (const p of seriesAll) {
        if (p.ts < yEnd && p.ts >= yEnd - ANCHOR_BEFORE_MS) endAnchor = p
        if (p.ts >= yEnd) break
      }
      if (!startAnchor || !endAnchor || !(startAnchor.value > 0) || endAnchor.ts <= startAnchor.ts) continue
      const { pct, abs } = computeModifiedDietz({
        startValue: startAnchor.value, endValue: endAnchor.value,
        startTs: startAnchor.ts + 1, endTs: endAnchor.ts,
        transactions: txs, convert, baseCurrency,
      })
      if (isFinite(pct)) calendarYears.push({ year, pct, abs, partial: false })
    }
  }
