...
// 'bi_import' es el estado de cuenta de BANCO (lib/parsers/biParser.js), no de
// tarjeta. Faltaba, así que corregir una de esas filas no enseñaba nada: la
// descripción la escribió el banco igual que en un estado de tarjeta, y es
// exactamente el caso donde aprender el comercio sirve.
const MACHINE_SOURCES = new Set(['card_import', 'bi_import'])

export function isMachineDescribed(tx) {
  if (MACHINE_SOURCES.has(String(tx?.source || ''))) return true
