...
//
// `blockedByScroller`: el dedo empezó DENTRO de un contenedor con scroll propio
// que todavía puede subir (su `scrollTop > 0`). Ese tirón le pertenece a él, no
// a nosotros. Sin esta condición, con el documento arriba pero una lista
// interna a media altura, jalar hacia abajo dispara un refresco en vez de
// scrollear la lista: pasa hoy en `PortfolioGrowthChart` y `PortfolioMap`, y
// sobre todo en `FinanceTransactionList`, que ES el contenido de Finanzas.
// Si esa lista ya está en su propio tope, el gesto pasa de largo hacia acá,
// que es justo lo que se espera.
//
// `blockedByDragHandle`: el dedo empezó sobre un elemento que declara su propio
// arrastre (`[data-ptr-ignore]`). El caso concreto es el calendario del plan de
// ingresos en Finanzas: arrastrar un cuadrito HACIA ABAJO con la página arriba
// cumple todas las condiciones de un tirón, así que sin esto el gesto se
// reclamaría, le haría preventDefault al arrastre y en vez de mover el ingreso
// a otro mes dispararía un refresco. Se resuelve igual que el scroller
// interno: subiendo el DOM desde el `target` del touchstart.
export function shouldClaim({
  dx = 0, dy = 0, atTop = false, insideMain = true, blockedByScroller = false,
  blockedByDragHandle = false,
} = {}) {
  if (!atTop || !insideMain || blockedByScroller || blockedByDragHandle) return false
