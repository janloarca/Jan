...
export function parseIBKRXmlFile(xmlText) {
  ...
  // Dedupe transactions: a Flex report can repeat a row across pages, and the
  // deterministic doc id in bulkImport is the last line of defense. Prefer the
  // broker id; fall back to the same composite the doc id is built from.
  const seenTx = new Set()
  const dedupedTransactions = transactions.filter((tx) => {
    const amt = Math.round((tx.totalAmount || 0) * 100)
    const key = tx._ibkrTxnId
      ? `id:${tx._ibkrTxnId}`
      : `${tx.date}-${(tx.symbol || '').toUpperCase()}-${tx.type}-${amt}`
