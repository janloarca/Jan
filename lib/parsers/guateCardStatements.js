...
//   3. A payment TO the card enters as INCOME ('Salario'), and its reversal as
//      negative income. Decisión del usuario (21 ago 2026): contablemente es
//      una transferencia entre cuentas propias, pero para quien solo importa
//      tarjetas el sueldo vive en el banco y el mes se lee como pura pérdida.
//      La fila conserva `kind:'payment'` justamente para que un import futuro
//      del estado bancario pueda netearla en vez de contar el mismo dinero dos
//      veces. `excluded` se queda como bucket (ya nada cae ahí hoy) porque es
//      parte del contrato con el importador y con reconcileStatement.
...
// Un pago A la tarjeta.
//
// ⛔ Decisión del usuario (21 ago 2026): estos entran como INGRESO, categoría
// Salario. Antes se excluían por ser una transferencia entre cuentas propias,
// que es lo contablemente correcto, pero deja el mes leyéndose como pura
// pérdida para quien solo importa tarjetas: el sueldo vive en el banco y nunca
// aparece.
//
// Lo que hace segura la decisión: la fila CONSERVA su `kind: 'payment'` aunque
// sea ingreso. Ese dato es lo que permite que el día que se importe el estado
// de la cuenta bancaria, el cruce reconozca la salida del banco como el mismo
// dinero ya registrado desde la tarjeta en vez de sumarlo dos veces (el sueldo
// contado una vez al salir del banco y otra al llegar a la tarjeta).
const PAYMENT_RE = /GRACIAS POR SU PAGO|PAGO POR INTERNET|PAGO RECIBIDO|PAGO A CAPITAL|PAGO A INTERESES|PAGO EN AGENCIA|PAGO BANCA|PAGO DE TARJETA|PAGO EN LINEA|PAGO EN LÍNEA|PAGO DE DEUDA|PAGO ACH|PAGO MOVIL|PAGO MÓVIL|^ABONO\b|^DEPOSITO\b|^DEPÓSITO\b|TRANSFERENCIA RECIBIDA/i
// La CORRECCIÓN de un pago lo revierte (Q1,731.26 en un estado real). Con el
// pago como ingreso, su corrección tiene que ser un ingreso NEGATIVO: si se
// quedara excluida, el ingreso revertido seguiría contando.
const PAYMENT_ADJUSTMENT_RE = /CORRECCION A PAGO|CORRECCIÓN A PAGO/i
// Promociones de la tarjeta: cashback, reintegros, bonificaciones. Dinero nuevo
// del banco, no un pago y no la devolución de una compra.
//
// `DEVOLUCION` a secas queda FUERA a propósito: una devolución de un comercio
// es un reembolso, y desde el cambio anterior es un gasto NEGATIVO que se
// descuenta de su categoría. Meterla acá la volvería ingreso otra vez, que es
// justo el defecto que ese cambio eliminó. 'TE DEVUELVE' sí entra porque es la
// marca de cashback de G&T, no una devolución de comercio.
const CASHBACK_RE = /TE DEVUELVE|^CR\.?\s*\d+\s*%|CREDITO P\/CARGOS BONIFICABLES|CRÉDITO P\/CARGOS|BONIFICAB|BONIFICACION|BONIFICACIÓN|REINTEGRO|CASHBACK|CASH BACK|RECOMPENSA|PROMOCION|PROMOCIÓN|PROMO\b|PUNTOS|MILLAS/i
// El crédito por intereses bonificados es un cashback con una diferencia que
// importa: bonifica un CARGO DE INTERESES que este parser NO importa. Ese
// cargo no es una fila fechada de la tabla de movimientos: vive en el bloque
// de resumen del estado (visto en un estado real de G&T: rebate de Q92.25 en
// movimientos contra un cargo de Q125.97 solo en el resumen), y el texto
// exacto de esas líneas de resumen no está fijado por ningún estado que se
// conserve (los PDF reales no se commitean y su texto extraído se borró).
// Leer dinero de una línea nunca vista sería inventarlo, así que el cargo no
// se sintetiza: estas filas se MARCAN (parseCardStatement) y la vista previa
// del importador lo dice, para que el ingreso sin su gasto enfrente no quede
// mudo. Solo la marca "CREDITO P/CARGOS": una bonificación o promo genérica
// no promete ningún cargo escondido.
const INTEREST_REBATE_RE = /CR[EÉ]DITO P\/CARGOS/i
...
function buildTx({ date, postedDate, description, location, amount, currency, kind, wallet }) {
  // A REFUND is the reversal of a purchase, not income: getting Q500 back for
  // a returned jacket did not earn Q500, it means Q500 less was spent. So it
  // is filed as a NEGATIVE expense in the merchant's own category, which makes
  // that category report net spend instead of gross.
  //
  // This also nets a money-out-money-back round trip with no special case for
  // it: both legs classify from the same merchant name, so they land in the
  // same category and cancel. On a real statement Q7,422 went out to a betting
  // site and Q7,311 came back; as income that read as Q7,311 earned and
  // Q7,422 of entertainment, when the truth is a Q110.64 loss.
  //
  // CASHBACK stays income: it is new money from the bank, tied to no single
  // purchase, so there is no category to net it against. Va a su propia
  // categoría, 'Promoción de tarjeta', en vez de mezclarse con el ingreso
  // genérico: un reintegro del banco no es lo mismo que un sueldo.
  //
  // Un PAGO a la tarjeta entra como ingreso 'Salario', y su CORRECCIÓN como
  // ingreso NEGATIVO por la misma mecánica que el reembolso: lo que revierte
  // una entrada de dinero tiene que restar de esa entrada, no quedarse afuera.
  const isRefund = kind === 'refund'
  const isNegativeIncome = kind === 'payment-adjustment'
  const isIncome = kind === 'cashback' || kind === 'payment' || isNegativeIncome
  ...
  return {
    ...
    amount: (isRefund || isNegativeIncome) ? -amount : amount,
    currency,
...
// La FORMA del estado de Contecnica/BI, para cuando el nombre del emisor no
// se puede leer.
//
// Caso real: un estado de la MISMA tarjeta (VISA Clásica ••9856) llegó sin la
// palabra "CONTECNICA" en el texto extraído — el logo del emisor es una imagen
// en ese PDF, así que el extractor no lo ve. El detector la exigía, el estado
// caía al camino de IA, y ahí la IA busca POSICIONES de un portafolio: el
// usuario terminó viendo "La IA no encontró posiciones en el PDF" sobre un
// estado de tarjeta perfectamente legible. Verificado: con el detector
// satisfecho, el parser lee las 39 filas y cuadra al centavo por los cuatro
// lados.
//
// Tres marcas que tienen que aparecer JUNTAS. La fecha de corte con sus tres
// grupos separados (`16      08      2026`) es la más específica de las tres, y
// además es funcionalmente necesaria: sin ella el parser no tiene fecha de
// corte. Un falso positivo tampoco pasa en silencio, que es lo que vuelve
// aceptable reconocer por forma: la lectura se compara contra los totales que
// el propio estado imprime, así que un formato que no era este se delata solo.
function looksLikeContecnicaLayout(t) {
  return /MOVIMIENTOS EN QUETZALES/i.test(t)
    && /TOTAL\s+QUETZALES/i.test(t)
    && /Fecha de corte:\s+\d{1,2}\s+\d{1,2}\s+\d{4}/i.test(t)
}

export function detectCardStatement(text) {
  const t = String(text || '')
  // El nombre del emisor cuando SÍ se puede leer: es inequívoco.
  ...
  // Sin nombre de emisor, la forma. Va AL FINAL a propósito: así un estado de
  // G&T o de BAC se reconoce por su propio nombre antes de que esta regla
  // pueda mirarlo siquiera.
  if (looksLikeContecnicaLayout(t)) return 'bi'
  return null
}
...
export function parseCardStatement(text, { rules = [] } = {}) {
  ...
  // El cargo que estas filas bonifican no está en el archivo (ver
  // INTEREST_REBATE_RE): se marcan para que la vista previa lo DIGA en vez de
  // dejar el mes con un ingreso que no tiene su gasto enfrente, en silencio.
  // Va acá y no en cada parser porque la regla es la misma para los tres
  // bancos: una copia por banco es cómo una se queda atrás.
  parsed.interestRebates = parsed.transactions
    .filter((t) => t.kind === 'cashback' && INTEREST_REBATE_RE.test(t.description))
    .map((t) => ({ amount: t.amount, currency: t.currency, description: t.description }))
  return parsed
}
