...
// Every money amount on a line, with its END column (numbers are right-aligned,
// so the END is what identifies the column) and BAC's trailing minus.
// Two guards learned from real rows:
//   - "\d[\d,]*\.\d{2}" over "\d{1,3}(,...)*": an ungrouped "1234.56" must not
//     half-match as "234.56".
//   - the bare ".\d{2}" form (Contecnica prints ".00"/".06" totals) demands a
//     WHITESPACE boundary before it, or it fires inside addresses and
//     references ("2C.20-83", "PAGO RECIBIDO...406") and the row gets thrown
//     away as "too many amounts". Both misfires came from real statements.
function amountTokens(line) {
  const out = []
  const re = /(?:^|\s)(\d[\d,]*\.\d{2}|\.\d{2})(-)?/g
  let m
...
function buildTx({ date, postedDate, description, location, amount, currency, kind, wallet }) {
  ...
  // A fee is a bank charge no matter what the description echoes: "MEMBRESIA
  // CLUB BI" would otherwise match the 'club' merchant rule and file the
  // card's membership fee under Entretenimiento.
...
export function parseCardStatement(text) {
  ...
  if (bank === 'bi') return parseContecnica(text)
  if (bank === 'gyt') return parseGyT(text)
  if (bank === 'bac') return parseBac(text)
  return null
}
