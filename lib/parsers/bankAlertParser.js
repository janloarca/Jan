...
import { chargeDayISO } from '../localDate'
...
//
// ⛔ Se evalua sobre el ENCABEZADO, igual que DECLINED_HINTS y por la MISMA
// razon: clasificar como 'credit' termina en `skip` (lib/alertIngest.js), o sea
// el gasto no se escribe Y el correo se marca leido, con un barrido que corre
// una vez al dia. Descartar un cobro REAL lo hace desaparecer sin dejar rastro;
// guardar un reverso de mas deja una fila visible que el usuario borra. Ante la
// duda, no se descarta.
//
// Y esta lista es MAS propensa al pie de pagina que su hermana: "devolucion",
// "abono" y "reintegro" viven en el boilerplate bancario ("politica de
// devoluciones", "abono a su cuenta"), asi que sobre el texto COMPLETO un cobro
// real cuyo pie las mencione se tragaba entero.
...
export function parseBankAlert({ subject = '', text = '', html = '', receivedAt, defaultCurrency = 'GTQ', offsetMinutes = null } = {}) {
  ...
  // El día de LLEGADA, visto desde donde estaba quien pagó y no desde UTC.
  //
  // Esto es el respaldo para una alerta que no imprime fecha, y hasta ahora
  // tomaba los diez primeros caracteres de una cadena Zulu: en Guatemala eso
  // ROTA A LAS 6 DE LA TARDE, así que una compra de la noche del último día del
  // mes se archivaba en el mes SIGUIENTE. En Flujo eso no es un día de
  // diferencia, es dinero cambiado de mes.
  //
  // El desfase no se adivina: sale de la cabecera Date del propio mensaje, que
  // `lib/emailIngest.js` ya parsea para colocar la hora de pared y que hasta hoy
  // no se usaba para el DÍA. Sin desfase, el comportamiento es idéntico al de
  // antes.
  const receivedISO = receivedAt ? (chargeDayISO(receivedAt, offsetMinutes) || undefined) : undefined
  ...
  return {
    ...
    kind: CREDIT_HINTS.test(alertHead(subject, body)) ? 'credit'
