...
function parseDate(val) {
  ...
  if (/^\d{8}$/.test(s)) out = `${s.slice(0, 4)}-${s.slice(4, 6)}-${s.slice(6, 8)}`
    ...
    // YYYYMM rows come from PortfolioAnalyst MONTHLY NAV exports: the value is
    // the month-END NAV. Stamping them month-start put Jan-31's NAV (deposit
    // and gain already inside) on 'YYYY-01-01', where findYearStartAnchor
    // picked it as the YTD baseline and Modified Dietz subtracted January's
    // flows a second time. Date.UTC(y, m, 0) is the last day of month m
    // (1-indexed): '202601' -> '2026-01-31'.
    const y = +s.slice(0, 4), m = +s.slice(4, 6)
    out = new Date(Date.UTC(y, m, 0)).toISOString().slice(0, 10)
    // FASE NU. La fila del mes EN CURSO no es un cierre de mes: PortfolioAnalyst
    // la emite igual (YYYYMM) pero su valor es el NAV al DIA DEL EXPORT, y el
    // fin de mes todavia no ocurrio. Estamparla en el futuro (el CSV real del
    // usuario venia con '202609' -> '2026-09-30' un 4 de septiembre) archiva
    // un valor con una fecha que no existe: el "ultimo snapshot" pasa a ser
    // uno del futuro, y cuando esa fecha llegue afirmaria que la cuenta valia
    // eso al cierre. Se recorta a HOY, que es lo mas tarde que ese numero
    // puede ser cierto; un mes ya cerrado no cambia.
...
function parseActivityStatementSections(sections) {
  ...
  // IBKR titles this section differently segun como se genero el statement:
  // "Open Positions" en el compacto, "Long Open Positions" (y "Short Open
  // Positions") en el detallado. Anclar en /^open pos/ hacia que un statement
  // detallado parseara CERO posiciones y cayera a filas basura.
  //
  // ⛔ Ese /^open pos/ ya no esta, y su ausencia es parte del arreglo: la regla
  // de arriba cubre las tres formas legitimas, mientras que el prefijo ademas
  // matcheaba "Open Position Summary", que es una seccion de PortfolioAnalyst
  // (la lee parsePortfolioAnalystSections mas abajo, con otras columnas). Con
  // las dos reglas, el MISMO archivo se parseaba dos veces y la lectura mala
  // ganaba el merge. Ver el guard de asset category, justo abajo.
  const openPosKeys = Object.keys(sections).filter(k => /open positions?$/i.test(k.trim()))
  for (const openPosKey of openPosKeys) {
    ...
    // ⛔ Sin columna de categoria esta seccion NO es la Open Positions de un
    // Activity Statement: esa columna es parte de su forma. Y el costo de
    // seguir es que `mapAssetCategory('')` cae a su default 'Stock', o sea se
    // AFIRMA un tipo que aca nadie leyo. Eso convirtio una fila de EFECTIVO
    // ("USD - United States Dollar", 82.53 dolares) en una posicion de mercado,
    // y como "USD" tambien es el ticker de un ETF real, useMarketPrices la
    // cotizo a ~88.50: 7,304.27 inventados sobre una cuenta de 10,008.97.
    // Rehusar es mejor que adivinar; la seccion de PA se lee bien mas abajo.
  ...
  // A "Trade Summary" is a LIFETIME AGGREGATE per symbol (quantity bought,
  // average price bought, quantity sold...) with no dates at all. The old
  // fallback accepted it when no real Trades section existed, so a
  // PortfolioAnalyst export produced 77 "purchases" all stamped with today's
  // date: not one of them a real transaction, and every one of them landing in
  // the user's movement ledger. There is no trade history to salvage from an
  // aggregate, so no section is better than a summary.
  const tradesKey = Object.keys(sections).find(k => /^trade/i.test(k) && !/summar(y|ies)/i.test(k))
  if (tradesKey) {
  ...
  for (const [sectionRe, txType] of [
    ...
    const amtIdx = findCol(headers, 'amount')
    const curIdx = findCol(headers, 'currency')
    // PortfolioAnalyst ships a real Symbol column here; the Activity Statement
    // does not and hides the ticker inside the description. Reading the column
    // when it exists is both more accurate and what keeps this section from
    // being claimed twice (see the guard below).
    const symIdx = findCol(headers, 'symbol')

    // Both this parser and the PortfolioAnalyst one understand a "Dividends"
    // section, and parseIBKRFile concatenates their output. On a PA export that
    // meant every dividend was imported TWICE (124 rows became 248 and income
    // doubled from 339.01 to 678.02), because this path could not read PA's
    // layout and filed its copies under a generic CASH symbol. PA's own parser
    // handles that shape better, so stand down when it will run.
    if (txType === 'DIVIDEND' && symIdx >= 0 && descIdx < 0) continue
    ...
    for (const row of rows) {
      ...
      const symbol = (symIdx >= 0 ? (row[symIdx] || '').trim().toUpperCase() : '')
        || (desc.match(/^([A-Z0-9.]{1,10})\s*\(/) || [])[1] || 'CASH'
