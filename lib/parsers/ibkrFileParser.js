...
export function isIBKRPerformanceReport(text) {
  ...
  const hasPositions = lower.includes('open positions') && (lower.includes('symbol') || lower.includes('quantity'))
  if ((hasPA || hasBenchmark || hasKeyStats) && !hasPositions) return true
  return false
}
...
function parsePortfolioAnalystSections(sections) {
  ...
  // Set acquisitionDate on positions from earliest equityHistory date
  if (equityHistory.length > 0 && positions.length > 0) {
    const dates = equityHistory.map(e => e.date).filter(Boolean).sort()
...
export function parseIBKRFile(textOrData, headers) {
  if (typeof textOrData === 'string') {
    if (/,Header,/.test(textOrData) || /,Data,/.test(textOrData)) {
      ...
      const hasOpenPositions = !!sections['Open Positions']
      const hasTrades = !!(sections['Trades'] || sections['Trades - Realized P/L'])

      let pa = { positions: [], trades: [], cashPositions: [], equityHistory: [], benchmarks: [] }
      if (hasOpenPositions || hasTrades || activity.positions.length > 0) {
      ...
      const isPerformanceReport = !hasOpenPositions && !hasTrades && positions.length === 0
