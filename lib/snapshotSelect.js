...
// ¿Ya hay una observación REAL de portafolio en esta fecha?
//
// ⛔ FASE JW. La usa la calibración para no escribir encima de un día que ya
// está medido. Antes esa pregunta se contestaba con una lista literal de
// fuentes (`'ibkr'` o `'daily'`), que dejaba fuera 'backfill' — desde FASE
// GD/HN, casi todos los días históricos — más 'manual', 'ibkr_quarterly' y los
// docs viejos sin `_source`. Como una calibración GLOBAL se guarda en el id
// PLANO de la fecha, sobre uno de esos días hacía merge encima del doc real.
//
// La pregunta correcta no es de qué fuente viene sino si hay un valor ahí. Una
// calibración propia no cuenta (recalibrar el mismo día sí se permite), y un
// ancla por cuenta tampoco (vive en su propio id compuesto).
export function hasRealObservationAt(snapshots, dateStr) {
  if (!dateStr) return null
  for (const s of snapshots || []) {
    if (!s || s.date !== dateStr || s._calibrated || s._account) continue
