...
export function projectToGoal({
  ...
  if (!Number.isFinite(y0) || !Number.isFinite(yN) || yN < y0) {
    return { years: [], points: [], endValue: start, totalSaved: 0, totalGrowth: 0, reachedYear: null, gap: hasGoal ? goal - start : 0, goalValue: hasGoal ? goal : null }
  }
  ...
  const years = []
  // Los puntos MENSUALES de todo el recorrido, para que la gráfica tenga un
  // eje uniforme. Con puntos anuales después del primer año, los meses de este
  // año y los años siguientes compartirían el mismo ancho y la pendiente del
  // arranque se vería doce veces más empinada de lo que es.
  const points = [{ year: y0, month: Math.max(0, fromMonth) - 1, value: start }]
  ...
  for (let year = y0; year <= yN; year++) {
    ...
    for (let m = startMonth; m <= 11; m++) {
      ...
      saved += monthSaved
      growth += monthGrowth
      points.push({ year, month: m, value })
    }
...
// Cuánto puede sobresalir la meta por encima del recorrido antes de que
// dibujarla deje de valer la pena. Con la meta muy por arriba, el eje se
// estira para alcanzarla y la trayectoria queda aplastada contra el piso: se
// pierde la única información que la gráfica podía dar (la FORMA de cómo
// crece) a cambio de una línea que ya dice el texto de arriba.
const MAX_GOAL_RATIO = 1.6

// Margen arriba y abajo del viewBox, en las mismas unidades del 0-100.
const PAD_Y = 6

// La geometría de la gráfica del recorrido hacia la meta, en un viewBox de
// 100x100. Pura: no sabe de SVG ni de React, solo devuelve coordenadas.
//
// Devuelve `goalOffChart` cuando la meta queda fuera de rango, en vez de
// dibujarla igual o de callarse: el consumidor lo DICE, porque una gráfica sin
// la línea de meta y una donde la meta simplemente no se ve son la misma
// imagen con dos significados opuestos.
export function goalChartGeometry(points = [], goalValue = null) {
  const vals = (points || []).map((p) => Number(p.value)).filter((v) => Number.isFinite(v))
  if (vals.length < 2) return null

  const goal = Number(goalValue)
  const hasGoal = Number.isFinite(goal) && goal > 0
  const runMax = Math.max(...vals)
  const runMin = Math.min(...vals)
  const goalFits = hasGoal && goal <= runMax * MAX_GOAL_RATIO

  const max = goalFits ? Math.max(runMax, goal) : runMax
  const min = Math.min(runMin, goalFits ? goal : runMin)
  const span = max - min
  // Sin movimiento no hay forma que mostrar: una línea plana pegada al borde
  // se lee como un bloque en blanco, que es peor que no tener gráfica.
  if (!(span > 0)) return null

  // Margen vertical para que el trazo no se coma su propio grosor contra el
  // borde: sin él, el punto más alto queda cortado a la mitad y la línea se ve
  // truncada justo donde termina el recorrido, que es lo que uno viene a mirar.
  const y = (v) => PAD_Y + (1 - (v - min) / span) * (100 - PAD_Y * 2)
  const x = (i) => (i / (vals.length - 1)) * 100
  const pts = vals.map((v, i) => `${x(i).toFixed(2)},${y(v).toFixed(2)}`)

  // El primer punto que cruza la meta, para marcarlo. Puede no existir aunque
  // la meta quepa en el eje (el recorrido se queda corto).
  let crossIndex = -1
  if (hasGoal) crossIndex = vals.findIndex((v) => v >= goal)

  return {
    line: pts.join(' '),
    area: `0,100 ${pts.join(' ')} 100,100`,
    goalY: goalFits ? y(goal) : null,
    goalOffChart: hasGoal && !goalFits,
    cross: crossIndex >= 0 ? { x: x(crossIndex), y: y(vals[crossIndex]), index: crossIndex } : null,
  }
}
