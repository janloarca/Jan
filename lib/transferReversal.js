...
export function transferReversalPlan(tx, items) {
  ...
  if (toId && !toItem) missing.push('to')
    ...
    if (isDebt) {
      ...
      // FASE OB. Si el destino tiene HOY menos de lo que recibió (lo gastó
      // después), `debitFields` recortaría el saldo a CERO y la diferencia se
      // perdería sin decirlo: un fondo que recibió 300 y quedó en 0 se
      // "revertía" escribiendo 0, o sea la transferencia desaparecía y los
      // 300 no volvían al origen completos. Se rehúsa y se dice: el usuario
      // ajusta el saldo del destino primero, y entonces sí se deshace.
      const fields = credit.amount > before + DUST ? null : debitFields(toItem, credit.amount)
...
// Qué decirle al usuario ANTES de borrar.
//
// Borrar una transferencia ahora MUEVE DINERO en dos cuentas, así que no puede
// cerrarse en silencio: es la lección de FASE JD5 ("una acción que mueve dinero
// y se cierra en silencio no se puede diagnosticar") aplicada a la acción
// inversa. Vive acá, y no en cada pantalla, porque son DOS superficies las que
// borran (la tarjeta de movimientos recientes y el historial de la cuenta) y dos
// copias de esta redacción es exactamente cómo una se queda atrás.
//
// Devuelve líneas ya armadas; el formato del monto lo pone quien llama, porque
// `formatCurrency` vive en la capa de componentes.
export function reversalLines(plan, lang, fmt) {
  if (!plan) return []
  const es = lang !== 'en'
  ...
  if (plan.from) {
    lines.push(`${es ? 'Devuelve' : 'Returns'} ${money(plan.from.amount, plan.from.currency)} ${es ? 'a' : 'to'} ${plan.from.name}`)
  }
  if (plan.to) {
    lines.push(plan.kind === 'debt'
      ? `${es ? 'Sube la deuda de' : 'Raises'} ${plan.to.name} ${es ? 'en' : 'by'} ${money(plan.to.amount, plan.to.currency)}`
      : `${es ? 'Quita' : 'Removes'} ${money(plan.to.amount, plan.to.currency)} ${es ? 'de' : 'from'} ${plan.to.name}`)
  }
  // Una cuenta borrada no tiene saldo que devolver, y callarlo haría que la
  // cuenta de lo que se mueve no cuadre con lo que el usuario ve.
  if (plan.missing.length) {
    lines.push(es
      ? 'Una de las cuentas ya no existe: solo se ajusta la que queda.'
      : 'One of the accounts no longer exists: only the remaining one is adjusted.')
  }
  if (plan.refused.length) {
    lines.push(es
      ? 'Una de las cuentas no tiene saldo suficiente para devolver este movimiento, asi que no se puede deshacer. Ajusta su saldo primero.'
      : 'One of the accounts does not hold enough to give this movement back, so this cannot be undone. Adjust its balance first.')
  }
  return lines
}
