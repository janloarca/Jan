...
// Las fuentes que miden UNA cuenta del broker y no el portafolio. Mismo par que
// BROKER_NAV_SOURCES (components/dashboard/utils.js) y que el BROKER_NAV de
// lib/corruptSnapshots.js: se declara acá para que este módulo puro no importe
// desde components/, que es la dirección equivocada de la dependencia.
const BROKER_NAV_SOURCES = ['ibkr', 'ibkr_quarterly']
...
// FASE HN. El NAV REAL del broker, por fecha, desde los docs que el broker ya
// dejó en Firestore: el Equity Summary del Flex (~365 días, un valor por día
// hábil, `_source:'ibkr'`) y la transcripción trimestral de PortfolioAnalyst
// (`_source:'ibkr_quarterly'`, FASE DL: un valor al cierre de cada trimestre,
// para lo anterior a esos ~365 días). Es la medición del propio broker: no hay
// nada mejor con que reconstruir esa parte del portafolio, y estábamos
// estimándola.
//
// FASE NV. Hasta acá el default era SOLO 'ibkr', así que un NAV transcrito por
// trimestre era invisible para toda la cadena de composición y reparación (el
// backfill, "Reparar ahora", el arranque del broker en el desglose del YTD y el
// juez de calibraciones por cuenta), mientras la Hoja y `staleBackfillDates` ya
// lo trataban como observación real del broker: dos partes de la app con dos
// ideas de qué es "el NAV del broker". Ahora entra con dos reglas:
//   · PRIORIDAD: en una misma fecha, el NAV DIARIO gana SIEMPRE al trimestral,
//     sin importar el orden de los docs. La transcripción sale de una imagen
//     y el Flex de la contabilidad del broker; cuando los dos existen no hay
//     razón para preferir el más grueso (planQuarterlyNavWrite ya evita
//     escribirlos juntos desde FASE KA, pero los docs viejos pueden traerlos).
//   · ARRASTRE: el mismo tope de 4 días que el NAV diario (fin de semana +
//     feriado pegado). Un cierre de trimestre describe SU fecha; arrastrarlo
//     a lo ancho del trimestre archivaría meses enteros desde una sola
//     medición gruesa, que es justo lo que este módulo prohíbe. Lejos del
//     cierre, el día sigue siendo un hueco honesto (invariante 2).
// Consecuencia deliberada: un portafolio cuyo único NAV del broker es la
// transcripción trimestral pasa a COMPONER (la mitad del broker deja de
// estimarse hold-flat y los días sin NAV arrastrable no se escriben), que es
// lo que los invariantes 1 y 2 de la serie histórica ya exigían.
const NAV_SOURCE_RANK = { ibkr: 2, ibkr_quarterly: 1 }
export function buildNavByDate(snapshots, brokerNavSources = BROKER_NAV_SOURCES) {
  ...
  for (const s of snapshots || []) {
    ...
    const rank = NAV_SOURCE_RANK[s._source] ?? 0
    const prev = rankByDate.get(s.date)
    // Una fuente más fina ya ocupa la fecha: la más gruesa no la pisa.
    if (prev != null && prev > rank) continue
    // Con varios docs de la misma fecha y la MISMA fuente (slot plano +
    // paralelo) gana el último visto, como siempre.
    out.set(s.date, v)
    rankByDate.set(s.date, rank)
  }
  return out
}
...
// ⛔ FASE NP. La TERCERA vía, y la única que NO necesita al broker: el propio
// archivo contradiciéndose entre días vecinos.
//
// El caso que la obligó, tres reportes seguidos con el MISMO número al centavo
// (9,305.22) pese a que FASE NL y FASE NN ya estaban desplegadas: el ancla del
// 1 de enero vale 9,305.22 y el día siguiente, medido en vivo, ~5,555. La
// gráfica ya lo rotula "Mayor caída del período -40.3% (1 ene → 2 ene)", o sea
// la app venía SEÑALANDO el defecto sin poder repararlo.
//
// Por qué las dos hermanas no alcanzaban, y es lo que hay que entender antes
// de tocar esto:
//   · contradictedCalibrationDates (FASE NL) y divergentDailyDates exigen
//     `c.composed`, o sea NAV REAL del broker para ESE día. Con el broker
//     conectado pero sin NAV arrastrable a esa fecha, `composeDailyTotals` ni
//     siquiera produce una entrada para el día: no hay nada contra qué juzgar
//     y tampoco nada con qué reescribir. FASE NL dejó ese hueco anotado
//     ("un portafolio SIN broker conectado conserva su calibración pase lo que
//     pase"); esto lo cierra por el otro lado.
//   · contradictedAccountCalibrations (FASE NN) juzga la especie POR CUENTA.
// Y la evidencia estaba en el archivo todo el tiempo: el día de al lado.
//
// La forma de la prueba, y por qué NO es una banda estadística: se PROYECTA el
// valor del vecino hasta la fecha del ancla aplicándole los flujos que hubo en
// medio (`vecino_posterior = ancla + flujos`, y al revés para el anterior).
// Si los dos vecinos proyectan lo mismo y el ancla dice otra cosa, el ancla es
// un pico aislado: ningún movimiento de mercado produce eso, porque un cambio
// REAL de nivel deja a los dos vecinos en niveles DISTINTOS (la firma de
// round-trip que corruptSnapshotRunIds ya usa desde FASE FR).
//
// Las restricciones, cada una con su test:
//   · solo calibraciones GLOBALES (sin `_account`), y JAMÁS una observación
//     real. Una calibración no es un monto tecleado: es un valor DESPEJADO de
//     un porcentaje (FASE NL), y por eso se puede juzgar contra mediciones.
//   · el vecino tiene que ser una observación REAL y estar CERCA
//     (`maxNeighborDays`, 3 por defecto: cubre un fin de semana). Uno de hace
//     dos semanas puede diferir legítimamente más que la banda y la prueba
//     dejaría de significar algo.
//   · un doc de NAV de broker NO puede hacer de vecino: mide UNA cuenta y no
//     el portafolio (la lección de FASE MI).
//   · con vecinos de los DOS lados, si NO coinciden entre sí no hay veredicto:
//     ahí pasó un cambio real de nivel y no se puede saber de qué lado cae el
//     ancla. Es el caso que protege una caída de mercado genuina.
//   · con UN solo vecino la prueba es más débil (un salto real de un día es
//     indistinguible), así que se exige un desvío mucho mayor (`loneGapRatio`,
//     25%) en vez de la banda de 8%. Se dice de frente: esta mitad es la floja.
//
// `flowsUSD` llega YA convertido, con la misma forma que consume
// corruptSnapshotRunIds (`{ts, type, amount}`): una segunda conversión acá
// sería una segunda definición de "cuánto entró" que puede divergir.
export function selfContradictedCalibrationDates(snapshots, flowsUSD = [], {
  tol = 0.08, minAbs = 50, maxNeighborDays = 3, loneGapRatio = 0.25,
} = {}) {
  const DAY = 86400000
  const cals = []
  const real = []
  for (const s of snapshots || []) {
    if (!s || !s.date || s._account) continue
    const v = snapshotAssetsUSD(s)
    if (!isFinite(v) || v <= 0) continue
    const ts = Date.parse(`${s.date}T00:00:00Z`)
    if (!isFinite(ts)) continue
    if (s._calibrated) cals.push({ date: s.date, ts, value: v })
    else if (!BROKER_NAV_SOURCES.includes(s._source || '')) real.push({ ts, value: v })
  }
  if (cals.length === 0 || real.length === 0) return []
  real.sort((a, b) => a.ts - b.ts)

  const flows = (flowsUSD || []).filter(
    (f) => f && isFinite(f.ts) && isFinite(f.amount) && f.amount > 0
  )
  // Neto CON SIGNO de lo que entró entre dos instantes: un depósito y un retiro
  // del mismo monto no explican nada, y contarlos por separado dejaría pasar
  // una calibración envenenada.
  //
  // La ventana es INCLUSIVA en los dos extremos, a propósito. Un snapshot
  // fechado D debería ser el cierre de D, así que un flujo del propio D ya
  // estaría adentro y no debería contar hacia el día siguiente; pero una
  // calibración no es un cierre, es un valor DESPEJADO de un % medido en un
  // momento que nadie registró. Ante esa ambigüedad se elige el lado
  // conservador: si un flujo del mismo día PODRÍA explicar el salto, no se
  // toca la calibración. Equivocarse acá cuesta dejar un ancla mala a la
  // vista; equivocarse al revés cuesta descartar una correcta.
  const netFlow = (fromTs, toTs) => {
    let net = 0
    for (const f of flows) {
      if (f.ts < fromTs || f.ts > toTs) continue
      net += f.type === 'WITHDRAWAL' ? -f.amount : f.amount
    }
    return net
  }
  ...
  for (const c of cals) {
    ...
    let next = null
    for (const r of real) {
      const d = Math.abs(r.ts - c.ts)
      if (d === 0 || d > maxNeighborDays * DAY) continue
      if (r.ts < c.ts) { if (!prev || r.ts > prev.ts) prev = r }
      else if (!next || r.ts < next.ts) next = r
    }
    // Lo que cada vecino dice que el ancla debería valer, ya descontados los
    // flujos que hubo en medio.
    const fromPrev = prev ? prev.value + netFlow(prev.ts, c.ts) : null
    const fromNext = next ? next.value - netFlow(c.ts, next.ts) : null
    let expected = null
    let band = null
    if (fromPrev != null && fromNext != null) {
      const spread = Math.abs(fromPrev - fromNext)
      const ref = Math.max(fromPrev, fromNext)
      // Los vecinos discrepan entre sí: pasó algo real y no hay veredicto.
      if (spread > Math.max(minAbs, ref * tol)) continue
      expected = (fromPrev + fromNext) / 2
      band = Math.max(minAbs, expected * tol)
    } else {
      expected = fromPrev != null ? fromPrev : fromNext
      if (expected == null) continue
      band = Math.max(minAbs, expected * loneGapRatio)
    }
    if (!(expected > 0)) continue
    if (Math.abs(c.value - expected) > band) out.push(c.date)
  }
  return out
}
