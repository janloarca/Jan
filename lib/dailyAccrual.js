...
// del usuario). `balanceAsOf` es un SEGUNDO corte, además de la compra: es la
// fecha desde la cual el saldo tecleado ya es cierto (ver lib/liquidYield.js).
// Si cae DENTRO del mes que se está devengando, los días de ESE mes que ya
// pasaron antes de la foto están adentro del número que el usuario tecleó, y
// solo los de DESPUÉS son devengo nuevo. Sin este corte, el asentamiento de
// fin de mes de una cuenta que reinvierte volvía a devengar el mes ENTERO
// encima de un saldo que ya contenía media parte de él: un fondo de $5,000
// con balanceAsOf al 15 de agosto escribía $16.68 (agosto completo) cuando lo
// que faltaba por devengar eran los 16 días de después ($8.60).
//
// El corte que manda es el MÁS TARDE de los dos (compra, balanceAsOf), y por
// eso alcanza con tomar el máximo antes de comparar: un mes ANTERIOR a ambos
// sigue devengando entero, uno POSTERIOR a ambos también (balanceAsOf deja de
// pesar en cuanto su mes ya pasó), y solo el mes en que cae el más tardío de
// los dos se recorta. Sin `balanceAsOf` (callers viejos, o el caso en que no
// aplica: ver `monthlyIncomeAmount`) el comportamiento es idéntico al de
// siempre.
export function accrualDaysInMonth({ year, monthIndex, acquisitionDay, balanceAsOf } = {}) {
  const total = daysInMonth(year, monthIndex)
  if (!Number.isFinite(total) || total <= 0) return 0
  const cutoff = [acquisitionDay, balanceAsOf].filter(Boolean).sort().pop() || null
  if (!cutoff) return total
  const acqY = Number(cutoff.slice(0, 4))
  const acqM = Number(cutoff.slice(5, 7)) - 1
  const acqD = Number(cutoff.slice(8, 10))
  if (!Number.isFinite(acqY) || !Number.isFinite(acqM) || !Number.isFinite(acqD)) return total
  // Mes anterior al corte: cero. El motor ya lo filtra antes de llegar acá
  ...
  // El MES del corte: desde el día siguiente hasta fin de mes. El día del
  // corte en sí no devenga: el dinero entró (o la foto se selló) ese día y la
  // primera acreditación es la del día siguiente, igual que la compra.
...
export function monthlyAccrual({ balance, annualRatePct, year, monthIndex, acquisitionDay, balanceAsOf } = {}) {
  const bal = Number(balance)
  const rate = Number(annualRatePct)
  if (!Number.isFinite(bal) || bal <= 0) return 0
  if (!Number.isFinite(rate) || rate === 0) return 0
  const days = accrualDaysInMonth({ year, monthIndex, acquisitionDay, balanceAsOf })
  if (days <= 0) return 0
