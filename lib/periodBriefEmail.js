...
import { renderEmail, portfolioHero, HIDDEN_TOTAL_NOTE } from './emailLayout'
...
const KINDS = {
  monthly: {
    ...
    preheader: 'Your month in Chispudo: how the portfolio moved, the income you collected, and where the market closed. Open to see the numbers.',
  },
  annual: {
    ...
    preheader: 'Your year in Chispudo: how the portfolio moved, the income you collected, and where the market closed. Open to see the numbers.',
  },
}
 ...
 * @param {boolean} [input.showNetWorth]   decisión del usuario; por default el
 *                                         total NO viaja en el correo (FASE OO)
 */
export function buildPeriodBriefEmail({ kind = 'monthly', periodLabel, portfolio, market, attachmentsInfo = {}, showNetWorth = false }) {
  const k = KINDS[kind] || KINDS.monthly
  const cur = portfolio?.currency || 'USD'
  const sections = []

  // ── Tu período ──
  // Misma decisión que el semanal (FASE OO): el total es lo que un teléfono
  // enseña en la lista del buzón sin desbloquearse, así que por default no
  // viaja y la cifra principal es la variación del período.
  const hero = portfolioHero({
    showTotal: showNetWorth,
    total: { label: 'Net worth', value: fmtMoney(portfolio?.netWorth, cur) },
    change: {
      label: k.periodRow,
      combo: (portfolio?.periodPct != null || portfolio?.periodAbs != null)
        ? combo(portfolio.periodAbs, portfolio.periodPct, cur) : null,
      abs: portfolio?.periodAbs != null && isFinite(portfolio.periodAbs) ? fmtSigned(portfolio.periodAbs, cur) : null,
      pct: fmtPct(portfolio?.periodPct),
      suffix: k.deltaSuffix,
    },
    asOf: portfolio?.asOf ? `as of ${portfolio.asOf}` : null,
  })
  ...
  // Una ausencia deliberada se DICE, o el correo se lee como roto.
  if (!showNetWorth) paragraphs.push(HIDDEN_TOTAL_NOTE)

  sections.push({ hero, heading: k.heading, rows, paragraphs, cta: { label: 'Open Chispudo', url: `${APP_URL}/dashboard` } })
  ...
  const { html, text } = renderEmail({
    ...
    // Sin un solo número: es lo que se lee en la lista del buzón.
...
export function buildMonthlyBriefEmail({ monthLabel, portfolio, market, attachmentsInfo, showNetWorth }) {
  return buildPeriodBriefEmail({
    kind: 'monthly', periodLabel: monthLabel, market, attachmentsInfo, showNetWorth,
    portfolio: portfolio ? { ...portfolio, periodAbs: portfolio.monthAbs, periodPct: portfolio.monthPct } : portfolio,
  })
}

export function buildAnnualBriefEmail({ yearLabel, portfolio, market, attachmentsInfo, showNetWorth }) {
  return buildPeriodBriefEmail({
    kind: 'annual', periodLabel: yearLabel, market, attachmentsInfo, showNetWorth,
