...
export function buildPeriodBriefEmail({ kind = 'monthly', periodLabel, portfolio, market, attachmentsInfo = {} }) {
  ...
  // ── Tu período ──
  // El patrimonio va como cifra principal con su variación al lado, no como
  // una fila más: es lo primero que alguien quiere ver al abrir el correo.
  const hero = {
    label: 'Net worth',
    value: fmtMoney(portfolio?.netWorth, cur),
    delta: (portfolio?.periodPct != null || portfolio?.periodAbs != null)
      ? `${combo(portfolio.periodAbs, portfolio.periodPct, cur)} ${k.deltaSuffix}`
      : null,
