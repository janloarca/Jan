...
// La firma que decide si dos filas DEL MISMO ARCHIVO son la misma fila.
//
// La referencia del banco va adentro cuando existe, y esa es la mitad que
// importa: dos Uber de Q75 el mismo día, dos cafés en el mismo local, dos
// retiros del mismo cajero son cobros REALES y distintos, y sin la referencia
// comparten fecha, monto, tipo y descripción, o sea colapsaban en uno solo. Con
// referencia no pueden colapsar nunca, porque el banco les dio identidad.
//
// Sin referencia siguen colapsando (no hay con qué distinguirlas), pero ya no se
// descartan: caen en el bucket `repeats`, que el usuario puede rescatar.
function signatureKey(tx) {
  const ref = tx.reference ? String(tx.reference).trim() : ''
  const base = `${String(tx.date).slice(0, 10)}|${cents(tx.amount)}|${tx.type}|${normalizeDesc(tx.description)}`
...
export function matchStatement(parsedTxs, existingTxs) {
  ...
  for (const parsed of parsedTxs || []) {
    // Una fila que se repite dentro del archivo NO es una fila ya registrada, y
    // mezclarlas en el mismo bucket era mentir sobre la única de las dos que
    // puede ser un cobro real: se mostraba bajo "Ya registradas: se omiten"
    // sobre algo que nunca se registró. Va a su propio bucket, con su propio
    // rótulo y rescatable.
    const sig = signatureKey(parsed)
    if (seenInFile.has(sig)) {
      repeats.push({ parsed, reason: 'in-file-duplicate' })
      continue
    }
    seenInFile.add(sig)
  ...
  // `exact` ahora solo contiene filas que de verdad matchearon algo ya
  // registrado, así que su rótulo ("Ya registradas") por fin es cierto.
  // `repeats` son filas que se repiten dentro del archivo: pueden ser un cobro
  // real repetido y por eso el caller tiene que ofrecerlas, nunca descartarlas.
  return { newTxs, exact, likely, repeats }
}
