 ...
 */
export async function buildWeeklyBriefForUser({ db, uid, prefs = {}, market = null, now = new Date(), cache = null }) {
  ...
  const ctx = await loadUserPortfolioContext({ db, uid, prefs, cache })
  if (!ctx) return null
  const { items: enriched, transactions, augmented, netWorth, totalAssets, baseCurrency, convert,
    annualDividends, estimatedAnnualIncome, profileName } = ctx
  ...
  try {
    const { buffer, filename } = await renderReportPdf({
      ...
      // FASE IE8: sin estas dos, el reporte del SEMANAL imprimía
      // "Dividends, trailing 12 months $0.00" mientras el del mensual, con
      // los MISMOS datos, decía $784.87 (los dos PDFs que mandó el usuario en
      // la misma tanda). Son cifras de 12 meses y de tasas declaradas: no
      // dependen del período que cubre el correo.
  ...
  const { subject, html, text } = buildWeeklyBriefEmail({
    ...
    // FASE OO. El patrimonio en el cuerpo del correo es opt-in: por default no
    // viaja, porque la vista previa del teléfono lo enseña sin desbloquear.
    showNetWorth: prefs?.emailShowNetWorth === true,
  })
