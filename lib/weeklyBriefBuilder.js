...
import { getItemValue, isExcludedFromNetWorth, augmentSnapshots } from '../components/dashboard/utils'
...
export async function buildWeeklyBriefForUser({ db, uid, prefs = {}, market = null, now = new Date() }) {
  ...
  // Los snapshots van AUMENTADOS, igual que en el dashboard: un día cuyo único
  // doc es NAV solo-broker mide UNA cuenta, y sin completarlo con los activos
  // manuales el reporte lo lee como si el portafolio entero valiera eso. En la
  // primera prueba real eso produjo un piso de $9,900 a media semana y una
  // "máxima caída del período" de -62.9% sobre una semana que se movió 0.44%.
