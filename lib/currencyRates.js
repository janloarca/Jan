...
// ¿La tarjeta de tipo de cambio tiene algo que decir? Con una sola moneda (y
// siendo esa el ancla) diría "1 USD = 1 USD". La usan la tarjeta Y el tablero
// (que decide si le reserva media fila), para que las dos no puedan discrepar.
export function showsRateTable(items) {
  return buildRateRows({ currencies: usedCurrencies(items) }).length >= 2
}
