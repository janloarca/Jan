...
export async function generateReport({
  ...
  if (data.institutions.length > 0) {
    ...
    tableHead([
      ...
      { label: `${t('Ganancia YTD', 'YTD Gain')} ³`, x: M + 145, right: true },
      { label: `${t('Retorno YTD', 'YTD Return')} ³`, x: pageW - M, right: true },
    ])
    data.institutions.forEach((r) => {
      row([
        ...
        { text: r.ytdGain != null ? fmt(r.ytdGain) : '-', x: M + 145, right: true },
        { text: fmtPct(r.ytdRetPct), x: pageW - M, right: true },
      ], { h: 5.5 })
    })
    if (data.ytdUnexplained) {
      row([
  ...
  if (data.currencies.length > 0 || data.geography.length > 0) {
    ...
    data.geography.slice(0, 6).forEach((r) => {
      row([
        { text: r.key === 'Unknown' || r.key === 'Other' ? t('Otros', 'Other') : r.key, x: colX2 },
