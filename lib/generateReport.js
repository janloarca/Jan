export async function generateReport({ items, snapshots, transactions, netWorth, totalAssets, lang, returnYTD, annualDividends, profileName, baseCurrency = 'USD', convert }) {
  ...
  // All monetary inputs (netWorth, enriched item prices) arrive already converted to
  // the user's base currency — format with that currency, not a hardcoded USD.
  const locale = lang === 'es' ? 'es-GT' : 'en-US'
  const fmt = (v) => new Intl.NumberFormat(locale, { style: 'currency', currency: baseCurrency, maximumFractionDigits: 0 }).format(v || 0)
  // Snapshots store USD values — convert them to base currency before charting.
  const snapToBase = (v) => (convert && baseCurrency !== 'USD') ? convert(v, 'USD', baseCurrency) : v
  ...
  function addPageFooter(pageNum, totalPages) {
    ...
    doc.text(`Chispudo · chispu.xyz · ${dateStr}`, 15, pageH - 8)
    doc.text(`${pageNum}/${totalPages}`, pageW - 15, pageH - 8, { align: 'right' })
  }
  ...
  doc.text(t('Reporte de Portfolio', 'Portfolio Report'), pageW / 2, 70, { align: 'center' })

  doc.setFontSize(14)
  doc.setTextColor(100, 116, 139)
  doc.text(dateStr, pageW / 2, 82, { align: 'center' })

  if (profileName) {
    doc.setFontSize(12)
    doc.setTextColor(148, 163, 184)
    doc.text(profileName, pageW / 2, 94, { align: 'center' })
  }

  // Cover summary metrics
  const coverMetrics = [
    { label: t('Patrimonio Neto', 'Net Worth'), value: fmt(netWorth) },
    { label: t('Total Activos', 'Total Assets'), value: fmt(totalAssets) },
    { label: t('Retorno YTD', 'YTD Return'), value: fmtPct(returnYTD || 0) },
    { label: t('Posiciones', 'Holdings'), value: String(items.length) },
  ]

  const coverBoxW = 38
  const coverStartX = (pageW - (coverBoxW * 4 + 12)) / 2
  coverMetrics.forEach((m, i) => {
    const x = coverStartX + i * (coverBoxW + 4)
    doc.setFillColor(30, 41, 59)
    doc.roundedRect(x, 115, coverBoxW, 28, 2, 2, 'F')
    doc.setFontSize(7)
    doc.setTextColor(100, 116, 139)
    doc.text(m.label, x + coverBoxW / 2, 124, { align: 'center' })
    doc.setFontSize(13)
    doc.setTextColor(241, 245, 249)
    doc.text(m.value, x + coverBoxW / 2, 135, { align: 'center' })
  })

  doc.setFontSize(10)
  doc.setTextColor(16, 185, 129)
  doc.text('Chispudo', pageW / 2, pageH - 25, { align: 'center' })
  doc.setFontSize(8)
  doc.setTextColor(100, 116, 139)
  doc.text('chispu.xyz', pageW / 2, pageH - 18, { align: 'center' })

  // ═══ PAGE 1: ALLOCATION + TOP HOLDINGS ═══
  doc.addPage()
  doc.setFillColor(15, 23, 42)
  doc.rect(0, 0, pageW, pageH, 'F')
  doc.setFillColor(59, 130, 246)
  ...
  doc.setFontSize(9)
  doc.setTextColor(16, 185, 129)
  doc.text('Chispudo', 15, 14)
  doc.setTextColor(100, 116, 139)
  doc.text(t('Reporte de Portfolio', 'Portfolio Report'), 55, 14)
  doc.text(dateStr, pageW - 15, 14, { align: 'right' })
  ...
  if (snapshots.length >= 2) {
    const sparkData = [...snapshots]
      ...
      .map((s) => snapToBase(s.netWorthUSD ?? s.totalActivosUSD ?? 0))
      .filter((v) => v > 0)
  ...
  if (totalDebt > 0) {
    const debtY = ty + 4
    doc.setDrawColor(30, 45, 69)
    doc.line(15, debtY, pageW - 15, debtY)
    doc.setFontSize(8)
    doc.setTextColor(239, 68, 68)
    doc.text(`${t('Deudas totales', 'Total debts')}: ${fmt(totalDebt)}`, 15, debtY + 6)
    doc.setTextColor(148, 163, 184)
    doc.text(`${t('Patrimonio Neto', 'Net Worth')} = ${fmt(totalVal)} - ${fmt(totalDebt)} = ${fmt(totalVal - totalDebt)}`, 90, debtY + 6)
  }

  // ═══ PAGE 3: TAX & MATURITY ═══
  const taxItems = items.filter((it) => it.taxJurisdiction && !it.isDebt)
  const maturingItems = items.filter((it) => it.maturityDate && new Date(it.maturityDate) > now)

  if (taxItems.length > 0 || maturingItems.length > 0) {
    doc.addPage()
    doc.setFillColor(15, 23, 42)
    doc.rect(0, 0, pageW, pageH, 'F')
    doc.setFillColor(59, 130, 246)
    ...
    doc.setFontSize(12)
    doc.setTextColor(241, 245, 249)
    doc.text(t('Detalle Fiscal y Vencimientos', 'Tax & Maturity Detail'), 15, 18)
    doc.setDrawColor(30, 45, 69)
    ...
    if (taxItems.length > 0) {
      doc.setFontSize(10)
      doc.setTextColor(241, 245, 249)
      doc.text(t('Resumen por Jurisdicción', 'Jurisdiction Summary'), 15, pg2Y)
      pg2Y += 8

      const byJurisdiction = {}
      taxItems.forEach((it) => {
        const j = it.taxJurisdiction
        if (!byJurisdiction[j]) byJurisdiction[j] = { count: 0, value: 0 }
        byJurisdiction[j].count++
        byJurisdiction[j].value += Math.abs((it.quantity || 0) * (it.currentPrice || it.purchasePrice || 0))
      })

      doc.setFontSize(7)
      doc.setTextColor(100, 116, 139)
      doc.text(t('País', 'Country'), 15, pg2Y)
      doc.text(t('Posiciones', 'Positions'), 50, pg2Y)
      doc.text(t('Valor', 'Value'), 95, pg2Y, { align: 'right' })
      doc.text('% Port.', 115, pg2Y, { align: 'right' })
      pg2Y += 5

      Object.entries(byJurisdiction).sort((a, b) => b[1].value - a[1].value).forEach(([j, data]) => {
        doc.setFontSize(8)
        doc.setTextColor(241, 245, 249)
        doc.text(j, 15, pg2Y)
        doc.setTextColor(148, 163, 184)
        doc.text(String(data.count), 50, pg2Y)
        doc.text(fmt(data.value), 95, pg2Y, { align: 'right' })
        doc.text(`${totalVal > 0 ? ((data.value / totalVal) * 100).toFixed(1) : '0.0'}%`, 115, pg2Y, { align: 'right' })
        pg2Y += 5.5
      })
    ...
    if (maturingItems.length > 0) {
      doc.setFontSize(10)
      doc.setTextColor(241, 245, 249)
      doc.text(t('Próximos Vencimientos', 'Upcoming Maturities'), 15, pg2Y)
      pg2Y += 8

      doc.setFontSize(7)
      doc.setTextColor(100, 116, 139)
      doc.text(t('Instrumento', 'Instrument'), 15, pg2Y)
      doc.text(t('Vencimiento', 'Maturity'), 80, pg2Y)
      doc.text(t('Valor', 'Value'), 130, pg2Y, { align: 'right' })
      doc.text(t('Acción', 'Action'), 155, pg2Y)
      pg2Y += 5
      ...
      maturingItems
        ...
        .forEach((it) => {
          const val = Math.abs((it.quantity || 0) * (it.currentPrice || it.purchasePrice || 0))
          const days = Math.ceil((new Date(it.maturityDate) - now) / 86400000)
          doc.setFontSize(8)
          doc.setTextColor(241, 245, 249)
          doc.text((it.name || it.symbol || '').slice(0, 28), 15, pg2Y)
          doc.setTextColor(days <= 90 ? 239 : days <= 365 ? 245 : 148, days <= 90 ? 68 : days <= 365 ? 158 : 163, days <= 90 ? 68 : days <= 365 ? 11 : 184)
          doc.text(it.maturityDate.slice(0, 10), 80, pg2Y)
          doc.setTextColor(16, 185, 129)
          doc.text(fmt(val), 130, pg2Y, { align: 'right' })
          doc.setTextColor(100, 116, 139)
          const action = it.maturityAction === 'convert_equity' ? t('Convertir', 'Convert') : it.maturityAction === 'auto_renew' ? t('Renovar', 'Renew') : t('Capital', 'Capital')
          doc.text(action, 155, pg2Y)
          pg2Y += 5.5
        })
    }
  ...
  for (let i = 1; i <= totalPages; i++) {
    doc.setPage(i)
    if (i > 1) addPageFooter(i - 1, totalPages - 1)
  }

  // Include the profile in the filename so same-day reports for different
  // profiles/entities don't overwrite each other in the downloads folder.
