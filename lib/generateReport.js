...
import { attributionRefusalText } from './ytdAttribution'
...
// FASE HZ. Construye el documento y lo DEVUELVE, en vez de descargarlo. El
// correo semanal necesita los bytes del PDF en el servidor (nodemailer los
// adjunta) y el navegador necesita la descarga: separar "armar" de "guardar"
// es lo que permite que ambos usen el MISMO generador en vez de dos.
export async function buildReportDoc({
  ...
  period = 'ytd',
  // Arranque explícito de la ventana (FASE IE6, correo anual): el reporte del
  // año que acaba de CERRAR mide desde su 1 de enero, no desde el del año en
  // curso, que es donde ancla periodRange('ytd'). Aditivo: sin él, idéntico.
  periodStartTs = null,
  // FASE KP. `audience: 'share'` es el PDF que baja el CLIENTE de un link
  // compartido, y es una ALLOWLIST: omite instituciones (la promesa del tab
  // Compartir), la columna de cantidades (el leak test del payload ya prohíbe
  // qty), jurisdicciones, el detalle de deudas, sectores y comisiones; agrega
  // la portada "Preparado para X por Y" y las disclosures del link. El default
  // 'owner' es byte-coherente con lo de siempre (fijado por los tests de
  // correo y por generateReportShare.test.js).
  audience = 'owner', clientLabel = null, advisor = null,
}) {
  ...
  const data = buildReportData({
    ...
    baseCurrency, convert, profileName, period, periodStartTs, now,
  })
  ...
  const periodLabel = (() => {
    ...
    if (k === 'week') {
      const from = new Date(data.meta.periodStartTs)
  ...
  doc.setTextColor(...GRAY)
  if (shareAud) {
    // La portada de un documento para CLIENTE: a quién se preparó y quién lo
    // prepara, con la firma del asesor cuando existe.
    if (clientLabel) { doc.text(`${t('Preparado para', 'Prepared for')} ${clientLabel}`, M, y); y += 6 }
    const by = [profileName, advisor?.firm].filter(Boolean).join(' · ')
    if (by) { doc.text(`${t('Preparado por', 'Prepared by')} ${by}`, M, y); y += 6 }
    const contact = [advisor?.phone, advisor?.email].filter(Boolean).join(' · ')
    if (contact) { doc.setFontSize(8); doc.text(contact, M, y); doc.setFontSize(10); y += 6 }
  } else if (profileName) { doc.text(profileName, M, y); y += 6 }
  ...
  // ══ 2. RENDIMIENTO POR HORIZONTE + REFERENCIA + RIESGO + COSTOS ══
  sectionTitle('Rendimiento', 'Performance')
  tableHead([
    { label: t('Período', 'Period'), x: M },
    { label: `${t('Retorno', 'Return')} ¹`, x: M + 70, right: true },
    { label: t('Ganancia', 'Gain'), x: M + 110, right: true },
    { label: t('Medido desde', 'Measured from'), x: pageW - M, right: true },
  ])
  const horizonLabels = {
  ...
  data.performance.forEach((p) => {
    row([
      { text: horizonLabels[p.key], x: M },
      { text: fmtPct(p.pct), x: M + 70, right: true },
      { text: p.abs != null ? fmt(p.abs) : '-', x: M + 110, right: true, muted: true },
      { text: p.pct != null && p.fromTs ? fmtDate(p.fromTs) : '-', x: pageW - M, right: true, muted: true },
    ], { h: 5.5 })
  })
  const extraLines = []
  if (data.calendarYears.length > 0) {
    const cyText = data.calendarYears
      .map((cy) => `${cy.year}${cy.partial ? ` (${t('parcial', 'partial')})` : ''} ${fmtPct(cy.pct)}`)
      .join(' · ')
    extraLines.push(`${t('Años calendario', 'Calendar years')}: ${cyText}`)
  }
  if (data.benchmark && !shareAud) extraLines.push(`${t('Referencia', 'Benchmark')}: ${data.benchmark.name} YTD ${fmtPct(data.benchmark.ytdPct)}`)
  if (data.risk.volatilityPct != null || data.risk.maxDrawdown || data.risk.sharpe != null || data.risk.beta != null) {
    const parts = []
    if (data.risk.volatilityPct != null) parts.push(`${t('volatilidad anualizada', 'annualized volatility')} ${data.risk.volatilityPct.toFixed(1)}%`)
    if (data.risk.sharpe != null) parts.push(`Sharpe ${data.risk.sharpe.toFixed(2)}`)
    if (data.risk.beta != null) parts.push(`beta vs ${data.benchmark?.name || 'benchmark'} ${data.risk.beta.toFixed(2)}`)
    if (data.risk.maxDrawdown) parts.push(`${t('máxima caída del período', 'period max drawdown')} ${data.risk.maxDrawdown.pct.toFixed(1)}% (${fmtDate(data.risk.maxDrawdown.fromTs)} ${t('a', 'to')} ${fmtDate(data.risk.maxDrawdown.toTs)})`)
    extraLines.push(`${t('Riesgo', 'Risk')}: ${parts.join(' · ')}`)
  }
  if (data.fees.totalEntryFees > 0 && !shareAud) {
    extraLines.push(`${t('Comisiones de entrada pagadas', 'Entry fees paid')}: ${fmt(data.fees.periodEntryFees)} ${t('en el período', 'in period')} · ${fmt(data.fees.totalEntryFees)} ${t('desde el inicio', 'since inception')}`)
  }
  extraLines.forEach((line) => {
    row([{ text: line, x: M, muted: true }], { size: 7, h: 4.5 })
  })
  y += 4
  ...
  tableHead([
    ...
    // La cantidad de unidades NO va en el PDF del cliente: el leak test del
    // payload ya prohíbe `qty` en lo publicado, y el papel no puede filtrar
    // lo que la página esconde.
    ...(shareAud ? [] : [{ label: t('Cant.', 'Qty'), x: M + 98, right: true }]),
    { label: t('Valor', 'Value'), x: M + 126, right: true },
    { label: '% Port.', x: M + 141, right: true },
    { label: `${t('Retorno', 'Return')} ²`, x: pageW - M, right: true },
  ])
  data.holdings.forEach((h) => {
    // Cuando el nombre completo no cabe, el que cede es el NOMBRE y el que se
    // conserva entero es el SÍMBOLO: cortar la cadena ya armada producía
    // "Cuenta monetaria (BANCO-INDUSTRIAL", con el paréntesis abierto y el
    // ticker partido justo donde estaba el dato que identifica la posición
    // (reporte real del usuario). Se corta en el último espacio para no
    // partir una palabra, y sin puntos suspensivos: el "…" no existe en la
    // fuente estándar de jsPDF y se perdería al escribirlo.
    const label = (() => {
      const MAXW = 34
      ...
      if (!sym || sym === name) return fit(name || sym, MAXW)
      const full = `${name} (${sym})`
      if (full.length <= MAXW) return full
      const room = MAXW - (sym.length + 3) // lo que queda tras " (SYM)"
      return room >= 8 ? `${fit(name, room)} (${sym})` : fit(sym, MAXW)
    })()
    row([
      { text: label, x: M },
      { text: `${h.type || '-'}${h.subtype ? `/${h.subtype}` : ''}`.slice(0, 18), x: M + 62, muted: true },
      ...(shareAud ? [] : [{ text: h.qty.toLocaleString(locale, { maximumFractionDigits: 4 }), x: M + 98, right: true, muted: true }]),
  ...
  // una pared de "-" no informa nada. Y en el PDF del CLIENTE la sección no
  // existe: nombra instituciones, que es lo que el tab Compartir promete no
  // revelar jamás.
  if (!shareAud && data.institutions.length > 0) {
    ...
    if (!hasAttribution) {
      const why = data.meta.breakdownReason
      ...
      const lines = doc.splitTextToSize(why, CW)
  ...
  // ══ 7. EXPOSICIÓN ══
  if (data.currencies.length > 0 || data.geography.length > 0 || data.sectors.length > 0) {
    sectionTitle('Exposición por moneda, geografía y sector', 'Currency, Geography & Sector Exposure')
    ...
    // El sector tampoco viaja en el payload del link: mismo contrato en papel.
    if (!shareAud && data.sectors.length > 0) {
      ensure(10)
      doc.setFont('helvetica', 'bold')
      doc.setFontSize(7.5)
      doc.setTextColor(...GRAY)
      doc.text(t('Sector', 'Sector'), M, y)
      y += 5
      data.sectors.slice(0, 8).forEach((r) => {
        row([
          { text: (r.key === 'Unknown' ? t('Otros', 'Other') : r.key).slice(0, 24), x: M },
          { text: fmt(r.value), x: M + 62, right: true },
          { text: `${r.pct.toFixed(1)}%`, x: M + 80, right: true, muted: true },
        ], { h: 5 })
      })
  ...
  if (data.maturities.length > 0) {
    ...
    data.maturities.forEach((r) => {
      ...
      row([
        { text: String(r.name).slice(0, 30), x: M },
        { text: fmtDate(r.date), x: M + 78, muted: true },
  ...
  // ══ 9. JURISDICCIÓN ══
  if (!shareAud && data.jurisdictions.length > 0) {
  ...
  // ══ 10. DEUDAS ══
  // El detalle de deudas nombra cada pasivo del dueño: fuera del PDF del
  // cliente (el total de deuda sigue en la caja de KPIs, igual que el link).
  if (!shareAud && data.debts.length > 0) {
  ...
  const notes = [
    ...
    // El footnote de la atribución por cuenta habla de una sección que el PDF
    // del cliente no lleva: se omite junto con ella.
    ...(shareAud ? [] : [
    ...
    // Las disclosures del link, también en papel: el PDF viaja solo y no puede
    // apoyarse en el pie de la página que lo generó.
    ...(shareAud ? [
      t('Este resumen complementa, no sustituye, su estado de cuenta oficial. No se garantiza un rendimiento mínimo; cifras según los registros del titular.',
        'This summary complements, and does not replace, your official account statement. No minimum return is guaranteed; figures are based on the owner\'s records.'),
    ] : []),
    ...
    t('Glosario. Retorno (Modified Dietz): ganancia del período neta de depósitos y retiros, ponderando cada movimiento por el tiempo que estuvo invertido. Volatilidad anualizada: qué tanto varían los retornos alrededor de su promedio, escalado a un año. Máxima caída: la peor pérdida de pico a valle del período. Sharpe: retorno sobre la tasa libre de riesgo por unidad de volatilidad (mayor a 1 se considera bueno). Beta: sensibilidad frente al benchmark (1 se mueve igual que el mercado).',
      'Glossary. Return (Modified Dietz): period gain net of deposits and withdrawals, weighting each movement by time invested. Annualized volatility: how much returns vary around their average, scaled to one year. Max drawdown: the worst peak-to-trough loss in the period. Sharpe: return above the risk-free rate per unit of volatility (above 1 is considered good). Beta: sensitivity to the benchmark (1 moves with the market).'),
  ]
  notes.forEach((n) => {
  ...
  const profileSlug = profileName
    ...
    : ''
  const filename = `chispudo-report${profileSlug}-${now.toISOString().split('T')[0]}.pdf`
  return { doc, filename }
}

// Camino del NAVEGADOR: arma y descarga. Firma intacta para sus callers.
export async function generateReport(opts) {
  const { doc, filename } = await buildReportDoc(opts)
  doc.save(filename)
  return filename
}

// Camino del SERVIDOR (cron): arma y devuelve los bytes para adjuntar.
// jsPDF corre en Node; lo único que no existe ahí es la descarga.
