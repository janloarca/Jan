...
// FASE NS. Una posición de IBKR que el usuario YA VENDIÓ no tiene item hoy, así
// que ninguna fila por item la puede representar: su historia se escribe bajo
// esta llave sintética (`prefijo + institución + '__' + SÍMBOLO`), con la misma
// convención que el bucket (jamás un id real). PortfolioSpreadsheet.jsx y
// lib/monthlySpreadsheet.js arman la MISMA cadena.
export const IBKR_CLOSED_KEY_PREFIX = '__ibkr_closed__'

// FASE NS. Qué posiciones de IBKR se VENDIERON por completo, según el ledger de
// trades, y con qué eventos se reconstruye la cantidad que se tenía cada mes.
//
// El defecto que cierra, medido sobre el archivo real del usuario: 7 de las 18
// posiciones que el Flex traía se vendieron, y en la Hoja un mes pasado sumaba
// solo las posiciones que existen HOY (reconstruidas) más el bucket de las no
// medidas. Una posición vendida no aportaba NADA, así que la fila de
// Interactive Brokers de esos meses quedaba corta frente al NAV real por el
// valor entero de lo vendido. Es la otra mitad de "no lee que cambié de
// posiciones": la primera era el parser (FASE NR), esta es la reconstrucción.
//
// La cantidad se ACUMULA desde cero hacia adelante (Σ compras − Σ ventas hasta
// el cierre del mes), a diferencia de `qtyFromTx`, que rebobina desde la
// cantidad de HOY: acá no hay cantidad de hoy, la posición ya no existe.
//
// ⛔ Solo entra un ledger AUTOCONSISTENTE y CERRADO, y las tres restricciones
// tienen cada una su test:
//  - la cantidad acumulada nunca baja de cero (una venta que excede lo comprado
//    dice que la posición ANTECEDE al ledger, y el Flex solo trae ~365 días:
//    ahí no se sabe cuánto se tenía y se deja en el bucket, como siempre);
//  - termina en cero (si el ledger dice que se sigue teniendo y no hay item, o
//    falta la venta o el item se borró a mano: en los dos casos inventarle una
//    fila hoy sería afirmar una tenencia que la app ya negó);
//  - ningún item VIVO lleva ese símbolo, de cualquier fuente: una posición
//    manual con el mismo ticker ya se reconstruye por su cuenta, y sumarle esta
//    la contaría dos veces.
// Sin ningún item de IBKR vivo no se devuelve nada: borrar la cuenta del broker
// significa que no aparece en el historial (FASE GL), y esto también.
//
// La institución sale del item vivo de la MISMA cuenta (`_ibkrAccountId`), y si
// no, de la más común entre los items de IBKR: los trades no la traen.
const FOREX_PAIR_RE = /^[A-Z]{3}\.[A-Z]{3}$/
const CHART_SYMBOL_RE = /^[A-Z0-9._\-^=]{1,20}$/i
export function closedIbkrLedgers(items, transactions) {
  const live = (items || []).filter(it => it && it.id && it._source === 'ibkr')
  if (live.length === 0 || !transactions || transactions.length === 0) return []

  const liveSymbols = new Set((items || []).map(it => (it?.symbol || '').toUpperCase()).filter(Boolean))
  const instByAccount = new Map()
  const instCount = new Map()
  for (const it of live) {
    const inst = it.institution || ''
    if (it._ibkrAccountId && !instByAccount.has(it._ibkrAccountId)) instByAccount.set(it._ibkrAccountId, inst)
    instCount.set(inst, (instCount.get(inst) || 0) + 1)
  }
  const defaultInst = [...instCount.entries()].sort((a, b) => b[1] - a[1])[0]?.[0] || ''

  const bySym = new Map()
  for (const tx of transactions) {
    if (!tx || tx._source !== 'ibkr') continue
    const ty = (tx.type || '').toUpperCase()
    if (ty !== 'BUY' && ty !== 'SELL') continue
    const sym = (tx.symbol || '').toUpperCase()
    if (!sym || sym === 'CASH' || FOREX_PAIR_RE.test(sym) || !CHART_SYMBOL_RE.test(sym)) continue
    if (liveSymbols.has(sym)) continue
    const d = parseUTCDate(tx.date)
    const q = Math.abs(Number(tx.quantity) || 0)
    if (!d || !q) continue
    if (!bySym.has(sym)) bySym.set(sym, { symbol: sym, events: [], accountId: tx._ibkrAccountId || '', currency: tx.currency || 'USD' })
    const g = bySym.get(sym)
    g.events.push({ ts: d.getTime(), delta: ty === 'BUY' ? q : -q, price: Number(tx.pricePerUnit) || 0 })
  }
  ...
  for (const g of bySym.values()) {
    ...
    for (const ev of g.events) {
      running += ev.delta
      if (running < -EPS) { consistent = false; break }
    }
    if (!consistent || Math.abs(running) > EPS) continue
    const last = g.events[g.events.length - 1]
    out.push({
      symbol: g.symbol,
      institution: instByAccount.get(g.accountId) ?? defaultInst,
      currency: g.currency,
      events: g.events.map(({ ts, delta }) => ({ ts, delta })),
      lastTrade: { ts: last.ts, price: last.price, currency: g.currency },
    })
  }
  return out
}

// Cantidad que se tenía al cierre del mes, acumulada desde cero.
function closedQtyAt(events, monthEnd) {
  const end = monthEnd.getTime()
  let qty = 0
  for (const ev of events) if (ev.ts <= end) qty += ev.delta
  return qty > 1e-6 ? qty : 0
}
...
// `acqOverride` (FASE OA): la compuerta que el camino de mercado ya resolvio
// (fecha real > lote mas viejo > creacion). Sin ella, el respaldo de un
// activo cotizado volvia a caer a enero del anio de creacion y rellenaba los
// meses anteriores al lote que el propio camino de mercado dejo vacios.
// FASE OA. La compuerta de existencia de un activo de MERCADO sin fecha de
// adquisicion: fecha real > lote mas viejo del simbolo (el alta manual crea
// uno con su fecha) > enero del anio de creacion (la regla de un estatico).
// Vive aparte de `effectiveAcqDate` a proposito: aquella la lee la
// reconstruccion estatica (congelada F) y no cambia.
function marketAcqDate(it, symLots) {
  const real = parseUTCDate(it.acquisitionDate)
  if (real) return real
  const earliestLot = (symLots || [])
    .map(l => parseUTCDate(l.acquisitionDate)).filter(Boolean).sort((a, b) => a - b)[0] || null
  return earliestLot || effectiveAcqDate(it)
}

function fillFallback(result, it, months, convert, baseCurrency, acqOverride = null) {
  const acqDate = acqOverride || effectiveAcqDate(it)
  ...
  if (transactions && transactions.length) {
    ...
    for (const tx of transactions) {
      ...
      if (ty === 'BUY' || ty === 'SELL') {
        ...
        //
        // ⛔ EXTENSIÓN (FASE OK, aprobada explícitamente por el usuario el 7 sep
        // 2026). Vender con destino "Queda en el portafolio" (el dinero se
        // acredita a OTRO ítem, `_destinationItemId`) NO crea ningún WITHDRAWAL
        // para el ítem VENDIDO: `buildSaleTransactions` (lib/saleTx.js) solo
        // agrega esa fila cuando el destino es "Sale del portafolio". Sin ella,
        // el ítem vendido se queda SIN NINGÚN evento de balance propio, y
        // `applyStaticHistory` reconstruye su pasado desde `curVal` (que ya es
        // 0, porque vender pone currentPrice/purchasePrice en cero) sin nada
        // que deshacer: TODOS los meses, incluidos los que el activo tuvo valor
        // real, colapsan a $0.00. Reproducido: un bono de $6,000 vendido con
        // destino "queda en el portafolio" mostraba $0.00 en cada mes de su
        // historia en la Hoja, aunque estuvo sano meses antes de venderse.
        //
        // El arreglo espeja lo que YA pasaba (por accidente) con destino "Sale
        // del portafolio": ahí el WITHDRAWAL compañero SÍ empuja un evento
        // -monto para el ítem vendido y la reconstrucción vuelve a andar
        // (ancla el valor pre-venta al PRECIO DE VENTA, no al de compra — la
        // MISMA convención "hold-flat + deshacer" que ya rige para una cuenta
        // VACIADA, FASE LW/MB, y no algo que esta extensión inventa). Acá se
        // hace EXPLÍCITO: toda venta con destino declarado también empuja un
        // evento -monto para el ítem VENDIDO, en la MISMA fecha y moneda que
        // el que ya recibe el destino.
        //
        // Por qué NO duplica el caso "Sale del portafolio": los dos destinos
        // son MUTUAMENTE EXCLUYENTES por construcción — `buildSaleTransactions`
        // solo agrega `_destinationItemId` cuando NO hay WITHDRAWAL compañera,
        // y solo agrega la WITHDRAWAL cuando NO hay `_destinationItemId` — así
        // que esta rama nueva y la reconstrucción vía WITHDRAWAL nunca compiten
        // por el mismo ítem. Cero contacto con la fórmula (numerador,
        // denominador, comisión de entrada): cambia solo QUÉ movimiento entra a
        // la reconstrucción del valor, como las extensiones de FASE IX/MZ/GU.
        // El candado de 3.94% pasa sin tocarse: VITALI no se vende en ese test.
        if (ty === 'SELL' && tx._destinationItemId && itemById.get(tx._destinationItemId) && d) {
          ...
          if (amtRaw > 0) {
            ...
            if (tx._linkedItemId && itemById.get(tx._linkedItemId)) {
              pushBalance(tx._linkedItemId, { ts: d.getTime(), amount: -amount })
            }
          }
        }
        if (!sym || !d || !q) continue
...
//   'unavailable'  el proveedor NO contesto (cuota, red, 5xx)
//
// ⛔ 'unavailable' es una fuente aparte y no un caso de 'flat', porque la
// diferencia decide si el resultado se puede GUARDAR. Un bono cuyo simbolo no
// cotiza contesta 'flat' y eso es su naturaleza: cachearlo es correcto. Un
// fetch que fallo no midio nada, y cachearlo congela el mes para siempre (el
// chequeo de "ya se calculo" lo encuentra cubierto y no lo vuelve a pedir).
// Es el invariante 5 de la serie historica una capa mas abajo: un consumidor
// que PERSISTE nunca escribe desde una respuesta degradada.
...
export async function getHistoricalItemValues(items, months, convert, baseCurrency, lots, transactions, snapshots, diag) {
  ...
  const bailToBucket = (it, source) => {
    if (it._source !== 'ibkr') return false
    unmeasuredIbkr.add(it)
    noteSource(diag, it, source)
    return true
  }
  ...
  for (let i = 0; i < marketItems.length; i += BATCH_SIZE) {
    ...
    await Promise.all(batch.map(async (it) => {
      try {
        ...
        if (!res.ok) { if (bailToBucket(it, 'unavailable')) return; applyStaticHistory(result, it, months, convert, baseCurrency, balanceEventsById, reinvestIdx); noteSource(diag, it, 'unavailable'); return }
        const data = await safeJson(res)
        const prices = data.prices || []
        if (prices.length === 0) { if (bailToBucket(it, 'flat')) return; applyStaticHistory(result, it, months, convert, baseCurrency, balanceEventsById, reinvestIdx); noteSource(diag, it, 'flat'); return }
        ...
        if (sameCur && latestClose > 0 && itUnit > 0) {
          ...
          if (ratio > 10 || ratio < 0.1) {
            if (bailToBucket(it, 'flat')) return
        ...
        // FASE OA. Sin fecha de adquisicion, el held-flat de abajo no tenia
        // compuerta: una accion agregada por el recorrido guiado (que no
        // pregunta fecha) valia lo de HOY en TODOS los meses y anios pasados,
        // como si se hubiera tenido desde siempre. La compuerta cae a la
        // evidencia mas cercana: el lote mas viejo del simbolo (el alta manual
        // crea uno con su fecha), y si no hay, la misma regla que un activo
        // estatico (enero del anio en que se creo el doc). Una fecha REAL sigue
        // mandando; IBKR sigue exento por `dateUnreliable`.
        const acqDate = marketAcqDate(it, symLots)
        ...
        fillFallback(result, it, months, convert, baseCurrency, acqDate)
        noteSource(diag, it, 'market')
      } catch (err) {
        console.error(`[historicalValues] Failed for ${it.symbol}:`, err.message)
        if (bailToBucket(it, 'unavailable')) return
        fillFallback(result, it, months, convert, baseCurrency)
        noteSource(diag, it, 'unavailable')
      }
    }))
  }

  // FASE NS. Las posiciones ya VENDIDAS, reconstruidas desde el ledger.
  //
  // Va DESPUÉS del camino de mercado y ANTES del bucket a propósito: escribe
  // bajo llaves sintéticas propias (IBKR_CLOSED_KEY_PREFIX), nunca bajo un id
  // de item, así que ni compite con las filas de arriba ni entra al `_covers`
  // del bucket, que solo nombra ids.
  //
  // Solo se pide precio para un símbolo que SÍ se tuvo en alguno de los meses
  // pedidos: con un ledger de ~365 días y una tabla de 2023, casi ningún símbolo
  // aplica a los años viejos, y cada símbolo es una llamada al proveedor.
  //
  // ⛔ Un fallo del proveedor sobre una posición vendida se anota como 'skipped'
  // y NO como 'unavailable', y la diferencia es deliberada: 'unavailable' hace
  // que el mes entero no se guarde (invariante 5), que es lo correcto para una
  // posición que el usuario TIENE. Pero la ruta de precios contesta 503 tanto
  // ante un proveedor caído como ante un ticker que YA NO EXISTE (una vendida
  // puede estar deslistada: el caso real trae "GOLD.OLD"), y con 'unavailable'
  // un solo ticker muerto congelaría el año en "sin guardar" para siempre. La
  // fila de una vendida es best-effort: sin ella el mes se ve exactamente como
  // se veía antes de esta fase, y "Recalcular" la vuelve a pedir.
  const closedLedgers = closedIbkrLedgers(items, transactions)
  const closedToFetch = closedLedgers
    .map(cl => ({ ...cl, qtyByMonth: Object.fromEntries(months.map(mk => [mk, closedQtyAt(cl.events, getMonthEndDate(mk))])) }))
    .filter(cl => Object.values(cl.qtyByMonth).some(q => q > 0))
  for (let i = 0; i < closedToFetch.length; i += BATCH_SIZE) {
    const batch = closedToFetch.slice(i, i + BATCH_SIZE)
    await Promise.all(batch.map(async (cl) => {
      const key = `${IBKR_CLOSED_KEY_PREFIX}${cl.institution}__${cl.symbol}`
      const note = (source) => { if (diag) diag[key] = { name: cl.symbol, symbol: cl.symbol, source, closed: true } }
      try {
        const res = await authFetch(`/api/prices/chart?symbol=${encodeURIComponent(cl.symbol)}&range=${range}&interval=1mo`)
        if (!res.ok) { note('skipped'); return }
        const data = await safeJson(res)
        const prices = (data && data.prices) || []
        if (prices.length === 0) { note('flat'); return }
        const priceCurrency = data.currency || 'USD'
        const priceByMonth = {}
        prices.forEach(p => {
          if (p.close != null && p.date) priceByMonth[getMonthKey(new Date(p.date))] = p.close
        })
        // Guard de colisión de ticker, con la única referencia que una vendida
        // tiene: el precio de su ÚLTIMO trade contra el cierre del proveedor en
        // ese mismo mes. Un símbolo que resolvió a otro instrumento se delata
        // por un orden de magnitud, igual que en el camino de mercado.
        const lastMk = getMonthKey(new Date(cl.lastTrade.ts))
        const ref = priceByMonth[lastMk]
        if (cl.lastTrade.price > 0 && ref > 0 && cl.lastTrade.currency === priceCurrency) {
          const ratio = ref / cl.lastTrade.price
          if (ratio > 10 || ratio < 0.1) { note('flat'); return }
        }
        let wrote = false
        months.forEach(mk => {
          const qty = cl.qtyByMonth[mk]
          const price = priceByMonth[mk]
          if (!(qty > 0) || price == null) return
          let val = qty * price
          if (convert && priceCurrency !== (baseCurrency || 'USD')) val = convert(val, priceCurrency, baseCurrency || 'USD')
          if (!(val > 0)) return
          // Categoría 'stocks' fija: un trade no trae categoría de activo, y
          // una posición viva de IBKR entra a la app como `type: 'Stock'` para
          // todo lo que el Flex marca STK (acciones Y ETFs), o sea aterriza en
          // esta misma categoría. Es la misma fila donde habría estado viva.
          result[mk][key] = {
            value: val, symbol: cl.symbol, category: 'stocks', institution: cl.institution,
            // La identidad de "vendida" viaja en la LLAVE (IBKR_CLOSED_KEY_PREFIX),
            // no en un campo: un campo que nadie lee es como alguien mas adelante
            // cree que sirve para algo (guardian noOrphanPersistedFields, FASE MV).
            estimated: false, _syntheticIbkr: true,
          }
          wrote = true
        })
        note(wrote ? 'market' : 'flat')
      } catch (err) {
        console.error(`[historicalValues] Failed for sold ${cl.symbol}:`, err.message)
        note('skipped')
      }
    }))
  }
  ...
  for (const it of items) {
    ...
    // FASE OA: un activo de mercado sin fecha se gatea igual que en su camino
    // de arriba (lote mas viejo > creacion); si no, esta red volvia a rellenar
    // con el valor de HOY los meses anteriores al lote que ese camino dejo
    // vacios a proposito.
    const acqDate = marketSet.has(it)
      ? marketAcqDate(it, hasLots ? lots.filter(l => (l.symbol || '').toUpperCase() === (it.symbol || '').toUpperCase()) : [])
      : effectiveAcqDate(it)
