  ...
  for (let i = 0; i < marketItems.length; i += BATCH_SIZE) {
    ...
    await Promise.all(batch.map(async (it) => {
      try {
        ...
        if (!res.ok) { applyStaticHistory(result, it, months, convert, baseCurrency, balanceEventsById, reinvestIdx); noteSource(diag, it, 'flat'); return }
        const data = await safeJson(res)
        const prices = data.prices || []
        if (prices.length === 0) { applyStaticHistory(result, it, months, convert, baseCurrency, balanceEventsById, reinvestIdx); noteSource(diag, it, 'flat'); return }
