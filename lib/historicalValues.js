...
export async function getHistoricalItemValues(items, months, convert, baseCurrency, lots, transactions) {
  ...
  if (transactions && transactions.length) {
    ...
    for (const tx of transactions) {
      ...
      if (ty === 'BUY' || ty === 'SELL') {
        ...
        if (tx._linkedItemId) {
          const arr = balanceEventsById[tx._linkedItemId] = balanceEventsById[tx._linkedItemId] || []
          arr.push({ ts: d.getTime(), amount: delta })
        }
        if (ty === 'DIVIDEND') {
          const sym = (tx.symbol || (linked && (linked.symbol || linked.name)) || '').toUpperCase()
          if (sym) (reinvestBySym[sym] = reinvestBySym[sym] || []).push({ ts: d.getTime(), amount })
        }
      }
    }
  }
  ...
  staticItems.forEach(it => {
    ...
    months.forEach(mk => {
      ...
      // Reverse any deposits/dividends/withdrawals that happened AFTER this month
      // so the value reflects the balance as of month-end.
      let val = curVal
      if (allEvents.length) {
        const end = monthEnd.getTime()
