...
function fillFallback(result, it, months, convert, baseCurrency) {
  ...
  const rawPrice = it._originalPurchasePrice ?? it.purchasePrice ?? it._originalPrice ?? 0
      ...
      try {
        ...
        months.forEach(mk => {
          ...
          if (txEvents.length > 0) {
            ...
            // current position was held throughout, gated by acquisition date.
            qty = (!acqDate || monthEnd >= acqDate) ? currentQty : 0
          }
          if (qty <= 0) return
