...
export async function getHistoricalItemValues(items, months, convert, baseCurrency, lots, transactions, snapshots) {
  ...
  if (ibkrItems.length && snapshots && snapshots.length) {
    ...
    // month), converting the stored USD NAV to baseCurrency. Includes
    // 'ibkr_quarterly' (transcribed from Portfolio Analyst for history a Flex
    // Query can't reach) alongside 'ibkr' (synced) — a transcribed quarter used
    // to be silently dropped here, so a user who filled in years of quarterly
    // history still saw those months fall back to flat current-value estimates.
