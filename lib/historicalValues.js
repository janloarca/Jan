...
// Acquisition gate for historical reconstruction. Order of evidence:
//   1. The exact acquisitionDate when the user provided one (always wins).
//   2. The earliest real BUY trade for the symbol (firstBuyTs) — a trade PROVES
//      the asset existed that day, so crypto synced from Ledger/exchange (which
//      stamps no acquisitionDate) still reconstructs from its first purchase
//      (e.g. Jan 2021) instead of the year it was added to the app.
//   3. Jan 1 of the year the asset was ADDED to the app (createdAt). createdAt
//      is an import/add timestamp, NOT an acquisition date — using it directly
//      would blank a position imported mid-year for every earlier month of that
//      same year. Snapping it to the start of its year hides the asset in PRIOR
//      years it didn't exist while still showing it across its add-year.
// Returns null when none is available (no gate).
function effectiveAcqDate(it, firstBuyTs) {
  const acq = parseUTCDate(it.acquisitionDate)
  if (acq) return acq
  if (firstBuyTs != null) return new Date(firstBuyTs)
  const created = parseUTCDate(it.createdAt)
  if (created) return new Date(Date.UTC(created.getUTCFullYear(), 0, 1))
  return null
}

function fillFallback(result, it, months, convert, baseCurrency, firstBuyTs) {
  const acqDate = effectiveAcqDate(it, firstBuyTs)
...
function applyStaticHistory(result, it, months, convert, baseCurrency, balanceEventsById, reinvestBySym, firstBuyTs) {
  const acqDate = effectiveAcqDate(it, firstBuyTs)
  ...
  months.forEach(mk => {
    ...
    result[mk][it.id] = { value: val, symbol: it.name || it.symbol || '', category: it._category || '', institution: it.institution || '', estimated: true }
  })
}
...
export async function getHistoricalItemValues(items, months, convert, baseCurrency, lots, transactions, snapshots) {
  ...
  // Earliest real BUY per symbol — evidence the asset existed that day. Feeds
  // the acquisition gate (effectiveAcqDate) for items with no acquisitionDate,
  // so synced crypto reconstructs from its first purchase, not its add-year.
  const firstBuyBySym = {}
  Object.entries(txBySymbol).forEach(([sym, events]) => {
    for (const ev of events) {
      if (ev.delta > 0 && (firstBuyBySym[sym] == null || ev.ts < firstBuyBySym[sym])) {
        firstBuyBySym[sym] = ev.ts
      }
    }
  })
  const firstBuyTsFor = (it) => firstBuyBySym[(it.symbol || '').toUpperCase()] ?? null
  ...
  staticItems.forEach(it => {
    if (!it.id) return
    applyStaticHistory(result, it, months, convert, baseCurrency, balanceEventsById, reinvestBySym, firstBuyTsFor(it))
  })
  ...
  for (let i = 0; i < marketItems.length; i += BATCH_SIZE) {
    ...
    await Promise.all(batch.map(async (it) => {
      try {
        ...
        if (!res.ok) { applyStaticHistory(result, it, months, convert, baseCurrency, balanceEventsById, reinvestBySym, firstBuyTsFor(it)); return }
        const data = await safeJson(res)
        const prices = data.prices || []
        if (prices.length === 0) { applyStaticHistory(result, it, months, convert, baseCurrency, balanceEventsById, reinvestBySym, firstBuyTsFor(it)); return }
        ...
        // Gate for the held-flat branch: the explicit acquisitionDate wins; a
        // real BUY trade (firstBuyBySym) is the fallback evidence — items with
        // neither stay ungated, exactly as before.
        const acqDate = parseUTCDate(it.acquisitionDate)
          || (firstBuyTsFor(it) != null ? new Date(firstBuyTsFor(it)) : null)
        ...
        fillFallback(result, it, months, convert, baseCurrency, firstBuyTsFor(it))
      } catch (err) {
  ...
  for (const it of items) {
    ...
    const acqDate = effectiveAcqDate(it, firstBuyTsFor(it))
