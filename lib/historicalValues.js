...
function applyStaticHistory(result, it, months, convert, baseCurrency, balanceEventsById, reinvestBySym, trueStatic = false) {
  const acqDate = effectiveAcqDate(it)
...
//   reinvestBySym:     { [SYMBOL]: [{ ts, amount }] }   reinvested income
  ...
  if (transactions && transactions.length) {
    ...
    for (const tx of transactions) {
      ...
      if (ty === 'BUY' || ty === 'SELL') {
        ...
        if (!(amtRaw > 0) || !d) continue
        const cur = tx.currency || baseCurrency || 'USD'
        // The amount is denominated in the SENDING account's currency; one
        // conversion to base covers both ends, because these events are always
        // in base currency (same as the DEPOSIT path above).
        const amount = convert && cur !== (baseCurrency || 'USD') ? convert(amtRaw, cur, baseCurrency || 'USD') : amtRaw
        const ts = d.getTime()
        ...
        if (fromId) pushBalance(fromId, { ts, amount: -amount })
        if (toId) pushBalance(toId, { ts, amount })
      }
    }
  }
  return { balanceEventsById, reinvestBySym, txBySymbol }
}
...
export async function getHistoricalItemValues(items, months, convert, baseCurrency, lots, transactions, snapshots) {
  ...
  const { balanceEventsById, reinvestBySym, txBySymbol } =
  ...
  staticItems.forEach(it => {
    if (!it.id) return
    applyStaticHistory(result, it, months, convert, baseCurrency, balanceEventsById, reinvestBySym, true)
  })
  ...
  // IBKR positions: NEVER reconstructed from real per-position history here —
  // only the ACCOUNT's total NAV is known for a past month (from equity
  // snapshots), never which of TODAY's names it was split across (a stock
  // bought last month didn't exist in the portfolio two years ago). Splitting
  // that total across today's holdings and labeling each slice with a real
  // ticker (the old behavior) reads as "we know NOVO-NORDISK was worth $78.71
  // in Jun 2023" when all that's actually known is the account's total.
  // Collapsed into ONE synthetic bucket per institution+category instead —
  // see IBKR_UNKNOWN_KEY_PREFIX (PortfolioSpreadsheet.jsx renders it as
  // "Unknown positions" and must build the SAME key to find it).
  ...
  if (ibkrItems.length) {
    ...
    for (const { institution, category, items: groupItems } of groups.values()) {
      ...
      months.forEach(mk => {
        ...
        if (value > 0) result[mk][key] = { value, category, institution, estimated, _syntheticIbkr: true }
      })
    }
  }
