...
// El arranque del broker DESPEJADO del propio archivo, no estimado por precios.
//
// Con un broker conectado, el ancla del año que usa el encabezado es un doc
// COMPUESTO (FASE HN): `NAV real del broker + reconstrucción de lo manual`.
// Eso es una identidad exacta, así que el arranque del broker se despeja:
//
//     arranque_broker = ancla − Σ(arranques de las cuentas manuales)
//
// Por qué importa: los arranques por cuenta reconstruyen el broker por PRECIOS
// mientras el ancla lleva su NAV REAL, y el residuo que hacía rehusar el panel
// era exactamente esa diferencia (caso del usuario: $1,093 de descuadre sobre
// una tolerancia de $237, que NO bajaba por más veces que se regenerara el
// historial, porque no era historial a medio regenerar: eran dos medidas
// distintas de la misma cantidad). Despejado, el reparto cuadra por
// construcción y encima usa el valor verdadero.
//
// Rehúsa (null) antes que inventar: sin ancla, sin cuentas manuales medidas, o
// si el despeje sale negativo (los arranques manuales solos ya exceden el
// ancla, o sea el problema está en otro lado y este atajo lo taparía).
// Elige, de la serie que devuelve el API de historial, el par de puntos con
// desglose por ítem (byKey) que alimenta el panel: el que cae EXACTAMENTE en el
// ancla que usó el encabezado, y el último.
//
// La versión anterior exigía que el punto con desglose fuera el PRIMERO de la
// serie (`withKeys[0].ts === anchorTs`) y, si no, descartaba el desglose
// entero. Eso no dejaba al panel sin datos: lo mandaba a los RESPALDOS (columna
// de diciembre del Spreadsheet para lo estático, held-flat para lo demás), que
// es de donde salían las filas que no coincidían con las gráficas escopadas. La
// garantía que importa es medir en el ancla, no que ese punto venga primero.
//
// Devuelve null cuando no hay un punto en el ancla o no hay al menos dos puntos
// con desglose: ahí el panel sí prefiere no afirmar nada.
export function pickAnchorBreakdown(points, anchorTs) {
  if (!Array.isArray(points) || anchorTs == null) return null
  const withKeys = points.filter((p) => p && p.byKey)
  if (withKeys.length < 2) return null
  const anchorPoint = withKeys.find((p) => p.ts === anchorTs)
  if (!anchorPoint) return null
  // `startTs` viaja con el desglose (FASE IU) porque el punto en el que se
  // mide NO tiene por que ser la fecha del ancla: el caller pasa el primer
  // punto con total positivo de la serie. Si esos dos instantes se separan, el
  // panel mide el arranque de cada cuenta en un dia distinto al que dice, y en
  // una cuenta volatil eso solo se nota como un desvio que no cuadra con nada.
  return { start: anchorPoint.byKey, end: withKeys[withKeys.length - 1].byKey, startTs: anchorPoint.ts }
}

export function deriveBrokerStart({ anchor, manualStarts }) {
  if (!isFinite(anchor) || anchor <= 0) return null
  if (!Array.isArray(manualStarts) || manualStarts.length === 0) return null
  let sum = 0
  for (const v of manualStarts) {
    if (!isFinite(v)) return null
    sum += v
  }
  const derived = anchor - sum
...
export function attributeYtd({ accounts, portfolioStart, headlineGain, unattributedFlow = 0 }, diag) {
  ...
  const rows = accounts.map((a) => {
    ...
    return {
      ...
      // Cuánto del flujo neteado es dinero movido entre cuentas del propio
      // usuario. Puramente informativo (no entra en ninguna cuenta): existe
      // para poder explicar por qué esta fila puede diferir de la gráfica
      // escopada de la misma cuenta, que no netea esos movimientos.
      internal: isFinite(a.internal) ? a.internal : 0,
      gain,
