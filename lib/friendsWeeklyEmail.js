...
export function weekLabel(now = new Date()) {
  ...
  // Una semana puede cruzar el año, y el rótulo va en el ASUNTO. Con un solo
  // año al final, "Dec 28-Jan 3, 2027" fechaba el 28 de diciembre en 2027,
  // cuando es de 2026. Cuando los dos extremos caen en años distintos se dice
  // cada uno con el suyo.
  const y0 = start.getUTCFullYear()
  const y1 = end.getUTCFullYear()
  if (y0 !== y1) return `${f(start)}, ${y0}-${f(end)}, ${y1}`
  return `${f(start)}-${f(end)}, ${y1}`
}
...
export function standingSentence({ rows }) {
  ...
  // ⛔ Un EMPATE no es estar adelante ni atrás. Con `Math.abs` y sin caso para
  // cero, dos personas con el mismo % recibían el mismo domingo frases
  // opuestas ("1st, 0.00 points ahead" y "1st, 0.00 points behind") según en
  // qué orden hubiera quedado el arreglo, y la segunda se contradecía a sí
  // misma en la misma oración. Con el ranking de competencia las dos van 1º.
  const tied = (a, b) => a != null && b != null && a === b
  if (leader.uid === you.uid) {
    const second = ranked.find((r) => r.uid !== you.uid)
    if (second && tied(second.ytd, you.ytd)) {
      return `You are tied for 1st of ${ranked.length} with ${second.displayName}.`
    }
  ...
  if (tied(leader.ytd, you.ytd)) {
    return `You are tied for 1st of ${ranked.length} with ${leader.displayName}.`
  }
...
/**
 * @param {Array}  groups  [{ name, scope, pendingCount, rows, streak }]
 * @param {Date}   now
...
export function buildFriendsWeeklyEmail({ groups = [], now = new Date(), manageUrl = 'https://chispu.xyz/friends' } = {}) {
  ...
  sections.push({
    ...
    paragraphs: [
      ...
      ...(anyMovement
        // La flecha se dibuja en TODAS las filas (la de Bob cuenta a quién pasó
        // Bob), así que la leyenda no puede hablar solo de vos.
        ? ['The arrow counts how many people that person passed, or was passed by, among those who were also here last week.']
        : []),
  ...
  const { html, text } = renderEmail({
    ...
    // FASE OO. Este correo nunca lleva montos (solo porcentajes, la regla de
    // lib/friendsStats.js), pero sí NOMBRES de otras personas y sus puestos, y
    // eso tampoco tiene por qué leerse en una pantalla bloqueada.
    preheader: 'How your friend groups are doing this week, who is leading, and where you placed in each one. Open to see the standings.',
    sections,
    manageUrl,
