...
export default function RecurringTransactions({ goals, onSaveGoals, lang }) {
  ...
  return (
    <div className="bg-[#1e293b]/80 rounded-xl border border-[#334155]/50 p-4 sm:p-5">
      ...
      {showForm && (
        <div className="mb-4 p-3 bg-[#0f172a] rounded-lg border border-[#334155]/50 space-y-3">
          <div className="flex gap-2">
            <button
              onClick={() => setForm({ ...form, isInflow: true })}
              className={`flex-1 py-1.5 text-xs rounded-lg border transition-colors ${
                form.isInflow ? 'bg-emerald-600/20 border-emerald-500/30 text-emerald-400' : 'border-[#334155] text-slate-400'
              }`}
            >
              ↓ {t('Ingreso', 'Inflow')}
            </button>
            <button
              ...
              className={`flex-1 py-1.5 text-xs rounded-lg border transition-colors ${
                !form.isInflow ? 'bg-red-600/20 border-red-500/30 text-red-400' : 'border-[#334155] text-slate-400'
              }`}
            >
              ↑ {t('Gasto', 'Outflow')}
            </button>
          ...
          <div className="grid grid-cols-2 gap-2">
            <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
              ...
              className="px-3 py-2 bg-[#1e293b] border border-[#334155] rounded-lg text-xs text-white placeholder-slate-600 focus:outline-none focus:border-blue-500/50" />
          ...
          <div className="grid grid-cols-2 gap-2">
            <select value={form.frequency} onChange={(e) => setForm({ ...form, frequency: e.target.value })}
              className="px-3 py-2 bg-[#1e293b] border border-[#334155] rounded-lg text-xs text-white focus:outline-none focus:border-blue-500/50">
      ...
      {recurring.length > 0 && (
        <>
          ...
          <div className="pt-3 border-t border-[#334155]/30 space-y-1.5">
            ...
            <div className="flex justify-between text-xs font-medium pt-1 border-t border-[#334155]/20">
