import { computeModifiedDietz, solveDietzStartValue, accountKeyOfItem, heldFlatAccountValueUSD, combineAccountCalibrations, calibrationAnchorDate, calibrationKindOf, calibrationCoveredByRealData, fitSeriesToAnchors, dedupeCalibrations } from '../utils'
...
// dedupeCalibrations keeps one doc per (date, kind, account): the latest
// capturedAt wins, so a recalibration replaces the stale one everywhere the
// anchors are consumed (chart constraints, return badges).
describe('dedupeCalibrations', () => {
  it('keeps the most recent capturedAt per date+kind+account', () => {
    const out = dedupeCalibrations([
      { date: '2026-01-01', _calibrationKind: 'ytd', targetPct: 6, capturedAt: '2026-08-04', netWorthUSD: 100 },
      { date: '2026-01-01', _calibrationKind: 'ytd', targetPct: 8, capturedAt: '2026-08-05', netWorthUSD: 200 },
    ])
    expect(out).toHaveLength(1)
    expect(out[0].targetPct).toBe(8)
    expect(out[0].netWorthUSD).toBe(200)
  })

  it('treats global and per-account docs as different slots', () => {
    const out = dedupeCalibrations([
      { date: '2026-01-01', _calibrationKind: 'ytd', capturedAt: '2026-08-05' },
      { date: '2026-01-01', _calibrationKind: 'ytd', _account: 'ibkr', capturedAt: '2026-08-04' },
    ])
    expect(out).toHaveLength(2)
  })

  it('legacy docs without capturedAt lose against stamped ones, and dateless docs drop', () => {
    const out = dedupeCalibrations([
      { date: '2026-01-01', _calibrationKind: 'ytd', targetPct: 5 },
      { date: '2026-01-01', _calibrationKind: 'ytd', targetPct: 7, capturedAt: '2026-08-05' },
      { _calibrationKind: 'ytd', targetPct: 99 },
    ])
    expect(out).toHaveLength(1)
    expect(out[0].targetPct).toBe(7)
  })
})
