  ...
  return (
    <div className="relative">
      <button onClick={() => setShowMenu(!showMenu)}
        ...
        className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-slate-300 hover:text-white bg-[#0d1117]/50 border border-[#21262d]/50 rounded-lg transition-colors">
      ...
      {showMenu && (
        <>
          ...
          <div className="absolute top-full left-0 mt-1 w-56 bg-[#161b22] border border-[#21262d] rounded-xl shadow-xl z-50 overflow-hidden" role="listbox">
            <button onClick={() => { onSelect('__all__'); setShowMenu(false) }}
              ...
              className={`w-full flex items-center gap-2 px-4 py-2.5 text-sm text-left transition-colors ${
                ...
                activePortfolio === '__all__' ? 'bg-blue-500/10 text-blue-400' : 'text-slate-300 hover:bg-[#1c2129]'
              }`}>
              <span>📊</span> {allLabel}
            </button>
              ...
              <div key={p.id} className="flex items-center group">
                <button onClick={() => { onSelect(p.id); setShowMenu(false) }}
                  ...
                  className={`flex-1 flex items-center gap-2 px-4 py-2.5 text-sm text-left transition-colors ${
                    ...
                    activePortfolio === p.id ? 'bg-blue-500/10 text-blue-400' : 'text-slate-300 hover:bg-[#1c2129]'
                  }`}>
              ...
              {showAdd ? (
                <div className="p-2 flex gap-1.5">
                  <input value={newName} onChange={(e) => setNewName(e.target.value)}
                    ...
                    className="flex-1 px-2 py-1.5 text-xs bg-[#0d1117] border border-[#21262d] rounded text-white placeholder-slate-600" />
                ...
                <button onClick={() => setShowAdd(true)}
                  className="w-full px-4 py-2.5 text-sm text-slate-500 hover:text-emerald-400 hover:bg-[#1c2129] text-left transition-colors">
