...
import { planCellEdit, editNeedsAnswer, accruesInBalance, ANSWER_CORRECTION, ANSWER_RETURN } from '@/lib/spreadsheetEdit'
...
export default function PortfolioSpreadsheet({ items, snapshots, lang, onUpdateItem, onEditItem, onAddTransaction, returnYTD, netWorth, convert, baseCurrency, onSaveItemSnapshots, onLoadItemSnapshots, lots, transactions, onRegisterRecalculate }) {
  ...
  // corrección".
  const commitPatch = useCallback(async (item, patch, income) => {
    ...
    try {
      ...
      setSaveMsg(income
        ? (lang === 'es' ? `${label}: guardado y rendimiento registrado` : `${label}: saved, yield recorded`)
        : (lang === 'es' ? `${label}: guardado` : `${label}: saved`))
  ...
  const answerPendingEdit = useCallback(async (answer) => {
    ...
    await commitPatch(item, patch, plan.income)
  ...
  const handleValueUpdate = useCallback(async (item, newVal) => {
    ...
    await commitPatch(item, patch, null)
          ...
          <div className="mx-4 mb-2 px-3 py-3 rounded-lg text-xs space-y-2"
            ...
            <div className="flex flex-wrap gap-2 pt-0.5">
              ...
              <button type="button" onClick={() => answerPendingEdit(ANSWER_RETURN)}
                ...
                {liquida ? t('Son intereses que ganó', 'It earned interest') : t('De verdad cambió de valor', 'It really changed value')}
              </button>
            ...
            <p style={{ color: 'var(--text-muted)' }}>
              {liquida
                ? t('Corregir un número no mueve tu ganancia ni tu pérdida. Los intereses sí se registran como ingreso y aparecen en Ingresos Pasivos.',
                    'Correcting a number does not move your gain or loss. Interest is recorded as income and shows up in Passive Income.')
                : t('Corregir un número no mueve tu ganancia ni tu pérdida. Un cambio de valor real sí la mueve.',
                    'Correcting a number does not move your gain or loss. A real change in value does.')}
            </p>
          </div>
        )
      })()}
