    ...
    <>
      <div className="fixed bottom-0 left-0 right-0 z-40 sm:hidden bg-[#0f172a]/95 backdrop-blur-sm border-t border-[#334155]">
        <div className="flex items-center justify-around h-14 px-2">
          <button onClick={onSearch} aria-label={t('Buscar', 'Search')} className="flex flex-col items-center gap-0.5 text-slate-400 hover:text-white transition-colors p-2.5 min-w-[44px] min-h-[44px] justify-center">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
            </svg>
            <span className="text-[10px]">{t('Buscar', 'Search')}</span>
          </button>
          <button onClick={onAdd} aria-label={t('Agregar activo', 'Add asset')} className="flex flex-col items-center gap-0.5 text-slate-400 hover:text-white transition-colors p-2.5 min-w-[44px] min-h-[44px] justify-center">
