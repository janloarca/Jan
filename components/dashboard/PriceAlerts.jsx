...
export default function PriceAlerts({ alerts, items, onAddAlert, onDeleteAlert, lang }) {
  ...
  return (
    <div className="bg-theme-card rounded-2xl border border-glass-border p-5 card-primary">
      ...
      {showForm && (
        <div className="bg-theme-base rounded-lg p-3 mb-3 border border-glass-border/50 space-y-2">
          <div className="flex gap-2">
            <select value={symbol} onChange={(e) => setSymbol(e.target.value)}
              className="flex-1 bg-theme-card border border-glass-border rounded-lg px-3 py-2 text-sm text-white">
            ...
            <select value={direction} onChange={(e) => setDirection(e.target.value)}
              className="bg-theme-card border border-glass-border rounded-lg px-3 py-2 text-sm text-white">
          ...
          <div className="flex gap-2">
            <input type="number" value={targetPrice} onChange={(e) => setTargetPrice(e.target.value)}
              ...
              className="flex-1 bg-theme-card border border-glass-border rounded-lg px-3 py-2 text-sm text-white placeholder-slate-500" />
      ...
      {activeAlerts.length > 0 && (
        <div className="space-y-1.5 mb-2">
          {activeAlerts.map((alert) => (
            <div key={alert.id} className="flex items-center justify-between py-2 px-2.5 rounded-lg bg-theme-base/50 border border-glass-border/30">
